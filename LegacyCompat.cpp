...
void registerLegacyV39Routes(const std::shared_ptr<CoreService>& service) {
  ...
  reg1("/api/character/skills/{1}/use", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      if (roomId > 0) {
        ...
        if (!battleRows.empty() && battleRows[0]["battle_active"].as<bool>()) {
  ...
  // 用途：修改怪物相關資料或操作。 修改時請同步檢查前端呼叫、權限驗證與相關資料表。
  ...
  reg1("/api/rooms/{1}/world", drogon::Get,
    ...
    runHandler(cb, [&] {
      ...
      out["npcs"] = selectRows1(service->database(),
          "SELECT row_to_json(x)::text AS json_text FROM (SELECT rn.*,nt.name,nt.role_name,nt.description,nt.image_url,nt.merchant_enabled,nt.merchant_title,nt.attributes,nt.skills,nt.config FROM room_npcs rn JOIN npc_templates nt ON nt.id=rn.npc_template_id WHERE rn.room_id=$1 ORDER BY rn.npc_template_id) x", roomId);
  ...
  reg1("/api/admin/tasks/{1}/complete", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      service->database()->execSqlSync(
          ...
          "magic_points=magic_points+$4,ritual_points=ritual_points+$5,weird_coins=weird_coins+$6,game_coins=game_coins+$7,"
          ...
          std::max<std::int64_t>(0,intValue(rewards["ritual_points"])),
  ...
  reg1("/api/rooms/{1}/actions", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      auto triggered = evaluateRuleEvent(service, roomId, user.id, "PLAYER_ACTION", payload, false);
      if (triggered["blocked"].asBool()) throw ApiError(drogon::k403Forbidden, "此行動被規則阻止");
      service->addEvent(roomId, user.id, "action", payload);
      Json::Value out; out["ok"] = true; out["triggered"] = triggered["matched"]; out["rule_engine"] = triggered; out["room"] = service->roomSnapshot(roomId); reply(cb, std::move(out));
    }, "提交行動失敗");
  });
  ...
  reg0("/api/character/items/use", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      if ((hasPermanentElementGain || magicPointGain > 0 || ritualPointGain > 0) &&
  ...
  reg1("/api/rooms/{1}/teams", drogon::Get,
    ...
    runHandler(cb, [&] {
      ...
      else details["visions"]=selectRows1(service->database(),"SELECT row_to_json(v)::text AS json_text FROM personal_visions v WHERE room_id=$1 AND user_id=$2 ORDER BY created_at DESC",roomId,user.id);
      out["details"]=details;
  ...
  // [功能備註｜房間／跑團｜POST /api/rooms/{1}/teams/{2}/move]
  // 用途：建立/執行房間／跑團相關資料或操作。 修改時請同步檢查前端呼叫、權限驗證與相關資料表。
  reg2("/api/rooms/{1}/teams/{2}/move",drogon::Post,
       [service](const Request& req,Callback& cb,const std::string& roomIdText,const std::string& teamIdText){runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomIdText);const auto teamId=positiveId(teamIdText);requireRoomAccess(service,user,roomId);const auto nodeId=intValue(bodyOrEmpty(req)["node_id"]);if(nodeId<=0)throw ApiError(drogon::k400BadRequest,"請指定地圖節點");if(service->database()->execSqlSync("SELECT 1 FROM room_team_members WHERE team_id=$1 AND user_id=$2",teamId,user.id).empty()&&!user.isAdmin)throw ApiError(drogon::k403Forbidden,"你不在這個隊伍");if(service->database()->execSqlSync("SELECT 1 FROM room_map_nodes WHERE id=$1 AND room_id=$2 AND hidden=FALSE",nodeId,roomId).empty())throw ApiError(drogon::k404NotFound,"找不到可前往地點");Json::Value payload=ruleMapNodePayload(service,roomId,nodeId);payload["text"]="隊伍移動到新的地圖節點";payload["team_id"]=Json::Int64(teamId);const auto movedMembers=service->database()->execSqlSync("SELECT user_id FROM room_team_members WHERE team_id=$1 ORDER BY user_id",teamId);for(const auto& moved:movedMembers){const auto movedUserId=moved["user_id"].as<std::int64_t>();auto attempt=evaluateRuleEvent(service,roomId,movedUserId,"PLAYER_MOVE_ATTEMPT",payload,false);if(attempt["blocked"].asBool())throw ApiError(drogon::k403Forbidden,"隊伍中有成員受到規則限制，無法前往此地點");}service->database()->execSqlSync("UPDATE room_teams SET current_map_node_id=$1,updated_at=CURRENT_TIMESTAMP WHERE id=$2 AND room_id=$3",nodeId,teamId,roomId);service->database()->execSqlSync("UPDATE room_members SET current_map_node_id=$1 WHERE room_id=$2 AND user_id IN (SELECT user_id FROM room_team_members WHERE team_id=$3)",nodeId,roomId,teamId);service->addEvent(roomId,user.id,"team_move",payload);for(const auto& moved:movedMembers)evaluateRuleEvent(service,roomId,moved["user_id"].as<std::int64_t>(),"PLAYER_MOVED",payload,false);Json::Value out;out["ok"]=true;reply(cb,std::move(out));},"移動隊伍失敗");});
  ...
  reg0("/api/magic-studies", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      out["studies"]=genericList(service->database(),"magic_studies","category ASC,rank ASC,name ASC,id ASC","active=TRUE");
  ...
  reg0("/api/ritual-studies", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      out["studies"]=genericList(service->database(),"ritual_studies","category ASC,rank ASC,name ASC,id ASC","active=TRUE");
      out["learned"]=selectRows1(service->database(),
  ...
  reg1("/api/ritual-studies/{1}/learn", drogon::Post,
    ...
    runHandler(cb,[&]{
      ...
      const auto cost=std::max<std::int64_t>(0,intValue(study["point_cost"]));
      const auto update=service->database()->execSqlSync(
        "UPDATE character_cards SET ritual_points=ritual_points-$1,updated_at=CURRENT_TIMESTAMP WHERE user_id=$2 AND ritual_points>=$1 RETURNING ritual_points",cost,user.id);
      ...
      try{
        ...
        service->database()->execSqlSync("UPDATE character_cards SET ritual_points=ritual_points+$1 WHERE user_id=$2",cost,user.id);
        throw;
      }
      Json::Value out;out["ok"]=true;out["study"]=study;out["character"]=characterRaw(service->database(),user.id);reply(cb,std::move(out));
    },"學習儀式失敗");
  ...
  reg0("/api/admin/magic-studies",drogon::Get,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);Json::Value out;out["studies"]=genericList(service->database(),"magic_studies","category ASC,rank ASC,name ASC,id ASC");reply(cb,std::move(out));},"讀取魔法學資料庫失敗");
  });
  reg0("/api/admin/magic-studies",drogon::Post,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);auto body=bodyOrEmpty(req);const auto name=trimmed(stringValue(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"魔法名稱不能空白");body["name"]=name.substr(0,120);body["point_cost"]=Json::Int64(std::max<std::int64_t>(0,intValue(body["point_cost"],1)));Json::Value out;out["ok"]=true;out["study"]=genericInsert(service->database(),"magic_studies",body);reply(cb,std::move(out));},"建立魔法失敗");
  });
  registerAdminPatchDelete1(service,"/api/admin/magic-studies/{1}","magic_studies","study");
  ...
  reg0("/api/admin/ritual-studies",drogon::Post,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);auto body=bodyOrEmpty(req);const auto name=trimmed(stringValue(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"儀式名稱不能空白");body["name"]=name.substr(0,120);body["point_cost"]=Json::Int64(std::max<std::int64_t>(0,intValue(body["point_cost"],1)));Json::Value out;out["ok"]=true;out["study"]=genericInsert(service->database(),"ritual_studies",body);reply(cb,std::move(out));},"建立儀式失敗");
  });
  registerAdminPatchDelete1(service,"/api/admin/ritual-studies/{1}","ritual_studies","study");

  // [功能備註｜魔法學／儀式學｜POST /api/admin/players/{1}/study-points]
  // 用途：DM 發放或扣除魔法學／儀式學點數。點數最低維持 0。
  reg1("/api/admin/players/{1}/study-points",drogon::Post,
    ...
    runHandler(cb,[&]{
      ...
      const std::string field=kind=="magic"?"magic_points":"ritual_points";
      service->database()->execSqlSync("UPDATE character_cards SET "+field+"=GREATEST(0,"+field+"+$1),updated_at=CURRENT_TIMESTAMP WHERE user_id=$2",delta,userId);
      Json::Value out;out["ok"]=true;out["character"]=characterRaw(service->database(),userId);reply(cb,std::move(out));
    },"調整學習點數失敗");
  });
  ...
  // 支援 skills/items/materials。前端可貼 TSV/CSV/JSON，整理成 rows 後由 C++ 再次驗證。
  reg0("/api/admin/bulk-import/preview",drogon::Post,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);const auto body=bodyOrEmpty(req);const auto kind=stringValue(body["kind"]);const auto rows=body["rows"];if(kind!="skills"&&kind!="items"&&kind!="materials"&&kind!="magics"&&kind!="rituals")throw ApiError(drogon::k400BadRequest,"匯入種類只支援 skills/items/materials/magics/rituals");if(!rows.isArray())throw ApiError(drogon::k400BadRequest,"rows 必須是陣列");if(rows.size()>5000)throw ApiError(drogon::k400BadRequest,"單次最多 5000 筆");Json::Value normalized{Json::arrayValue},errors{Json::arrayValue};std::size_t line=0;for(const auto&r:rows){++line;if(!r.isObject()){Json::Value e;e["row"]=static_cast<Json::UInt64>(line);e["error"]="資料列不是物件";errors.append(e);continue;}auto name=trimmed(stringValue(r["name"]));if(name.empty()){Json::Value e;e["row"]=static_cast<Json::UInt64>(line);e["error"]="缺少 name";errors.append(e);continue;}Json::Value x=r;x["name"]=name.substr(0,120);if(kind=="skills"){if(!x.isMember("category"))x["category"]="一般";if(!x.isMember("skill_type"))x["skill_type"]=stringValue(x["type"],"active");if(!x.isMember("level"))x["level"]=1;if(!x.isMember("description"))x["description"]="";if(!x.isMember("resources"))x["resources"]=Json::Value{Json::objectValue};if(!x.isMember("data"))x["data"]=Json::Value{Json::objectValue};}
        else if(kind=="magics"||kind=="rituals"){if(!x.isMember("category"))x["category"]=(kind=="magics"?"一般魔法":"一般儀式");if(!x.isMember("rank"))x["rank"]="G";if(!x.isMember("description"))x["description"]="";if(!x.isMember("point_cost"))x["point_cost"]=1;if(!x.isMember("prerequisite_ids"))x["prerequisite_ids"]=Json::Value{Json::arrayValue};if(!x.isMember("effects"))x["effects"]=Json::Value{Json::objectValue};if(!x.isMember("active"))x["active"]=true;}
        else {if(!x.isMember("category")||stringValue(x["category"]).empty())x["category"]=(kind=="materials"?"材料":"道具");if(!x.isMember("item_type")||stringValue(x["item_type"]).empty())x["item_type"]=(kind=="materials"?"material":"normal");if(!x.isMember("description"))x["description"]="";if(!x.isMember("effects"))x["effects"]=Json::Value{Json::objectValue};if(!x.isMember("consume_on_use"))x["consume_on_use"]=(kind!="materials");}normalized.append(x);}Json::Value out;out["ok"]=errors.empty();out["kind"]=kind;out["valid_count"]=static_cast<Json::UInt64>(normalized.size());out["error_count"]=static_cast<Json::UInt64>(errors.size());out["rows"]=normalized;out["errors"]=errors;reply(cb,std::move(out));},"預覽批量匯入失敗");
  });
  ...
  reg0("/api/admin/bulk-import/commit",drogon::Post,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);const auto body=bodyOrEmpty(req);const auto kind=stringValue(body["kind"]);const auto rows=body["rows"];if(kind!="skills"&&kind!="items"&&kind!="materials"&&kind!="magics"&&kind!="rituals")throw ApiError(drogon::k400BadRequest,"匯入種類不支援");if(!rows.isArray()||rows.empty())throw ApiError(drogon::k400BadRequest,"沒有可匯入資料");if(rows.size()>5000)throw ApiError(drogon::k400BadRequest,"單次最多 5000 筆");const std::string table=kind=="skills"?"skill_templates":kind=="magics"?"magic_studies":kind=="rituals"?"ritual_studies":"item_templates";std::int64_t created=0,updated=0;Json::Value failures{Json::arrayValue};std::size_t line=0;for(const auto&r:rows){++line;try{if(!r.isObject())throw ApiError(drogon::k400BadRequest,"資料列不是物件");auto row=r;const auto name=trimmed(stringValue(row["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"缺少 name");row["name"]=name.substr(0,120);if(kind=="materials"){row["category"]="材料";if(!row.isMember("item_type"))row["item_type"]="material";}const auto existing=service->database()->execSqlSync("SELECT id FROM \""+table+"\" WHERE name=$1 ORDER BY id LIMIT 1",name);if(existing.empty()){genericInsert(service->database(),table,row);++created;}else{genericUpdate(service->database(),table,existing[0]["id"].as<std::int64_t>(),row);++updated;}}catch(const std::exception&e){Json::Value f;f["row"]=static_cast<Json::UInt64>(line);f["name"]=stringValue(r["name"]);f["error"]=e.what();failures.append(f);}}Json::Value out;out["ok"]=failures.empty();out["created"]=Json::Int64(created);out["updated"]=Json::Int64(updated);out["failed"]=static_cast<Json::UInt64>(failures.size());out["failures"]=failures;reply(cb,std::move(out));},"批量匯入失敗");
  });
