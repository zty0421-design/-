  ...
  if (type == "monster") {
    const auto rows = service->database()->execSqlSync(R"SQL(
      SELECT jsonb_build_object(
        ...
        'config',mt.config,'ai_behavior',mt.ai_behavior,
...
Json::Value resolveBasicCombatAttack(const std::shared_ptr<CoreService>& service,
  ...
  event["text"] = (attackerType == "monster" ? "👹 " : attackerType == "npc" ? "🧑 " : "") +
                  attackerName + " 攻擊「" + target.name + "」：攻擊 " +
...
Json::Value startBattleFromSystem(const std::shared_ptr<CoreService>& service,
  ...
  service->database()->execSqlSync(R"SQL(
    UPDATE room_members rm SET
      ...
      spirit=LEAST(rm.spirit,GREATEST(0,cc.spirit)),
...
Json::Value CoreService::characterByUser(std::int64_t userId) const {
  ...
  character["max_hp"] = std::max<std::int64_t>(0, nonNegative(character["constitution"]) * 2);
  character["max_spirit"] = nonNegative(character["spirit"]);
  character["endurance"] = (nonNegative(character["constitution"]) +
                            nonNegative(character["spirit"])) / 2;
  character["will"] = nonNegative(character["spirit"]) * 8 / 10;
  character["sanity"] = nonNegative(character["spirit"]) * 12 / 10;
...
Json::Value CoreService::roomSnapshot(std::int64_t roomId) const {
  ...
  const auto members = database_->execSqlSync(R"SQL(
    SELECT jsonb_build_object(
      ...
      'role',rm.role,'hp',rm.hp,'max_hp',rm.max_hp,
      'spirit',rm.spirit,'max_spirit',rm.max_spirit,
      'current_sanity',rm.current_sanity,'max_sanity',rm.max_sanity,
      ...
      'current_map_node_id',rm.current_map_node_id,'offline_ai_enabled',rm.offline_ai_enabled,
      'offline_ai_active',rm.offline_ai_active,'last_seen_at',rm.last_seen_at,
      'username',u.username,'is_admin',u.is_admin,
      'character_id',crb.character_id,
      'class_resources',COALESCE(cc.class_resources,'{}'::jsonb),
      'avatar_url',COALESCE(cc.avatar_url,''),'pollution',COALESCE(cc.pollution,0)
    )::text AS json_text
    FROM room_members rm
  ...
  const auto npcs = database_->execSqlSync(R"SQL(
    SELECT jsonb_build_object(
      ...
      'role_name',nt.role_name,'image_url',nt.image_url,'merchant_enabled',nt.merchant_enabled,
      'merchant_title',nt.merchant_title,'status',rn.status,'combat_side',rn.combat_side,
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback) {
        runHandler(callback, [&] {
          ...
          const auto inserted = self->database()->execSqlSync(R"SQL(
            INSERT INTO monster_templates(name,category,rank,description,max_hp,attributes,config)
            VALUES($1,$2,$3,$4,$5,$6::jsonb,$7::jsonb) RETURNING id
          )SQL", name, category, rank, description, maxHp, compactJson(attributes),
              compactJson(config));
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback,
        ...
        runHandler(callback, [&] {
          ...
          const auto templates = self->database()->execSqlSync(
              "SELECT id,name,max_hp,attributes::text AS attributes_text "
          ...
          const auto fifth = std::max(nonNegative(attributes["great_way"]),
                                      std::max(nonNegative(attributes["faith"]),
                                               nonNegative(attributes["fifth"])));
          const auto inserted = self->database()->execSqlSync(R"SQL(
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback) {
        runHandler(callback, [&] {
          ...
          const auto rows = user.isAdmin
                                ? self->database()->execSqlSync(
                                      "SELECT row_to_json(r)::text AS json_text FROM rooms r "
                                      "ORDER BY r.id DESC")
                                : self->database()->execSqlSync(
                                      "SELECT row_to_json(r)::text AS json_text FROM rooms r "
                                      "JOIN room_members rm ON rm.room_id=r.id "
                                      "WHERE rm.user_id=$1 ORDER BY r.id DESC",
                                      user.id);
          Json::Value response;
  ...
  drogon::app().registerHandler(
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback,
        ...
        runHandler(callback, [&] {
          ...
          auto combat = resolveBasicCombatAttack(self, roomId, actorType, actorId,
                                                 candidates[picked]);
          self->addEvent(roomId, user.id, "combat_attack", combat["event_payload"]);
