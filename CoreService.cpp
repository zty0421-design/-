  ...
  if (type == "monster") {
    const auto rows = service->database()->execSqlSync(R"SQL(
      SELECT jsonb_build_object(
        ...
        'boss_phases',mt.boss_phases,'config',mt.config,'ai_behavior',mt.ai_behavior,
  ...
  if (type == "npc") {
    const auto rows = service->database()->execSqlSync(R"SQL(
      SELECT jsonb_build_object(
        ...
        'attributes',nt.attributes,'config',nt.config,'ai_behavior',nt.ai_behavior
      )::text AS json_text
...
void CoreService::initializeDatabase() {
  const std::vector<std::string> migrations = {
      ...
      R"SQL(CREATE TABLE IF NOT EXISTS character_cards (
        ...
        attribute_points BIGINT NOT NULL DEFAULT 50,
...
Json::Value CoreService::characterByUser(std::int64_t userId) const {
  ...
  try {
    ...
    auto targetData = [&](std::int64_t targetId, const std::string& expectedType) -> Json::Value {
      ...
      return parseJson(targetRows[0]["data_text"].as<std::string>(), Json::Value{Json::objectValue});
    };
...
Json::Value CoreService::roomSnapshot(std::int64_t roomId) const {
  ...
  const auto members = database_->execSqlSync(R"SQL(
    SELECT jsonb_build_object(
      ...
      'avatar_url',COALESCE(to_jsonb(cc)->>'avatar_url',''),
  ...
  try {
    ...
    SELECT jsonb_build_object(
      ...
      'role_name',nt.role_name,'image_url',nt.image_url,
  ...
  const auto events = database_->execSqlSync(R"SQL(
    SELECT jsonb_build_object(
      ...
      'created_at',e.created_at,'username',u.username,'character_name',c.name
    )::text AS json_text
    FROM (
      SELECT * FROM events WHERE room_id=$1 ORDER BY id DESC LIMIT 100
    ) e
    LEFT JOIN users u ON u.id=e.user_id
    LEFT JOIN character_cards c ON c.user_id=e.user_id
    ORDER BY e.id ASC
  )SQL", roomId);
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback) {
        runHandler(callback, [&] {
          ...
          const auto newPoints = body.isMember("attribute_points")
                                     ...
                                     : oldPoints;
          const auto tier = std::clamp<std::int64_t>(nonNegative(current["tier"], 1), 1, 10);
          const std::array<std::int64_t, 10> caps = {50, 100, 180, 320, 600,
                                                     1100, 2000, 3600, 6500, 10000};
          for (std::size_t index = 0; index < fields.size(); ++index) {
            const auto oldValue = nonNegative(current[fields[index]]);
            attributes[index] = body.isMember(fields[index])
                                    ? nonNegative(body[fields[index]])
                                    : oldValue;
            if (attributes[index] > caps[static_cast<std::size_t>(tier - 1)]) {
              throw ApiError(drogon::k400BadRequest,
                             "目前階級的單項屬性超過上限");
            }
            oldTotal += oldValue;
            newTotal += attributes[index];
          }
          if (newTotal + newPoints != oldTotal + oldPoints) {
            throw ApiError(
                drogon::k400BadRequest,
                "屬性點數不正確，力量、敏捷、體質、精神與剩餘屬性點的總和必須保持不變");
          }
          ...
          if (body.isMember(allowedFifth)) {
            ...
            if (nextMax < currentValue) {
              throw ApiError(drogon::k400BadRequest,
                             "新上限不能低於目前剩餘值；請先由技能、效果或 DM 降低目前值");
            }
            if (west)
              faith = nextMax;
            else
  ...
  drogon::app().registerHandler(
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback) {
        runHandler(callback, [&] {
          ...
          const auto maxHp = std::max<std::int64_t>(1, nonNegative(body["max_hp"], 100));
          ...
          const auto inserted = self->database()->execSqlSync(R"SQL(
            INSERT INTO monster_templates(name,category,rank,description,public_hint,image_url,max_hp,investigation_dc,
              attributes,skills,passive_skills,great_way,boss_phases,drops,ai_level,ai_behavior,config,element_affinity,effects)
            VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10::jsonb,$11::jsonb,$12::jsonb,$13::jsonb,$14::jsonb,$15,$16,$17::jsonb,$18::jsonb,$19::jsonb) RETURNING id
              ...
              compactJson(drops), aiLevel, aiBehavior, compactJson(config), compactJson(elementAffinity), compactJson(effects));
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback) {
        runHandler(callback, [&] {
          ...
          self->database()->execSqlSync(
              ...
              "VALUES($1,$2,'gm',$3) ON CONFLICT(room_id,user_id) DO UPDATE SET role='gm'",
  ...
  drogon::app().registerHandler(
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback) {
        runHandler(callback, [&] {
          ...
          const auto character = self->characterByUser(user.id);
          if (character.isNull()) throw ApiError(drogon::k409Conflict, "請先建立角色卡");
          const auto displayName = clipped(stringValue(character["name"], user.username), 30);
          self->database()->execSqlSync(
              "INSERT INTO room_members(room_id,user_id,role,display_name) "
  ...
  drogon::app().registerHandler(
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback,
        ...
        runHandler(callback, [&] {
          ...
          const auto roomId = parseRoomId(roomIdText);
          const auto updated = self->database()->execSqlSync(
              "UPDATE rooms SET status='active',saved=FALSE,battle_active=FALSE,round=1,"
              ...
              "closed_at=NULL WHERE id=$1 RETURNING id",
              roomId);
  ...
  drogon::app().registerHandler(
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback,
        ...
        runHandler(callback, [&] {
          ...
          auto combat = selectedSkill.isObject()
              ? resolveBasicCombatAttack(self, roomId, actorType, actorId, target, selectedSkill)
              : resolveBasicCombatAttack(self, roomId, actorType, actorId, target);
          self->addEvent(roomId, user.id, selectedSkill.isObject() ? "combat_skill" : "combat_attack", combat["event_payload"]);
          Json::Value combatRuleEvent = combat; combatRuleEvent["source"] = selectedSkill.isObject() ? "ai_monster_skill" : "ai_basic_attack"; combatRuleEvent["ai_actor_type"] = actorType;
          if (selectedSkill.isObject()) {
            evaluateRuleEvent(self, roomId, user.id, "COMBAT_SKILL_USED", combatRuleEvent, false);
            const auto resourceCost = std::max<std::int64_t>(0, integerValue(selectedSkill["resource_cost"]));
          ...
          combat.removeMember("event_payload");
          if (selectedSkill.isObject()) {
            const auto actionCost = std::clamp<std::int64_t>(integerValue(selectedSkill["action_cost"], 1), 1, 5);
            if (actionCost > 1) adjustCombatActions(self, roomId, actorType, actorId, -(actionCost - 1));
          }
          advanceCombatAction(self, roomId, actorType, actorId, user.id,
                              selectedSkill.isObject() ? "AI 怪物技能" : "AI 基本攻擊");
          RoomSocket::broadcastSnapshotForRoom(roomId);
          Json::Value response;
          response["ok"] = true;
          response["combat"] = combat;
          response["ai_result"]["skill_name"] = selectedSkill.isObject() ? stringValue(selectedSkill["name"], "怪物技能") : "基本攻擊";
          if (selectedSkill.isObject()) response["ai_result"]["skill"] = selectedSkill;
