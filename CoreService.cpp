...
bool endBattleIfResolved(const std::shared_ptr<CoreService>& service,
  ...
  finishBattle(service, roomId, eventUserId,
               enemy ? "⚠️ 戰鬥結束：玩家／友方已無可戰鬥單位。"
                     : "🏁 戰鬥結束：敵方已無可戰鬥單位。");
  return true;
}
...
std::string combatAttributeLabel(const std::string& key) {
...
Json::Value CoreService::roomSnapshot(std::int64_t roomId) const {
  ...
  const auto members = database_->execSqlSync(R"SQL(
    SELECT jsonb_build_object(
      ...
      'pollution',COALESCE(cc.pollution,0)
    )::text AS json_text
    FROM room_members rm
  ...
  const auto npcs = database_->execSqlSync(R"SQL(
    SELECT jsonb_build_object(
      ...
      'role_name',nt.role_name,'status',rn.status,'combat_side',rn.combat_side,
...
void CoreService::addEvent(std::int64_t roomId, std::int64_t userId,
  ...
  database_->execSqlSync(
      "INSERT INTO events(room_id,user_id,type,payload) VALUES($1,$2,$3,$4::jsonb)",
