...
void RoomSocket::handleNewMessage(
  ...
  try {
    ...
    if (event == "room:enter") {
      ...
      sendEvent(connection, "room:snapshot", service->roomSnapshot(*roomId));
      return;
    }
...
void RoomSocket::handleConnectionClosed(
  ...
  if (oldRoom) broadcastPresence(*oldRoom);
}
...
void RoomSocket::broadcastSnapshotForRoom(std::int64_t roomId) {
  ...
  if (!service) return;
  try {
    broadcastEvent(roomId, "room:snapshot", service->roomSnapshot(roomId));
  } catch (const std::exception& error) {
    LOG_ERROR << "snapshot broadcast failed: " << error.what();
  }
}
