# TRPG Online C++ 52-cpp.13 Render 編譯 Hotfix 自我檢查報告

## 本次修正

- 修正 `LegacyCompat.cpp` 隊伍資料頁的兩個 C++ 編譯錯誤。
- `personal_visions`：`selectRows1(...)` → `selectRows2(...)`。
- `room_team_invites`：`selectRows1(...)` → `selectRows2(...)`。
- 完整掃描 `LegacyCompat.cpp`：`selectRows` / `selectRows1` / `selectRows2` 的呼叫參數數量目前全部一致。
- `release_check.py` 已加入同類錯誤的自動回歸檢查。
- 沒有變更資料庫 schema、既有 244/244 v39 API、頭像上傳、規則戰鬥或其他遊戲功能。
...
PASS SQL selectRows helper 參數數量一致（避免 Render too many arguments）
...
SUMMARY pass=84 fail=0 warn=2
```

## 尚需 Render 驗證

目前工作容器沒有完整 Drogon headers，也沒有真實 `DATABASE_URL`，因此完整 C++/Drogon 連結與 PostgreSQL 端到端仍交由 Render 驗證。這次 Render 已明確定位到 `selectRows1` 參數數量錯誤，本包已針對該錯誤修正。
