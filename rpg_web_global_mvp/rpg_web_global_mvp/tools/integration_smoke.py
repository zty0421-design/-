...
def run(args: argparse.Namespace) -> None:
    ...
    require(version_info.get("version") == "65-cpp.26.15", "unexpected server version")
    ...
    require(status.get("version") == "65-cpp.26.15", "unexpected C++ version")
    ...
    # 65-cpp.26.5: task creation must normalize empty/zero nullable foreign keys instead of violating PostgreSQL FKs.
    task_smoke = api.request(
        "POST", "/api/admin/tasks", expected=200, token=admin_token,
        payload={
            "title": "C++任務建立修正" + secrets.token_hex(2),
            "task_type": "public",
            "target_user_id": 0,
            "profession_id": 0,
            "required_faction_id": 0,
            "min_tier": 1,
            "description": "65-cpp.26.5 task nullable FK regression",
            "requirements": "",
            "rewards": {},
        },
    ).body["task"]
    require(task_smoke.get("target_user_id") is None, "public task target_user_id was not normalized to null")
    require(task_smoke.get("profession_id") is None, "non-profession task profession_id was not normalized to null")
    task_smoke_id = int(task_smoke["id"])
    api.request("DELETE", f"/api/admin/tasks/{task_smoke_id}", expected=200, token=admin_token)
    passed("task creation normalizes nullable foreign keys")
    ...
    granted_attributes = api.request(
        "POST", f"/api/admin/players/{player_id}/attribute-points/grant", expected=200, token=admin_token,
        payload={"amount": 7},
    ).body
    granted_character = granted_attributes["character"]
    require(int(granted_attributes.get("granted", 0)) == 7, "DM free attribute grant amount mismatch")
    require(int(granted_character.get("attribute_tier_budget", -1)) == 50, "tier fixed budget changed after DM grant")
    require(int(granted_character.get("attribute_bonus", -1)) == 7, "DM bonus attribute budget missing")
    require(int(granted_character.get("attribute_budget", -1)) == 57, "effective attribute budget did not include DM grant")
    require(int(granted_character.get("attribute_points", -1)) == 57, "DM grant did not increase remaining free points")
    passed("DM grants persistent extra free attribute points")
    ...
    require(int(character.get("attribute_budget", -1)) == 57, "tier-1 effective attribute budget mismatch after DM grant")
    require(int(character.get("attribute_spent", -1)) == 40, "five-base-stat spent total mismatch")
    require(int(character.get("attribute_points", -1)) == 17, "remaining base attribute points mismatch after DM grant")
    ...
    # 65-cpp.26.15: core content can be edited repeatedly, and hidden recipes can be discovered by material experiments.
    edit_suffix = secrets.token_hex(3)
    editable_item_name = "smoke-edit-item-" + edit_suffix
    editable_item = api.request(
        "POST", "/api/admin/item-templates", expected=200, token=admin_token,
        payload={"name": editable_item_name, "category": "測試材料", "item_type": "normal", "rank": "G", "description": "v1", "effects": {}},
    ).body["item"]
    editable_item_id = int(editable_item["id"])
    api.request("PATCH", f"/api/admin/item-templates/{editable_item_id}", expected=200, token=admin_token, payload={"description": "v2"})
    api.request("PATCH", f"/api/admin/item-templates/{editable_item_id}", expected=200, token=admin_token, payload={"rank": "F", "description": "v3"})
    item_rows = api.request("GET", "/api/admin/item-templates", expected=200, token=admin_token).body["items"]
    edited_item = next(x for x in item_rows if int(x.get("id", 0)) == editable_item_id)
    require(edited_item.get("rank") == "F" and edited_item.get("description") == "v3", "item template repeated PATCH failed")

    editable_skill_name = "smoke-edit-skill-" + edit_suffix
    editable_skill = api.request(
        "POST", "/api/admin/skill-templates", expected=200, token=admin_token,
        payload={"name": editable_skill_name, "category": "測試", "skill_type": "active", "level": 1, "rank": "G", "description": "v1", "data": {}},
    ).body["skill"]
    editable_skill_id = int(editable_skill["id"])
    api.request("PATCH", f"/api/admin/skill-templates/{editable_skill_id}", expected=200, token=admin_token, payload={"description": "v2"})
    api.request("PATCH", f"/api/admin/skill-templates/{editable_skill_id}", expected=200, token=admin_token, payload={"rank": "E", "description": "v3"})
    skill_rows = api.request("GET", "/api/admin/skill-templates", expected=200, token=admin_token).body["skills"]
    edited_skill = next(x for x in skill_rows if int(x.get("id", 0)) == editable_skill_id)
    require(edited_skill.get("rank") == "E" and edited_skill.get("description") == "v3", "skill template repeated PATCH failed")

    experiment_material_name = "smoke-mat-" + edit_suffix
    experiment_output_name = "smoke-discovery-output-" + edit_suffix
    editable_recipe_name = "smoke-hidden-recipe-" + edit_suffix
    editable_recipe = api.request(
        "POST", "/api/admin/recipes", expected=200, token=admin_token,
        payload={
            "name": editable_recipe_name,
            "recipe_type": "一般",
            "rank": "F",
            "description": "v1",
            "materials": [{"name": experiment_material_name, "category": "試驗材料", "quantity": 1}],
            "output": {"type": "item", "name": experiment_output_name, "category": "測試成品", "quantity": 1},
            "config": {"success_rate_mode": "rank", "discoverable_by_experiment": True, "hidden_until_discovered": True, "failure_consumes": True, "access": "public", "currency_type": "none", "currency_cost": 0},
        },
    ).body["recipe"]
    editable_recipe_id = int(editable_recipe["id"])
    api.request("PATCH", f"/api/admin/recipes/{editable_recipe_id}", expected=200, token=admin_token, payload={"description": "v2"})
    api.request("PATCH", f"/api/admin/recipes/{editable_recipe_id}", expected=200, token=admin_token, payload={"rank": "G", "description": "v3"})
    recipe_rows = api.request("GET", "/api/admin/recipes", expected=200, token=admin_token).body["recipes"]
    edited_recipe = next(x for x in recipe_rows if int(x.get("id", 0)) == editable_recipe_id)
    require(edited_recipe.get("rank") == "G" and edited_recipe.get("description") == "v3", "recipe repeated PATCH failed")
    require(int((edited_recipe.get("materials") or [{}])[0].get("quantity", 0)) == 1, "partial recipe PATCH reset materials")

    visible_before = api.request("GET", "/api/recipes", expected=200, token=player_token).body["recipes"]
    require(not any(int(x.get("id", 0)) == editable_recipe_id for x in visible_before), "hidden experimental recipe leaked before discovery")
    inventory_for_experiment = list(character.get("inventory") or [])
    inventory_for_experiment.append({"name": experiment_material_name, "category": "試驗材料", "quantity": 1})
    character = api.request(
        "PATCH", f"/api/admin/characters/{character['id']}", expected=200, token=admin_token,
        payload={"inventory": inventory_for_experiment},
    ).body["character"]
    experiment = api.request(
        "POST", "/api/recipes/experiment", expected=200, token=player_token,
        payload={"materials": [{"name": experiment_material_name, "category": "試驗材料", "quantity": 1}]},
    ).body
    require(experiment.get("matched") is True and experiment.get("success") is True, "G-rank recipe experiment should succeed at the default 100% base rate")
    require(experiment.get("newly_discovered") is True, "successful experiment did not discover hidden recipe")
    require(int(experiment.get("base_success_rate", -1)) == 100, "G-rank crafting base success rate mismatch")
    crafted_inventory = (experiment.get("character") or {}).get("inventory") or []
    crafted_output = next((x for x in crafted_inventory if isinstance(x, dict) and x.get("name") == experiment_output_name), None)
    require(crafted_output is not None and crafted_output.get("rank") == "G", "crafted output did not inherit recipe rank")
    visible_after = api.request("GET", "/api/recipes", expected=200, token=player_token).body["recipes"]
    require(any(int(x.get("id", 0)) == editable_recipe_id for x in visible_after), "discovered recipe did not become visible")
    character = experiment.get("character") or character

    api.request("DELETE", f"/api/admin/recipes/{editable_recipe_id}", expected=200, token=admin_token)
    api.request("DELETE", f"/api/admin/item-templates/{editable_item_id}", expected=200, token=admin_token)
    api.request("DELETE", f"/api/admin/skill-templates/{editable_skill_id}", expected=200, token=admin_token)
    passed("repeated content edits and experimental recipe discovery")
    ...
    # 65-cpp.26.9: a partial DM PATCH must not silently reset rank or other omitted fields.
    api.request("PATCH", f"/api/admin/shop/{bulk_shop_item['id']}", expected=200, token=admin_token, payload={"rank":"B"})
    api.request("PATCH", f"/api/admin/shop/{bulk_shop_item['id']}", expected=200, token=admin_token, payload={"active":False})
    patched_shop = api.request("GET", "/api/admin/shop", expected=200, token=admin_token).body["items"]
    patched_shop_item = next(x for x in patched_shop if int(x.get("id",0)) == int(bulk_shop_item["id"]))
    require(patched_shop_item.get("rank") == "B", "partial shop PATCH reset rank")
    require(patched_shop_item.get("active") is False, "partial shop PATCH did not update active state")
    passed("shop partial PATCH preserves rank")
    ...
    # 65-cpp.26.2: portrait is independent from avatar and can also be overridden by DM.
    portrait_url = "https://example.invalid/cpp-smoke-portrait.png"
    portrait_character = api.request(
        "PATCH", "/api/me/portrait", expected=200, token=player_token,
        payload={"portrait_url": portrait_url},
    ).body["character"]
    require(portrait_character.get("portrait_url") == portrait_url, "player portrait was not persisted")
    dm_portrait_url = "https://example.invalid/cpp-smoke-portrait-by-dm.png"
    dm_portrait_character = api.request(
        "PATCH", f"/api/admin/players/{me['user']['id']}/portrait", expected=200, token=admin_token,
        payload={"portrait_url": dm_portrait_url},
    ).body["character"]
    require(dm_portrait_character.get("portrait_url") == dm_portrait_url, "DM could not update another player's portrait")
    refreshed_portrait = api.request("GET", "/api/character", expected=200, token=player_token).body["character"]
    require(refreshed_portrait.get("portrait_url") == dm_portrait_url, "DM portrait update was not visible to the player")
    passed("player portrait and DM portrait override")

    # 65-cpp.26.4: multiple player/NPC portraits can be named, switched and safely deleted.
    multi_a_url = "https://example.invalid/cpp-smoke-portrait-normal.png"
    multi_b_url = "https://example.invalid/cpp-smoke-portrait-battle.png"
    multi_a = api.request(
        "POST", "/api/me/portraits", expected=200, token=player_token,
        payload={"label": "普通", "portrait_url": multi_a_url, "activate": True, "is_default": True},
    ).body
    multi_a_id = int(multi_a["id"])
    multi_b = api.request(
        "POST", "/api/me/portraits", expected=200, token=player_token,
        payload={"label": "戰鬥", "portrait_url": multi_b_url, "activate": True},
    ).body
    multi_b_id = int(multi_b["id"])
    player_portraits = api.request("GET", "/api/me/portraits", expected=200, token=player_token).body["portraits"]
    require(any(int(x.get("id", 0)) == multi_b_id and x.get("is_active") for x in player_portraits), "second player portrait did not become active")
    switched = api.request(
        "PATCH", f"/api/me/portraits/{multi_a_id}", expected=200, token=player_token, payload={"activate": True},
    ).body
    require(switched["character"].get("portrait_url") == multi_a_url, "player portrait switch did not update character portrait_url")
    deleted = api.request("DELETE", f"/api/me/portraits/{multi_a_id}", expected=200, token=player_token).body
    require(deleted["character"].get("portrait_url") == multi_b_url, "deleting active portrait did not fall back to remaining portrait")

    npc_variant = api.request(
        "POST", "/api/admin/npc-templates", expected=200, token=admin_token,
        payload={"name": "C++多立繪NPC" + secrets.token_hex(2), "role_name": "整合測試", "max_hp": 100},
    ).body["npc_template"]
    npc_variant_id = int(npc_variant["id"])
    npc_portrait = api.request(
        "POST", f"/api/admin/npc-templates/{npc_variant_id}/portraits", expected=200, token=admin_token,
        payload={"label": "普通", "portrait_url": "https://example.invalid/cpp-smoke-npc-portrait.png", "activate": True},
    ).body
    npc_portrait_id = int(npc_portrait["id"])
    npc_portraits = api.request(
        "GET", f"/api/admin/npc-templates/{npc_variant_id}/portraits", expected=200, token=admin_token,
    ).body["portraits"]
    require(any(int(x.get("id", 0)) == npc_portrait_id and x.get("is_active") for x in npc_portraits), "NPC portrait variant did not become active")
    api.request("DELETE", f"/api/admin/npc-portrait-variants/{npc_portrait_id}", expected=200, token=admin_token)
    api.request("DELETE", f"/api/admin/npc-templates/{npc_variant_id}", expected=200, token=admin_token)
    passed("multi portrait player and NPC lifecycle")
    ...
    require(int(ascended_character.get("attribute_budget", -1)) == 107, "tier 2 effective budget must preserve the +7 DM bonus")
    require(int(ascended_character.get("attribute_points", -1)) == 67,
            "tier 2 should preserve 40 allocated points, +7 DM bonus, and expose 67 remaining")
    ...
    passed("ascension ritual advances big tier and preserves DM extra attribute budget")
    ...
    portrait_upload = request_json(base_url, "/api/me/portrait/upload", token=player_token, method="POST", body={"image_base64": tiny_png})
    require(str(portrait_upload.get("portrait_url", "")).startswith("/api/portrait-images/"), "player portrait upload did not return internal image URL")
    portrait_get = requests.get(base_url + portrait_upload["portrait_url"], timeout=20)
    require(portrait_get.status_code == 200 and portrait_get.headers.get("content-type", "").startswith("image/png"), "uploaded portrait image endpoint failed")
    passed("portrait upload persistence")
    ...
    # 64-cpp.25.4：說話與行動共用 /actions；說話會產生 CHARACTER_SPOKE / SOUND_EMITTED，但不需要戰鬥行動。
    speech = api.request(
        "POST", f"/api/rooms/{room_id}/actions", expected=200, token=player_token,
        payload={"interaction_type": "speech", "text": "悄聲測試", "volume": "whisper"},
    ).body
    require(speech.get("ok") is True and speech.get("interaction_type") == "speech", "speech interaction receipt missing")
    require(int(speech.get("action", {}).get("noise", -1)) == 3, "speech volume did not map to whisper noise")
    require(
        isinstance(speech.get("snapshot"), dict)
        and event_exists(speech["snapshot"], "speech", "text", "悄聲測試"),
        "speech interaction did not persist",
    )

    wait = api.request(
        "POST", f"/api/rooms/{room_id}/actions", expected=200, token=player_token,
        payload={"interaction_type": "wait"},
    ).body
    require(wait.get("ok") is True and wait.get("interaction_type") == "wait", "wait interaction receipt missing")
    require(
        isinstance(wait.get("snapshot"), dict)
        and event_exists(wait["snapshot"], "wait", "text", "待機"),
        "wait interaction did not persist",
    )
    passed("unified speech, action and wait interactions")
    ...
    if not args.skip_websocket:
        ...
        try:
            ...
            websocket.wait_for(
                "room:snapshot",
                lambda data: event_exists(data, "speech", "text", "C++ WebSocket 測試"),
            )
            websocket.send_event(
            ...
            websocket.close()
        passed("native WebSocket ticket handshake, enter, legacy speech and dice")
