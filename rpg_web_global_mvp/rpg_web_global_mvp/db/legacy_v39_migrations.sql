...
CREATE TABLE IF NOT EXISTS dungeon_submissions (
      id BIGSERIAL PRIMARY KEY,
      user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      title VARCHAR(160) NOT NULL,
      submission_type VARCHAR(30) NOT NULL DEFAULT 'suggestion',
      concept TEXT NOT NULL DEFAULT '',
      recommended_level VARCHAR(20) NOT NULL DEFAULT '',
...
-- [73-cpp.34.25.33 紙本第十一批] 遊戲群：跨房間的持久群組、成員與群組留言。
...
CREATE TABLE IF NOT EXISTS game_group_members (
      group_id BIGINT NOT NULL REFERENCES game_groups(id) ON DELETE CASCADE,
      user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS game_group_messages (
      id BIGSERIAL PRIMARY KEY,
      group_id BIGINT NOT NULL REFERENCES game_groups(id) ON DELETE CASCADE,
      user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_game_group_members_user ON game_group_members(user_id,group_id);
CREATE INDEX IF NOT EXISTS idx_game_group_messages_group_time ON game_group_messages(group_id,created_at DESC);
...
-- [73-cpp.34.25.26] 諸神圖鑑／神明 AI。使用 ALTER 兼容既有 PostgreSQL。
ALTER TABLE faith_deities ADD COLUMN IF NOT EXISTS true_name VARCHAR(180) NOT NULL DEFAULT '';
ALTER TABLE faith_deities ADD COLUMN IF NOT EXISTS aliases JSONB NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE faith_deities ADD COLUMN IF NOT EXISTS portrait_url TEXT NOT NULL DEFAULT '';
ALTER TABLE faith_deities ADD COLUMN IF NOT EXISTS lore TEXT NOT NULL DEFAULT '';
ALTER TABLE faith_deities ADD COLUMN IF NOT EXISTS ai_config JSONB NOT NULL DEFAULT '{"enabled":false,"response_chance":35,"tone":"solemn","devotion_gain":1,"favor_gain_chance":10,"response_lines":[],"silent_text":"祈禱落入寂靜，沒有得到明確回應。"}'::jsonb;
...
-- =========================================================
-- 73-cpp.34.25.23：紙本清單第一批｜神秘學／禁忌知識與代價／特殊職業道具／商店多分類
-- =========================================================
ALTER TABLE character_cards ADD COLUMN IF NOT EXISTS mysticism_points BIGINT NOT NULL DEFAULT 0;
ALTER TABLE character_cards ADD COLUMN IF NOT EXISTS forbidden_knowledge JSONB NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE character_cards ADD COLUMN IF NOT EXISTS forbidden_cost BIGINT NOT NULL DEFAULT 0;

CREATE TABLE IF NOT EXISTS mysticism_studies (
  id BIGSERIAL PRIMARY KEY,
  name VARCHAR(120) NOT NULL UNIQUE,
  category VARCHAR(80) NOT NULL DEFAULT '一般神秘學',
  rank VARCHAR(10) NOT NULL DEFAULT 'G',
  description TEXT NOT NULL DEFAULT '',
  point_cost BIGINT NOT NULL DEFAULT 1,
  prerequisite_ids JSONB NOT NULL DEFAULT '[]'::jsonb,
  required_item VARCHAR(180) NOT NULL DEFAULT '',
  consume_item BOOLEAN NOT NULL DEFAULT FALSE,
  forbidden BOOLEAN NOT NULL DEFAULT FALSE,
  forbidden_cost BIGINT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS character_mysticisms (
  user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  mysticism_id BIGINT NOT NULL REFERENCES mysticism_studies(id) ON DELETE CASCADE,
  learned_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY(user_id,mysticism_id)
);
CREATE INDEX IF NOT EXISTS idx_mysticism_studies_active ON mysticism_studies(active,category,rank,name,id);
CREATE INDEX IF NOT EXISTS idx_character_mysticisms_user ON character_mysticisms(user_id);

ALTER TABLE item_templates ADD COLUMN IF NOT EXISTS study_domains JSONB NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE item_templates ADD COLUMN IF NOT EXISTS profession_requirements JSONB NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE shop_items ADD COLUMN IF NOT EXISTS category_ids JSONB NOT NULL DEFAULT '[]'::jsonb;


-- =========================================================
-- [功能備註｜73-cpp.34.25.29 任務期限／優先度 + 探索歷史]
-- =========================================================
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS priority VARCHAR(20) NOT NULL DEFAULT 'normal';
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS recommended_party_size INTEGER NOT NULL DEFAULT 1;
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ;
CREATE INDEX IF NOT EXISTS idx_tasks_priority_deadline ON tasks(status,priority,expires_at);

CREATE TABLE IF NOT EXISTS room_exploration_records (
  id BIGSERIAL PRIMARY KEY,
  room_id BIGINT NOT NULL REFERENCES rooms(id) ON DELETE CASCADE,
  user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  map_node_id BIGINT REFERENCES room_map_nodes(id) ON DELETE SET NULL,
  mode VARCHAR(30) NOT NULL DEFAULT 'environment',
  target_name VARCHAR(180) NOT NULL DEFAULT '',
  skill_name VARCHAR(180) NOT NULL DEFAULT '',
  roll INTEGER NOT NULL DEFAULT 0,
  success_rate INTEGER NOT NULL DEFAULT 0,
  success BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE INDEX IF NOT EXISTS idx_room_exploration_records_user ON room_exploration_records(room_id,user_id,created_at DESC);


-- =========================================================
-- [功能備註｜73-cpp.34.25.30 世界異常設定 + 戰鬥深化]
-- =========================================================
CREATE TABLE IF NOT EXISTS room_anomaly_profiles (
  room_id BIGINT PRIMARY KEY REFERENCES rooms(id) ON DELETE CASCADE,
  enabled BOOLEAN NOT NULL DEFAULT FALSE,
  name VARCHAR(160) NOT NULL DEFAULT '世界異常',
  intensity INTEGER NOT NULL DEFAULT 0 CHECK (intensity BETWEEN 0 AND 100),
  public_title VARCHAR(160) NOT NULL DEFAULT '',
  public_description TEXT NOT NULL DEFAULT '',
  tags JSONB NOT NULL DEFAULT '[]'::jsonb,
  combat_notes TEXT NOT NULL DEFAULT '',
...
-- 73-cpp.34.25.34｜紙本清單第十二批：特殊職業道具深化
ALTER TABLE item_templates ADD COLUMN IF NOT EXISTS profession_requirement_mode VARCHAR(16) NOT NULL DEFAULT 'any';
