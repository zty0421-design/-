...
// Forward declarations for helpers used by early task/NPC compatibility code.
std::string trimmed(std::string value);
int randomInt(int low, int high);


// [功能備註｜73-cpp.34.22 結構化任務步驟] 任務不再只有一段文字 requirements；
// 每個步驟都有固定 type / id / 目標與數量，C++ 會在建立與進度更新時驗證。
Json::Value normalizeTaskSteps(const Json::Value& raw) {
  Json::Value out{Json::arrayValue};
  if (!raw.isArray()) return out;
  static const std::set<std::string> allowed = {
      "custom", "investigate", "reach_location", "interact_npc", "escort_npc",
      "cooperative_seal", "place_anchor", "perform_ritual", "defeat_target", "collect",
      "boss_phase", "rule_event"};
  std::size_t index = 0;
  for (const auto& source : raw) {
    if (!source.isObject() || index >= 64) break;
    Json::Value step{Json::objectValue};
    auto id = trimmed(stringValue(source["id"]));
    if (id.empty() || !std::regex_match(id, std::regex("^[A-Za-z0-9_-]{1,80}$")))
      id = "step_" + std::to_string(index + 1);
    auto type = trimmed(stringValue(source["type"], "custom"));
    if (!allowed.count(type)) type = "custom";
    auto title = trimmed(stringValue(source["title"]));
    if (title.size() > 160) title.resize(160);
    if (title.empty()) {
      const std::unordered_map<std::string,std::string> labels = {
          {"custom","完成任務步驟"},{"investigate","完成調查"},{"reach_location","抵達指定地點"},
          {"interact_npc","與指定 NPC 互動"},{"escort_npc","護送指定 NPC"},
          {"cooperative_seal","與 NPC 協力封印"},{"place_anchor","安插錨點"},
          {"perform_ritual","完成指定儀式"},{"defeat_target","擊敗指定目標"},{"collect","收集指定物品"},
          {"boss_phase","等待 Boss 進入指定階段"},{"rule_event","等待規則事件"}};
      title = labels.at(type);
    }
    auto description = trimmed(stringValue(source["description"]));
    if (description.size() > 1200) description.resize(1200);
    auto location = trimmed(stringValue(source["location_name"]));
    if (location.size() > 120) location.resize(120);
    auto anchorName = trimmed(stringValue(source["anchor_name"], "任務錨點"));
    if (anchorName.size() > 120) anchorName.resize(120);
    auto targetName = trimmed(stringValue(source["target_name"]));
    if (targetName.size() > 160) targetName.resize(160);
    auto itemName = trimmed(stringValue(source["item_name"]));
    if (itemName.size() > 120) itemName.resize(120);
    step["id"] = id;
    step["type"] = type;
    step["title"] = title;
    step["description"] = description;
    step["required"] = Json::Int64(std::clamp<std::int64_t>(intValue(source["required"], 1), 1, 999));
    step["npc_template_id"] = Json::Int64(std::max<std::int64_t>(0, intValue(source["npc_template_id"])));
    step["monster_template_id"] = Json::Int64(std::max<std::int64_t>(0, intValue(source["monster_template_id"])));
    step["ritual_id"] = Json::Int64(std::max<std::int64_t>(0, intValue(source["ritual_id"])));
    step["map_node_id"] = Json::Int64(std::max<std::int64_t>(0, intValue(source["map_node_id"])));
    step["boss_phase"] = Json::Int64(std::clamp<std::int64_t>(intValue(source["boss_phase"], 1), 1, 99));
    auto eventName = trimmed(stringValue(source["event_name"]));
    if (eventName.size() > 120) eventName.resize(120);
    step["event_name"] = eventName;
    step["required_anchor_count"] = Json::Int64(std::clamp<std::int64_t>(intValue(source["required_anchor_count"]), 0, 999));
    step["requires_previous"] = source.get("requires_previous", index > 0).asBool();
    step["auto_progress"] = source.get("auto_progress", type == "reach_location" || type == "perform_ritual" || type == "boss_phase" || type == "rule_event").asBool();
    Json::Value dependencies{Json::arrayValue};
    if (source["depends_on_step_ids"].isArray()) {
      for (const auto& dep : source["depends_on_step_ids"]) {
        const auto key = trimmed(stringValue(dep));
        if (!key.empty() && key.size() <= 80 && std::regex_match(key, std::regex("^[A-Za-z0-9_-]{1,80}$"))) dependencies.append(key);
      }
    }
    step["depends_on_step_ids"] = dependencies;
    step["location_name"] = location;
    step["anchor_name"] = anchorName.empty() ? "任務錨點" : anchorName;
    step["target_name"] = targetName;
    step["item_name"] = itemName;
    step["requires_same_node"] = source.get("requires_same_node", true).asBool();
    step["unique_nodes"] = source.get("unique_nodes", type == "place_anchor").asBool();
    step["seal_target_on_complete"] = source.get("seal_target_on_complete", type == "cooperative_seal").asBool();
    step["player_can_progress"] = source.get("player_can_progress", true).asBool();
    auto progressScope = trimmed(stringValue(source["progress_scope"], "personal"));
    if (progressScope != "team") progressScope = "personal";
    step["progress_scope"] = progressScope;
    step["optional"] = source.get("optional", false).asBool();
    auto activationVariable = trimmed(stringValue(source["activation_variable"]));
    if (activationVariable.size() > 80) activationVariable.resize(80);
    step["activation_variable"] = activationVariable;
    auto activationOperator = trimmed(stringValue(source["activation_operator"], ">="));
    static const std::set<std::string> compareOps = {"==", "!=", ">", ">=", "<", "<="};
    if (!compareOps.count(activationOperator)) activationOperator = ">=";
    step["activation_operator"] = activationOperator;
    step["activation_value"] = Json::Int64(intValue(source["activation_value"]));
    auto changeVariable = trimmed(stringValue(source["variable_change_name"]));
    if (changeVariable.size() > 80) changeVariable.resize(80);
    step["variable_change_name"] = changeVariable;
    auto changeMode = trimmed(stringValue(source["variable_change_mode"], "add"));
    if (changeMode != "set") changeMode = "add";
    step["variable_change_mode"] = changeMode;
    step["variable_change_value"] = Json::Int64(intValue(source["variable_change_value"]));
    auto failureEvent = trimmed(stringValue(source["failure_event_name"]));
    if (failureEvent.size() > 120) failureEvent.resize(120);
    step["failure_event_name"] = failureEvent;
    out.append(step);
    ++index;
  }
  return out;
...
const Json::Value* findTaskStep(const Json::Value& steps, const std::string& stepId) {
  if (!steps.isArray()) return nullptr;
  for (const auto& step : steps) if (stringValue(step["id"]) == stepId) return &step;
  return nullptr;
}

// [功能備註｜73-cpp.34.25 任務分支／變數] 分支啟用、失敗條件與變數全部由 C++ 判定，前端只負責編輯。
bool taskCompareNumber(std::int64_t left, const std::string& op, std::int64_t right) {
...
void ensureTaskVariables(const Json::Value& task, Json::Value& progress) {
  if (!progress.isObject()) progress = Json::Value{Json::objectValue};
  if (!progress["variables"].isObject()) progress["variables"] = Json::Value{Json::objectValue};
  if (task["task_variables"].isObject()) {
    for (const auto& name : task["task_variables"].getMemberNames()) {
      if (!progress["variables"].isMember(name)) progress["variables"][name] = Json::Int64(intValue(task["task_variables"][name]));
    }
  }
}

bool taskStepActive(const Json::Value& step, const Json::Value& progress) {
  const auto name = trimmed(stringValue(step["activation_variable"]));
  if (name.empty()) return true;
  const auto current = intValue(progress["variables"][name]);
  return taskCompareNumber(current, stringValue(step["activation_operator"], ">="), intValue(step["activation_value"]));
}

bool applyTaskStepVariableChange(const Json::Value& step, Json::Value& progress) {
  const auto name = trimmed(stringValue(step["variable_change_name"]));
  if (name.empty()) return false;
  if (!progress["variables"].isObject()) progress["variables"] = Json::Value{Json::objectValue};
  const auto value = intValue(step["variable_change_value"]);
  const auto next = stringValue(step["variable_change_mode"], "add") == "set" ? value : intValue(progress["variables"][name]) + value;
  progress["variables"][name] = Json::Int64(next);
  return true;
}

Json::Value normalizeTaskVariables(const Json::Value& raw) {
  Json::Value out{Json::objectValue};
  if (!raw.isObject()) return out;
  std::size_t count = 0;
  for (const auto& name0 : raw.getMemberNames()) {
    auto name = trimmed(name0); if (name.empty() || name.size() > 80 || count >= 64) continue;
    out[name] = Json::Int64(std::clamp<std::int64_t>(intValue(raw[name0]), -1000000000LL, 1000000000LL)); ++count;
  }
  return out;
}

Json::Value normalizeTaskFailureConditions(const Json::Value& raw) {
  Json::Value out{Json::arrayValue}; if (!raw.isArray()) return out;
  std::size_t count = 0;
  for (const auto& src : raw) {
    if (!src.isObject() || count >= 32) break;
    Json::Value x{Json::objectValue}; auto type = trimmed(stringValue(src["type"], "event")); if (type != "variable") type = "event";
    x["type"] = type; auto message = trimmed(stringValue(src["message"], "任務失敗條件成立")); if (message.size() > 240) message.resize(240); x["message"] = message;
    if (type == "event") { auto e = trimmed(stringValue(src["event_name"])); if (e.size() > 120) e.resize(120); if (e.empty()) continue; x["event_name"] = e; }
    else { auto n = trimmed(stringValue(src["variable"])); if (n.size() > 80) n.resize(80); if (n.empty()) continue; auto op=trimmed(stringValue(src["operator"], ">=")); static const std::set<std::string> ops={"==","!=",">",">=","<","<="}; if(!ops.count(op))op=">="; x["variable"]=n; x["operator"]=op; x["value"]=Json::Int64(intValue(src["value"])); }
    out.append(x); ++count;
  }
  return out;
}

std::string taskFailureReason(const Json::Value& task, const Json::Value& progress, const std::string& eventName = {}) {
  if (!task["task_failure_conditions"].isArray()) return {};
  for (const auto& c : task["task_failure_conditions"]) {
    const auto type = stringValue(c["type"], "event");
    bool hit = false;
    if (type == "event") hit = !eventName.empty() && eventName == stringValue(c["event_name"]);
    else if (type == "variable") hit = taskCompareNumber(intValue(progress["variables"][stringValue(c["variable"])]), stringValue(c["operator"], ">="), intValue(c["value"]));
    if (hit) return stringValue(c["message"], "任務失敗條件成立");
  }
  return {};
}

bool taskStepsComplete(const Json::Value& steps, const Json::Value& progress) {
  if (!steps.isArray() || steps.empty()) return true;
  const auto stepProgress = progress["steps"].isObject() ? progress["steps"] : Json::Value{Json::objectValue};
  for (const auto& step : steps) {
    if (!taskStepActive(step, progress)) continue;
    if (step.get("optional", false).asBool()) continue;
    const auto id = stringValue(step["id"]);
    const auto required = std::max<std::int64_t>(1, intValue(step["required"], 1));
    if (id.empty()) continue;
    const auto current = std::max<std::int64_t>(0, intValue(stepProgress[id]["count"]));
    if (current < required && !stepProgress[id].get("completed", false).asBool()) return false;
  }
  return true;
}

bool taskStepDependenciesComplete(const Json::Value& steps, const Json::Value& progress, const Json::Value& step) {
  const auto id = stringValue(step["id"]);
  const auto stepProgress = progress["steps"].isObject() ? progress["steps"] : Json::Value{Json::objectValue};
  auto completed = [&](const std::string& key) {
    const auto* dep = findTaskStep(steps, key);
    if (!dep) return false;
    if (!taskStepActive(*dep, progress)) return true;
    const auto required = std::max<std::int64_t>(1, intValue((*dep)["required"], 1));
    return stepProgress[key].get("completed", false).asBool() || intValue(stepProgress[key]["count"]) >= required;
  };
  if (step.get("requires_previous", false).asBool()) {
    std::string previous;
    for (const auto& candidate : steps) {
      const auto cid = stringValue(candidate["id"]);
      if (cid == id) break;
      previous = cid;
    }
    if (!previous.empty() && !completed(previous)) return false;
  }
  if (step["depends_on_step_ids"].isArray()) {
    for (const auto& dep : step["depends_on_step_ids"]) {
      const auto key = stringValue(dep);
      if (!key.empty() && !completed(key)) return false;
    }
  }
  return true;
}
...
Json::Value advanceCharacterTier(Db db, std::int64_t userId, std::int64_t nextTier,
  ...
  const auto rows = db->execSqlSync(
      "SELECT tier,level,agility,strength,constitution,spirit,great_way,faith,faction,attribute_points FROM character_cards WHERE user_id=$1", userId);
  ...
  // [功能備註｜65-cpp.26.5 DM 自由屬性點] DM 額外給予的自由屬性點會形成額外額度，
  // 玩家花掉後仍屬於角色永久總額；登階時只增加新的大階固定額度，不會把 DM 額外額度洗掉。
  const auto currentFixedBudget = characterTierBudget(currentTier);
  const auto currentRemaining = std::max<std::int64_t>(0, rows[0]["attribute_points"].as<std::int64_t>());
  const auto currentTotalBudget = spent + currentRemaining;
  const auto bonusBudget = std::max<std::int64_t>(0, currentTotalBudget - currentFixedBudget);
  const auto newBudget = characterTierBudget(nextTier) + bonusBudget;
  if (spent > newBudget) throw ApiError(drogon::k409Conflict, "現有基礎屬性已超過下一階總額度，請先由 DM 修正角色卡");
  ...
  out["attribute_tier_budget"] = Json::Int64(characterTierBudget(nextTier));
  out["attribute_bonus"] = Json::Int64(bonusBudget);
...
// [功能備註｜73-cpp.34.7 NPC 自主交流]
// NPC 不再只等待玩家按「互動」：同場景的可對話 NPC 會觀察玩家的說話／行動，
// 依 AI 行為、AI 等級、自訂台詞與簡單情境判斷，自主在聊天室發言。
std::vector<std::int64_t> npcSpeechAudience(
    const std::shared_ptr<CoreService>& service,
    std::int64_t roomId,
    std::int64_t npcNodeId) {
  std::vector<std::int64_t> audience;
  const auto rows = service->database()->execSqlSync(R"SQL(
    SELECT rm.user_id,
           COALESCE(t.current_map_node_id,rm.current_map_node_id,r.current_map_node_id) AS node_id
  ...
  for (const auto& row : rows) {
    if (npcNodeId <= 0 || row["node_id"].isNull() || row["node_id"].as<std::int64_t>() == npcNodeId)
      audience.push_back(row["user_id"].as<std::int64_t>());
  }
  return audience;
}

std::vector<std::string> npcConfiguredLines(const Json::Value& config) {
  std::vector<std::string> lines;
  const auto& raw = config["ai_lines"];
  if (raw.isArray()) {
    for (const auto& item : raw) {
      auto line = trimmed(stringValue(item));
      if (!line.empty()) lines.push_back(std::move(line));
    }
  } else if (raw.isString()) {
    std::stringstream ss(raw.asString());
    std::string line;
...
bool containsAnyNpcCue(const std::string& text,
                       std::initializer_list<const char*> cues) {
  for (const auto* cue : cues) if (text.find(cue) != std::string::npos) return true;
  return false;
}

std::string npcJudgementLine(const std::string& behavior,
                             const std::string& interactionType,
                             const std::string& text,
                             const std::vector<std::string>& customLines) {
  // 自訂台詞優先，但遇到明確危險／調查／戰鬥情境時仍會產生「判斷」而不是純隨機念台詞。
  const bool danger = containsAnyNpcCue(text, {"危險","陷阱","爆炸","毒","詭異","怪物","敵人","追","逃"});
  const bool attack = containsAnyNpcCue(text, {"攻擊","砍","射","打","殺","施放","技能","戰鬥"});
  const bool investigate = containsAnyNpcCue(text, {"調查","搜索","搜尋","觀察","檢查","線索","判斷","怎麼看","你覺得"});
  const bool help = containsAnyNpcCue(text, {"幫忙","支援","一起","跟我","救","保護"});

  std::vector<std::string> contextual;
  if (danger) {
    if (behavior == "aggressive") contextual = {"有危險，但拖下去只會更糟。我建議先掌握主動。","我判斷風險已經出現，先找出威脅來源。"};
    else if (behavior == "defensive") contextual = {"先別往前。我判斷這裡有風險，至少先確認退路。","這裡不安全。先拉開距離，再決定下一步。"};
    else contextual = {"我的判斷是：先確認危險從哪裡來，再行動。","情況不太對，現在最重要的是先把未知因素縮小。"};
  } else if (attack) {
    if (behavior == "aggressive") contextual = {"可以打。我建議先處理最有威脅的目標。","如果要動手，就別讓對方有重新組織的時間。"};
    else if (behavior == "defensive") contextual = {"我不反對戰鬥，但先確認我們有退路。","先看對方的反應再出手，硬碰硬未必划算。"};
    else contextual = {"我的判斷是可以行動，但最好先確認目標和代價。","要出手的話，先決定誰負責壓制、誰留後手。"};
  } else if (investigate) {
    contextual = {"我會先看不合理的地方。通常真正的線索就在那些細節裡。","我的判斷是先把已知和猜測分開，這樣比較不容易被誤導。","先查能被驗證的部分。剩下的再當作假設。"};
  } else if (help) {
    contextual = {"可以，我會配合。你先說你打算做到哪一步。","我會跟上，但如果情況改變，我會自己判斷。","好。我負責補你沒注意到的地方。"};
  } else if (interactionType == "action") {
    if (behavior == "story") contextual = {"我記住這個選擇了。之後它可能會影響別的事情。","這一步看起來普通，但我覺得後面會有連鎖反應。"};
    else contextual = {"我看到了。先觀察結果，再決定要不要繼續。","這個做法可行，不過我會保留一條退路。","我暫時沒有反對，但接下來要注意周圍的變化。"};
  } else {
    if (behavior == "story") contextual = {"這句話我會記得。也許之後會變得重要。","聽起來像是某件事的開頭。"};
    else if (behavior == "aggressive") contextual = {"我懂你的意思。要做就快一點。","可以，但別把主動權讓出去。"};
    else if (behavior == "defensive") contextual = {"我聽到了。不過我建議先確認安全。","先保留餘地，我不想因為一句話就把退路堵死。"};
    else contextual = {"嗯，我的判斷是先看看情況怎麼變。","我聽到了。這件事還有幾個地方值得確認。"};
  }

  if (!contextual.empty()) return contextual[static_cast<std::size_t>(randomInt(0, static_cast<int>(contextual.size()-1)))];
  if (!customLines.empty()) return customLines[static_cast<std::size_t>(randomInt(0, static_cast<int>(customLines.size()-1)))];
  return "我在看。先別急著下結論。";
}

Json::Value runAutonomousNpcSpeech(
    const std::shared_ptr<CoreService>& service,
    std::int64_t roomId,
    std::int64_t sourceUserId,
    const std::string& interactionType,
    const std::string& sourceText) {
  Json::Value spoken{Json::arrayValue};
  if (interactionType != "speech" && interactionType != "action" && interactionType != "wait") return spoken;

  std::int64_t sourceNode = 0;
  if (const auto node = effectiveInteractionNode(service, roomId, sourceUserId)) sourceNode = *node;
  const auto rows = service->database()->execSqlSync(R"SQL(
    SELECT rn.map_node_id,rn.status AS room_status,
           nt.id,nt.name,nt.role_name,nt.image_url,nt.portrait_url,
           nt.ai_level,nt.ai_behavior,nt.speech_enabled,nt.config::text AS config_text
    FROM room_npcs rn
    JOIN npc_templates nt ON nt.id=rn.npc_template_id
    WHERE rn.room_id=$1 AND nt.speech_enabled=TRUE
    ORDER BY nt.ai_level DESC,nt.id
  )SQL", roomId);

  int emitted = 0;
  for (const auto& row : rows) {
    if (emitted >= 2) break; // 同一次玩家互動最多兩位 NPC 插話，避免聊天室被洗版。
    const auto npcId = row["id"].as<std::int64_t>();
    const auto npcNode = row["map_node_id"].isNull() ? 0 : row["map_node_id"].as<std::int64_t>();
    if (sourceNode > 0 && npcNode > 0 && sourceNode != npcNode) continue;
    const auto status = row["room_status"].as<std::string>();
    if (status == "removed" || status == "dead" || status == "hidden") continue;

    const auto config = parseJson(row["config_text"].as<std::string>(), Json::Value{Json::objectValue});
    if (config.isMember("autonomous_speech") && !config["autonomous_speech"].asBool()) continue;
    const auto name = row["name"].as<std::string>();
    const bool mentioned = sourceText.find("@" + name) != std::string::npos || sourceText.find(name) != std::string::npos;
    const auto aiLevel = std::clamp<std::int64_t>(row["ai_level"].as<std::int64_t>(), 0, 10);
    auto chance = intValue(config["autonomous_speech_chance"], 28 + aiLevel * 6);
    chance = std::clamp<std::int64_t>(chance, 0, 100);
    if (mentioned) chance = 100;
    if (interactionType == "wait") chance = std::min<std::int64_t>(chance, 20);
    if (chance <= 0 || randomInt(1,100) > chance) continue;

    // 同一 NPC 短時間內不連續插話；被直接點名時可立即回答。
    if (!mentioned) {
      const auto recent = service->database()->execSqlSync(
          "SELECT 1 FROM events WHERE room_id=$1 AND type='speech' "
          "AND payload->>'speaker_kind'='npc' AND payload->>'npc_template_id'=$2 "
          "AND created_at>CURRENT_TIMESTAMP-INTERVAL '12 seconds' LIMIT 1",
          roomId, std::to_string(npcId));
      if (!recent.empty()) continue;
    }

    const auto behavior = row["ai_behavior"].as<std::string>();
    const auto custom = npcConfiguredLines(config);
    auto line = npcJudgementLine(behavior, interactionType, sourceText, custom);
    // 沒有明確情境時才有機率採用 NPC 自訂口頭禪，保留角色個性。
    if (!custom.empty() && randomInt(1,100) <= 35)
      line = custom[static_cast<std::size_t>(randomInt(0, static_cast<int>(custom.size()-1)))];
    ...
    payload["npc_template_id"] = Json::Int64(npcId);
    payload["npc_name"] = name;
    payload["npc_role"] = row["role_name"].as<std::string>();
    payload["npc_avatar_url"] = row["image_url"].as<std::string>();
    payload["npc_portrait_url"] = row["portrait_url"].as<std::string>();
    payload["source_user_id"] = Json::Int64(sourceUserId);
    payload["source_interaction_type"] = interactionType;
    payload["ai_behavior"] = behavior;
    payload["ai_judgement"] = true;
    appendAudienceIds(payload, npcSpeechAudience(service, roomId, npcNode));
    service->addEvent(roomId, 0, "speech", payload);

    Json::Value item{Json::objectValue};
    item["npc_template_id"] = Json::Int64(npcId);
    item["npc_name"] = name;
    item["text"] = line;
    spoken.append(item);
    ++emitted;
...
// [功能備註｜73-cpp.34.25.22 人格值／控制權]
// 每次玩家互動只載入一次人格設定，後續「搶奪控制」與「自我意識回應」共用同一份資料，
// 避免為新增人格值機制額外增加一輪 character_cards 查詢。
struct PersonalityRuntimeContext {
  bool found{false};
...
PersonalityRuntimeContext loadPersonalityRuntimeContext(
    const std::shared_ptr<CoreService>& service,
    std::int64_t roomId,
    std::int64_t userId) {
  PersonalityRuntimeContext context;
  const auto rows = service->database()->execSqlSync(
      "SELECT cc.id,cc.name,cc.personality_profile::text AS profile_text FROM character_cards cc "
      "JOIN character_room_bindings crb ON crb.character_id=cc.id AND crb.user_id=cc.user_id "
      "WHERE crb.room_id=$1 AND cc.user_id=$2 LIMIT 1", roomId, userId);
  if (rows.empty()) return context;
  context.found = true;
  context.characterId = rows[0]["id"].as<std::int64_t>();
  context.characterName = rows[0]["name"].as<std::string>();
  context.profile = parseJson(rows[0]["profile_text"].as<std::string>(), Json::Value{Json::objectValue});
  if (!context.profile.isObject()) context.profile = Json::Value{Json::objectValue};
  return context;
}

std::vector<std::string> personalityTakeoverActions(const Json::Value& profile) {
  std::vector<std::string> lines;
  const auto& raw = profile["takeover_actions"];
  if (raw.isArray()) {
    for (const auto& item : raw) {
      auto line = trimmed(stringValue(item));
      if (!line.empty()) lines.push_back(std::move(line));
...
std::int64_t personalityMaxValue(const Json::Value& profile) {
  return std::clamp<std::int64_t>(intValue(profile["personality_max_value"], 100), 1, 1000000);
}

std::int64_t personalityValueDeltaForContext(const Json::Value& profile,
                                              const std::string& interactionType,
                                              const std::string& text) {
  const auto& rules = profile["personality_value_rules"];
  if (!rules.isObject()) return 0;
  std::int64_t delta = intValue(rules[interactionType], 0);
  if (containsAnyNpcCue(text, {"危險","陷阱","毒","詭異","怪物","敵人","追","逃"})) delta += intValue(rules["danger"], 0);
  if (containsAnyNpcCue(text, {"攻擊","砍","射","打","施放","技能","戰鬥"})) delta += intValue(rules["combat"], 0);
  if (containsAnyNpcCue(text, {"調查","搜索","搜尋","觀察","檢查","線索","判斷"})) delta += intValue(rules["investigate"], 0);
  return std::clamp<std::int64_t>(delta, -1000000, 1000000);
}

Json::Value maybeApplyPersonalityTakeover(
    const std::shared_ptr<CoreService>& service,
    PersonalityRuntimeContext& context,
  ...
  if (!context.found || !context.profile.isObject()) return result;
  auto& profile = context.profile;
  if (!profile.get("ai_enabled", false).asBool() || !profile.get("takeover_enabled", true).asBool()) return result;

  const auto maxValue = personalityMaxValue(profile);
  const auto value = std::clamp<std::int64_t>(intValue(profile["personality_value"], 0), 0, maxValue);
  const auto threshold = std::clamp<std::int64_t>(intValue(profile["takeover_threshold"], std::min<std::int64_t>(80,maxValue)), 1, maxValue);
  const auto chance = std::clamp<std::int64_t>(intValue(profile["takeover_chance"], 100), 0, 100);
  const auto remainingBefore = std::max<std::int64_t>(0,intValue(profile["takeover_remaining_actions"],0));
  const bool continuing = remainingBefore > 0;
  if (!continuing && (value < threshold || chance <= 0 || randomInt(1, 100) > chance)) return result;

  const auto originalType = interactionType;
  const auto originalText = text;
  const auto actions = personalityTakeoverActions(profile);
  if (!actions.empty()) {
    text = actions[static_cast<std::size_t>(randomInt(0, static_cast<int>(actions.size() - 1)))];
  } else if (interactionType == "speech") {
    text = "先別說了。這一次由我來決定要說什麼。";
  } else {
    text = "人格暫時接管身體，依自己的判斷採取行動。";
  }
  // 原本選擇待機時，人格接管代表它拒絕保持靜止，轉成真正的 action。
  if (interactionType == "wait") interactionType = "action";

  const auto cost = std::clamp<std::int64_t>(intValue(profile["takeover_cost"], 25), 0, maxValue);
  const auto duration = std::clamp<std::int64_t>(intValue(profile["takeover_duration_actions"], 1), 1, 100);
  const auto nextValue = continuing ? value : std::max<std::int64_t>(0, value - cost);
  const auto remainingAfter = continuing ? std::max<std::int64_t>(0,remainingBefore-1) : std::max<std::int64_t>(0,duration-1);
  // 這裡只計算結果，不先寫回資料庫。等規則引擎確認本次接管行動真的成立後才扣值，
  // 避免被 DM 規則阻止時人格值仍被消耗。
  profile["personality_value"] = Json::Int64(nextValue);
  profile["takeover_remaining_actions"] = Json::Int64(remainingAfter);

  result["triggered"] = true;
  result["personality_name"] = trimmed(stringValue(profile["name"], "內在人格"));
  result["original_interaction_type"] = originalType;
  result["original_text"] = originalText;
  result["interaction_type"] = interactionType;
  result["text"] = text;
  result["value_before"] = Json::Int64(value);
  result["value_after"] = Json::Int64(nextValue);
  result["threshold"] = Json::Int64(threshold);
  result["max_value"] = Json::Int64(maxValue);
  result["cost"] = Json::Int64(continuing ? 0 : cost);
  result["continuing"] = continuing;
  result["remaining_before"] = Json::Int64(remainingBefore);
  result["remaining_after"] = Json::Int64(remainingAfter);
  result["duration_actions"] = Json::Int64(duration);
  return result;
}

std::vector<std::string> personalityConfiguredLines(const Json::Value& profile) {
  std::vector<std::string> lines;
  const auto& raw = profile["ai_lines"];
  if (raw.isArray()) {
    for (const auto& item : raw) {
      auto line = trimmed(stringValue(item));
      if (!line.empty()) lines.push_back(std::move(line));
...
std::string personalitySelfAwareLine(const Json::Value& profile,
                                     const std::string& interactionType,
                                     const std::string& text) {
  const auto name = trimmed(stringValue(profile["name"], "人格"));
  const auto custom = personalityConfiguredLines(profile);
  const bool danger = containsAnyNpcCue(text, {"危險","陷阱","毒","詭異","怪物","敵人","追","逃"});
  const bool attack = containsAnyNpcCue(text, {"攻擊","砍","射","打","施放","技能","戰鬥"});
  const bool investigate = containsAnyNpcCue(text, {"調查","搜索","搜尋","觀察","檢查","線索","判斷"});
  const bool hesitate = containsAnyNpcCue(text, {"不知道","猶豫","不確定","怎麼辦","要不要"});
  std::vector<std::string> lines;
  if (danger) lines = {"我感覺到了。這裡有問題，別只看表面。","先活著再說。我會替你盯著那些不對勁的地方。"};
  else if (attack) lines = {"你已經決定動手了？那至少把退路留好。","我不會替你做決定，但我會記住你為什麼出手。"};
  else if (investigate) lines = {"把事實和猜測分開。我會幫你記住矛盾的地方。","這裡值得查。我對某些細節有種不舒服的熟悉感。"};
  else if (hesitate) lines = {"你在猶豫。我知道。先選一個你能承擔結果的答案。","別急著問別人。先問你自己真正害怕失去什麼。"};
  else if (interactionType == "wait") lines = {"你停下來了。很好，我正好需要一點時間整理剛才的事。","安靜不代表什麼都沒發生。我還在想。"};
  else if (interactionType == "speech") lines = {"我聽見你說的了。你真的相信那句話嗎？","這句話我會記著。也許之後你會需要我提醒你。"};
  else lines = {"我知道你做了什麼。我會把這次選擇記下來。","我沒有控制你，但我有自己的看法。這一步值得觀察。"};
  if (!custom.empty() && randomInt(1,100) <= 40)
    return custom[static_cast<std::size_t>(randomInt(0, static_cast<int>(custom.size()-1)))];
  auto line = lines[static_cast<std::size_t>(randomInt(0, static_cast<int>(lines.size()-1)))];
  if (!name.empty() && name != "人格" && randomInt(1,100) <= 25) line = name + "：" + line;
  return line;
}

Json::Value runSelfAwarePersonalityAi(
    const std::shared_ptr<CoreService>& service,
    std::int64_t roomId,
    std::int64_t userId,
    PersonalityRuntimeContext& context,
  ...
  Json::Value result{Json::objectValue};
  result["triggered"] = false;
  if (interactionType != "speech" && interactionType != "action" && interactionType != "wait") return result;
  if (!context.found || !context.profile.isObject()) return result;
  auto& profile = context.profile;
  if (!profile.get("ai_enabled", false).asBool()) return result;
  const auto awareness = std::clamp<std::int64_t>(intValue(profile["self_awareness_level"], 5), 0, 10);
  if (awareness <= 0) return result;
  auto chance = std::clamp<std::int64_t>(intValue(profile["ai_response_chance"], 15 + awareness * 6), 0, 100);
  if (interactionType == "wait") chance = std::min<std::int64_t>(chance, 35);
  if (randomInt(1,100) > chance) return result;
  const auto recent = service->database()->execSqlSync(
      "SELECT 1 FROM events WHERE room_id=$1 AND user_id=$2 AND type='speech' "
      "AND payload->>'speaker_kind'='personality' AND created_at>CURRENT_TIMESTAMP-INTERVAL '10 seconds' LIMIT 1",
      roomId, userId);
  if (!recent.empty()) return result;

  const auto name = trimmed(stringValue(profile["name"], "內在人格"));
  const auto line = personalitySelfAwareLine(profile, interactionType, sourceText);
  Json::Value payload{Json::objectValue};
  payload["interaction_type"] = "speech";
  payload["text"] = line;
  payload["action"] = line;
  payload["volume"] = "inner";
  payload["speaker_kind"] = "personality";
  payload["personality_name"] = name.empty() ? "內在人格" : name;
  payload["source_user_id"] = Json::Int64(userId);
  payload["source_interaction_type"] = interactionType;
  payload["self_awareness_level"] = Json::Int64(awareness);
  payload["ai_judgement"] = true;
  payload["private_inner_voice"] = true;
  payload["audience_user_ids"] = Json::arrayValue;
  payload["audience_user_ids"].append(Json::Int64(userId));

  // 自我意識成功形成一次主動判斷後累積人格值；上限、基礎增量與情境增減全部可由 DM 修改。
  const auto maxValue = personalityMaxValue(profile);
  const auto valueBefore = std::clamp<std::int64_t>(intValue(profile["personality_value"], 0), 0, maxValue);
  const auto valueGain = std::clamp<std::int64_t>(intValue(profile["personality_value_gain"], 2), -maxValue, maxValue);
  const auto contextDelta = personalityValueDeltaForContext(profile, interactionType, sourceText);
  const auto valueAfter = std::clamp<std::int64_t>(valueBefore + valueGain + contextDelta, 0, maxValue);
  if (valueAfter != valueBefore && context.characterId > 0) {
    service->database()->execSqlSync(
        "UPDATE character_cards SET personality_profile=jsonb_set(personality_profile,'{personality_value}',to_jsonb($1::bigint),true),updated_at=CURRENT_TIMESTAMP WHERE id=$2",
        valueAfter, context.characterId);
    profile["personality_value"] = Json::Int64(valueAfter);
  }
  payload["personality_value_before"] = Json::Int64(valueBefore);
  payload["personality_value_after"] = Json::Int64(valueAfter);
  payload["personality_value_max"] = Json::Int64(maxValue);
  payload["personality_context_delta"] = Json::Int64(contextDelta);
  payload["takeover_threshold"] = Json::Int64(std::clamp<std::int64_t>(intValue(profile["takeover_threshold"], std::min<std::int64_t>(80,maxValue)), 1, maxValue));
  service->addEvent(roomId, userId, "speech", payload);
  result["triggered"] = true;
  result["name"] = payload["personality_name"];
  result["text"] = line;
  result["self_awareness_level"] = Json::Int64(awareness);
  result["personality_value_before"] = Json::Int64(valueBefore);
  result["personality_value_after"] = Json::Int64(valueAfter);
  result["personality_value_max"] = Json::Int64(maxValue);
  result["personality_context_delta"] = Json::Int64(contextDelta);
  return result;
}
...
Json::Value normalizeItemStudyDomains(const Json::Value& raw) {
  static const std::unordered_set<std::string> allowed = {"魔法學", "儀式學", "神秘學"};
  Json::Value out{Json::arrayValue};
  std::unordered_set<std::string> seen;
  if (!raw.isArray()) return out;
  for (const auto& value : raw) {
    const auto name = trimmed(stringValue(value));
    if (!allowed.contains(name) || seen.contains(name)) continue;
    seen.insert(name); out.append(name);
  }
  return out;
}

std::string normalizeProfessionRequirementMode(std::string mode) {
  mode = trimmed(mode);
  if (mode == "main" || mode == "sub") return mode;
  return "any";
}
...
std::string adminContentTable(const std::string& type) {
  static const std::unordered_map<std::string, std::string> tables{
      {"skills","skill_templates"}, {"items","item_templates"},
      {"recipes","recipes"}, {"monsters","monster_templates"},
      {"npcs","npc_templates"}};
  const auto it=tables.find(type);
  if(it==tables.end()) throw ApiError(drogon::k400BadRequest,"不支援的內容類型");
  return it->second;
}

Json::Value adminContentSnapshot(const Db& db,const std::string& type,std::int64_t id) {
  const auto table=adminContentTable(type);
  const auto rows=db->execSqlSync(
      "SELECT row_to_json(t)::text AS json_text FROM \""+table+"\" t WHERE id=$1",id);
  if(rows.empty()) throw ApiError(drogon::k404NotFound,"找不到內容");
  return rowJson(rows);
}

void saveAdminContentVersion(const Db& db,const std::string& type,std::int64_t id,
                             std::int64_t userId,const std::string& note) {
  const auto snapshot=adminContentSnapshot(db,type,id);
  const auto snapshotText=compactJson(snapshot);
  const auto latest=db->execSqlSync(
      "SELECT snapshot::text AS snapshot_text FROM admin_content_versions "
      "WHERE content_type=$1 AND content_id=$2 ORDER BY created_at DESC,id DESC LIMIT 1",type,id);
  if(!latest.empty()){
    const auto previous=parseJson(latest[0]["snapshot_text"].as<std::string>(),Json::Value{Json::objectValue});
    if(compactJson(previous)==snapshotText)return;
  }
  db->execSqlSync(
      "INSERT INTO admin_content_versions(content_type,content_id,snapshot,change_note,created_by) "
      "VALUES($1,$2,$3::jsonb,$4,$5)",
      type,id,snapshotText,note,userId);
  db->execSqlSync(
      "DELETE FROM admin_content_versions WHERE id IN ("
      "SELECT id FROM admin_content_versions WHERE content_type=$1 AND content_id=$2 "
...
bool replaceAdminContentStrings(Json::Value& value,const std::string& oldName,const std::string& newName) {
  bool changed=false;
  if(value.isString()){
    auto text=value.asString();
    std::size_t pos=0;
    while(!oldName.empty()&&(pos=text.find(oldName,pos))!=std::string::npos){
      text.replace(pos,oldName.size(),newName);pos+=newName.size();changed=true;
    }
    if(changed)value=text;
    return changed;
  }
  if(value.isArray()){for(Json::ArrayIndex i=0;i<value.size();++i)changed=replaceAdminContentStrings(value[i],oldName,newName)||changed;return changed;}
  if(value.isObject()){for(const auto& key:value.getMemberNames())changed=replaceAdminContentStrings(value[key],oldName,newName)||changed;}
  return changed;
}

Json::Value adminContentDependencies(const Db& db,const std::string& type,std::int64_t id) {
  const auto current=adminContentSnapshot(db,type,id);
  const auto name=trimmed(stringValue(current["name"]));
  Json::Value deps{Json::arrayValue};
  if(name.empty())return deps;
  const std::array<std::pair<const char*,const char*>,5> sources{{{"skills","skill_templates"},{"items","item_templates"},{"recipes","recipes"},{"monsters","monster_templates"},{"npcs","npc_templates"}}};
  for(const auto& [depType,table]:sources){
    const auto rows=db->execSqlSync(
      "SELECT id,name,row_to_json(t)::text AS json_text FROM \""+std::string(table)+"\" t WHERE row_to_json(t)::text ILIKE $1 ORDER BY id DESC LIMIT 50",
      "%"+name+"%");
    for(const auto& row:rows){
      const auto depId=row["id"].as<std::int64_t>();
      if(std::string(depType)==type&&depId==id)continue;
      Json::Value d;d["type"]=depType;d["id"]=Json::Int64(depId);d["name"]=row["name"].as<std::string>();d["reason"]="內容中出現「"+name+"」";deps.append(d);
      if(deps.size()>=60)break;
    }
    if(deps.size()>=60)break;
  }
  return deps;
...
Json::Value sanitizedContentPatch(const std::string& type,const Json::Value& patch) {
  Json::Value out{Json::objectValue};
  const std::unordered_map<std::string,std::set<std::string>> allowed{
    {"skills",{"rank","category","skill_type"}},
    {"items",{"rank","category","item_type"}},
    {"recipes",{"rank","recipe_type"}},
    {"monsters",{"rank","category","ai_behavior"}},
    {"npcs",{"role_name","ai_behavior","merchant_enabled","speech_enabled"}}
  };
  const auto it=allowed.find(type);
  if(it==allowed.end()) throw ApiError(drogon::k400BadRequest,"不支援的內容類型");
  if(!patch.isObject()) return out;
  for(const auto& key:patch.getMemberNames()) if(it->second.contains(key)) out[key]=patch[key];
  return out;
}
...
void syncShopItemCategory(const Db& db, Json::Value& body) {
  // [功能備註｜73-cpp.34.25.24 商店多分類] category_ids 為正式多分類；category_id 保留作主要分類與舊版相容。
  if (body.isMember("category_ids")) {
    Json::Value normalized{Json::arrayValue};
    std::unordered_set<std::int64_t> seen;
    if (body["category_ids"].isArray()) {
      for (const auto& raw : body["category_ids"]) {
        const auto id = intValue(raw);
        if (id <= 0 || seen.contains(id)) continue;
        const auto rows = db->execSqlSync("SELECT 1 FROM shop_categories WHERE id=$1 AND active=TRUE", id);
        if (rows.empty()) throw ApiError(drogon::k400BadRequest, "商店分類不存在或已停用");
        seen.insert(id); normalized.append(Json::Int64(id));
      }
    }
    body["category_ids"] = normalized;
    if (normalized.size() > 0 && (!body.isMember("category_id") || body["category_id"].isNull() || intValue(body["category_id"]) <= 0))
      body["category_id"] = normalized[0];
  }
  if (body.isMember("category_id")) {
    ...
    if (!body.isMember("category_ids")) { Json::Value ids{Json::arrayValue}; ids.append(Json::Int64(categoryId)); body["category_ids"] = ids; }
    return;
  }
...
// [功能備註｜65-cpp.26.9 商店管理安全更新]
// 商店建立／修改共用欄位清洗；PATCH 只處理實際送來的欄位，避免只改分類或啟用狀態時把品階重設為 G。
void normalizeShopItemWrite(Json::Value& body, bool creating) {
  if (body.isMember("name")) {
    const auto name = clipped(trimmed(stringValue(body["name"])), 120);
    if (name.empty()) throw ApiError(drogon::k400BadRequest, "商品名稱不能空白");
    body["name"] = name;
  } else if (creating) {
    throw ApiError(drogon::k400BadRequest, "請輸入商品名稱");
  }
  if (body.isMember("rank")) body["rank"] = normalizeContentRank(stringValue(body["rank"], "G"));
  else if (creating) body["rank"] = "G";
  if (body.isMember("item_type")) {
    const auto type = stringValue(body["item_type"], "item");
    if (type != "item" && type != "equipment" && type != "weapon")
      throw ApiError(drogon::k400BadRequest, "商品類型只能是 item/equipment/weapon");
  } else if (creating) body["item_type"] = "item";
  if (body.isMember("price_weird")) body["price_weird"] = Json::Int64(std::max<std::int64_t>(0, intValue(body["price_weird"])));
  if (body.isMember("quantity")) body["quantity"] = Json::Int64(std::clamp<std::int64_t>(intValue(body["quantity"], 1), 1, 9999));
  if (body.isMember("stock")) body["stock"] = Json::Int64(std::max<std::int64_t>(-1, intValue(body["stock"], -1)));
  if (body.isMember("purchase_limit")) body["purchase_limit"] = Json::Int64(std::max<std::int64_t>(-1, intValue(body["purchase_limit"], -1)));
  if (body.isMember("discount_percent")) body["discount_percent"] = std::clamp(body["discount_percent"].asDouble(), 0.0, 100.0);
  if (body.isMember("sell_percent")) body["sell_percent"] = std::clamp(body["sell_percent"].asDouble(), 0.0, 100.0);
  if (body.isMember("description")) body["description"] = clipped(stringValue(body["description"]), 4000);
}
...
// [功能備註｜65-cpp.26.15 合成研究／品階成功率]
// 新配方預設依品階決定基礎成功率；舊配方若沒有 success_rate_mode，仍沿用原 success_rate，避免改版後數值突變。
std::int64_t defaultRecipeRankSuccessRate(const std::string& rawRank) {
  const auto rank = normalizeContentRank(rawRank.empty() ? "G" : rawRank);
  static const std::unordered_map<std::string, std::int64_t> rates{
      {"G",100},{"F",95},{"E",90},{"D",82},{"C",74},
      {"B",65},{"A",55},{"S",45},{"SS",35},{"SSS",25}};
  const auto it = rates.find(rank);
  return it == rates.end() ? 100 : it->second;
}

std::int64_t recipeBaseSuccessRate(const Json::Value& recipe) {
  const auto cfg = recipe["config"].isObject() ? recipe["config"] : Json::Value{Json::objectValue};
  const auto mode = trimmed(stringValue(cfg["success_rate_mode"]));
  if (mode == "manual") {
    return std::clamp<std::int64_t>(intValue(cfg["success_rate"], defaultRecipeRankSuccessRate(stringValue(recipe["rank"], "G"))), 0, 100);
  }
  if (mode == "rank") return defaultRecipeRankSuccessRate(stringValue(recipe["rank"], "G"));
  // 26.14 以前沒有 mode；保留原本手動 success_rate。
  if (cfg.isMember("success_rate")) return std::clamp<std::int64_t>(intValue(cfg["success_rate"], 100), 0, 100);
  return defaultRecipeRankSuccessRate(stringValue(recipe["rank"], "G"));
}

bool characterOwnsSkillTemplateCpp(const Json::Value& character, std::int64_t templateId) {
  if (templateId <= 0 || !character["skills"].isArray()) return templateId <= 0;
  for (const auto& skill : character["skills"]) {
    if (intValue(skill["template_id"], intValue(skill["id"])) == templateId) return true;
  }
  return false;
}

std::int64_t characterCraftSuccessBonusCpp(const Json::Value& character, const std::string& category) {
  if (!character["skills"].isArray()) return 0;
  ...
  const auto target = lower(category);
  std::int64_t bonus = 0;
  for (const auto& skill : character["skills"]) {
    const auto data = skill["data"].isObject() ? skill["data"] : skill;
    const auto amount = intValue(data["craft_success_bonus"]);
    if (!amount) continue;
    const auto own = lower(stringValue(data["craft_success_category"]));
    if (own.empty() || own == "全部" || own == "all" || own == "*" || own == target) bonus += amount;
  }
  return bonus;
}

std::string recipeMaterialSignature(const Json::Value& rawMaterials) {
  if (!rawMaterials.isArray()) return {};
  std::vector<std::string> parts;
  for (const auto& material : rawMaterials) {
    auto name = trimmed(stringValue(material["name"], stringValue(material["item"])));
    auto category = trimmed(stringValue(material["category"]));
    const auto qty = std::max<std::int64_t>(1, intValue(material.isMember("quantity") ? material["quantity"] : material["qty"], 1));
    std::transform(name.begin(), name.end(), name.begin(), [](unsigned char c){ return static_cast<char>(std::tolower(c)); });
    std::transform(category.begin(), category.end(), category.begin(), [](unsigned char c){ return static_cast<char>(std::tolower(c)); });
    if (!name.empty()) parts.push_back(name + "\x1f" + category + "\x1f" + std::to_string(qty));
  }
...
bool recipeIsDiscovered(const Db& db, std::int64_t userId, std::int64_t recipeId) {
  return !db->execSqlSync(
      "SELECT 1 FROM character_recipe_discoveries WHERE user_id=$1 AND recipe_id=$2 AND discovered=TRUE",
      userId, recipeId).empty();
}

void recordRecipeExperiment(const Db& db, std::int64_t userId, std::int64_t recipeId,
                            const Json::Value& materials, bool success, int roll,
                            std::int64_t successRate, bool consumed) {
  if (recipeId > 0) {
    db->execSqlSync(
        "INSERT INTO character_recipe_discoveries(user_id,recipe_id,discovered,attempts,successful_attempts,last_attempt_at,discovered_at) "
        "VALUES($1,$2,$3,1,$4,CURRENT_TIMESTAMP,CASE WHEN $3 THEN CURRENT_TIMESTAMP ELSE NULL END) "
        "ON CONFLICT(user_id,recipe_id) DO UPDATE SET discovered=character_recipe_discoveries.discovered OR EXCLUDED.discovered,"
        "attempts=character_recipe_discoveries.attempts+1,successful_attempts=character_recipe_discoveries.successful_attempts+EXCLUDED.successful_attempts,"
        "last_attempt_at=CURRENT_TIMESTAMP,discovered_at=CASE WHEN character_recipe_discoveries.discovered_at IS NULL AND EXCLUDED.discovered THEN CURRENT_TIMESTAMP ELSE character_recipe_discoveries.discovered_at END,updated_at=CURRENT_TIMESTAMP",
        userId, recipeId, success, success ? 1 : 0);
  }
  db->execSqlSync(
      "INSERT INTO recipe_experiment_attempts(user_id,matched_recipe_id,materials,material_signature,success,roll,success_rate,consumed) "
      "VALUES($1,NULLIF($2,0),$3::jsonb,$4,$5,$6,$7,$8)",
      userId, recipeId, compactJson(materials), recipeMaterialSignature(materials), success, roll, successRate, consumed);
}
...
void registerAdminPatchDelete1(const std::shared_ptr<CoreService>& service,
  ...
  reg1(path, drogon::Patch, [service, table, singularKey](const Request& req, Callback& cb, const std::string& idText) {
    runHandler(cb, [&] {
      ...
      if(table=="skill_templates") saveAdminContentVersion(service->database(),"skills",id,user.id,"修改技能");
      Json::Value out;
      out["ok"] = true;
...
Json::Value advanceStructuredTaskStepsForEvent(const std::shared_ptr<CoreService>& service,
  ...
  Json::Value out{Json::objectValue}; out["progressed"] = Json::arrayValue;
  if (!service || roomId <= 0 || userId <= 0 || eventName.empty()) return out;
  const auto rows = service->database()->execSqlSync(
      "SELECT row_to_json(t)::text AS task_text,tp.progress::text AS progress_text "
      "FROM task_participants tp JOIN tasks t ON t.id=tp.task_id "
      "WHERE tp.user_id=$1 AND tp.status IN ('accepted','assigned') AND t.status='open' ORDER BY t.id", userId);
  bool changedAny = false;
  for (const auto& row : rows) {
    auto task = parseJson(row["task_text"].as<std::string>());
    auto steps = task["task_steps"].isArray() ? task["task_steps"] : Json::Value{Json::arrayValue};
    auto progress = parseJson(row["progress_text"].as<std::string>(), Json::Value{Json::objectValue});
    if (!progress.isObject()) progress = Json::Value{Json::objectValue};
    ensureTaskVariables(task, progress);
    if (!progress["steps"].isObject()) progress["steps"] = Json::Value{Json::objectValue};
    const auto eventFailure = taskFailureReason(task, progress, eventName);
    if (!eventFailure.empty()) {
      progress["failed"] = true; progress["failure_reason"] = eventFailure; progress["failure_event"] = eventName;
      service->database()->execSqlSync("UPDATE task_participants SET status='failed',progress=$1::jsonb WHERE task_id=$2 AND user_id=$3", compactJson(progress), intValue(task["id"]), userId);
      Json::Value f; f["task_id"] = task["id"]; f["reason"] = eventFailure; out["failed"] = f; changedAny = true; continue;
    }
    bool changedTask = false;
    for (const auto& step : steps) {
      if (!taskStepActive(step, progress)) continue;
      if (!stringValue(step["failure_event_name"]).empty() && eventName == stringValue(step["failure_event_name"])) {
        progress["failed"] = true; progress["failure_reason"] = "任務環節失敗：" + stringValue(step["title"]); progress["failure_event"] = eventName;
        service->database()->execSqlSync("UPDATE task_participants SET status='failed',progress=$1::jsonb WHERE task_id=$2 AND user_id=$3", compactJson(progress), intValue(task["id"]), userId); changedAny = true; break;
      }
      if (!step.get("auto_progress", false).asBool()) continue;
      if (!taskStepDependenciesComplete(steps, progress, step)) continue;
      const auto stepId = stringValue(step["id"]); if (stepId.empty()) continue;
      const auto required = std::max<std::int64_t>(1, intValue(step["required"], 1));
      const bool sharedStep = stringValue(step["progress_scope"], "personal") == "team";
      auto current = std::max<std::int64_t>(0, intValue(progress["steps"][stepId]["count"]));
      if (sharedStep) {
        const auto sharedRows = service->database()->execSqlSync(
            "SELECT progress::text AS progress_text FROM task_participants WHERE task_id=$1 AND status IN ('accepted','assigned')", intValue(task["id"]));
        for (const auto& sharedRow : sharedRows) {
          const auto sharedProgress = parseJson(sharedRow["progress_text"].as<std::string>(), Json::Value{Json::objectValue});
          current = std::max<std::int64_t>(current, intValue(sharedProgress["steps"][stepId]["count"]));
        }
        const auto recent = service->database()->execSqlSync(
            "SELECT 1 FROM task_step_actions WHERE room_id=$1 AND task_id=$2 AND step_id=$3 AND action_type=$4 "
            "AND created_at > CURRENT_TIMESTAMP - INTERVAL '1500 milliseconds' LIMIT 1",
            roomId, intValue(task["id"]), std::string(stepId), std::string("auto:") + eventName);
        if (!recent.empty()) continue;
      }
      if (current >= required || progress["steps"][stepId].get("completed", false).asBool()) continue;
      const auto type = stringValue(step["type"], "custom");
      bool matched = false;
      if (type == "reach_location" && eventName == "PLAYER_MOVED") {
        const auto wantedNode = intValue(step["map_node_id"]);
        const auto movedNode = intValue(payload["node_id"], intValue(payload["target_node"]["id"]));
        const auto wantedName = trimmed(stringValue(step["location_name"]));
        const auto movedName = trimmed(stringValue(payload["target_node"]["name"]));
        matched = (wantedNode > 0 && wantedNode == movedNode) || (!wantedName.empty() && !movedName.empty() && wantedName == movedName);
      } else if (type == "perform_ritual" && eventName == "RITUAL_SUCCEEDED") {
        const auto wanted = intValue(step["ritual_id"]);
        matched = wanted <= 0 || wanted == intValue(payload["ritual_id"]);
      } else if (type == "boss_phase" && eventName == "BOSS_PHASE_CHANGED") {
        const auto wantedPhase = std::max<std::int64_t>(1, intValue(step["boss_phase"], 1));
        const auto actualPhase = std::max<std::int64_t>(1, intValue(payload["phase"], intValue(payload["phase_index"]) + 1));
        matched = actualPhase >= wantedPhase;
        const auto wantedMonster = intValue(step["monster_template_id"]);
        if (matched && wantedMonster > 0) {
          const auto roomMonsterId = intValue(payload["monster_id"]);
          matched = roomMonsterId > 0 && !service->database()->execSqlSync(
              "SELECT 1 FROM room_monsters WHERE id=$1 AND room_id=$2 AND monster_template_id=$3", roomMonsterId, roomId, wantedMonster).empty();
        }
      } else if (type == "rule_event") {
        const auto wanted = trimmed(stringValue(step["event_name"]));
        matched = !wanted.empty() && wanted == eventName;
      }
      if (!matched) continue;
      current = std::min<std::int64_t>(required, current + 1);
      auto& sp = progress["steps"][stepId]; if (!sp.isObject()) sp = Json::Value{Json::objectValue};
      const bool wasCompleted = sp.get("completed", false).asBool();
      sp["count"] = Json::Int64(current); sp["required"] = Json::Int64(required); sp["completed"] = current >= required;
      if (!wasCompleted && current >= required) applyTaskStepVariableChange(step, progress);
      sp["last_room_id"] = Json::Int64(roomId); sp["auto_event"] = eventName;
      const auto nodeId = intValue(payload["node_id"], intValue(payload["target_node"]["id"])); if (nodeId > 0) sp["last_map_node_id"] = Json::Int64(nodeId);
      Json::Value actionData; actionData["task_title"] = stringValue(task["title"]); actionData["step_title"] = stringValue(step["title"]); actionData["event_name"] = eventName; actionData["count"] = Json::Int64(current); actionData["required"] = Json::Int64(required);
      service->database()->execSqlSync(
          "INSERT INTO task_step_actions(room_id,task_id,step_id,user_id,action_type,map_node_id,data) VALUES($1,$2,$3,$4,$5,NULLIF($6,0),$7::jsonb)",
          roomId, intValue(task["id"]), std::string(stepId), userId, std::string("auto:") + eventName, nodeId > 0 ? nodeId : 0, compactJson(actionData));
      if (sharedStep) {
        const auto peers = service->database()->execSqlSync(
            "SELECT user_id,progress::text AS progress_text FROM task_participants WHERE task_id=$1 AND status IN ('accepted','assigned')", intValue(task["id"]));
        for (const auto& peer : peers) {
          auto pp = parseJson(peer["progress_text"].as<std::string>(), Json::Value{Json::objectValue});
          if (!pp.isObject()) pp = Json::Value{Json::objectValue};
          if (!pp["steps"].isObject()) pp["steps"] = Json::Value{Json::objectValue};
          ensureTaskVariables(task, pp);
          auto& psp = pp["steps"][stepId]; if (!psp.isObject()) psp = Json::Value{Json::objectValue};
          const bool peerWasCompleted = psp.get("completed", false).asBool();
          psp["count"] = Json::Int64(current); psp["required"] = Json::Int64(required); psp["completed"] = current >= required;
          if (!peerWasCompleted && current >= required) applyTaskStepVariableChange(step, pp);
          psp["last_room_id"] = Json::Int64(roomId); psp["shared_by_user_id"] = Json::Int64(userId);
          pp["all_complete"] = taskStepsComplete(steps, pp);
          service->database()->execSqlSync("UPDATE task_participants SET progress=$1::jsonb WHERE task_id=$2 AND user_id=$3",
              compactJson(pp), intValue(task["id"]), peer["user_id"].as<std::int64_t>());
        }
      }
      Json::Value item; item["task_id"] = task["id"]; item["step_id"] = stepId; item["count"] = Json::Int64(current); item["required"] = Json::Int64(required); item["shared"] = sharedStep; out["progressed"].append(item);
      changedTask = changedAny = true;
    }
    if (changedTask) {
      const auto fail = taskFailureReason(task, progress);
      if (!fail.empty()) { progress["failed"] = true; progress["failure_reason"] = fail; progress["all_complete"] = false; service->database()->execSqlSync("UPDATE task_participants SET status='failed',progress=$1::jsonb WHERE task_id=$2 AND user_id=$3", compactJson(progress), intValue(task["id"]), userId); }
      else { progress["all_complete"] = taskStepsComplete(steps, progress); service->database()->execSqlSync("UPDATE task_participants SET progress=$1::jsonb WHERE task_id=$2 AND user_id=$3", compactJson(progress), intValue(task["id"]), userId); }
    }
  }
  out["changed"] = changedAny;
  if (changedAny) RoomSocket::broadcastSnapshotForRoom(roomId);
  return out;
}
...
Json::Value buildCharacterBackupCheckpoint(const Db& db, std::int64_t userId) {
  ...
  // [功能備註｜65-cpp.26.15 配方研究備份] 配方發現屬於角色長期進度，角色備份／還原必須一併保存。
  related["recipe_discoveries"] = selectRows1(db, "SELECT row_to_json(x)::text AS json_text FROM character_recipe_discoveries x WHERE user_id=$1 ORDER BY recipe_id", userId);
...
void restoreCharacterBackupCheckpoint(const Db& db, std::int64_t userId,
  ...
  const std::array<std::pair<const char*,const char*>,9> groups={{{"statuses","character_statuses"},{"great_ways","character_great_ways"},{"faiths","character_faiths"},{"reputations","character_reputations"},{"clues","character_clues"},{"rules","rule_discoveries"},{"item_identifications","item_identifications"},{"tasks","task_participants"},{"recipe_discoveries","character_recipe_discoveries"}}};
...
void registerLegacyV39Routes(const std::shared_ptr<CoreService>& service) {
  ...
  // [功能備註｜73-cpp.34.17 DM 內容工作流]
  // 批量修改、伺服器草稿、版本歷史、依賴查詢。核心資料變更仍由 C++ 驗證與 PostgreSQL 保存。
  reg0("/api/admin/content/bulk-update", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb,[&]{
      const auto user=requireUser(service,req,true);
      const auto body=bodyOrEmpty(req);
      const auto type=stringValue(body["type"]);
      const auto table=adminContentTable(type);
      const auto patch=sanitizedContentPatch(type,body["patch"]);
      if(patch.getMemberNames().empty()) throw ApiError(drogon::k400BadRequest,"沒有可批量修改的欄位");
      if(!body["ids"].isArray()||body["ids"].empty()) throw ApiError(drogon::k400BadRequest,"請至少選擇一筆資料");
      Json::Value updated{Json::arrayValue};
      for(const auto& rawId:body["ids"]){
        const auto id=intValue(rawId);
        if(id<=0) continue;
        saveAdminContentVersion(service->database(),type,id,user.id,"批量修改前");
        updated.append(genericUpdate(service->database(),table,id,patch));
      }
      Json::Value out;out["ok"]=true;out["updated"]=updated;out["count"]=Json::UInt64(updated.size());reply(cb,std::move(out));
    },"批量修改失敗");
  });

  reg2("/api/admin/content/{1}/{2}/versions", drogon::Get,
       [service](const Request& req, Callback& cb,const std::string& type,const std::string& idText){
    runHandler(cb,[&]{
      requireUser(service,req,true);
      const auto id=positiveId(idText);
      (void)adminContentTable(type);
      const auto rows=service->database()->execSqlSync(
        "SELECT id,content_type,content_id,change_note,created_by,created_at,"
        "snapshot::text AS snapshot_text FROM admin_content_versions "
        "WHERE content_type=$1 AND content_id=$2 ORDER BY created_at DESC,id DESC LIMIT 30",type,id);
      Json::Value versions{Json::arrayValue};
      for(const auto& row:rows){
        Json::Value v;v["id"]=Json::Int64(row["id"].as<std::int64_t>());
        v["content_type"]=row["content_type"].as<std::string>();
        v["content_id"]=Json::Int64(row["content_id"].as<std::int64_t>());
        v["change_note"]=row["change_note"].as<std::string>();
        if(!row["created_by"].isNull())v["created_by"]=Json::Int64(row["created_by"].as<std::int64_t>());
        v["created_at"]=row["created_at"].as<std::string>();
        v["snapshot"]=parseJson(row["snapshot_text"].as<std::string>(),Json::Value{Json::objectValue});
        versions.append(v);
      }
      Json::Value out;out["versions"]=versions;reply(cb,std::move(out));
    },"讀取版本歷史失敗");
  });

  reg2("/api/admin/content/{1}/{2}/versions", drogon::Post,
       [service](const Request& req, Callback& cb,const std::string& type,const std::string& idText){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req,true);
      const auto id=positiveId(idText);
      const auto note=trimmed(stringValue(bodyOrEmpty(req)["note"],"手動保存版本"));
      saveAdminContentVersion(service->database(),type,id,user.id,note.empty()?"手動保存版本":note);
      Json::Value out;out["ok"]=true;reply(cb,std::move(out));
    },"保存版本失敗");
  });

  reg3("/api/admin/content/{1}/{2}/restore/{3}", drogon::Post,
       [service](const Request& req,Callback& cb,const std::string& type,const std::string& idText,const std::string& versionText){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req,true);
      const auto id=positiveId(idText),versionId=positiveId(versionText);
      const auto table=adminContentTable(type);
      const auto rows=service->database()->execSqlSync(
        "SELECT snapshot::text AS snapshot_text FROM admin_content_versions "
        "WHERE id=$1 AND content_type=$2 AND content_id=$3",versionId,type,id);
      if(rows.empty()) throw ApiError(drogon::k404NotFound,"找不到指定版本");
      saveAdminContentVersion(service->database(),type,id,user.id,"還原前自動備份");
      auto snapshot=parseJson(rows[0]["snapshot_text"].as<std::string>(),Json::Value{Json::objectValue});
      snapshot.removeMember("id");snapshot.removeMember("created_at");snapshot.removeMember("updated_at");
      Json::Value out;out["ok"]=true;out["content"]=genericUpdate(service->database(),table,id,snapshot);reply(cb,std::move(out));
    },"還原版本失敗");
  });

  reg2("/api/admin/content/{1}/{2}/dependencies", drogon::Get,
       [service](const Request& req,Callback& cb,const std::string& type,const std::string& idText){
    runHandler(cb,[&]{
      requireUser(service,req,true);
      const auto id=positiveId(idText);
      auto deps=adminContentDependencies(service->database(),type,id);
      Json::Value out;out["dependencies"]=deps;out["count"]=Json::UInt64(deps.size());reply(cb,std::move(out));
    },"讀取依賴關係失敗");
  });

  // [73-cpp.34.20 DM 共用效果模板]
  // [73-cpp.34.22 共用掉落池] DM 可重用怪物戰利品組合；怪物以 config.drop_pool_ids 引用。
  reg0("/api/admin/drop-pools", drogon::Get,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);Json::Value out;out["pools"]=genericList(service->database(),"admin_drop_pools");reply(cb,std::move(out));},"讀取共用掉落池失敗");
  });
  reg0("/api/admin/drop-pools", drogon::Post,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);const auto body=bodyOrEmpty(req);const auto name=trimmed(stringValue(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"請輸入掉落池名稱");Json::Value payload;payload["name"]=name;payload["category"]=trimmed(stringValue(body["category"],"一般"));payload["description"]=stringValue(body["description"]);payload["entries"]=body["entries"].isArray()?body["entries"]:Json::Value{Json::arrayValue};Json::Value out;out["ok"]=true;out["pool"]=genericInsert(service->database(),"admin_drop_pools",payload);reply(cb,std::move(out));},"建立共用掉落池失敗");
  });
  reg1("/api/admin/drop-pools/{1}", drogon::Put,[service](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{requireUser(service,req,true);const auto id=positiveId(idText);const auto body=bodyOrEmpty(req);const auto name=trimmed(stringValue(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"請輸入掉落池名稱");Json::Value payload;payload["name"]=name;payload["category"]=trimmed(stringValue(body["category"],"一般"));payload["description"]=stringValue(body["description"]);payload["entries"]=body["entries"].isArray()?body["entries"]:Json::Value{Json::arrayValue};Json::Value out;out["ok"]=true;out["pool"]=genericUpdate(service->database(),"admin_drop_pools",id,payload);reply(cb,std::move(out));},"修改共用掉落池失敗");
  });
  reg1("/api/admin/drop-pools/{1}", drogon::Delete,[service](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{requireUser(service,req,true);genericDelete(service->database(),"admin_drop_pools",positiveId(idText));Json::Value out;out["ok"]=true;reply(cb,std::move(out));},"刪除共用掉落池失敗");
  });

  reg0("/api/admin/effect-templates", drogon::Get,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);Json::Value out;out["templates"]=selectRows(service->database(),"SELECT row_to_json(e)::text AS json_text FROM admin_effect_templates e WHERE active=TRUE ORDER BY category,name,id");reply(cb,std::move(out));},"讀取共用效果模板失敗");
  });
  reg0("/api/admin/effect-templates", drogon::Post,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);const auto body=bodyOrEmpty(req);const auto name=trimmed(stringValue(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"請輸入效果模板名稱");const auto category=trimmed(stringValue(body["category"],"一般"));const auto effects=body["effects"].isArray()?body["effects"]:Json::Value{Json::arrayValue};const auto rows=service->database()->execSqlSync("INSERT INTO admin_effect_templates(name,category,effects) VALUES($1,$2,$3::jsonb) RETURNING row_to_json(admin_effect_templates)::text AS json_text",name,category,compactJson(effects));Json::Value out;out["ok"]=true;out["template"]=rowJson(rows);reply(cb,std::move(out));},"建立共用效果模板失敗");
  });
  reg1("/api/admin/effect-templates/{1}", drogon::Put,[service](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{requireUser(service,req,true);const auto id=positiveId(idText);const auto body=bodyOrEmpty(req);const auto name=trimmed(stringValue(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"請輸入效果模板名稱");const auto category=trimmed(stringValue(body["category"],"一般"));const auto effects=body["effects"].isArray()?body["effects"]:Json::Value{Json::arrayValue};const auto rows=service->database()->execSqlSync("UPDATE admin_effect_templates SET name=$1,category=$2,effects=$3::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=$4 RETURNING row_to_json(admin_effect_templates)::text AS json_text",name,category,compactJson(effects),id);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到效果模板");Json::Value out;out["ok"]=true;out["template"]=rowJson(rows);reply(cb,std::move(out));},"修改共用效果模板失敗");
  });
  reg1("/api/admin/effect-templates/{1}", drogon::Delete,[service](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{requireUser(service,req,true);const auto rows=service->database()->execSqlSync("UPDATE admin_effect_templates SET active=FALSE,updated_at=CURRENT_TIMESTAMP WHERE id=$1 RETURNING id",positiveId(idText));if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到效果模板");Json::Value out;out["ok"]=true;reply(cb,std::move(out));},"刪除共用效果模板失敗");
  });

  // [73-cpp.34.20 引用同步改名 + 安全刪除]
  reg2("/api/admin/content/{1}/{2}/rename-sync", drogon::Post,[service](const Request& req,Callback& cb,const std::string& type,const std::string& idText){
    runHandler(cb,[&]{const auto user=requireUser(service,req,true);const auto id=positiveId(idText);const auto current=adminContentSnapshot(service->database(),type,id);const auto oldName=trimmed(stringValue(current["name"]));const auto newName=trimmed(stringValue(bodyOrEmpty(req)["new_name"]));if(oldName.empty()||newName.empty())throw ApiError(drogon::k400BadRequest,"舊名稱或新名稱不可為空");if(oldName==newName){Json::Value out;out["ok"]=true;out["updated_references"]=0;reply(cb,std::move(out));return;}auto deps=adminContentDependencies(service->database(),type,id);std::int64_t changed=0;for(const auto& dep:deps){const auto depType=stringValue(dep["type"]);const auto depId=intValue(dep["id"]);auto snap=adminContentSnapshot(service->database(),depType,depId);if(replaceAdminContentStrings(snap,oldName,newName)){saveAdminContentVersion(service->database(),depType,depId,user.id,"同步改名前自動備份");snap.removeMember("id");snap.removeMember("created_at");snap.removeMember("updated_at");genericUpdate(service->database(),adminContentTable(depType),depId,snap);++changed;}}saveAdminContentVersion(service->database(),type,id,user.id,"改名前自動備份");Json::Value patch;patch["name"]=newName;genericUpdate(service->database(),adminContentTable(type),id,patch);Json::Value out;out["ok"]=true;out["old_name"]=oldName;out["new_name"]=newName;out["updated_references"]=Json::Int64(changed);reply(cb,std::move(out));},"同步改名失敗");
  });
  reg2("/api/admin/content/{1}/{2}/safe-delete", drogon::Delete,[service](const Request& req,Callback& cb,const std::string& type,const std::string& idText){
    runHandler(cb,[&]{const auto user=requireUser(service,req,true);const auto id=positiveId(idText);const bool force=req->getParameter("force")=="1";auto deps=adminContentDependencies(service->database(),type,id);if(!force&&!deps.empty()){Json::Value out;out["ok"]=false;out["blocked"]=true;out["dependencies"]=deps;out["count"]=Json::UInt64(deps.size());reply(cb,std::move(out),drogon::k409Conflict);return;}saveAdminContentVersion(service->database(),type,id,user.id,"刪除前自動備份");const auto table=adminContentTable(type);const auto rows=service->database()->execSqlSync("DELETE FROM \""+table+"\" WHERE id=$1 RETURNING id",id);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到內容");Json::Value out;out["ok"]=true;out["forced"]=force;reply(cb,std::move(out));},"安全刪除失敗");
  });

  // [73-cpp.34.20 DM 戰鬥控制中心]
  reg1("/api/admin/rooms/{1}/combat-center", drogon::Get,[service](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{requireUser(service,req,true);const auto roomId=positiveId(roomText);Json::Value out;const auto rr=service->database()->execSqlSync("SELECT row_to_json(r)::text AS json_text FROM rooms r WHERE id=$1",roomId);if(rr.empty())throw ApiError(drogon::k404NotFound,"找不到房間");out["room"]=rowJson(rr);out["players"]=selectRows1(service->database(),"SELECT row_to_json(x)::text AS json_text FROM (SELECT rm.user_id AS id,COALESCE(NULLIF(cc.name,''),rm.display_name,u.username) AS name,rm.hp,rm.max_hp,rm.spirit,rm.max_spirit,rm.current_sanity,rm.max_sanity,rm.turn_actions_remaining AS actions,rm.combat_state FROM room_members rm JOIN users u ON u.id=rm.user_id LEFT JOIN character_cards cc ON cc.user_id=rm.user_id WHERE rm.room_id=$1 AND rm.role='player' ORDER BY rm.joined_at) x",roomId);out["monsters"]=selectRows1(service->database(),"SELECT row_to_json(x)::text AS json_text FROM (SELECT rm.id,mt.name,rm.status,rm.current_hp AS hp,rm.max_hp,rm.current_spirit AS spirit,rm.max_spirit,rm.turn_actions_remaining AS actions,rm.boss_phase_index,rm.boss_phase_name FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 ORDER BY rm.id) x",roomId);out["npcs"]=selectRows1(service->database(),"SELECT row_to_json(x)::text AS json_text FROM (SELECT rn.npc_template_id AS id,nt.name,rn.status,rn.combat_side,rn.current_hp AS hp,rn.max_hp,rn.current_spirit AS spirit,rn.turn_actions_remaining AS actions FROM room_npcs rn JOIN npc_templates nt ON nt.id=rn.npc_template_id WHERE rn.room_id=$1 ORDER BY nt.name) x",roomId);reply(cb,std::move(out));},"讀取戰鬥控制中心失敗");
  });
  reg1("/api/admin/rooms/{1}/gm-command", drogon::Post,[service](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{const auto user=requireUser(service,req,true);const auto roomId=positiveId(roomText);const auto body=bodyOrEmpty(req);const auto op=stringValue(body["op"]);Json::Value out;out["ok"]=true;if(op=="summon_monster"){const auto name=trimmed(stringValue(body["name"]));const auto count=std::clamp<std::int64_t>(intValue(body["count"],1),1,20);const auto tr=service->database()->execSqlSync("SELECT id,name,max_hp,attributes::text AS attributes_text,great_way::text AS great_way_text FROM monster_templates WHERE name ILIKE $1 ORDER BY CASE WHEN name=$2 THEN 0 ELSE 1 END,id LIMIT 1","%"+name+"%",name);if(tr.empty())throw ApiError(drogon::k404NotFound,"找不到怪物模板："+name);const auto tid=tr[0]["id"].as<std::int64_t>(),hp=std::max<std::int64_t>(1,tr[0]["max_hp"].as<std::int64_t>());const auto attrs=parseJson(tr[0]["attributes_text"].as<std::string>());const auto spirit=std::max<std::int64_t>(0,intValue(attrs["spirit"]));Json::Value ids{Json::arrayValue};for(std::int64_t i=0;i<count;++i){const auto ins=service->database()->execSqlSync("INSERT INTO room_monsters(room_id,monster_template_id,status,current_hp,max_hp,current_spirit,max_spirit) VALUES($1,$2,'encountered',$3,$3,$4,$4) RETURNING id",roomId,tid,hp,spirit);ids.append(Json::Int64(ins[0]["id"].as<std::int64_t>()));}Json::Value ev;ev["text"]="DM 快捷召喚："+tr[0]["name"].as<std::string>()+" × "+std::to_string(count);service->addEvent(roomId,user.id,"system",ev);out["instance_ids"]=ids;out["count"]=Json::Int64(count);}else if(op=="give_coin"){const auto target=trimmed(stringValue(body["target"]));const auto amount=intValue(body["amount"]);const auto currency=stringValue(body["currency"],"weird");if(target.empty()||amount==0)throw ApiError(drogon::k400BadRequest,"請指定角色與數量");const auto col=currency=="game"?"game_coins":"weird_coins";const auto rows=service->database()->execSqlSync("UPDATE character_cards cc SET "+std::string(col)+"=GREATEST(0,"+std::string(col)+"+$1),updated_at=CURRENT_TIMESTAMP FROM users u WHERE u.id=cc.user_id AND (cc.name ILIKE $2 OR u.username ILIKE $2) RETURNING cc.user_id,cc.name",amount,"%"+target+"%");if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到角色："+target);out["affected"]=Json::UInt64(rows.size());}else if(op=="move_all"){const auto nodeName=trimmed(stringValue(body["node"]));const auto nr=service->database()->execSqlSync("SELECT id,name FROM room_map_nodes WHERE room_id=$1 AND name ILIKE $2 ORDER BY CASE WHEN name=$3 THEN 0 ELSE 1 END,id LIMIT 1",roomId,"%"+nodeName+"%",nodeName);if(nr.empty())throw ApiError(drogon::k404NotFound,"找不到地圖節點："+nodeName);const auto nodeId=nr[0]["id"].as<std::int64_t>();service->database()->execSqlSync("UPDATE room_members SET current_map_node_id=$1 WHERE room_id=$2",nodeId,roomId);service->database()->execSqlSync("UPDATE rooms SET current_map_node_id=$1,updated_at=CURRENT_TIMESTAMP WHERE id=$2",nodeId,roomId);out["node_id"]=Json::Int64(nodeId);out["node_name"]=nr[0]["name"].as<std::string>();}else throw ApiError(drogon::k400BadRequest,"不支援的 GM 指令");RoomSocket::broadcastSnapshotForRoom(roomId);reply(cb,std::move(out));},"執行 GM 快捷指令失敗");
  });

  reg1("/api/admin/content-drafts/{1}", drogon::Get,
       [service](const Request& req,Callback& cb,const std::string& key){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req,true);
      const auto rows=service->database()->execSqlSync(
        "SELECT payload::text AS payload_text,updated_at FROM admin_content_drafts WHERE user_id=$1 AND draft_key=$2",
        user.id,key);
      Json::Value out;
      if(rows.empty())out["draft"]=Json::nullValue;
      else{out["draft"]["payload"]=parseJson(rows[0]["payload_text"].as<std::string>(),Json::Value{Json::objectValue});out["draft"]["updated_at"]=rows[0]["updated_at"].as<std::string>();}
      reply(cb,std::move(out));
    },"讀取草稿失敗");
  });
  reg1("/api/admin/content-drafts/{1}", drogon::Put,
       [service](const Request& req,Callback& cb,const std::string& key){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req,true);
      const auto body=bodyOrEmpty(req);
      const auto payload=body["payload"].isObject()?body["payload"]:Json::Value{Json::objectValue};
      service->database()->execSqlSync(
        "INSERT INTO admin_content_drafts(user_id,draft_key,payload) VALUES($1,$2,$3::jsonb) "
        "ON CONFLICT(user_id,draft_key) DO UPDATE SET payload=EXCLUDED.payload,updated_at=CURRENT_TIMESTAMP",
        user.id,key,compactJson(payload));
      Json::Value out;out["ok"]=true;reply(cb,std::move(out));
    },"保存草稿失敗");
  });
  reg1("/api/admin/content-drafts/{1}", drogon::Delete,
       [service](const Request& req,Callback& cb,const std::string& key){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req,true);
      service->database()->execSqlSync("DELETE FROM admin_content_drafts WHERE user_id=$1 AND draft_key=$2",user.id,key);
      Json::Value out;out["ok"]=true;reply(cb,std::move(out));
    },"刪除草稿失敗");
  });

  // [功能備註｜73-cpp.34.19 DM 編輯器儀表板｜GET /api/admin/editor-dashboard]
  // 由 C++ 直接彙整 PostgreSQL，避免前端自行猜測資料庫狀態。
  reg0("/api/admin/editor-dashboard", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb,[&]{
      requireUser(service,req,true);
      auto countSql=[&](const std::string& sql)->std::int64_t{
        const auto rows=service->database()->execSqlSync(sql);
        return rows.empty()?0:rows[0]["n"].as<std::int64_t>();
      };
      Json::Value counts;
      counts["online_players"]=Json::Int64(countSql("SELECT COUNT(DISTINCT rm.user_id)::BIGINT AS n FROM room_members rm JOIN users u ON u.id=rm.user_id WHERE u.is_admin=FALSE AND rm.last_seen_at >= CURRENT_TIMESTAMP-INTERVAL '10 minutes'"));
      counts["rooms"]=Json::Int64(countSql("SELECT COUNT(*)::BIGINT AS n FROM rooms"));
      counts["skills"]=Json::Int64(countSql("SELECT COUNT(*)::BIGINT AS n FROM skill_templates"));
      counts["items"]=Json::Int64(countSql("SELECT COUNT(*)::BIGINT AS n FROM item_templates"));
      counts["recipes"]=Json::Int64(countSql("SELECT COUNT(*)::BIGINT AS n FROM recipes"));
      counts["monsters"]=Json::Int64(countSql("SELECT COUNT(*)::BIGINT AS n FROM monster_templates"));
      counts["npcs"]=Json::Int64(countSql("SELECT COUNT(*)::BIGINT AS n FROM npc_templates"));
      counts["drafts"]=Json::Int64(countSql("SELECT COUNT(*)::BIGINT AS n FROM admin_content_drafts"));
      Json::Value recentVersions{Json::arrayValue};
      for(const auto& row:service->database()->execSqlSync("SELECT content_type,content_id,change_note,created_at FROM admin_content_versions ORDER BY created_at DESC,id DESC LIMIT 12")){
        Json::Value x;x["content_type"]=row["content_type"].as<std::string>();x["content_id"]=Json::Int64(row["content_id"].as<std::int64_t>());x["change_note"]=row["change_note"].as<std::string>();x["created_at"]=row["created_at"].as<std::string>();recentVersions.append(x);
      }
      Json::Value recentDrafts{Json::arrayValue};
      for(const auto& row:service->database()->execSqlSync("SELECT draft_key,updated_at FROM admin_content_drafts ORDER BY updated_at DESC LIMIT 12")){
        Json::Value x;x["draft_key"]=row["draft_key"].as<std::string>();x["updated_at"]=row["updated_at"].as<std::string>();recentDrafts.append(x);
      }
      Json::Value out;out["counts"]=counts;out["recent_versions"]=recentVersions;out["recent_drafts"]=recentDrafts;reply(cb,std::move(out));
    },"讀取 DM 編輯器儀表板失敗");
  });
  ...
  reg0("/api/recipes", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      const auto user = requireUser(service, req);
      const auto all = genericList(service->database(), "recipes", "id ASC");
      Json::Value visible{Json::arrayValue};
      for (const auto& recipe : all) {
        const auto cfg = recipe["config"].isObject() ? recipe["config"] : Json::Value{Json::objectValue};
        const bool hidden = cfg.get("hidden_until_discovered", false).asBool();
        const bool discovered = !hidden || recipeIsDiscovered(service->database(), user.id, intValue(recipe["id"]));
        if (!discovered) {
          if (!cfg.get("preview_before_discovery", true).asBool()) continue;
          Json::Value preview{Json::objectValue};
          preview["id"] = recipe["id"];
          preview["name"] = recipe["name"];
          preview["rank"] = recipe["rank"];
          preview["recipe_type"] = recipe["recipe_type"];
          preview["craft_category"] = stringValue(cfg["craft_category"], stringValue(recipe["recipe_type"], "一般"));
          preview["discoverable_by_experiment"] = cfg.get("discoverable_by_experiment", false).asBool();
          preview["discovered"] = false;
          preview["locked_preview"] = true;
          visible.append(std::move(preview));
          continue;
        }
        auto row = recipe;
        row["discovered"] = discovered;
        row["locked_preview"] = false;
        row["rank_success_rate"] = Json::Int64(defaultRecipeRankSuccessRate(stringValue(recipe["rank"], "G")));
        row["base_success_rate"] = Json::Int64(recipeBaseSuccessRate(recipe));
        visible.append(std::move(row));
      }
      Json::Value out;
      out["recipes"] = std::move(visible);
      reply(cb, std::move(out));
    });
  });
  ...
  reg1("/api/recipes/{1}/craft", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      if (config.get("hidden_until_discovered", false).asBool() &&
          !recipeIsDiscovered(service->database(), user.id, recipeId)) {
        throw ApiError(drogon::k403Forbidden, "這個配方尚未被你發現");
      }
      if (stringValue(config["access"], "public") == "skill") {
        const auto requiredSkillId = intValue(config["required_skill_template_id"]);
        if (requiredSkillId <= 0 || !characterOwnsSkillTemplateCpp(character, requiredSkillId))
          throw ApiError(drogon::k403Forbidden, "缺少必要製作技能");
      }
      auto inventory = inventoryOf(character);
      ...
      const auto baseSuccessRate = recipeBaseSuccessRate(recipe);
      const auto craftBonus = characterCraftSuccessBonusCpp(character, stringValue(config["craft_category"], stringValue(recipe["recipe_type"], "一般")));
      const auto successRate = std::clamp<std::int64_t>(baseSuccessRate + craftBonus, 0, 100);
      ...
      if (success) {
        ...
        auto craftedOutput = output;
        if (!craftedOutput.isMember("rank") || trimmed(stringValue(craftedOutput["rank"])).empty())
          craftedOutput["rank"] = normalizeContentRank(stringValue(recipe["rank"], "G"));
        ...
        if (outputType == "special_currency") {
          ...
          Json::Value crafted = craftedOutput; crafted["name"] = outputName; crafted["quantity"] = Json::Int64(1);
          ...
          inventory = changeInventory(std::move(inventory), outputName, outputQty, outputCategory, craftedOutput);
        }
      }
      service->database()->execSqlSync(
      ...
      out["success"] = success;
      out["roll"] = roll;
      out["base_success_rate"] = Json::Int64(baseSuccessRate);
      out["skill_bonus"] = Json::Int64(craftBonus);
  ...
  // [功能備註｜65-cpp.26.15 配方試驗｜POST /api/recipes/experiment]
  // 玩家可直接拿背包材料做未知配方試驗。只有完全符合 DM 設定且允許試驗的配方才會產生反應；成功後永久發現配方。
  reg0("/api/recipes/experiment", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      const auto user = requireUser(service, req);
      const auto body = bodyOrEmpty(req);
      const auto materials = body["materials"].isArray() ? body["materials"] : Json::Value{Json::arrayValue};
      if (materials.empty()) throw ApiError(drogon::k400BadRequest, "請至少放入一種材料");
      ...
      for (const auto& material : materials) {
        ...
        const auto category = trimmed(stringValue(material["category"]));
        const auto quantity = std::max<std::int64_t>(1, intValue(material.isMember("quantity") ? material["quantity"] : material["qty"], 1));
        if (name.empty()) throw ApiError(drogon::k400BadRequest, "試驗材料名稱不能空白");
        if (inventoryQuantity(inventory, name, category) < quantity)
          throw ApiError(drogon::k400BadRequest, "材料不足：" + name + " 需要 " + std::to_string(quantity));
      }

      const auto signature = recipeMaterialSignature(materials);
      const auto recipes = genericList(service->database(), "recipes", "id ASC");
      Json::Value matched{Json::objectValue};
      for (const auto& recipe : recipes) {
        const auto cfg = recipe["config"].isObject() ? recipe["config"] : Json::Value{Json::objectValue};
        if (!cfg.get("discoverable_by_experiment", false).asBool()) continue;
        if (recipeMaterialSignature(recipe["materials"]) != signature) continue;
        matched = recipe;
        break;
      }

      if (!matched.isObject() || intValue(matched["id"]) <= 0) {
        recordRecipeExperiment(service->database(), user.id, 0, materials, false, 0, 0, false);
        Json::Value out;
        out["ok"] = true;
        out["matched"] = false;
        out["success"] = false;
        out["consumed"] = false;
        out["message"] = "沒有形成可辨識的配方反應，材料未消耗";
        out["character"] = character;
        reply(cb, std::move(out));
        return;
      }

      const auto recipeId = intValue(matched["id"]);
      const auto cfg = matched["config"].isObject() ? matched["config"] : Json::Value{Json::objectValue};
      if (stringValue(cfg["access"], "public") == "skill") {
        const auto requiredSkillId = intValue(cfg["required_skill_template_id"]);
        if (requiredSkillId <= 0 || !characterOwnsSkillTemplateCpp(character, requiredSkillId))
          throw ApiError(drogon::k403Forbidden, "材料似乎有反應，但你缺少必要製作技能");
      }

      auto weirdCoins = std::max<std::int64_t>(0, intValue(character["weird_coins"]));
      auto gameCoins = std::max<std::int64_t>(0, intValue(character["game_coins"]));
      const auto currency = stringValue(cfg["currency_type"], "none");
      const auto cost = std::max<std::int64_t>(0, intValue(cfg["currency_cost"]));
      if (currency == "weird" && weirdCoins < cost) throw ApiError(drogon::k400BadRequest, "配方反應需要額外詭幣，但目前不足");
      if (currency == "game" && gameCoins < cost) throw ApiError(drogon::k400BadRequest, "配方反應需要額外遊戲幣，但目前不足");

      const auto baseSuccessRate = recipeBaseSuccessRate(matched);
      const auto craftBonus = characterCraftSuccessBonusCpp(character, stringValue(cfg["craft_category"], stringValue(matched["recipe_type"], "一般")));
      const auto successRate = std::clamp<std::int64_t>(baseSuccessRate + craftBonus, 0, 100);
      const auto roll = randomInt(1, 100);
      const bool success = roll <= successRate;
      const bool consume = success || !cfg.isMember("failure_consumes") || cfg["failure_consumes"].asBool();
      ...
      if (consume) {
        for (const auto& material : materials) {
          ...
          const auto category = trimmed(stringValue(material["category"]));
          const auto quantity = std::max<std::int64_t>(1, intValue(material.isMember("quantity") ? material["quantity"] : material["qty"], 1));
          inventory = changeInventory(std::move(inventory), name, -quantity, category);
        }
        if (currency == "weird") weirdCoins -= cost;
        if (currency == "game") gameCoins -= cost;
      }
      ...
      auto equipment = character["equipment"].isArray() ? character["equipment"] : Json::Value{Json::arrayValue};
      auto weapons = character["weapons"].isArray() ? character["weapons"] : Json::Value{Json::arrayValue};
      const bool alreadyDiscovered = recipeIsDiscovered(service->database(), user.id, recipeId);
      if (success) {
        const auto output = matched["output"].isObject() ? matched["output"] : Json::Value{Json::objectValue};
        auto craftedOutput = output;
        if (!craftedOutput.isMember("rank") || trimmed(stringValue(craftedOutput["rank"])).empty())
          craftedOutput["rank"] = normalizeContentRank(stringValue(matched["rank"], "G"));
        const auto outputType = stringValue(output["type"], "item");
        const auto outputName = trimmed(stringValue(output["name"], stringValue(matched["name"], "未命名成品")));
        const auto outputQty = std::max<std::int64_t>(1, intValue(output["quantity"], 1));
        const auto outputCategory = stringValue(output["category"]);
        if (outputType == "special_currency") {
          const auto code = trimmed(stringValue(output["currency_code"], stringValue(output["code"], outputName)));
          if (code.empty() || service->database()->execSqlSync("SELECT 1 FROM special_currency_types WHERE code=$1 AND active=TRUE", code).empty())
            throw ApiError(drogon::k400BadRequest, "試驗配方指定的特殊貨幣不存在");
          service->database()->execSqlSync(
              "INSERT INTO character_special_currencies(user_id,currency_code,amount) VALUES($1,$2,$3) "
              "ON CONFLICT(user_id,currency_code) DO UPDATE SET amount=character_special_currencies.amount+EXCLUDED.amount,updated_at=CURRENT_TIMESTAMP",
              user.id, code, outputQty);
      ...
      service->database()->execSqlSync(
          "UPDATE character_cards SET inventory=$1::jsonb,equipment=$2::jsonb,weapons=$3::jsonb,weird_coins=$4,game_coins=$5,updated_at=CURRENT_TIMESTAMP WHERE user_id=$6",
          compactJson(inventory), compactJson(equipment), compactJson(weapons), weirdCoins, gameCoins, user.id);
      recordRecipeExperiment(service->database(), user.id, recipeId, materials, success, roll, successRate, consume);
      ...
      out["newly_discovered"] = success && !alreadyDiscovered;
      if (success) {
        auto discoveredRecipe = matched;
        discoveredRecipe["discovered"] = true;
        discoveredRecipe["base_success_rate"] = Json::Int64(baseSuccessRate);
        out["recipe"] = std::move(discoveredRecipe);
        out["message"] = alreadyDiscovered ? "試驗成功，已再次製作出成品" : "試驗成功，發現了新配方！";
      } else {
        out["message"] = "材料產生了明顯反應，但這次試驗沒有成功";
      }
      out["character"] = characterRaw(service->database(), user.id);
      reply(cb, std::move(out));
    }, "配方試驗失敗");
  ...
  reg0("/api/admin/shop", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      requireUser(service, req, true);
      auto body = bodyOrEmpty(req);
      normalizeShopItemWrite(body, true);
  ...
  reg1("/api/admin/shop/{1}", drogon::Patch,
    ...
    runHandler(cb, [&] {
      requireUser(service, req, true);
      auto body = bodyOrEmpty(req);
      normalizeShopItemWrite(body, false);
      syncShopItemCategory(service->database(), body);
      Json::Value out;
      out["ok"] = true;
  ...
  reg1("/api/character/skills/{1}/use", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      Json::Value selected;
      std::int64_t growthTargetId = 0;
      std::int64_t growthXpOnUse = 0;
      double gearDamageMultiplier = 1.0;
      std::string growthGearName;
      for (const auto& skill : skills) {
        if (stringValue(skill["id"]) == skillId || std::to_string(intValue(skill["template_id"], -1)) == skillId) {
          selected = skill;
          break;
      ...
      // 裝備期間授予的技能使用 gear_<模板ID>。舊版前端已有此 ID，但後端只搜尋角色永久技能，
      // 因此這裡正式由 C++ 驗證「目前確實已裝備該來源」並把成長／技能倍率接入戰鬥核心。
      if (selected.isNull() && skillId.rfind("gear_", 0) == 0) {
        std::int64_t gearTemplateId = 0;
        try { gearTemplateId = std::stoll(skillId.substr(5)); } catch (...) { gearTemplateId = 0; }
        if (gearTemplateId > 0) {
          const auto equippedCharacter = service->characterByUser(user.id);
          auto findSource = [&](const Json::Value& list) -> Json::Value {
            if (!list.isArray()) return Json::Value{};
            for (const auto& gear : list) {
              if (!gear.isObject() || stringValue(gear["grant_mode"]) != "equipped") continue;
              bool grants = false;
              if (gear["granted_skill_template_ids"].isArray()) {
                for (const auto& rawId : gear["granted_skill_template_ids"]) {
                  if (intValue(rawId) == gearTemplateId) { grants = true; break; }
                }
              }
              if (grants) return gear;
            }
            return Json::Value{};
          };
          auto sourceGear = findSource(equippedCharacter["equipped_weapons"]);
          if (sourceGear.isNull()) sourceGear = findSource(equippedCharacter["equipped_equipment"]);
          if (sourceGear.isObject()) {
            const auto templateRows = service->database()->execSqlSync(
                "SELECT row_to_json(s)::text AS json_text FROM skill_templates s WHERE id=$1", gearTemplateId);
            if (!templateRows.empty()) {
              selected = rowJson(templateRows);
              selected["id"] = skillId;
              selected["template_id"] = Json::Int64(gearTemplateId);
              growthTargetId = intValue(sourceGear["affix_target_id"]);
              growthXpOnUse = std::max<std::int64_t>(0, intValue(sourceGear["growth_xp_on_use"], 1));
              growthGearName = stringValue(sourceGear["name"], "成長裝備");
              gearDamageMultiplier *= std::max(0.0, sourceGear.get("growth_damage_multiplier", 1.0).asDouble());
              if (sourceGear["skill_modifiers"].isArray()) {
                for (const auto& mod : sourceGear["skill_modifiers"]) {
                  if (intValue(mod["skill_template_id"]) == gearTemplateId)
                    gearDamageMultiplier *= std::max(0.0, mod.get("damage_multiplier", 1.0).asDouble());
                }
              }
            }
          }
      ...
      if (selected.isNull()) throw ApiError(drogon::k404NotFound, "找不到技能或目前未裝備技能來源");
      ...
      if (growthTargetId > 0) {
        auto data = selected["data"].isObject() ? selected["data"] : Json::Value{Json::objectValue};
        data["gear_damage_multiplier"] = std::clamp(gearDamageMultiplier, 0.0, 1000.0);
        data["gear_growth_target_id"] = Json::Int64(growthTargetId);
        data["gear_growth_source_name"] = growthGearName;
        selected["data"] = std::move(data);
      }
      ...
      if (growthTargetId > 0 && growthXpOnUse > 0) {
        const auto growth = grantGearGrowthXp(service, user.id, growthTargetId, growthXpOnUse);
        result["growth"]["enabled"] = growth.enabled;
        result["growth"]["changed"] = growth.changed;
        result["growth"]["levelled"] = growth.levelled;
        result["growth"]["rank_changed"] = growth.rankChanged;
        result["growth"]["target_id"] = Json::Int64(growth.targetId);
        result["growth"]["level"] = Json::Int64(growth.level);
        result["growth"]["xp"] = Json::Int64(growth.xp);
        result["growth"]["xp_needed"] = Json::Int64(growth.xpNeeded);
        result["growth"]["xp_gained"] = Json::Int64(growth.xpGained);
        result["growth"]["rank"] = growth.rank;
        result["growth"]["gear_name"] = growthGearName;
      }
  ...
  reg2("/api/character/gear/{1}/{2}/use", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      const auto growthXp = std::max<std::int64_t>(0, intValue(gear["growth_xp_on_use"], 1));
      const auto growth = growthXp > 0
                              ? grantHeldGearGrowthXpByIndex(service, user.id, kindText, index, growthXp)
                              : GearGrowthResult{};
      Json::Value out;
      out["ok"] = true;
      out["gear"] = gear;
      out["growth"]["enabled"] = growth.enabled;
      out["growth"]["changed"] = growth.changed;
      out["growth"]["levelled"] = growth.levelled;
      out["growth"]["rank_changed"] = growth.rankChanged;
      out["growth"]["target_id"] = Json::Int64(growth.targetId);
      out["growth"]["level"] = Json::Int64(growth.level);
      out["growth"]["xp"] = Json::Int64(growth.xp);
      out["growth"]["xp_needed"] = Json::Int64(growth.xpNeeded);
      out["growth"]["xp_gained"] = Json::Int64(growth.xpGained);
      out["growth"]["rank"] = growth.rank;
      out["character"] = service->characterByUser(user.id);
      reply(cb, std::move(out));
    }, "使用裝備失敗");
  });
  ...
  // [功能備註｜65-cpp.26.15 配方可重複修改｜PATCH /api/admin/recipes/{1}]
  reg1("/api/admin/recipes/{1}", drogon::Patch,
       [service](const Request& req, Callback& cb, const std::string& idText) {
    runHandler(cb, [&] {
      requireUser(service, req, true);
      auto body = bodyOrEmpty(req);
      ...
      Json::Value out;
      out["ok"] = true;
      out["recipe"] = genericUpdate(service->database(), "recipes", positiveId(idText), body);
      reply(cb, std::move(out));
    }, "修改合成配方失敗");
  });
  ...
  // [功能備註｜65-cpp.26.5 DM 自由屬性點｜POST /api/admin/players/{1}/attribute-points/grant]
  // DM 可直接給指定玩家額外自由屬性點；玩家只能在既有總額內重新分配，不能自行增生點數。
  reg1("/api/admin/players/{1}/attribute-points/grant", drogon::Post,
       [service](const Request& req, Callback& cb, const std::string& userIdText) {
    runHandler(cb, [&] {
      requireUser(service, req, true);
      const auto userId = positiveId(userIdText, "玩家 ID 無效");
      ensureCharacter(service->database(), userId);
      const auto body = bodyOrEmpty(req);
      const auto amount = intValue(body["amount"], intValue(body["delta"]));
      if (amount <= 0) throw ApiError(drogon::k400BadRequest, "給予點數必須大於 0");
      if (amount > 1000000000LL) throw ApiError(drogon::k400BadRequest, "單次給予點數過大");
      service->database()->execSqlSync(
          "UPDATE character_cards SET attribute_points=GREATEST(0,attribute_points)+$1,updated_at=CURRENT_TIMESTAMP WHERE user_id=$2",
          amount, userId);
      Json::Value out;
      out["ok"] = true;
      out["granted"] = Json::Int64(amount);
  ...
  reg0("/api/admin/npc-templates", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] { requireUser(service, req, true); auto body=bodyOrEmpty(req); auto attrs=body["attributes"].isObject()?body["attributes"]:Json::Value{Json::objectValue}; const auto con=std::max<std::int64_t>(0,intValue(attrs["constitution"])),spi=std::max<std::int64_t>(0,intValue(attrs["spirit"])); body["max_hp"]=Json::Int64(con*2); auto cfg=body["config"].isObject()?body["config"]:Json::Value{Json::objectValue}; cfg["derived_stats"]["max_hp"]=Json::Int64(con*2);cfg["derived_stats"]["endurance"]=Json::Int64((con+spi)/2);cfg["derived_stats"]["will"]=Json::Int64((spi*8)/10);cfg["derived_stats"]["sanity"]=Json::Int64((spi*12)/10);body["config"]=cfg; Json::Value out; out["ok"] = true; out["npc_template"] = genericInsert(service->database(), "npc_templates", body); reply(cb, std::move(out)); });
  });
  // [功能備註｜NPC｜PATCH/DELETE /api/admin/npc-templates/{1}]
  // 用途：NPC的共用管理 CRUD；修改欄位規則時請同步檢查資料表與前端管理頁。
  reg1("/api/admin/npc-templates/{1}", drogon::Patch, [service](const Request& req, Callback& cb, const std::string& idText){ runHandler(cb,[&]{ requireUser(service,req,true); const auto id=positiveId(idText); auto body=bodyOrEmpty(req); const auto rows=service->database()->execSqlSync("SELECT attributes::text AS attributes_text,config::text AS config_text FROM npc_templates WHERE id=$1",id); if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到 NPC"); auto attrs=parseJson(rows[0]["attributes_text"].as<std::string>(),Json::Value{Json::objectValue}); if(body["attributes"].isObject())for(const auto& k:body["attributes"].getMemberNames())attrs[k]=body["attributes"][k]; const auto con=std::max<std::int64_t>(0,intValue(attrs["constitution"])),spi=std::max<std::int64_t>(0,intValue(attrs["spirit"])); body["attributes"]=attrs;body["max_hp"]=Json::Int64(con*2);auto cfg=parseJson(rows[0]["config_text"].as<std::string>(),Json::Value{Json::objectValue});if(body["config"].isObject())for(const auto& k:body["config"].getMemberNames())cfg[k]=body["config"][k];cfg["derived_stats"]["max_hp"]=Json::Int64(con*2);cfg["derived_stats"]["endurance"]=Json::Int64((con+spi)/2);cfg["derived_stats"]["will"]=Json::Int64((spi*8)/10);cfg["derived_stats"]["sanity"]=Json::Int64((spi*12)/10);body["config"]=cfg;Json::Value out;out["ok"]=true;out["npc_template"]=genericUpdate(service->database(),"npc_templates",id,body);reply(cb,std::move(out));},"修改 NPC 失敗"); });
  reg1("/api/admin/npc-templates/{1}", drogon::Delete, [service](const Request& req, Callback& cb, const std::string& idText){runHandler(cb,[&]{requireUser(service,req,true);genericDelete(service->database(),"npc_templates",positiveId(idText));Json::Value out;out["ok"]=true;reply(cb,std::move(out));},"刪除 NPC 失敗");});
  // [功能備註｜65-cpp.26.11 NPC 管理 2.0] 一鍵複製 NPC 模板，保留設定但不複製房間狀態／關係／商店庫存。
  reg1("/api/admin/npc-templates/{1}/clone", drogon::Post, [service](const Request& req, Callback& cb, const std::string& idText) {
    runHandler(cb, [&] {
      requireUser(service, req, true);
      const auto id = positiveId(idText, "NPC ID 無效");
      const auto rows = service->database()->execSqlSync("SELECT row_to_json(n)::text AS json_text FROM npc_templates n WHERE id=$1", id);
      if (rows.empty()) throw ApiError(drogon::k404NotFound, "找不到 NPC");
      auto copy = rowJson(rows);
      copy.removeMember("id"); copy.removeMember("created_at"); copy.removeMember("updated_at");
      copy["name"] = trimmed(stringValue(copy["name"], "NPC")) + "（複製）";
      const auto imageUrl = stringValue(copy["image_url"]);
      const auto portraitUrl = stringValue(copy["portrait_url"]);
      // DB-backed image endpoints include the original NPC/variant identity. Do not clone those URLs,
      // otherwise editing/deleting the original image can make the clone point at the wrong record.
      copy["image_url"] = imageUrl.rfind("/api/npc-avatar-images/", 0) == 0 ? "" : imageUrl;
      copy["portrait_url"] = portraitUrl.rfind("/api/npc-portrait-variants/", 0) == 0 ? "" : portraitUrl;
      Json::Value out; out["ok"] = true; out["npc_template"] = genericInsert(service->database(), "npc_templates", copy); reply(cb, std::move(out));
    }, "複製 NPC 失敗");
  });

  // [功能備註｜73-cpp.34.25.32 副本投稿／建議] 玩家可提交概念；DM 才能查看全部投稿與審核備註。
  reg0("/api/dungeon-submissions", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      const auto user=requireUser(service,req); const auto body=bodyOrEmpty(req);
      const auto title=trimmed(stringValue(body["title"])); const auto concept=trimmed(stringValue(body["concept"]));
      if(title.empty()) throw ApiError(drogon::k400BadRequest,"請填寫投稿標題");
      if(title.size()>160||concept.size()>5000) throw ApiError(drogon::k400BadRequest,"投稿內容過長");
      auto type=stringValue(body["submission_type"],"suggestion"); if(type!="suggestion"&&type!="full")type="suggestion";
      Json::Value tags=body["tags"]; if(!tags.isArray())tags=Json::Value(Json::arrayValue);
      const auto rows=service->database()->execSqlSync("INSERT INTO dungeon_submissions(user_id,title,submission_type,concept,recommended_level,tags) VALUES($1,$2,$3,$4,$5,$6::jsonb) RETURNING *",user.id,title,type,concept,trimmed(stringValue(body["recommended_level"])),compactJson(tags));
      Json::Value out;out["ok"]=true;out["submission"]=rowJson(rows);reply(cb,std::move(out));
    },"提交副本建議失敗");
  });
  reg0("/api/admin/dungeon-submissions", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb,[&]{requireUser(service,req,true);Json::Value out(Json::arrayValue);auto rows=service->database()->execSqlSync("SELECT ds.*,u.username FROM dungeon_submissions ds JOIN users u ON u.id=ds.user_id ORDER BY CASE ds.status WHEN 'pending' THEN 0 WHEN 'reviewing' THEN 1 ELSE 2 END,ds.created_at DESC LIMIT 200");for(const auto&r:rows)out.append(rowJson(r));Json::Value body;body["submissions"]=out;reply(cb,std::move(body));},"讀取副本投稿失敗");
  });
  reg1("/api/admin/dungeon-submissions/{1}", drogon::Patch, [service](const Request& req, Callback& cb,const std::string&idText) {
    runHandler(cb,[&]{requireUser(service,req,true);const auto id=positiveId(idText);auto body=bodyOrEmpty(req);Json::Value patch;auto status=stringValue(body["status"]);if(status=="pending"||status=="reviewing"||status=="accepted"||status=="rejected")patch["status"]=status;if(body.isMember("admin_note"))patch["admin_note"]=trimmed(stringValue(body["admin_note"]));if(patch.empty())throw ApiError(drogon::k400BadRequest,"沒有可更新內容");Json::Value out;out["ok"]=true;out["submission"]=genericUpdate(service->database(),"dungeon_submissions",id,patch);reply(cb,std::move(out));},"更新副本投稿失敗");
  });

  // [功能備註｜73-cpp.34.25.33 遊戲群] 跨房間持久群組；只有群成員可讀成員/留言，只有群主或 DM 可修改/刪除。
  reg0("/api/game-groups", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);Json::Value groups(Json::arrayValue);const auto rows=service->database()->execSqlSync(
      "SELECT row_to_json(x)::text AS json_text FROM (SELECT g.id,g.owner_user_id,g.name,g.description,g.invite_code,gm.role,(SELECT COUNT(*)::BIGINT FROM game_group_members m2 WHERE m2.group_id=g.id) AS member_count,g.created_at,g.updated_at FROM game_groups g JOIN game_group_members gm ON gm.group_id=g.id WHERE gm.user_id=$1 ORDER BY g.updated_at DESC,g.id DESC) x",user.id);
      for(const auto&r:rows)groups.append(parseJson(r["json_text"].as<std::string>()));Json::Value out;out["groups"]=groups;reply(cb,std::move(out));},"讀取遊戲群失敗");
  });
  reg0("/api/game-groups", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto body=bodyOrEmpty(req);auto name=trimmed(stringValue(body["name"]));auto description=trimmed(stringValue(body["description"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"請輸入遊戲群名稱");name=clipped(name,120);description=clipped(description,2000);
      const auto rows=service->database()->execSqlSync("INSERT INTO game_groups(owner_user_id,name,description,invite_code) VALUES($1,$2,$3,UPPER(SUBSTRING(MD5(random()::text||clock_timestamp()::text||$1::text),1,8))) RETURNING row_to_json(game_groups)::text AS json_text",user.id,name,description);auto group=rowJson(rows);service->database()->execSqlSync("INSERT INTO game_group_members(group_id,user_id,role) VALUES($1,$2,'owner') ON CONFLICT(group_id,user_id) DO UPDATE SET role='owner'",intValue(group["id"]),user.id);Json::Value out;out["ok"]=true;out["group"]=group;reply(cb,std::move(out));},"建立遊戲群失敗");
  });
  reg0("/api/game-groups/join", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto code=trimmed(stringValue(bodyOrEmpty(req)["invite_code"]));if(code.empty())throw ApiError(drogon::k400BadRequest,"請輸入邀請碼");const auto rows=service->database()->execSqlSync("SELECT id FROM game_groups WHERE invite_code=UPPER($1) LIMIT 1",code);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到這個遊戲群");const auto groupId=rows[0]["id"].as<std::int64_t>();service->database()->execSqlSync("INSERT INTO game_group_members(group_id,user_id,role) VALUES($1,$2,'member') ON CONFLICT(group_id,user_id) DO NOTHING",groupId,user.id);Json::Value out;out["ok"]=true;out["group_id"]=Json::Int64(groupId);reply(cb,std::move(out));},"加入遊戲群失敗");
  });
  reg1("/api/game-groups/{1}", drogon::Get, [service](const Request& req, Callback& cb,const std::string&idText) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);if(!user.isAdmin&&service->database()->execSqlSync("SELECT 1 FROM game_group_members WHERE group_id=$1 AND user_id=$2",id,user.id).empty())throw ApiError(drogon::k403Forbidden,"你不是這個遊戲群的成員");const auto rows=service->database()->execSqlSync("SELECT row_to_json(g)::text AS json_text FROM game_groups g WHERE id=$1",id);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到遊戲群");Json::Value out;out["group"]=rowJson(rows);Json::Value members(Json::arrayValue);for(const auto&r:service->database()->execSqlSync("SELECT row_to_json(x)::text AS json_text FROM (SELECT gm.user_id,u.username,gm.role,gm.joined_at FROM game_group_members gm JOIN users u ON u.id=gm.user_id WHERE gm.group_id=$1 ORDER BY CASE gm.role WHEN 'owner' THEN 0 ELSE 1 END,gm.joined_at) x",id))members.append(parseJson(r["json_text"].as<std::string>()));out["members"]=members;Json::Value messages(Json::arrayValue);for(const auto&r:service->database()->execSqlSync("SELECT row_to_json(x)::text AS json_text FROM (SELECT m.id,m.user_id,u.username,m.message,m.created_at FROM game_group_messages m JOIN users u ON u.id=m.user_id WHERE m.group_id=$1 ORDER BY m.created_at DESC,m.id DESC LIMIT 50) x ORDER BY x.created_at,x.id",id))messages.append(parseJson(r["json_text"].as<std::string>()));out["messages"]=messages;reply(cb,std::move(out));},"讀取遊戲群內容失敗");
  });
  reg1("/api/game-groups/{1}", drogon::Patch, [service](const Request& req, Callback& cb,const std::string&idText) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);const auto rows=service->database()->execSqlSync("SELECT row_to_json(g)::text AS json_text FROM game_groups g WHERE id=$1",id);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到遊戲群");auto current=rowJson(rows);if(!user.isAdmin&&intValue(current["owner_user_id"])!=user.id)throw ApiError(drogon::k403Forbidden,"只有群主可以修改遊戲群");const auto body=bodyOrEmpty(req);auto name=body.isMember("name")?trimmed(stringValue(body["name"])):stringValue(current["name"]);auto desc=body.isMember("description")?trimmed(stringValue(body["description"])):stringValue(current["description"]);if(name.empty())throw ApiError(drogon::k400BadRequest,"遊戲群名稱不可為空");const auto updated=service->database()->execSqlSync("UPDATE game_groups SET name=$1,description=$2,updated_at=CURRENT_TIMESTAMP WHERE id=$3 RETURNING row_to_json(game_groups)::text AS json_text",clipped(name,120),clipped(desc,2000),id);Json::Value out;out["ok"]=true;out["group"]=rowJson(updated);reply(cb,std::move(out));},"修改遊戲群失敗");
  });
  reg1("/api/game-groups/{1}", drogon::Delete, [service](const Request& req, Callback& cb,const std::string&idText) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);const auto rows=service->database()->execSqlSync("SELECT owner_user_id FROM game_groups WHERE id=$1",id);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到遊戲群");if(!user.isAdmin&&rows[0]["owner_user_id"].as<std::int64_t>()!=user.id)throw ApiError(drogon::k403Forbidden,"只有群主可以刪除遊戲群");service->database()->execSqlSync("DELETE FROM game_groups WHERE id=$1",id);Json::Value out;out["ok"]=true;reply(cb,std::move(out));},"刪除遊戲群失敗");
  });
  reg1("/api/game-groups/{1}/messages", drogon::Post, [service](const Request& req, Callback& cb,const std::string&idText) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);if(service->database()->execSqlSync("SELECT 1 FROM game_group_members WHERE group_id=$1 AND user_id=$2",id,user.id).empty())throw ApiError(drogon::k403Forbidden,"你不是這個遊戲群的成員");auto message=trimmed(stringValue(bodyOrEmpty(req)["message"]));if(message.empty())throw ApiError(drogon::k400BadRequest,"請輸入留言");message=clipped(message,800);const auto ins=service->database()->execSqlSync("INSERT INTO game_group_messages(group_id,user_id,message) VALUES($1,$2,$3) RETURNING id",id,user.id,message);const auto msgId=ins[0]["id"].as<std::int64_t>();const auto rows=service->database()->execSqlSync("SELECT row_to_json(x)::text AS json_text FROM (SELECT m.id,m.user_id,u.username,m.message,m.created_at FROM game_group_messages m JOIN users u ON u.id=m.user_id WHERE m.id=$1) x",msgId);service->database()->execSqlSync("UPDATE game_groups SET updated_at=CURRENT_TIMESTAMP WHERE id=$1",id);Json::Value out;out["ok"]=true;out["message"]=rowJson(rows);reply(cb,std::move(out));},"送出遊戲群留言失敗");
  });
  reg1("/api/game-groups/{1}/leave", drogon::Post, [service](const Request& req, Callback& cb,const std::string&idText) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);const auto rows=service->database()->execSqlSync("SELECT owner_user_id FROM game_groups WHERE id=$1",id);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到遊戲群");if(rows[0]["owner_user_id"].as<std::int64_t>()==user.id)throw ApiError(drogon::k409Conflict,"群主不可直接退出；可先刪除遊戲群");service->database()->execSqlSync("DELETE FROM game_group_members WHERE group_id=$1 AND user_id=$2",id,user.id);Json::Value out;out["ok"]=true;reply(cb,std::move(out));},"退出遊戲群失敗");
  });
  ...
  reg1("/api/admin/monster-templates/{1}", drogon::Patch,
    ...
    runHandler(cb, [&] {
      ...
      { auto attrs=body["attributes"].isObject()?body["attributes"]:parseJson(currentRows[0]["attributes_text"].as<std::string>(),Json::Value{Json::objectValue}); const auto con=std::max<std::int64_t>(0,intValue(attrs["constitution"])),spi=std::max<std::int64_t>(0,intValue(attrs["spirit"])); body["max_hp"]=Json::Int64(con*2); config["derived_stats"]["max_hp"]=Json::Int64(con*2);config["derived_stats"]["endurance"]=Json::Int64((con+spi)/2);config["derived_stats"]["will"]=Json::Int64((spi*8)/10);config["derived_stats"]["sanity"]=Json::Int64((spi*12)/10); if(body["config"]["special_stats"].isObject())config["special_stats"]=body["config"]["special_stats"]; if(!config["special_stats"].isObject()){config["special_stats"]["endurance"]=true;config["special_stats"]["will"]=true;config["special_stats"]["sanity"]=true;} }
  ...
  reg0("/api/tasks/public", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      out["tasks"] = selectRows1(service->database(),
          "SELECT row_to_json(x)::text AS json_text FROM ("
          "SELECT t.*,tp.status AS participant_status,tp.progress AS participant_progress FROM tasks t LEFT JOIN task_participants tp ON tp.task_id=t.id AND tp.user_id=$1 "
          "WHERE t.status='open' AND t.task_type='public' AND (t.expires_at IS NULL OR t.expires_at>CURRENT_TIMESTAMP) ORDER BY CASE t.priority WHEN 'critical' THEN 0 WHEN 'high' THEN 1 ELSE 2 END,t.expires_at NULLS LAST,t.created_at DESC) x", user.id);
      reply(cb, std::move(out));
    }, "讀取公共任務失敗");
  });
  ...
  reg0("/api/tasks/my", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      out["tasks"] = selectRows1(service->database(),
          "SELECT row_to_json(x)::text AS json_text FROM ("
          "SELECT DISTINCT t.*,COALESCE(tp.status,CASE WHEN t.target_user_id=$1 THEN 'assigned' END) AS participant_status,tp.progress AS participant_progress "
  ...
  reg1("/api/tasks/{1}/accept", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      const auto tasks = service->database()->execSqlSync(
          "SELECT id,status,task_type,target_user_id,min_tier,expires_at,(expires_at IS NOT NULL AND expires_at<=CURRENT_TIMESTAMP) AS expired FROM tasks WHERE id=$1", taskId);
      ...
      if (!tasks[0]["expired"].isNull() && tasks[0]["expired"].as<bool>())
        throw ApiError(drogon::k400BadRequest, "任務期限已過，無法再接受");
      ...
      // [功能備註｜65-cpp.26.11 任務 2.0] 接取條件改由 C++ 真正驗證，前端只負責提示。
      const auto taskRows = service->database()->execSqlSync(
          "SELECT row_to_json(t)::text AS json_text FROM tasks t WHERE id=$1", taskId);
      const auto fullTask = rowJson(taskRows);
      const auto prereqs = fullTask["prerequisite_task_ids"].isArray() ? fullTask["prerequisite_task_ids"] : Json::Value{Json::arrayValue};
      for (const auto& prereq : prereqs) {
        const auto prereqId = intValue(prereq);
        if (prereqId <= 0) continue;
        if (service->database()->execSqlSync(
              "SELECT 1 FROM task_participants WHERE task_id=$1 AND user_id=$2 AND status='completed'", prereqId, user.id).empty())
          throw ApiError(drogon::k403Forbidden, "尚未完成必要的前置任務");
      }
      const auto factionId = intValue(fullTask["required_faction_id"]);
      if (factionId > 0) {
        const auto reps = service->database()->execSqlSync(
            "SELECT reputation FROM character_reputations WHERE user_id=$1 AND faction_id=$2", user.id, factionId);
        const auto reputation = reps.empty() ? 0 : reps[0]["reputation"].as<std::int64_t>();
        if (reputation < intValue(fullTask["min_reputation"], -9999))
          throw ApiError(drogon::k403Forbidden, "勢力聲望不足，無法接受此任務");
      }
      const auto exclusive = trimmed(stringValue(fullTask["exclusive_group"]));
      if (!exclusive.empty()) {
        const auto conflicts = service->database()->execSqlSync(
            "SELECT 1 FROM tasks t JOIN task_participants tp ON tp.task_id=t.id WHERE tp.user_id=$1 AND tp.status='completed' AND t.exclusive_group=$2 AND t.id<>$3 LIMIT 1",
            user.id, exclusive, taskId);
        if (!conflicts.empty()) throw ApiError(drogon::k403Forbidden, "你已完成互斥分支中的其他任務");
      }
      service->database()->execSqlSync(
          "INSERT INTO task_participants(task_id,user_id,status) VALUES($1,$2,'accepted') "
          "ON CONFLICT(task_id,user_id) DO UPDATE SET status=CASE WHEN task_participants.status='completed' THEN task_participants.status ELSE 'accepted' END,accepted_at=CURRENT_TIMESTAMP,completed_at=CASE WHEN task_participants.status='completed' THEN task_participants.completed_at ELSE NULL END",
          taskId, user.id);
      service->database()->execSqlSync(
          "INSERT INTO user_notifications(user_id,notification_type,title,message,data) VALUES($1,'task','已接受任務',$2,jsonb_build_object('task_id',$3))",
          user.id, stringValue(fullTask["title"], "任務"), taskId);
      Json::Value out; out["ok"] = true; reply(cb, std::move(out));
    }, "接受任務失敗");
  });


  // [功能備註｜73-cpp.34.22 任務步驟進度｜POST /api/rooms/{1}/tasks/{2}/steps/{3}/progress]
  // 玩家不是單純按「完成」：NPC 協力封印、抵達地點、安插錨點、完成儀式等會由 C++ 驗證房間狀態。
  reg3("/api/rooms/{1}/tasks/{2}/steps/{3}/progress", drogon::Post,
       [service](const Request& req, Callback& cb, const std::string& roomText,
                 const std::string& taskText, const std::string& stepIdText) {
    runHandler(cb, [&] {
      const auto user = requireUser(service, req);
      const auto roomId = positiveId(roomText, "房間 ID 無效");
      const auto taskId = positiveId(taskText, "任務 ID 無效");
      requireRoomAccess(service, user, roomId);
      const auto stepId = trimmed(stepIdText);
      if (stepId.empty() || stepId.size() > 80) throw ApiError(drogon::k400BadRequest, "任務步驟 ID 無效");

      const auto taskRows = service->database()->execSqlSync(
          "SELECT row_to_json(t)::text AS json_text FROM tasks t WHERE id=$1", taskId);
      if (taskRows.empty()) throw ApiError(drogon::k404NotFound, "找不到任務");
      const auto task = rowJson(taskRows);
      const auto steps = task["task_steps"].isArray() ? task["task_steps"] : Json::Value{Json::arrayValue};
      const auto* step = findTaskStep(steps, stepId);
      if (!step) throw ApiError(drogon::k404NotFound, "找不到任務步驟");
      const bool sharedStep = stringValue((*step)["progress_scope"], "personal") == "team";
      if (!step->get("player_can_progress", true).asBool())
        throw ApiError(drogon::k403Forbidden, "此任務步驟需要 DM 判定");

      const auto participantRows = service->database()->execSqlSync(
          "SELECT status,progress::text AS progress_text FROM task_participants WHERE task_id=$1 AND user_id=$2",
          taskId, user.id);
      if (participantRows.empty()) throw ApiError(drogon::k403Forbidden, "你尚未接取這個任務");
      const auto participantStatus = participantRows[0]["status"].as<std::string>();
      if (participantStatus == "completed") throw ApiError(drogon::k409Conflict, "此任務已完成");
      if (participantStatus != "accepted" && participantStatus != "assigned")
        throw ApiError(drogon::k403Forbidden, "目前不能推進這個任務");

      Json::Value progress = parseJson(participantRows[0]["progress_text"].as<std::string>(), Json::Value{Json::objectValue});
      if (!progress.isObject()) progress = Json::Value{Json::objectValue};
      ensureTaskVariables(task, progress);
      if (!progress["steps"].isObject()) progress["steps"] = Json::Value{Json::objectValue};
      if (!taskStepActive(*step, progress)) throw ApiError(drogon::k409Conflict, "此任務分支目前尚未啟用");
      if (!taskStepDependenciesComplete(steps, progress, *step))
        throw ApiError(drogon::k409Conflict, "前置任務環節尚未完成");
      auto& stepProgress = progress["steps"][stepId];
      if (!stepProgress.isObject()) stepProgress = Json::Value{Json::objectValue};
      const auto required = std::max<std::int64_t>(1, intValue((*step)["required"], 1));
      auto current = std::max<std::int64_t>(0, intValue(stepProgress["count"]));
      if (sharedStep) {
        const auto sharedRows = service->database()->execSqlSync(
            "SELECT progress::text AS progress_text FROM task_participants WHERE task_id=$1 AND status IN ('accepted','assigned')", taskId);
        for (const auto& sharedRow : sharedRows) {
          const auto sharedProgress = parseJson(sharedRow["progress_text"].as<std::string>(), Json::Value{Json::objectValue});
          current = std::max<std::int64_t>(current, intValue(sharedProgress["steps"][stepId]["count"]));
        }
      }
      if (current >= required || stepProgress.get("completed", false).asBool()) {
        Json::Value out; out["ok"] = true; out["progress"] = progress; out["all_complete"] = taskStepsComplete(steps, progress);
        reply(cb, std::move(out)); return;
      }
      ...
      const auto nodeRows = service->database()->execSqlSync(
          ...
          "LEFT JOIN room_team_members tm ON tm.room_id=rm.room_id AND tm.user_id=rm.user_id "
          "LEFT JOIN room_teams t ON t.id=tm.team_id WHERE rm.room_id=$1 AND rm.user_id=$2",
          roomId, user.id);
      const auto currentNodeId = (!nodeRows.empty() && !nodeRows[0]["node_id"].isNull())
          ? nodeRows[0]["node_id"].as<std::int64_t>() : 0;
      const auto locationName = trimmed(stringValue((*step)["location_name"]));
      if (!locationName.empty()) {
        if (currentNodeId <= 0 || service->database()->execSqlSync(
              "SELECT 1 FROM room_map_nodes WHERE id=$1 AND room_id=$2 AND name ILIKE $3",
              currentNodeId, roomId, locationName).empty())
          throw ApiError(drogon::k409Conflict, "你必須先到達指定地點：" + locationName);
      }

      const auto type = stringValue((*step)["type"], "custom");
      const auto sameNode = step->get("requires_same_node", true).asBool();
      const auto npcId = intValue((*step)["npc_template_id"]);
      const auto monsterId = intValue((*step)["monster_template_id"]);
      std::int64_t targetRoomMonsterId = 0;

      if (type == "interact_npc" || type == "escort_npc" || type == "cooperative_seal") {
        if (npcId <= 0) throw ApiError(drogon::k400BadRequest, "這個任務步驟尚未指定 NPC");
        const auto npcRows = service->database()->execSqlSync(
            "SELECT map_node_id,status FROM room_npcs WHERE room_id=$1 AND npc_template_id=$2", roomId, npcId);
        if (npcRows.empty()) throw ApiError(drogon::k409Conflict, "指定 NPC 目前不在這個房間");
        const auto npcStatus = npcRows[0]["status"].as<std::string>();
        if (npcStatus == "dead" || npcStatus == "defeated" || npcStatus == "removed")
          throw ApiError(drogon::k409Conflict, "指定 NPC 目前無法協助此任務");
        if (sameNode) {
          if (currentNodeId <= 0 || npcRows[0]["map_node_id"].isNull())
            throw ApiError(drogon::k409Conflict, "必須先讓你與指定 NPC 位於同一地點");
          if (npcRows[0]["map_node_id"].as<std::int64_t>() != currentNodeId)
            throw ApiError(drogon::k409Conflict, "指定 NPC 不在你目前的地點");
        }
      }

      if (type == "cooperative_seal") {
        const auto anchorNeed = std::max<std::int64_t>(0, intValue((*step)["required_anchor_count"]));
        if (anchorNeed > 0) {
          const auto anchorRows = sharedStep
            ? service->database()->execSqlSync(
                "SELECT count(*) AS count FROM room_task_anchors WHERE room_id=$1 AND task_id=$2", roomId, taskId)
            : service->database()->execSqlSync(
                "SELECT count(*) AS count FROM room_task_anchors WHERE room_id=$1 AND task_id=$2 AND user_id=$3", roomId, taskId, user.id);
          const auto have = anchorRows.empty() ? 0 : anchorRows[0]["count"].as<std::int64_t>();
          if (have < anchorNeed) throw ApiError(drogon::k409Conflict, "封印前需要先安插至少 " + std::to_string(anchorNeed) + " 個錨點");
        }
        const auto prerequisiteRitualId = intValue((*step)["ritual_id"]);
        if (prerequisiteRitualId > 0 && service->database()->execSqlSync(
              "SELECT 1 FROM ritual_instances WHERE room_id=$1 AND ritual_id=$2 AND status='success' ORDER BY completed_at DESC LIMIT 1",
              roomId, prerequisiteRitualId).empty())
          throw ApiError(drogon::k409Conflict, "封印前必須先完成指定儀式");
      }

      if (type == "cooperative_seal" && monsterId > 0) {
        if (sameNode && currentNodeId <= 0) throw ApiError(drogon::k409Conflict, "無法確認你目前的位置");
        const auto monsterRows = sameNode
          ? service->database()->execSqlSync(
              "SELECT id FROM room_monsters WHERE room_id=$1 AND monster_template_id=$2 AND map_node_id=$3 "
              "AND status NOT IN ('defeated','dead','sealed','removed') ORDER BY id LIMIT 1", roomId, monsterId, currentNodeId)
          : service->database()->execSqlSync(
              "SELECT id FROM room_monsters WHERE room_id=$1 AND monster_template_id=$2 "
              "AND status NOT IN ('defeated','dead','sealed','removed') ORDER BY id LIMIT 1", roomId, monsterId);
        if (monsterRows.empty()) throw ApiError(drogon::k409Conflict, "要封印的目標目前不在可作用範圍內");
        targetRoomMonsterId = monsterRows[0]["id"].as<std::int64_t>();
      }

      if (type == "reach_location" && locationName.empty())
        throw ApiError(drogon::k400BadRequest, "抵達地點步驟尚未設定地點名稱");

      if (type == "perform_ritual") {
        const auto ritualId = intValue((*step)["ritual_id"]);
        if (ritualId <= 0) throw ApiError(drogon::k400BadRequest, "這個任務步驟尚未指定儀式");
        if (service->database()->execSqlSync(
              "SELECT 1 FROM character_rituals WHERE user_id=$1 AND ritual_id=$2", user.id, ritualId).empty())
          throw ApiError(drogon::k403Forbidden, "你尚未學會指定儀式");
        if (service->database()->execSqlSync(
              "SELECT 1 FROM ritual_instances WHERE room_id=$1 AND ritual_id=$2 AND status='success' ORDER BY completed_at DESC LIMIT 1",
              roomId, ritualId).empty())
          throw ApiError(drogon::k409Conflict, "指定儀式尚未在此房間完成");
      }

      if (type == "defeat_target" && monsterId > 0) {
        const auto defeated = service->database()->execSqlSync(
            "SELECT 1 FROM room_monsters WHERE room_id=$1 AND monster_template_id=$2 AND status IN ('defeated','dead','sealed') LIMIT 1",
            roomId, monsterId);
        if (defeated.empty()) throw ApiError(drogon::k409Conflict, "指定目標尚未被擊敗或封印");
      }

      if (type == "collect") {
        const auto wanted = trimmed(stringValue((*step)["item_name"]));
        if (wanted.empty()) throw ApiError(drogon::k400BadRequest, "收集步驟尚未設定物品名稱");
        const auto character = characterRaw(service->database(), user.id);
        const auto inventory = inventoryOf(character);
        std::int64_t owned = 0;
        for (const auto& item : inventory) {
          const auto name = item.isString() ? item.asString() : stringValue(item["name"]);
          if (name == wanted) owned += item.isObject() ? std::max<std::int64_t>(1, intValue(item["quantity"], 1)) : 1;
        }
        if (owned < required) throw ApiError(drogon::k409Conflict, "指定物品數量不足：" + wanted);
        current = required - 1; // 此次驗證成功後直接完成收集步驟。
      }

      if (type == "place_anchor") {
        if (currentNodeId <= 0) throw ApiError(drogon::k409Conflict, "請先進入一個地圖節點再安插錨點");
        if (step->get("unique_nodes", true).asBool()) {
          const auto duplicate = sharedStep
            ? service->database()->execSqlSync(
                "SELECT 1 FROM room_task_anchors WHERE room_id=$1 AND task_id=$2 AND step_id=$3 AND map_node_id=$4 LIMIT 1",
                roomId, taskId, std::string(stepId), currentNodeId)
            : service->database()->execSqlSync(
                "SELECT 1 FROM room_task_anchors WHERE room_id=$1 AND task_id=$2 AND step_id=$3 AND user_id=$4 AND map_node_id=$5 LIMIT 1",
                roomId, taskId, std::string(stepId), user.id, currentNodeId);
          if (!duplicate.empty()) throw ApiError(drogon::k409Conflict, "這個地點已經安插過此任務的錨點");
        }
        const auto anchorName = trimmed(stringValue((*step)["anchor_name"], "任務錨點"));
        service->database()->execSqlSync(
            "INSERT INTO room_task_anchors(room_id,task_id,step_id,user_id,map_node_id,anchor_name,state) VALUES($1,$2,$3,$4,$5,$6,$7::jsonb)",
            roomId, taskId, std::string(stepId), user.id, currentNodeId, std::string(anchorName.empty() ? "任務錨點" : anchorName),
            compactJson(Json::Value{Json::objectValue}));
        const auto countRows = sharedStep
          ? service->database()->execSqlSync(
              "SELECT count(*) AS count FROM room_task_anchors WHERE room_id=$1 AND task_id=$2 AND step_id=$3", roomId, taskId, std::string(stepId))
          : service->database()->execSqlSync(
              "SELECT count(*) AS count FROM room_task_anchors WHERE room_id=$1 AND task_id=$2 AND step_id=$3 AND user_id=$4",
              roomId, taskId, std::string(stepId), user.id);
        current = countRows.empty() ? current : countRows[0]["count"].as<std::int64_t>() - 1;
      }

      const bool wasCompleted = stepProgress.get("completed", false).asBool();
      current = std::min<std::int64_t>(required, current + 1);
      stepProgress["count"] = Json::Int64(current);
      stepProgress["required"] = Json::Int64(required);
      stepProgress["completed"] = current >= required;
      if (!wasCompleted && current >= required) applyTaskStepVariableChange(*step, progress);
      stepProgress["last_room_id"] = Json::Int64(roomId);
      if (currentNodeId > 0) stepProgress["last_map_node_id"] = Json::Int64(currentNodeId);
      const auto variableFailure = taskFailureReason(task, progress);
      if (!variableFailure.empty()) { progress["failed"] = true; progress["failure_reason"] = variableFailure; }
      progress["all_complete"] = !progress.get("failed", false).asBool() && taskStepsComplete(steps, progress);
      if (progress.get("failed", false).asBool())
        service->database()->execSqlSync("UPDATE task_participants SET status='failed',progress=$1::jsonb WHERE task_id=$2 AND user_id=$3", compactJson(progress), taskId, user.id);
      else
        service->database()->execSqlSync("UPDATE task_participants SET progress=$1::jsonb WHERE task_id=$2 AND user_id=$3", compactJson(progress), taskId, user.id);
      if (sharedStep) {
        const auto peers = service->database()->execSqlSync(
            "SELECT user_id,progress::text AS progress_text FROM task_participants WHERE task_id=$1 AND status IN ('accepted','assigned') AND user_id<>$2",
            taskId, user.id);
        for (const auto& peer : peers) {
          auto pp = parseJson(peer["progress_text"].as<std::string>(), Json::Value{Json::objectValue});
          if (!pp.isObject()) pp = Json::Value{Json::objectValue};
          ...
          psp["count"] = Json::Int64(current); psp["required"] = Json::Int64(required); psp["completed"] = current >= required;
          if (!peerWasCompleted && current >= required) applyTaskStepVariableChange(*step, pp);
          psp["last_room_id"] = Json::Int64(roomId); psp["shared_by_user_id"] = Json::Int64(user.id);
          pp["all_complete"] = taskStepsComplete(steps, pp);
          service->database()->execSqlSync("UPDATE task_participants SET progress=$1::jsonb WHERE task_id=$2 AND user_id=$3",
              compactJson(pp), taskId, peer["user_id"].as<std::int64_t>());
        }
      }

      Json::Value actionData{Json::objectValue};
      actionData["task_title"] = stringValue(task["title"]);
      actionData["step_title"] = stringValue((*step)["title"]);
      actionData["count"] = Json::Int64(current);
      actionData["required"] = Json::Int64(required);
      service->database()->execSqlSync(
          "INSERT INTO task_step_actions(room_id,task_id,step_id,user_id,action_type,map_node_id,data) VALUES($1,$2,$3,$4,$5,NULLIF($6,0),$7::jsonb)",
          roomId, taskId, std::string(stepId), user.id, std::string(type), currentNodeId > 0 ? currentNodeId : 0, compactJson(actionData));

      if (type == "cooperative_seal" && current >= required && targetRoomMonsterId > 0 &&
          step->get("seal_target_on_complete", true).asBool()) {
        service->database()->execSqlSync(
            "UPDATE room_monsters SET status='sealed',updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND room_id=$2",
            targetRoomMonsterId, roomId);
      }

      Json::Value ev; ev["task_id"] = Json::Int64(taskId); ev["step_id"] = stepId;
      ev["text"] = "任務進度：「" + stringValue((*step)["title"], "任務步驟") + "」 " +
                   std::to_string(current) + "/" + std::to_string(required);
      service->addEvent(roomId, user.id, "task_progress", ev);
      RoomSocket::broadcastSnapshotForRoom(roomId);

      Json::Value out; out["ok"] = true; out["progress"] = progress;
      out["step_progress"] = stepProgress; out["all_complete"] = taskStepsComplete(steps, progress);
      out["sealed_room_monster_id"] = Json::Int64((type == "cooperative_seal" && current >= required) ? targetRoomMonsterId : 0);
      reply(cb, std::move(out));
    }, "更新任務步驟失敗");
  });

  // [功能備註｜73-cpp.34.25 任務行動歷史｜GET /api/rooms/{1}/tasks/{2}/history]
  reg2("/api/rooms/{1}/tasks/{2}/history", drogon::Get,
       [service](const Request& req, Callback& cb, const std::string& roomText, const std::string& taskText) {
    runHandler(cb, [&] {
      const auto user = requireUser(service, req);
      const auto roomId = positiveId(roomText, "房間 ID 無效");
      const auto taskId = positiveId(taskText, "任務 ID 無效");
      requireRoomAccess(service, user, roomId);
      if (!user.isAdmin && service->database()->execSqlSync(
            "SELECT 1 FROM task_participants WHERE task_id=$1 AND user_id=$2", taskId, user.id).empty())
        throw ApiError(drogon::k403Forbidden, "你不是此任務的參與者");
      Json::Value out;
      out["actions"] = selectRows2(service->database(),
          "SELECT row_to_json(x)::text AS json_text FROM ("
          "SELECT a.*,u.username,n.name AS map_node_name FROM task_step_actions a "
          "LEFT JOIN users u ON u.id=a.user_id LEFT JOIN room_map_nodes n ON n.id=a.map_node_id "
          "WHERE a.room_id=$1 AND a.task_id=$2 ORDER BY a.created_at DESC,a.id DESC LIMIT 200) x", roomId, taskId);
      reply(cb, std::move(out));
    }, "讀取任務行動歷史失敗");
  });

  // [功能備註｜73-cpp.34.22 任務錨點｜GET /api/rooms/{1}/task-anchors]
  reg1("/api/rooms/{1}/task-anchors", drogon::Get,
       [service](const Request& req, Callback& cb, const std::string& roomText) {
    runHandler(cb, [&] {
      const auto user = requireUser(service, req);
      const auto roomId = positiveId(roomText, "房間 ID 無效");
      requireRoomAccess(service, user, roomId);
      Json::Value out;
      out["anchors"] = selectRows1(service->database(),
          "SELECT row_to_json(x)::text AS json_text FROM ("
          "SELECT a.*,n.name AS map_node_name,t.title AS task_title FROM room_task_anchors a "
          "LEFT JOIN room_map_nodes n ON n.id=a.map_node_id LEFT JOIN tasks t ON t.id=a.task_id "
          "WHERE a.room_id=$1 ORDER BY a.created_at,a.id) x", roomId);
      reply(cb, std::move(out));
    }, "讀取任務錨點失敗");
  });
  ...
  reg0("/api/admin/tasks", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      requireUser(service, req, true);
      Json::Value out;
      auto tasks = selectRows(service->database(),
      ...
      for (auto& task : tasks) {
        task["participants"] = selectRows1(service->database(),
            "SELECT row_to_json(x)::text AS json_text FROM (SELECT tp.user_id,tp.status,tp.accepted_at,tp.completed_at,tp.progress,u.username,c.name AS character_name FROM task_participants tp JOIN users u ON u.id=tp.user_id LEFT JOIN character_cards c ON c.user_id=tp.user_id WHERE tp.task_id=$1 ORDER BY tp.completed_at NULLS FIRST,tp.accepted_at,u.username) x",
            intValue(task["id"]));
      }
  ...
  reg0("/api/admin/tasks", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      auto body = bodyOrEmpty(req);
      auto title = trimmed(stringValue(body["title"]));
      if (title.empty()) throw ApiError(drogon::k400BadRequest, "請輸入任務名稱");
      if (title.size() > 120) throw ApiError(drogon::k400BadRequest, "任務名稱過長");
      body["title"] = title;

      auto taskType = stringValue(body["task_type"]);
      if (taskType.empty()) taskType = intValue(body["target_user_id"]) > 0 ? "private" : "public";
      if (taskType != "public" && taskType != "private" && taskType != "profession")
        throw ApiError(drogon::k400BadRequest, "任務類型無效");
      body["task_type"] = taskType;

      // [功能備註｜65-cpp.26.5 任務建立修正] 前端 select 的空值曾被 Number('') 轉成 0，
      // genericInsert 會真的把 0 寫進 nullable FK，造成 users/profession_templates 外鍵失敗。
      // 後端現在一律把「未指定」正規化成 SQL NULL，並驗證真正需要指定的任務。
      auto targetUserId = intValue(body["target_user_id"]);
      if (taskType == "public") {
        body["target_user_id"] = Json::Value{Json::nullValue};
      } else {
        if (targetUserId <= 0) throw ApiError(drogon::k400BadRequest, "這種任務必須指定玩家");
        if (service->database()->execSqlSync("SELECT 1 FROM users WHERE id=$1", targetUserId).empty())
          throw ApiError(drogon::k404NotFound, "找不到指定玩家");
        body["target_user_id"] = Json::Int64(targetUserId);
      }

      auto professionId = intValue(body["profession_id"]);
      if (taskType == "profession") {
        if (professionId <= 0) throw ApiError(drogon::k400BadRequest, "職業升階任務必須選擇職業");
        if (service->database()->execSqlSync("SELECT 1 FROM profession_templates WHERE id=$1", professionId).empty())
          throw ApiError(drogon::k404NotFound, "找不到指定職業");
        body["profession_id"] = Json::Int64(professionId);
      } else {
        body["profession_id"] = Json::Value{Json::nullValue};
      }
      if (intValue(body["required_faction_id"]) <= 0)
        body["required_faction_id"] = Json::Value{Json::nullValue};
      body["min_tier"] = Json::Int64(std::clamp<std::int64_t>(intValue(body["min_tier"], 1), 1, 10));
      auto priority = trimmed(stringValue(body["priority"], "normal"));
      if (priority != "normal" && priority != "high" && priority != "critical") priority = "normal";
      body["priority"] = priority;
      body["recommended_party_size"] = Json::Int64(std::clamp<std::int64_t>(intValue(body["recommended_party_size"], 1), 1, 12));
      auto deadline = trimmed(stringValue(body["expires_at"]));
      if (deadline.empty()) body["expires_at"] = Json::Value{Json::nullValue};
      else {
        if (deadline.size() > 40 || !std::regex_match(deadline, std::regex("^[0-9]{4}-[0-9]{2}-[0-9]{2}([T ][0-9]{2}:[0-9]{2}(:[0-9]{2})?)?([+-][0-9]{2}:[0-9]{2}|Z)?$")))
          throw ApiError(drogon::k400BadRequest, "任務期限格式不正確");
        body["expires_at"] = deadline;
      }
      // [功能備註｜73-cpp.34.22 任務步驟] 由 C++ 正規化並驗證 NPC／怪物／儀式模板引用。
      body["task_steps"] = normalizeTaskSteps(body["task_steps"]);
      body["task_variables"] = normalizeTaskVariables(body["task_variables"]);
      body["task_failure_conditions"] = normalizeTaskFailureConditions(body["task_failure_conditions"]);
      for (const auto& step : body["task_steps"]) {
        const auto npcId = intValue(step["npc_template_id"]);
        const auto monsterId = intValue(step["monster_template_id"]);
        const auto ritualId = intValue(step["ritual_id"]);
        if (npcId > 0 && service->database()->execSqlSync("SELECT 1 FROM npc_templates WHERE id=$1", npcId).empty())
          throw ApiError(drogon::k404NotFound, "任務步驟指定的 NPC 不存在");
        if (monsterId > 0 && service->database()->execSqlSync("SELECT 1 FROM monster_templates WHERE id=$1", monsterId).empty())
          throw ApiError(drogon::k404NotFound, "任務步驟指定的怪物不存在");
        if (ritualId > 0 && service->database()->execSqlSync("SELECT 1 FROM ritual_studies WHERE id=$1", ritualId).empty())
          throw ApiError(drogon::k404NotFound, "任務步驟指定的儀式不存在");
      }

      Json::Value out; out["ok"] = true; out["task"] = genericInsert(service->database(), "tasks", body, extra);
      const auto createdTaskId = intValue(out["task"]["id"]);
      if (targetUserId > 0 && createdTaskId > 0) {
        service->database()->execSqlSync(
            "INSERT INTO task_participants(task_id,user_id,status) VALUES($1,$2,'accepted') ON CONFLICT(task_id,user_id) DO UPDATE SET status=CASE WHEN task_participants.status='completed' THEN task_participants.status ELSE 'accepted' END,accepted_at=CURRENT_TIMESTAMP",
            createdTaskId, targetUserId);
        service->database()->execSqlSync(
            "INSERT INTO user_notifications(user_id,notification_type,title,message,data) VALUES($1,'task','DM 指派新任務',$2,jsonb_build_object('task_id',$3))",
            targetUserId, title, createdTaskId);
      }
      reply(cb, std::move(out));
    }, "建立任務失敗");
  });
  ...
  reg1("/api/admin/tasks/{1}", drogon::Patch,
    ...
    runHandler(cb, [&] {
      requireUser(service, req, true);
      auto body = bodyOrEmpty(req);
      if (body.isMember("task_steps")) body["task_steps"] = normalizeTaskSteps(body["task_steps"]);
      if (body.isMember("task_variables")) body["task_variables"] = normalizeTaskVariables(body["task_variables"]);
      if (body.isMember("task_failure_conditions")) body["task_failure_conditions"] = normalizeTaskFailureConditions(body["task_failure_conditions"]);
      Json::Value out; out["ok"] = true; out["task"] = genericUpdate(service->database(), "tasks", positiveId(taskIdText), body); reply(cb, std::move(out));
    }, "更新任務失敗");
  });


  // [功能備註｜73-cpp.34.22 DM 任務環節判定｜POST /api/admin/tasks/{1}/step-progress]
  reg1("/api/admin/tasks/{1}/step-progress", drogon::Post,
       [service](const Request& req, Callback& cb, const std::string& taskText) {
    runHandler(cb, [&] {
      requireUser(service, req, true);
      const auto taskId = positiveId(taskText, "任務 ID 無效");
      const auto body = bodyOrEmpty(req);
      const auto userId = intValue(body["user_id"]);
      const auto stepId = trimmed(stringValue(body["step_id"]));
      if (userId <= 0 || stepId.empty()) throw ApiError(drogon::k400BadRequest, "請指定玩家與任務環節");
      const auto taskRows = service->database()->execSqlSync(
          "SELECT row_to_json(t)::text AS json_text FROM tasks t WHERE id=$1", taskId);
      if (taskRows.empty()) throw ApiError(drogon::k404NotFound, "找不到任務");
      const auto task = rowJson(taskRows);
      const auto steps = task["task_steps"].isArray() ? task["task_steps"] : Json::Value{Json::arrayValue};
      const auto* step = findTaskStep(steps, stepId);
      if (!step) throw ApiError(drogon::k404NotFound, "找不到任務環節");
      const auto participantRows = service->database()->execSqlSync(
          "SELECT progress::text AS progress_text,status FROM task_participants WHERE task_id=$1 AND user_id=$2", taskId, userId);
      if (participantRows.empty()) throw ApiError(drogon::k404NotFound, "此玩家尚未參與任務");
      Json::Value progress = parseJson(participantRows[0]["progress_text"].as<std::string>(), Json::Value{Json::objectValue});
      if (!progress.isObject()) progress = Json::Value{Json::objectValue};
      ensureTaskVariables(task, progress);
      if (!progress["steps"].isObject()) progress["steps"] = Json::Value{Json::objectValue};
      if (!taskStepActive(*step, progress)) throw ApiError(drogon::k409Conflict, "此分支目前未啟用");
      auto& p = progress["steps"][stepId]; if (!p.isObject()) p = Json::Value{Json::objectValue};
      const bool wasCompleted = p.get("completed", false).asBool();
      const auto required = std::max<std::int64_t>(1, intValue((*step)["required"], 1));
      std::int64_t count = intValue(body["count"], intValue(p["count"]));
      if (body.get("complete", false).asBool()) count = required;
      count = std::clamp<std::int64_t>(count, 0, required);
      p["count"] = Json::Int64(count); p["required"] = Json::Int64(required); p["completed"] = count >= required; p["dm_override"] = true;
      if (!wasCompleted && count >= required) applyTaskStepVariableChange(*step, progress);
      progress["all_complete"] = taskStepsComplete(steps, progress);
      service->database()->execSqlSync("UPDATE task_participants SET progress=$1::jsonb WHERE task_id=$2 AND user_id=$3", compactJson(progress), taskId, userId);
      Json::Value out; out["ok"] = true; out["progress"] = progress; out["all_complete"] = taskStepsComplete(steps, progress); reply(cb, std::move(out));
    }, "調整任務環節失敗");
  });

  // [功能備註｜73-cpp.34.25 任務除錯器｜GET /api/admin/tasks/{1}/debug/{2}]
  reg2("/api/admin/tasks/{1}/debug/{2}", drogon::Get,
       [service](const Request& req, Callback& cb, const std::string& taskText, const std::string& userText) {
    runHandler(cb, [&] {
      requireUser(service, req, true); const auto taskId=positiveId(taskText), userId=positiveId(userText);
      const auto tr=service->database()->execSqlSync("SELECT row_to_json(t)::text AS json_text FROM tasks t WHERE id=$1",taskId); if(tr.empty())throw ApiError(drogon::k404NotFound,"找不到任務");
      auto task=rowJson(tr); const auto pr=service->database()->execSqlSync("SELECT status,progress::text AS progress_text FROM task_participants WHERE task_id=$1 AND user_id=$2",taskId,userId); if(pr.empty())throw ApiError(drogon::k404NotFound,"此玩家尚未參與任務");
      auto progress=parseJson(pr[0]["progress_text"].as<std::string>(),Json::Value{Json::objectValue}); ensureTaskVariables(task,progress);
      Json::Value out; out["task_id"]=Json::Int64(taskId); out["user_id"]=Json::Int64(userId); out["status"]=pr[0]["status"].as<std::string>(); out["variables"]=progress["variables"]; out["all_complete"]=taskStepsComplete(task["task_steps"],progress); out["failure_reason"]=taskFailureReason(task,progress);
      Json::Value checks{Json::arrayValue}; for(const auto& step:task["task_steps"]){ Json::Value x; const auto id=stringValue(step["id"]); x["id"]=id; x["title"]=step["title"]; x["active"]=taskStepActive(step,progress); x["dependencies_complete"]=taskStepDependenciesComplete(task["task_steps"],progress,step); x["count"]=Json::Int64(intValue(progress["steps"][id]["count"])); x["required"]=step["required"]; x["completed"]=progress["steps"][id].get("completed",false).asBool(); Json::Value reasons{Json::arrayValue}; if(!x["active"].asBool())reasons.append("分支變數條件尚未成立"); if(!x["dependencies_complete"].asBool())reasons.append("前置環節尚未完成"); if(x["active"].asBool()&&!x["completed"].asBool()&&reasons.empty())reasons.append(step.get("auto_progress",false).asBool()?"等待遊戲事件":"等待玩家／DM 推進"); x["reasons"]=reasons; checks.append(x);} out["steps"]=checks; reply(cb,std::move(out));
    },"任務除錯失敗");
  });

  // [功能備註｜73-cpp.34.25 DM 任務變數｜POST /api/admin/tasks/{1}/variables]
  reg1("/api/admin/tasks/{1}/variables", drogon::Post,
       [service](const Request& req, Callback& cb, const std::string& taskText) {
    runHandler(cb,[&]{ requireUser(service,req,true); const auto taskId=positiveId(taskText); const auto body=bodyOrEmpty(req); const auto userId=intValue(body["user_id"]); auto name=trimmed(stringValue(body["name"])); if(userId<=0||name.empty()||name.size()>80)throw ApiError(drogon::k400BadRequest,"請指定玩家與變數名稱"); const auto tr=service->database()->execSqlSync("SELECT row_to_json(t)::text AS json_text FROM tasks t WHERE id=$1",taskId); if(tr.empty())throw ApiError(drogon::k404NotFound,"找不到任務"); auto task=rowJson(tr); const auto pr=service->database()->execSqlSync("SELECT progress::text AS progress_text FROM task_participants WHERE task_id=$1 AND user_id=$2",taskId,userId); if(pr.empty())throw ApiError(drogon::k404NotFound,"此玩家尚未參與任務"); auto progress=parseJson(pr[0]["progress_text"].as<std::string>(),Json::Value{Json::objectValue}); ensureTaskVariables(task,progress); const auto value=intValue(body["value"]); progress["variables"][name]=Json::Int64(stringValue(body["mode"],"set")=="add"?intValue(progress["variables"][name])+value:value); progress["all_complete"]=taskStepsComplete(task["task_steps"],progress); const auto fail=taskFailureReason(task,progress); if(!fail.empty()){progress["failed"]=true;progress["failure_reason"]=fail;service->database()->execSqlSync("UPDATE task_participants SET status='failed',progress=$1::jsonb WHERE task_id=$2 AND user_id=$3",compactJson(progress),taskId,userId);}else service->database()->execSqlSync("UPDATE task_participants SET progress=$1::jsonb WHERE task_id=$2 AND user_id=$3",compactJson(progress),taskId,userId); Json::Value out;out["ok"]=true;out["progress"]=progress;reply(cb,std::move(out)); },"調整任務變數失敗");
  });
  ...
  reg1("/api/admin/tasks/{1}/complete", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      // [功能備註｜73-cpp.34.22 任務完成保護] 有結構化步驟時，除非 DM 明確 force，否則所有步驟都必須完成。
      const auto steps = task["task_steps"].isArray() ? task["task_steps"] : Json::Value{Json::arrayValue};
      if (!steps.empty() && !body.get("force", false).asBool()) {
        const auto progressRows = service->database()->execSqlSync(
            "SELECT progress::text AS progress_text FROM task_participants WHERE task_id=$1 AND user_id=$2", taskId, targetUserId);
        auto progress = progressRows.empty() ? Json::Value{Json::objectValue}
            : parseJson(progressRows[0]["progress_text"].as<std::string>(), Json::Value{Json::objectValue});
        ensureTaskVariables(task, progress);
        if (progress.get("failed", false).asBool()) throw ApiError(drogon::k409Conflict, "此任務已失敗，不能正常完成");
        if (!taskStepsComplete(steps, progress))
          throw ApiError(drogon::k409Conflict, "任務步驟尚未全部完成；DM 可選擇強制完成");
      }
      ...
      // [功能備註｜65-cpp.26.5 任務完成屬性校正] 任務仍不直接發放舊制 attribute_points，
      // 但校正時只補足大階固定最低額度，不再覆蓋 DM 額外給予的自由屬性點。
      {
        const auto rankRows = service->database()->execSqlSync(
            "SELECT tier,agility,strength,constitution,spirit,great_way,faith,faction,attribute_points FROM character_cards WHERE user_id=$1",
            targetUserId);
        if (!rankRows.empty()) {
          ...
          const auto fixedRemaining = std::max<std::int64_t>(0, characterTierBudget(tier) - spent);
          const auto storedRemaining = std::max<std::int64_t>(0, rankRows[0]["attribute_points"].as<std::int64_t>());
          const auto remaining = std::max(storedRemaining, fixedRemaining);
          service->database()->execSqlSync(
      ...
      service->database()->execSqlSync(
          "INSERT INTO user_notifications(user_id,notification_type,title,message,data) VALUES($1,'task','任務完成',$2,jsonb_build_object('task_id',$3))",
          targetUserId, stringValue(task["title"], "任務"), taskId);
...
reg1("/api/rooms/{1}/actions", drogon::Post,
  ...
  runHandler(cb, [&] {
    ...
    // [功能備註｜73-cpp.34.25.22 人格值／搶奪控制權]
    // 先載入一次人格設定。達門檻時，本次玩家輸入會被人格覆蓋，之後仍照正常規則引擎處理，
    // 因此人格接管不能繞過 DM 規則、戰鬥限制或聲音判定。
    auto personalityContext = loadPersonalityRuntimeContext(service, roomId, user.id);
    Json::Value personalityTakeover{Json::objectValue};
    personalityTakeover["triggered"] = false;
    try {
      personalityTakeover = maybeApplyPersonalityTakeover(service, personalityContext, interactionType, text);
    } catch (const std::exception& error) {
      LOG_WARN << "personality takeover degraded for room " << roomId << ": " << error.what();
    }
    ...
    payload["noise"] = Json::Int64(noise);
    if (personalityTakeover.get("triggered", false).asBool()) {
      payload["speaker_kind"] = "personality_takeover";
      payload["personality_takeover"] = true;
      payload["personality_name"] = personalityTakeover["personality_name"];
      payload["original_interaction_type"] = personalityTakeover["original_interaction_type"];
      payload["original_text"] = personalityTakeover["original_text"];
      payload["personality_value_before"] = personalityTakeover["value_before"];
      payload["personality_value_after"] = personalityTakeover["value_after"];
      payload["takeover_threshold"] = personalityTakeover["threshold"];
      payload["personality_value_max"] = personalityTakeover["max_value"];
      payload["takeover_continuing"] = personalityTakeover["continuing"];
      payload["takeover_remaining_actions"] = personalityTakeover["remaining_after"];
    }
    ...
    // 接管行動已成功通過規則並寫入事件後，才正式扣除人格值。
    if (personalityTakeover.get("triggered", false).asBool() && personalityContext.characterId > 0) {
      try {
        service->database()->execSqlSync(
            "UPDATE character_cards SET personality_profile=jsonb_set(jsonb_set(personality_profile,'{personality_value}',to_jsonb($1::bigint),true),'{takeover_remaining_actions}',to_jsonb($2::bigint),true),updated_at=CURRENT_TIMESTAMP WHERE id=$3",
            intValue(personalityTakeover["value_after"], 0), intValue(personalityTakeover["remaining_after"], 0), personalityContext.characterId);
      } catch (const std::exception& error) {
        LOG_WARN << "personality takeover value commit degraded for room " << roomId << ": " << error.what();
      }
    }

    // [73-cpp.34.7] 玩家完成互動後，同場景 NPC 可以自行判斷並在聊天室回應。
    Json::Value npcSpeech{Json::arrayValue};
    try {
      npcSpeech = runAutonomousNpcSpeech(service, roomId, user.id, interactionType, text);
    } catch (const std::exception& error) {
      // NPC 自主發言屬附加 AI；失敗不能拖垮玩家的核心說話／行動。
      LOG_WARN << "autonomous NPC speech degraded for room " << roomId << ": " << error.what();
    }

    // [功能備註｜73-cpp.34.25.22 人格 AI／人格值／控制權]
    // 沒有發生控制權搶奪時，人格才進行額外內在思考；成功思考會增加人格值。
    // 搶奪與思考共用 personalityContext，不增加第二次角色卡讀取；仍共用本次 snapshot 廣播。
    Json::Value personalityAi{Json::objectValue};
    personalityAi["triggered"] = false;
    if (!personalityTakeover.get("triggered", false).asBool()) {
      try {
        personalityAi = runSelfAwarePersonalityAi(service, roomId, user.id, personalityContext, interactionType, text);
      } catch (const std::exception& error) {
        LOG_WARN << "self-aware personality AI degraded for room " << roomId << ": " << error.what();
      }
    }
    ...
    out["npc_speech"] = npcSpeech;
    out["personality_ai"] = personalityAi;
    out["personality_takeover"] = personalityTakeover;
    // 舊 action 用戶仍能從 rule_engine 取得第一個事件結果。
    if (ruleEvents.isArray() && !ruleEvents.empty()) out["rule_engine"] = ruleEvents[0];

    // [功能備註｜73-cpp.34.6 聊天交流] HTTP 互動也要主動推送房間快照，
    // 否則只有送話者看得到，其他玩家必須重整才會收到新對話。
    RoomSocket::broadcastSnapshotForRoom(roomId);
  ...
  reg0("/api/admin/item-templates", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] { requireUser(service, req, true); Json::Value out; out["ok"] = true; auto body=bodyOrEmpty(req);body["rank"]=normalizeContentRank(stringValue(body["rank"],"G"));body["study_domains"]=normalizeItemStudyDomains(body["study_domains"]);body["profession_requirement_mode"]=normalizeProfessionRequirementMode(stringValue(body["profession_requirement_mode"],"any"));out["item"] = genericInsert(service->database(), "item_templates", body); reply(cb, std::move(out)); });
  });
  // [功能備註｜65-cpp.26.15 道具／裝備模板可重複修改｜PATCH /api/admin/item-templates/{1}]
  reg1("/api/admin/item-templates/{1}", drogon::Patch,
       [service](const Request& req, Callback& cb, const std::string& idText) {
    runHandler(cb, [&] {
      requireUser(service, req, true);
      auto body = bodyOrEmpty(req);
      if (body.isMember("rank")) body["rank"] = normalizeContentRank(stringValue(body["rank"], "G"));
      if (body.isMember("study_domains")) body["study_domains"] = normalizeItemStudyDomains(body["study_domains"]);
      if (body.isMember("profession_requirement_mode")) body["profession_requirement_mode"] = normalizeProfessionRequirementMode(stringValue(body["profession_requirement_mode"], "any"));
      Json::Value out;
      out["ok"] = true;
      out["item"] = genericUpdate(service->database(), "item_templates", positiveId(idText), body);
      reply(cb, std::move(out));
    }, "修改道具／裝備模板失敗");
  });
  ...
  reg0("/api/character/items/use", drogon::Post, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      const auto requestedName = trimmed(stringValue(body["name"]));
      if (templateId <= 0 && requestedName.empty()) throw ApiError(drogon::k400BadRequest, "缺少道具模板 ID");
      const auto rows = templateId > 0
          ? service->database()->execSqlSync("SELECT row_to_json(i)::text AS json_text FROM item_templates i WHERE id=$1", templateId)
          : service->database()->execSqlSync("SELECT row_to_json(i)::text AS json_text FROM item_templates i WHERE name=$1 ORDER BY id LIMIT 1", requestedName);
      if (rows.empty()) throw ApiError(drogon::k404NotFound, "找不到道具模板");
      const auto item = rowJson(rows);
      auto character = characterRaw(service->database(), user.id);
      // [功能備註｜73-cpp.34.25.24 特殊職業道具] profession_requirements 為允許使用此道具的職業名稱；空陣列代表不限。
      const auto requirements = item["profession_requirements"].isArray() ? item["profession_requirements"] : Json::Value{Json::arrayValue};
      if (!requirements.empty()) {
        const auto mode = normalizeProfessionRequirementMode(stringValue(item["profession_requirement_mode"], "any"));
        auto normalized = [](std::string value) { value = trimmed(value); std::transform(value.begin(), value.end(), value.begin(), [](unsigned char c){ return static_cast<char>(std::tolower(c)); }); return value; };
        const auto mainClass = normalized(stringValue(character["main_class"]));
        std::unordered_set<std::string> subClasses;
        if (character["sub_classes"].isArray()) for (const auto& raw : character["sub_classes"]) { const auto n=normalized(stringValue(raw)); if(!n.empty()) subClasses.insert(n); }
        bool allowed=false; std::string requiredText;
        for (const auto& raw : requirements) { const auto display=trimmed(stringValue(raw));const auto n=normalized(display); if(n.empty()) continue; if(!requiredText.empty()) requiredText += "、"; requiredText += display; const bool mainMatch=!mainClass.empty()&&mainClass!="未設定"&&mainClass==n; const bool subMatch=subClasses.contains(n); if((mode=="main"&&mainMatch)||(mode=="sub"&&subMatch)||(mode=="any"&&(mainMatch||subMatch)))allowed=true; }
        if (!allowed) { const auto modeText = mode=="main"?"主職業":mode=="sub"?"副職業":"主／副職業"; throw ApiError(drogon::k403Forbidden, "此為特殊職業道具，需要" + modeText + "：" + requiredText); }
      }
      auto inventory = inventoryOf(character);
      ...
      const auto mysticismPointGain = std::max<std::int64_t>(0, intValue(effects["mysticism_points"]));
      const auto ritualXpGain = std::max<std::int64_t>(0, intValue(effects["ritual_xp"]));
      if ((hasPermanentElementGain || magicPointGain > 0 || ritualPointGain > 0 || mysticismPointGain > 0 || ritualXpGain > 0) &&
      ...
      if (roomId > 0) {
        requireRoomAccess(service, user, roomId);
        Json::Value rulePayload; rulePayload["item_template_id"] = Json::Int64(intValue(item["id"])); rulePayload["item"] = item; rulePayload["item_name"] = name;
        ...
        Json::Value payload; payload["text"] = stringValue(character["name"], user.username) + " 使用道具「" + name + "」"; payload["item_template_id"] = Json::Int64(intValue(item["id"])); payload["item"] = item; service->addEvent(roomId, user.id, "item_use", payload);
      ...
      service->database()->execSqlSync(
          ...
          "ritual_points_earned=ritual_points_earned+$4,mysticism_points=GREATEST(0,mysticism_points+$5),updated_at=CURRENT_TIMESTAMP WHERE user_id=$6",
          compactJson(inventory), compactJson(baseElementCaps), magicPointGain, ritualPointGain, mysticismPointGain, user.id);
  ...
  reg1("/api/rooms/{1}/world-systems", drogon::Get,
    ...
    runHandler(cb, [&] {
      ...
      const auto anomalyRows=service->database()->execSqlSync("SELECT row_to_json(a)::text AS json_text FROM room_anomaly_profiles a WHERE room_id=$1",roomId);
      if(anomalyRows.empty()){
        Json::Value anomaly; anomaly["enabled"]=false; anomaly["name"]="世界異常"; anomaly["intensity"]=0; anomaly["tags"]=Json::Value(Json::arrayValue); out["anomaly_profile"]=anomaly;
      }else{
        auto anomaly=rowJson(anomalyRows);
        if(!user.isAdmin){anomaly.removeMember("gm_notes"); anomaly.removeMember("combat_notes"); anomaly.removeMember("exploration_notes");}
        out["anomaly_profile"]=anomaly;
      }
      reply(cb,std::move(out));
    },"讀取世界系統失敗");
  });

  // [功能備註｜73-cpp.34.25.30 世界異常設定｜PATCH /api/admin/rooms/{1}/anomaly-profile]
  reg1("/api/admin/rooms/{1}/anomaly-profile",drogon::Patch,
       [service](const Request& req,Callback& cb,const std::string& roomIdText){runHandler(cb,[&]{
         requireUser(service,req,true); const auto roomId=positiveId(roomIdText); const auto body=bodyOrEmpty(req);
         const auto intensity=std::max<std::int64_t>(0,std::min<std::int64_t>(100,intValue(body["intensity"])));
         Json::Value tags{Json::arrayValue}; if(body["tags"].isArray()){for(const auto& tag:body["tags"]){const auto v=trimmed(stringValue(tag));if(!v.empty()&&tags.size()<12)tags.append(v.substr(0,40));}}
         const auto rows=service->database()->execSqlSync(
           "INSERT INTO room_anomaly_profiles(room_id,enabled,name,intensity,public_title,public_description,tags,combat_notes,exploration_notes,gm_notes,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9,$10,CURRENT_TIMESTAMP) ON CONFLICT(room_id) DO UPDATE SET enabled=EXCLUDED.enabled,name=EXCLUDED.name,intensity=EXCLUDED.intensity,public_title=EXCLUDED.public_title,public_description=EXCLUDED.public_description,tags=EXCLUDED.tags,combat_notes=EXCLUDED.combat_notes,exploration_notes=EXCLUDED.exploration_notes,gm_notes=EXCLUDED.gm_notes,updated_at=CURRENT_TIMESTAMP RETURNING row_to_json(room_anomaly_profiles)::text AS json_text",
           roomId,boolValue(body["enabled"],false),trimmed(stringValue(body["name"])).substr(0,160),intensity,trimmed(stringValue(body["public_title"])).substr(0,160),stringValue(body["public_description"]),compactJson(tags),stringValue(body["combat_notes"]),stringValue(body["exploration_notes"]),stringValue(body["gm_notes"]));
         Json::Value out;out["ok"]=true;out["anomaly_profile"]=rowJson(rows);reply(cb,std::move(out));
       },"儲存世界異常設定失敗");});
  ...
  reg2("/api/admin/rooms/{1}/monsters/{2}/boss-next",drogon::Post,
       [service](const Request& req,Callback& cb,const std::string& roomIdText,const std::string& monsterIdText){runHandler(cb,[&]{const auto user=requireUser(service,req,true);const auto roomId=positiveId(roomIdText);const auto monsterId=positiveId(monsterIdText);const auto body=bodyOrEmpty(req);const auto name=stringValue(body["name"],stringValue(body["phase_name"],"下一階段"));const auto rows=service->database()->execSqlSync("UPDATE room_monsters SET boss_phase_index=boss_phase_index+1,boss_phase_name=$1 WHERE id=$2 AND room_id=$3 RETURNING boss_phase_index",name,monsterId,roomId);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到怪物");Json::Value payload;payload["text"]="Boss 進入「"+name+"」";payload["monster_id"]=Json::Int64(monsterId);payload["phase_index"]=Json::Int64(rows[0]["boss_phase_index"].as<std::int64_t>());service->addEvent(roomId,user.id,"boss_phase",payload);payload["phase"]=Json::Int64(rows[0]["boss_phase_index"].as<std::int64_t>()+1);for(const auto& member:service->database()->execSqlSync("SELECT user_id FROM room_members WHERE room_id=$1 AND role='player'",roomId))advanceStructuredTaskStepsForEvent(service,roomId,member["user_id"].as<std::int64_t>(),"BOSS_PHASE_CHANGED",payload);Json::Value out;out["ok"]=true;out["room"]=service->roomSnapshot(roomId);reply(cb,std::move(out));},"切換 Boss 階段失敗");});
  ...
  reg0("/api/admin/backups/site/export", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      const std::array<const char*,113> names={"rooms","room_members","events","character_cards","character_room_bindings","extra_skills","bloodline_templates","summon_templates","summon_instances","recipes","profession_templates","skill_templates","shop_categories","shop_items","monster_templates","npc_templates","dungeon_templates","room_monsters","monster_discoveries","room_npcs","room_dungeons","combat_entity_statuses","combat_status_instances","combat_field_effects","clue_templates","character_clues","status_templates","character_statuses","room_map_nodes","hidden_rolls","item_templates","horror_rules","rule_discoveries","room_teams","room_team_members","team_shared_clues","team_shared_rules","personal_visions","team_shared_notes","team_messages","scheduled_world_events","room_chases","npc_relationships","search_containers","search_records","room_savepoints","character_savepoints","audit_logs","map_node_discoveries","npc_discoveries","item_identifications","clue_combinations","clue_combination_unlocks","great_way_templates","faith_deities","character_great_ways","character_faiths","custom_wheels","wheel_spins","faction_templates","character_reputations","npc_dialogue_entries","npc_dialogue_history","npc_shop_items","npc_shop_stock","faction_relations","tasks","task_participants","content_tags","entity_tags","user_favorites","user_notifications","codex_entries","codex_unlocks","shop_transactions","magic_studies","ritual_studies","mysticism_studies","ritual_instances","character_magics","character_rituals","character_mysticisms","character_ritual_research","rulebook_entries","room_team_invites","rule_engine_rules","rule_engine_rule_state","rule_engine_player_state","rule_engine_logs","avatar_images","portrait_images","character_portrait_variants","npc_portrait_variants","knowledge_shop_items","knowledge_shop_purchases","npc_task_offers","npc_avatar_images","special_currency_types","character_special_currencies","equipment_slot_types","affix_definitions","affix_pools","affix_pool_entries","reforge_profiles","affix_targets","character_recipe_discoveries","recipe_experiment_attempts","room_exploration_records","room_anomaly_profiles","dungeon_submissions","game_groups","game_group_members","game_group_messages"};
  ...
  reg1("/api/rooms/{1}/explore", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      auto recordExploration = [&](const std::string& target, std::int64_t roll, std::int64_t rate, bool success, const Json::Value& discoveries) {
        service->database()->execSqlSync(
          "INSERT INTO room_exploration_records(room_id,user_id,map_node_id,mode,target_name,skill_name,roll,success_rate,success,discoveries) VALUES($1,$2,NULLIF($3,0),$4,$5,$6,$7,$8,$9,$10::jsonb)",
          roomId,user.id,nodeId,mode,target,stringValue(skill["name"],"探查"),roll,rate,success,compactJson(discoveries));
      };
      std::int64_t dc = 50;
      ...
      if (mode == "npc") {
        ...
        recordExploration(targetName,roll,rate,success,found);
      ...
      if (mode == "item") {
        ...
        const auto item=rowJson(rows);
        if (!item.get("requires_identification",false).asBool()) { Json::Value out;out["ok"]=true;out["mode"]="item";out["success"]=true;out["roll"]=0;out["rate"]=100;found.append(name+" 不需要鑑定");recordExploration(name,0,100,true,found);out["found"]=found;reply(cb,std::move(out));return; }
        ...
        recordExploration(name,roll,rate,success,found);
      ...
      recordExploration(stringValue(node["name"],"目前環境"),roll,rate,success,found);
  ...
  // 用途：建立/執行房間／跑團相關資料或操作。 修改時請同步檢查前端呼叫、權限驗證與相關資料表。
  // [功能備註｜73-cpp.34.25.29 探索紀錄｜GET /api/rooms/{1}/exploration-history]
  // 探索歷史採按需讀取，避免每次房間初始載入都增加請求。
  reg1("/api/rooms/{1}/exploration-history", drogon::Get,
       [service](const Request& req, Callback& cb, const std::string& roomIdText) {
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomIdText);requireRoomAccess(service,user,roomId);Json::Value out;out["records"]=selectRows2(service->database(),"SELECT row_to_json(x)::text AS json_text FROM (SELECT r.*,n.name AS map_node_name FROM room_exploration_records r LEFT JOIN room_map_nodes n ON n.id=r.map_node_id WHERE r.room_id=$1 AND r.user_id=$2 ORDER BY r.created_at DESC LIMIT 30) x",roomId,user.id);reply(cb,std::move(out));},"讀取探索紀錄失敗");
  });
  ...
  // 用途：讀取/顯示其他系統相關資料或操作。 修改時請同步檢查前端呼叫、權限驗證與相關資料表。
  reg0("/api/pantheon",drogon::Get,[service](const Request& req,Callback& cb){runHandler(cb,[&]{const auto user=requireUser(service,req);Json::Value out;out["deities"]=user.isAdmin?genericList(service->database(),"faith_deities"):genericList(service->database(),"faith_deities","id ASC","active=TRUE");out["my_faiths"]=selectRows1(service->database(),"SELECT row_to_json(x)::text AS json_text FROM (SELECT c.*,d.name,d.true_name,d.title,d.aliases,d.domains,d.portrait_url,d.lore,d.ai_config,d.doctrine,d.description,d.max_rank,d.effects,d.granted_skill_template_ids FROM character_faiths c JOIN faith_deities d ON d.id=c.deity_id WHERE c.user_id=$1 ORDER BY c.created_at,d.id) x",user.id);reply(cb,std::move(out));},"讀取諸神殿失敗");});
  // [功能備註｜73-cpp.34.25.26 神明 AI｜POST /api/pantheon/{1}/pray]
  // 神諭判定完全在 C++ 後端執行；前端只播放結果，避免玩家竄改虔誠／神眷。
  reg1("/api/pantheon/{1}/pray", drogon::Post,
       [service](const Request& req, Callback& cb, const std::string& deityIdText) {
    runHandler(cb, [&] {
      const auto user=requireUser(service,req); const auto deityId=positiveId(deityIdText);
      const auto rows=service->database()->execSqlSync("SELECT row_to_json(d)::text AS json_text FROM faith_deities d WHERE id=$1 AND active=TRUE",deityId);
      if(rows.empty()) throw ApiError(drogon::k404NotFound,"找不到可祈禱的神祇");
      const auto deity=rowJson(rows); const auto cfg=deity["ai_config"].isObject()?deity["ai_config"]:Json::Value{Json::objectValue};
      if(!boolValue(cfg["enabled"],false)) throw ApiError(drogon::k400BadRequest,"此神祇目前沒有開放神諭 AI");
      const int chance=std::clamp<int>(static_cast<int>(intValue(cfg["response_chance"],35)),0,100);
      const bool responded=randomInt(1,100)<=chance;
      const auto silent=stringValue(cfg["silent_text"],"祈禱落入寂靜，沒有得到明確回應。");
      std::string text=silent;
      if(responded){
        std::vector<std::string> lines;
        if(cfg["response_lines"].isArray()) for(const auto& line:cfg["response_lines"]) if(line.isString()&&!line.asString().empty()) lines.push_back(line.asString());
        if(lines.empty()){
          const auto tone=stringValue(cfg["tone"],"solemn"), name=stringValue(deity["name"],"神祇");
          if(tone=="gentle") lines={"你的祈願已被聽見。先守住自己，再去守住你珍視的人。","不用急著尋找答案；你正在走的路本身也會留下答案。"};
          else if(tone=="cryptic") lines={"答案不在你所問之處，而在你刻意略過的那一步。","當第二個徵兆出現時，不要相信第一次得到的結論。"};
          else if(tone=="stern") lines={"祈禱不能替代選擇。去做你應該做的事。","若你要求力量，就先證明你能承受力量帶來的責任。"};
          else if(tone=="playful") lines={"你問得很認真，但命運可沒答應要一本正經地回答。","去看看你最不打算查看的地方，也許會有驚喜。"};
          else lines={"你的祈禱已抵達。遵循你的誓言，不要讓恐懼替你選擇。","道路仍在前方。保持敬意，也保持判斷。"};
          if(!name.empty()&&randomInt(1,100)<=20) lines.push_back(name+"的意志短暫掠過你的感知。");
        }
        text=lines[static_cast<std::size_t>(randomInt(0,static_cast<int>(lines.size()-1)))];
      }
      Json::Value out; out["ok"]=true; out["responded"]=responded; out["deity_id"]=Json::Int64(deityId); out["deity_name"]=deity["name"]; out["text"]=text;
      const auto faith=service->database()->execSqlSync("SELECT devotion,favor FROM character_faiths WHERE user_id=$1 AND deity_id=$2 AND active=TRUE",user.id,deityId);
      if(!faith.empty()){
        auto devotion=faith[0]["devotion"].as<std::int64_t>(), favor=faith[0]["favor"].as<std::int64_t>();
        if(responded){ devotion+=std::max<std::int64_t>(0,intValue(cfg["devotion_gain"],1)); if(randomInt(1,100)<=std::clamp<int>(static_cast<int>(intValue(cfg["favor_gain_chance"],10)),0,100)) favor=std::min<std::int64_t>(100,favor+1); service->database()->execSqlSync("UPDATE character_faiths SET devotion=$3,favor=$4,updated_at=CURRENT_TIMESTAMP WHERE user_id=$1 AND deity_id=$2",user.id,deityId,devotion,favor); }
        out["devotion"]=Json::Int64(devotion); out["favor"]=Json::Int64(favor);
      }
      reply(cb,std::move(out));
    },"祈禱神諭失敗");
  });
  ...
  reg0("/api/search", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb, [&] {
      ...
      const auto rows = service->database()->execSqlSync(
          ...
          "UNION ALL SELECT 'ritual',id::text,name,category,description FROM ritual_studies WHERE active=TRUE AND (name ILIKE '%'||$1||'%' OR category ILIKE '%'||$1||'%' OR description ILIKE '%'||$1||'%') UNION ALL SELECT 'mysticism',id::text,name,category,description FROM mysticism_studies WHERE active=TRUE AND (name ILIKE '%'||$1||'%' OR category ILIKE '%'||$1||'%' OR description ILIKE '%'||$1||'%') "
  ...
  // [功能備註｜65-cpp.26.11 通知中心 2.0] 一次將自己的未讀通知全部設為已讀。
  reg0("/api/notifications/read-all", drogon::Post, [service](const Request& req, Callback& cb){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto rows=service->database()->execSqlSync("UPDATE user_notifications SET read_at=CURRENT_TIMESTAMP WHERE user_id=$1 AND read_at IS NULL RETURNING id",user.id);Json::Value out;out["ok"]=true;out["updated"]=static_cast<Json::UInt64>(rows.size());reply(cb,std::move(out));},"全部標記已讀失敗");
  });
  ...
  // =========================================================
  // 73-cpp.34.25.23 紙本清單：神秘學／禁忌知識與代價
  // =========================================================
  reg0("/api/mysticism-studies", drogon::Get, [service](const Request& req, Callback& cb) {
    runHandler(cb,[&]{
      const auto user=requireUser(service,req);const auto character=characterRaw(service->database(),user.id);Json::Value out;
      out["points"]=Json::Int64(std::max<std::int64_t>(0,intValue(character["mysticism_points"])));
      out["forbidden_cost"]=Json::Int64(std::max<std::int64_t>(0,intValue(character["forbidden_cost"])));
      out["forbidden_knowledge"]=character["forbidden_knowledge"].isArray()?character["forbidden_knowledge"]:Json::Value{Json::arrayValue};
      out["studies"]=genericList(service->database(),"mysticism_studies","category ASC,rank ASC,name ASC,id ASC","active=TRUE");
      out["learned"]=selectRows1(service->database(),"SELECT row_to_json(x)::text AS json_text FROM (SELECT cm.learned_at,m.* FROM character_mysticisms cm JOIN mysticism_studies m ON m.id=cm.mysticism_id WHERE cm.user_id=$1 ORDER BY cm.learned_at,m.id) x",user.id);
      reply(cb,std::move(out));
    },"讀取神秘學失敗");
  });

  reg1("/api/mysticism-studies/{1}/learn",drogon::Post,[service](const Request& req,Callback& cb,const std::string&idText){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req);const auto id=positiveId(idText,"神秘學 ID 無效");
      const auto rows=service->database()->execSqlSync("SELECT row_to_json(m)::text AS json_text FROM mysticism_studies m WHERE id=$1 AND active=TRUE",id);
      if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到此神秘學");const auto study=rowJson(rows);
      if(!service->database()->execSqlSync("SELECT 1 FROM character_mysticisms WHERE user_id=$1 AND mysticism_id=$2",user.id,id).empty())throw ApiError(drogon::k409Conflict,"你已經掌握這項神秘學");
      if(study["prerequisite_ids"].isArray())for(const auto& raw:study["prerequisite_ids"]){const auto required=intValue(raw);if(required>0&&service->database()->execSqlSync("SELECT 1 FROM character_mysticisms WHERE user_id=$1 AND mysticism_id=$2",user.id,required).empty())throw ApiError(drogon::k400BadRequest,"尚未滿足神秘學前置條件");}
      auto character=characterRaw(service->database(),user.id);auto inventory=inventoryOf(character);const auto requiredItem=trimmed(stringValue(study["required_item"]));
      if(!requiredItem.empty()&&inventoryQuantity(inventory,requiredItem)<=0)throw ApiError(drogon::k400BadRequest,"需要先取得神秘學媒介：「"+requiredItem+"」");
      const auto cost=std::max<std::int64_t>(0,intValue(study["point_cost"]));const auto update=service->database()->execSqlSync("UPDATE character_cards SET mysticism_points=mysticism_points-$1,updated_at=CURRENT_TIMESTAMP WHERE user_id=$2 AND mysticism_points>=$1 RETURNING mysticism_points",cost,user.id);if(update.empty())throw ApiError(drogon::k400BadRequest,"神秘學點數不足");
      try{
        service->database()->execSqlSync("INSERT INTO character_mysticisms(user_id,mysticism_id) VALUES($1,$2)",user.id,id);
        if(!requiredItem.empty()&&study.get("consume_item",false).asBool()){inventory=changeInventory(std::move(inventory),requiredItem,-1);service->database()->execSqlSync("UPDATE character_cards SET inventory=$1::jsonb WHERE user_id=$2",compactJson(inventory),user.id);}
        if(study.get("forbidden",false).asBool()){
          const auto price=std::max<std::int64_t>(0,intValue(study["forbidden_cost"]));Json::Value knowledge=character["forbidden_knowledge"].isArray()?character["forbidden_knowledge"]:Json::Value{Json::arrayValue};Json::Value entry;entry["id"]=Json::Int64(id);entry["name"]=study["name"];entry["rank"]=study["rank"];entry["cost"]=Json::Int64(price);entry["cost_description"]=study["cost_description"];knowledge.append(entry);
          service->database()->execSqlSync("UPDATE character_cards SET forbidden_cost=forbidden_cost+$1,forbidden_knowledge=$2::jsonb,updated_at=CURRENT_TIMESTAMP WHERE user_id=$3",price,compactJson(knowledge),user.id);
        }
      }catch(...){service->database()->execSqlSync("DELETE FROM character_mysticisms WHERE user_id=$1 AND mysticism_id=$2",user.id,id);service->database()->execSqlSync("UPDATE character_cards SET mysticism_points=mysticism_points+$1 WHERE user_id=$2",cost,user.id);throw;}
      Json::Value out;out["ok"]=true;out["study"]=study;out["character"]=characterRaw(service->database(),user.id);reply(cb,std::move(out));
    },"學習神秘學失敗");
  });

  reg0("/api/admin/mysticism-studies",drogon::Get,[service](const Request& req,Callback& cb){runHandler(cb,[&]{requireUser(service,req,true);Json::Value out;out["studies"]=genericList(service->database(),"mysticism_studies","category ASC,rank ASC,name ASC,id ASC");reply(cb,std::move(out));},"讀取神秘學資料庫失敗");});
  reg0("/api/admin/mysticism-studies",drogon::Post,[service](const Request& req,Callback& cb){runHandler(cb,[&]{requireUser(service,req,true);auto body=bodyOrEmpty(req);const auto name=trimmed(stringValue(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"神秘學名稱不能空白");body["name"]=clipped(name,120);body["rank"]=normalizeContentRank(stringValue(body["rank"],"G"));body["point_cost"]=Json::Int64(std::max<std::int64_t>(0,intValue(body["point_cost"],1)));body["forbidden_cost"]=Json::Int64(std::max<std::int64_t>(0,intValue(body["forbidden_cost"])));body["required_item"]=clipped(trimmed(stringValue(body["required_item"])),180);Json::Value out;out["ok"]=true;out["study"]=genericInsert(service->database(),"mysticism_studies",body);reply(cb,std::move(out));},"建立神秘學失敗");});
  reg1("/api/admin/mysticism-studies/{1}",drogon::Patch,[service](const Request& req,Callback& cb,const std::string&idText){runHandler(cb,[&]{requireUser(service,req,true);auto body=bodyOrEmpty(req);if(body.isMember("rank"))body["rank"]=normalizeContentRank(stringValue(body["rank"]));if(body.isMember("point_cost"))body["point_cost"]=Json::Int64(std::max<std::int64_t>(0,intValue(body["point_cost"])));if(body.isMember("forbidden_cost"))body["forbidden_cost"]=Json::Int64(std::max<std::int64_t>(0,intValue(body["forbidden_cost"])));Json::Value out;out["ok"]=true;out["study"]=genericUpdate(service->database(),"mysticism_studies",positiveId(idText),body);reply(cb,std::move(out));},"更新神秘學失敗");});
  reg1("/api/admin/mysticism-studies/{1}",drogon::Delete,[service](const Request& req,Callback& cb,const std::string&idText){runHandler(cb,[&]{requireUser(service,req,true);genericDelete(service->database(),"mysticism_studies",positiveId(idText));Json::Value out;out["ok"]=true;reply(cb,std::move(out));},"刪除神秘學失敗");});
  ...
  reg1("/api/admin/players/{1}/study-points",drogon::Post,
    ...
    runHandler(cb,[&]{
      ...
      if(kind!="magic"&&kind!="ritual"&&kind!="mysticism")throw ApiError(drogon::k400BadRequest,"kind 必須是 magic、ritual 或 mysticism");
      if(kind=="magic"){
      ...
      }else if(kind=="mysticism"){
        service->database()->execSqlSync("UPDATE character_cards SET mysticism_points=GREATEST(0,mysticism_points+$1),updated_at=CURRENT_TIMESTAMP WHERE user_id=$2",delta,userId);
      }else{
  ...
  // [功能備註｜66-cpp.27 命隙／塔羅 API]
  // 大阿爾：重大命運主軸；小阿爾：較具體但可被行動改變的預兆。
  auto requireTarotDungeon=[service](const UserInfo& user,std::int64_t roomId,std::int64_t dungeonId){
    requireRoomAccess(service,user,roomId);
    const auto found=service->database()->execSqlSync("SELECT 1 FROM room_dungeons WHERE room_id=$1 AND dungeon_template_id=$2",roomId,dungeonId);
    if(found.empty())throw ApiError(drogon::k404NotFound,"這個房間沒有此副本");
  };
  auto drawTarotCard=[service](const std::string& arcana){
    const auto rows=service->database()->execSqlSync(
      "SELECT row_to_json(t)::text AS json_text FROM tarot_cards t WHERE arcana=$1 AND enabled=TRUE ORDER BY random() LIMIT 1",arcana);
    if(rows.empty())throw ApiError(drogon::k503ServiceUnavailable,"塔羅牌庫尚未初始化");
    return rowJson(rows);
  };
  auto miyoiId=[service]()->std::int64_t{
    const auto rows=service->database()->execSqlSync("SELECT id FROM npc_templates WHERE name='彌夜' ORDER BY id LIMIT 1");
    return rows.empty()?0:rows[0]["id"].as<std::int64_t>();
  };
  auto tarotOrientation=[service](){
    const auto rows=service->database()->execSqlSync("SELECT CASE WHEN random()<0.5 THEN 'upright' ELSE 'reversed' END AS v");
    return rows[0]["v"].as<std::string>();
  };
  auto tarotProphecy=[](const Json::Value& card,const std::string& orientation,const std::string& type){
    const auto name=stringValue(card["name"]);
    const auto words=orientation=="upright"?stringValue(card["upright_keywords"]):stringValue(card["reversed_keywords"]);
    if(type=="dungeon")return std::string{"副本的命運正向「"}+name+"」靠攏。"+words+"；真正的形式仍未確定。";
    if(type=="omen")return std::string{"近期最清晰的預兆是「"}+name+"」。"+words+"。這條未來仍可被你的選擇改變。";
    return std::string{"今日命運回應了「"}+name+"」。"+words+"。";
  };

  reg0("/api/tarot/cards",drogon::Get,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req);Json::Value out;out["major"]=Json::arrayValue;out["minor"]=Json::arrayValue;const auto rows=service->database()->execSqlSync("SELECT row_to_json(t)::text AS json_text FROM tarot_cards t WHERE enabled=TRUE ORDER BY arcana,card_no,suit");for(const auto&r:rows){auto c=parseJson(r["json_text"].as<std::string>());out[stringValue(c["arcana"])=="major"?"major":"minor"].append(c);}reply(cb,std::move(out));},"讀取塔羅牌庫失敗");
  });

  reg2("/api/tarot/status/{1}/{2}",drogon::Get,[service,requireTarotDungeon](const Request& req,Callback& cb,const std::string& roomText,const std::string& dungeonText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);const auto dungeonId=positiveId(dungeonText);requireTarotDungeon(user,roomId,dungeonId);const auto rows=service->database()->execSqlSync("SELECT reading_type,COUNT(*)::BIGINT AS n FROM tarot_readings WHERE room_id=$1 AND dungeon_template_id=$2 AND ((reading_type='dungeon') OR (user_id=$3 AND reading_date=CURRENT_DATE)) GROUP BY reading_type",roomId,dungeonId,user.id);Json::Value out;out["personal_available"]=true;out["omen_available"]=true;out["dungeon_available"]=true;for(const auto&r:rows){const auto type=r["reading_type"].as<std::string>();const bool used=r["n"].as<std::int64_t>()>0;if(type=="personal")out["personal_available"]=!used;if(type=="omen")out["omen_available"]=!used;if(type=="dungeon")out["dungeon_available"]=!used;}reply(cb,std::move(out));},"讀取占卜狀態失敗");
  });

  reg2("/api/tarot/personal/{1}/{2}",drogon::Post,[service,requireTarotDungeon,drawTarotCard,miyoiId,tarotOrientation,tarotProphecy](const Request& req,Callback& cb,const std::string& roomText,const std::string& dungeonText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);const auto dungeonId=positiveId(dungeonText);requireTarotDungeon(user,roomId,dungeonId);if(!service->database()->execSqlSync("SELECT 1 FROM tarot_readings WHERE user_id=$1 AND room_id=$2 AND dungeon_template_id=$3 AND reading_type='personal' AND reading_date=CURRENT_DATE",user.id,roomId,dungeonId).empty())throw ApiError(drogon::k409Conflict,"今天已經進行過個人命運占卜");auto card=drawTarotCard("major");const auto orientation=tarotOrientation();const auto effect=orientation=="upright"?card["upright_effect"]:card["reversed_effect"];const auto prophecy=tarotProphecy(card,orientation,"personal");const auto id=service->database()->execSqlSync("INSERT INTO tarot_readings(user_id,room_id,dungeon_template_id,npc_template_id,reading_type,card_id,orientation,effect_snapshot,prophecy_text,expires_at) VALUES($1,$2,$3,NULLIF($4,0),'personal',$5,$6,$7::jsonb,$8,CURRENT_DATE+INTERVAL '1 day') RETURNING id",user.id,roomId,dungeonId,miyoiId(),intValue(card["id"]),orientation,compactJson(effect),prophecy)[0]["id"].as<std::int64_t>();Json::Value out;out["ok"]=true;out["reading_id"]=Json::Int64(id);out["type"]="personal";out["card"]=card;out["orientation"]=orientation;out["effect"]=effect;out["prophecy"]=prophecy;reply(cb,std::move(out));},"個人命運占卜失敗");
  });

  reg2("/api/tarot/omen/{1}/{2}",drogon::Post,[service,requireTarotDungeon,drawTarotCard,miyoiId,tarotOrientation,tarotProphecy](const Request& req,Callback& cb,const std::string& roomText,const std::string& dungeonText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);const auto dungeonId=positiveId(dungeonText);requireTarotDungeon(user,roomId,dungeonId);if(!service->database()->execSqlSync("SELECT 1 FROM tarot_readings WHERE user_id=$1 AND room_id=$2 AND dungeon_template_id=$3 AND reading_type='omen' AND reading_date=CURRENT_DATE",user.id,roomId,dungeonId).empty())throw ApiError(drogon::k409Conflict,"今天已經看過一次命運預兆");auto card=drawTarotCard("minor");const auto orientation=tarotOrientation();const auto effect=orientation=="upright"?card["upright_effect"]:card["reversed_effect"];const auto prophecy=tarotProphecy(card,orientation,"omen");const auto id=service->database()->execSqlSync("INSERT INTO tarot_readings(user_id,room_id,dungeon_template_id,npc_template_id,reading_type,card_id,orientation,effect_snapshot,prophecy_text,expires_at) VALUES($1,$2,$3,NULLIF($4,0),'omen',$5,$6,$7::jsonb,$8,CURRENT_DATE+INTERVAL '1 day') RETURNING id",user.id,roomId,dungeonId,miyoiId(),intValue(card["id"]),orientation,compactJson(effect),prophecy)[0]["id"].as<std::int64_t>();Json::Value out;out["ok"]=true;out["reading_id"]=Json::Int64(id);out["type"]="omen";out["card"]=card;out["orientation"]=orientation;out["effect"]=effect;out["prophecy"]=prophecy;reply(cb,std::move(out));},"命運預兆占卜失敗");
  });

  reg2("/api/tarot/dungeon/{1}/{2}",drogon::Post,[service,requireTarotDungeon,drawTarotCard,miyoiId,tarotOrientation,tarotProphecy](const Request& req,Callback& cb,const std::string& roomText,const std::string& dungeonText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);const auto dungeonId=positiveId(dungeonText);requireTarotDungeon(user,roomId,dungeonId);if(!service->database()->execSqlSync("SELECT 1 FROM tarot_readings WHERE room_id=$1 AND dungeon_template_id=$2 AND reading_type='dungeon'",roomId,dungeonId).empty())throw ApiError(drogon::k409Conflict,"這個副本已經完成過副本命運占卜");auto card=drawTarotCard("major");const auto orientation=tarotOrientation();Json::Value effect=orientation=="upright"?card["upright_effect"]:card["reversed_effect"];effect["scope"]="dungeon";effect["rule_engine_tag"]="tarot_fate";const auto prophecy=tarotProphecy(card,orientation,"dungeon");const auto id=service->database()->execSqlSync("INSERT INTO tarot_readings(user_id,room_id,dungeon_template_id,npc_template_id,reading_type,card_id,orientation,effect_snapshot,prophecy_text) VALUES($1,$2,$3,NULLIF($4,0),'dungeon',$5,$6,$7::jsonb,$8) RETURNING id",user.id,roomId,dungeonId,miyoiId(),intValue(card["id"]),orientation,compactJson(effect),prophecy)[0]["id"].as<std::int64_t>();Json::Value out;out["ok"]=true;out["reading_id"]=Json::Int64(id);out["type"]="dungeon";out["card"]=card;out["orientation"]=orientation;out["effect"]=effect;out["prophecy"]=prophecy;reply(cb,std::move(out));},"副本命運占卜失敗");
  });

  reg2("/api/tarot/readings/{1}/{2}",drogon::Get,[service,requireTarotDungeon](const Request& req,Callback& cb,const std::string& roomText,const std::string& dungeonText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);const auto dungeonId=positiveId(dungeonText);requireTarotDungeon(user,roomId,dungeonId);Json::Value out;out["readings"]=Json::arrayValue;const auto rows=service->database()->execSqlSync("SELECT row_to_json(x)::text AS json_text FROM (SELECT r.*,c.name,c.arcana,c.suit,c.card_no,c.image_key FROM tarot_readings r JOIN tarot_cards c ON c.id=r.card_id WHERE r.room_id=$1 AND r.dungeon_template_id=$2 AND (r.reading_type='dungeon' OR r.user_id=$3) ORDER BY r.created_at DESC LIMIT 100) x",roomId,dungeonId,user.id);for(const auto&r:rows)out["readings"].append(parseJson(r["json_text"].as<std::string>()));reply(cb,std::move(out));},"讀取占卜紀錄失敗");
  });

  // [功能備註｜68-cpp.29 大阿爾克那完整命鬥規則 + 彌夜協戰 AI]
  auto fateMajorRule=[](std::int64_t cardNo,bool reversed){
    static const std::array<const char*,22> upright={
      "首擊獲得愚者幸運：命中率提高 12%","魔術師：攻擊傷害提高 15%","女祭司：防禦後下一擊命中率提高 18%","皇后：每次防禦恢復最大生命 8%","皇帝：防禦減傷加強，下一擊傷害提高 10%","教皇：每第三回合雙方獲得少量恢復","戀人：連續命中會累積 5% 傷害，失手重置","戰車：攻擊傷害提高 15%","力量：低生命時傷害提高 25%","隱者：命中率降低但成功命中傷害提高 20%","命運之輪：每回合命中率與傷害產生小幅命運波動","正義：命中率提高 10%，傷害降低 10%","倒吊人：防禦會犧牲少量生命換取強力反擊","死神：目標生命低於 30% 時傷害提高 30%","節制：攻擊與防禦後獲得小幅恢復","惡魔：傷害提高 35%，自身承受風險同步提高","高塔：每第四回合雙方受到最大生命 8% 的命運震盪","星星：每回合開始恢復少量生命","月亮：命中率波動，偶爾產生幻象閃避","太陽：命中率與傷害各提高 12%","審判：第一次降至危險生命時獲得一次 15% 最大生命救贖","世界：第十回合後傷害逐回合提高"
    };
    static const std::array<const char*,22> rev={
      "愚者逆位：命中率波動更大，但失手後下一擊大幅提高命中","魔術師逆位：傷害提高 20%，命中率降低 8%","女祭司逆位：隱藏資訊干擾，雙方命中率降低 10%","皇后逆位：防禦恢復減半，但攻擊提高 10%","皇帝逆位：防禦效果下降，攻擊提高 20%","教皇逆位：每第三回合不再恢復，改為短暫削弱傷害","戀人逆位：連續攻擊傷害提高，但失手反噬少量生命","戰車逆位：傷害提高 25%，命中率降低 8%","力量逆位：生命越低命中率越不穩定","隱者逆位：防禦強化但攻擊傷害降低","命運之輪逆位：命運波動幅度加倍","正義逆位：命中率降低 5%，傷害提高 10%","倒吊人逆位：防禦不再犧牲生命，但反擊加成降低","死神逆位：低生命斬殺加成消失，改為雙方恢復受抑制","節制逆位：恢復減少，傷害提高 12%","惡魔逆位：傷害提高 20%，較少反噬","高塔逆位：震盪減為 5%，但每三回合觸發","星星逆位：恢復減半，命中率降低 5%","月亮逆位：幻象減弱，命中率小幅降低","太陽逆位：只提高 8% 傷害與命中","審判逆位：救贖只恢復 8% 最大生命","世界逆位：第八回合開始逐回合提高傷害，但命中率下降"
    };
    if (cardNo < 0 || cardNo > 21) {
      return std::string("無特殊規則");
    }
    return std::string(reversed ? rev[cardNo] : upright[cardNo]);
  };
  // [功能備註｜67-cpp.28 命鬥場完整 PvP + 彌夜 AI 記憶]
  auto loadFateMatch=[service](std::int64_t matchId){
    const auto rows=service->database()->execSqlSync(
      "SELECT row_to_json(x)::text AS json_text FROM (SELECT m.*,c.name AS rule_card_name,c.card_no AS rule_card_no FROM fate_gap_pvp_matches m LEFT JOIN tarot_cards c ON c.id=m.rule_card_id WHERE m.id=$1) x",matchId);
    if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到命鬥場對局");
    return rowJson(rows);
  };
  auto fateParticipants=[](const Json::Value& match){return match["participants"].isArray()?match["participants"]:Json::Value{Json::arrayValue};};
  auto fateHasParticipant=[fateParticipants](const Json::Value& match,std::int64_t userId){for(const auto&v:fateParticipants(match))if(intValue(v)==userId)return true;return false;};
  auto appendMiyoiMemory=[service,miyoiId](std::int64_t roomId,std::int64_t userId,const std::string& text,const std::string& type,std::int64_t affinityDelta=0,std::int64_t trustDelta=0,std::int64_t hostilityDelta=0){
    const auto npcId=miyoiId();if(npcId<=0)return;
    Json::Value rel;const auto rows=service->database()->execSqlSync("SELECT row_to_json(r)::text AS json_text FROM npc_relationships r WHERE room_id=$1 AND npc_template_id=$2 AND user_id=$3",roomId,npcId,userId);
    if(rows.empty()){rel["affinity"]=0;rel["trust"]=0;rel["fear"]=0;rel["hostility"]=0;rel["memories"]=Json::arrayValue;}else rel=rowJson(rows);
    auto memories=rel["memories"].isArray()?rel["memories"]:Json::Value{Json::arrayValue};Json::Value mem;mem["text"]=text;mem["type"]=type;mem["at"]=std::to_string(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count());memories.append(mem);
    while(memories.size()>100){Json::Value cut{Json::arrayValue};for(Json::ArrayIndex i=1;i<memories.size();++i)cut.append(memories[i]);memories=std::move(cut);}
    const auto affinity=std::clamp<std::int64_t>(intValue(rel["affinity"])+affinityDelta,-100,100),trust=std::clamp<std::int64_t>(intValue(rel["trust"])+trustDelta,-100,100),hostility=std::clamp<std::int64_t>(intValue(rel["hostility"])+hostilityDelta,0,100);
    service->database()->execSqlSync("INSERT INTO npc_relationships(room_id,npc_template_id,user_id,affinity,trust,fear,hostility,memories) VALUES($1,$2,$3,$4,$5,$6,$7,$8::jsonb) ON CONFLICT(room_id,npc_template_id,user_id) DO UPDATE SET affinity=EXCLUDED.affinity,trust=EXCLUDED.trust,fear=EXCLUDED.fear,hostility=EXCLUDED.hostility,memories=EXCLUDED.memories,updated_at=CURRENT_TIMESTAMP",roomId,npcId,userId,affinity,trust,intValue(rel["fear"]),hostility,compactJson(memories));
  };

  reg0("/api/fate-gap/pvp",drogon::Post,[service,drawTarotCard,tarotOrientation,appendMiyoiMemory](const Request& req,Callback& cb){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto body=bodyOrEmpty(req);const auto roomId=positiveId(std::to_string(intValue(body["room_id"])),"房間 ID 無效");requireRoomAccess(service,user,roomId);const auto mode=stringValue(body["mode"],"duel");if(mode!="duel")throw ApiError(drogon::k400BadRequest,"目前命鬥場正式 PvP 先支援 1v1");const auto tarotMode=stringValue(body["tarot_mode"],"fate");Json::Value card;std::string orientation;if(tarotMode!="normal"){card=drawTarotCard("major");orientation=tarotOrientation();}Json::Value participants{Json::arrayValue};participants.append(Json::Int64(user.id));Json::Value state{Json::objectValue};state["round"]=0;state["turn_index"]=0;state["log"]=Json::arrayValue;const auto result=service->database()->execSqlSync("INSERT INTO fate_gap_pvp_matches(room_id,creator_user_id,mode,tarot_mode,participants,rule_card_id,rule_orientation,state) VALUES($1,$2,$3,$4,$5::jsonb,NULLIF($6,0),NULLIF($7,''),$8::jsonb) RETURNING id",roomId,user.id,mode,tarotMode,compactJson(participants),intValue(card["id"]),orientation,compactJson(state));const auto id=result[0]["id"].as<std::int64_t>();appendMiyoiMemory(roomId,user.id,"玩家建立了命鬥場對局 #"+std::to_string(id),"fate_duel",0,1,0);Json::Value out;out["ok"]=true;out["match_id"]=Json::Int64(id);out["rule_card"]=card;out["orientation"]=orientation;reply(cb,std::move(out));},"建立命鬥場對局失敗");
  });

  reg1("/api/fate-gap/pvp/room/{1}",drogon::Get,[service](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);Json::Value out;out["matches"]=selectRows1(service->database(),"SELECT row_to_json(x)::text AS json_text FROM (SELECT m.*,u.username AS creator_username,c.name AS rule_card_name,c.card_no AS rule_card_no FROM fate_gap_pvp_matches m JOIN users u ON u.id=m.creator_user_id LEFT JOIN tarot_cards c ON c.id=m.rule_card_id WHERE m.room_id=$1 AND m.status IN ('waiting','active') ORDER BY m.created_at DESC LIMIT 40) x",roomId);reply(cb,std::move(out));},"讀取命鬥場失敗");
  });

  reg1("/api/fate-gap/pvp/{1}",drogon::Get,[service,loadFateMatch,fateHasParticipant,fateMajorRule](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);auto match=loadFateMatch(id);requireRoomAccess(service,user,intValue(match["room_id"]));Json::Value out;match["rule_description"]=fateMajorRule(intValue(match["rule_card_no"],-1),stringValue(match["rule_orientation"])=="reversed");out["match"]=match;out["is_participant"]=fateHasParticipant(match,user.id);Json::Value players{Json::arrayValue};for(const auto&v:match["participants"]){const auto uid=intValue(v);const auto rows=service->database()->execSqlSync("SELECT row_to_json(x)::text AS json_text FROM (SELECT u.id,u.username,c.name,c.strength,c.constitution,c.agility,c.spirit FROM users u LEFT JOIN character_cards c ON c.user_id=u.id WHERE u.id=$1) x",uid);if(!rows.empty())players.append(rowJson(rows));}out["players"]=players;reply(cb,std::move(out));},"讀取命鬥場對局失敗");
  });

  reg1("/api/fate-gap/pvp/{1}/join",drogon::Post,[service,loadFateMatch,fateParticipants,fateHasParticipant,appendMiyoiMemory](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);auto match=loadFateMatch(id);const auto roomId=intValue(match["room_id"]);requireRoomAccess(service,user,roomId);if(stringValue(match["status"])!="waiting")throw ApiError(drogon::k409Conflict,"對局已經開始或結束");auto ps=fateParticipants(match);if(fateHasParticipant(match,user.id)){Json::Value out;out["ok"]=true;out["match"]=match;reply(cb,std::move(out));return;}if(ps.size()>=2)throw ApiError(drogon::k409Conflict,"1v1 對局已滿");ps.append(Json::Int64(user.id));service->database()->execSqlSync("UPDATE fate_gap_pvp_matches SET participants=$1::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=$2",compactJson(ps),id);appendMiyoiMemory(roomId,user.id,"玩家加入了命鬥場對局 #"+std::to_string(id),"fate_duel",0,1,0);Json::Value out;out["ok"]=true;out["match"]=loadFateMatch(id);reply(cb,std::move(out));},"加入命鬥場失敗");
  });

  reg1("/api/fate-gap/pvp/{1}/leave",drogon::Post,[service,loadFateMatch,fateParticipants,fateHasParticipant](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);auto match=loadFateMatch(id);requireRoomAccess(service,user,intValue(match["room_id"]));if(stringValue(match["status"])!="waiting")throw ApiError(drogon::k409Conflict,"對局開始後請使用投降");if(!fateHasParticipant(match,user.id))throw ApiError(drogon::k403Forbidden,"你不在這場對局中");Json::Value ps{Json::arrayValue};for(const auto&v:fateParticipants(match))if(intValue(v)!=user.id)ps.append(v);if(user.id==intValue(match["creator_user_id"]))service->database()->execSqlSync("UPDATE fate_gap_pvp_matches SET status='cancelled',participants=$1::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=$2",compactJson(ps),id);else service->database()->execSqlSync("UPDATE fate_gap_pvp_matches SET participants=$1::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=$2",compactJson(ps),id);Json::Value out;out["ok"]=true;reply(cb,std::move(out));},"離開命鬥場失敗");
  });

  reg1("/api/fate-gap/pvp/{1}/start",drogon::Post,[service,loadFateMatch,fateParticipants](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);auto match=loadFateMatch(id);requireRoomAccess(service,user,intValue(match["room_id"]));if(user.id!=intValue(match["creator_user_id"]))throw ApiError(drogon::k403Forbidden,"只有建立者可以開始");if(stringValue(match["status"])!="waiting")throw ApiError(drogon::k409Conflict,"對局不是等待狀態");auto ps=fateParticipants(match);if(ps.size()!=2)throw ApiError(drogon::k409Conflict,"1v1 需要兩名玩家");Json::Value hp{Json::objectValue};for(const auto&v:ps){const auto uid=intValue(v);const auto rr=service->database()->execSqlSync("SELECT COALESCE(NULLIF(max_hp,0),100) AS hp FROM room_members WHERE room_id=$1 AND user_id=$2",intValue(match["room_id"]),uid);hp[std::to_string(uid)]=Json::Int64(rr.empty()?100:std::max<std::int64_t>(1,rr[0]["hp"].as<std::int64_t>()));}Json::Value state{Json::objectValue};state["round"]=1;state["turn_index"]=0;state["hp"]=hp;state["winner_user_id"]=Json::Int64(0);state["log"]=Json::arrayValue;service->database()->execSqlSync("UPDATE fate_gap_pvp_matches SET status='active',state=$1::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=$2",compactJson(state),id);Json::Value out;out["ok"]=true;out["match"]=loadFateMatch(id);reply(cb,std::move(out));},"開始命鬥場失敗");
  });

  reg1("/api/fate-gap/pvp/{1}/act",drogon::Post,[service,loadFateMatch,fateParticipants,fateHasParticipant,appendMiyoiMemory](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);auto match=loadFateMatch(id);const auto roomId=intValue(match["room_id"]);requireRoomAccess(service,user,roomId);if(stringValue(match["status"])!="active")throw ApiError(drogon::k409Conflict,"對局尚未開始或已結束");if(!fateHasParticipant(match,user.id))throw ApiError(drogon::k403Forbidden,"你不在這場對局中");auto ps=fateParticipants(match);auto state=match["state"].isObject()?match["state"]:Json::Value{Json::objectValue};const auto idx=std::clamp<std::int64_t>(intValue(state["turn_index"]),0,1);if(intValue(ps[static_cast<Json::ArrayIndex>(idx)])!=user.id)throw ApiError(drogon::k409Conflict,"還沒輪到你");std::int64_t target=0;for(const auto&v:ps)if(intValue(v)!=user.id)target=intValue(v);if(target<=0)throw ApiError(drogon::k400BadRequest,"找不到對手");const auto body=bodyOrEmpty(req);const auto action=stringValue(body["action"],"attack");if(action!="attack"&&action!="guard")throw ApiError(drogon::k400BadRequest,"目前支援 attack / guard");auto hp=state["hp"].isObject()?state["hp"]:Json::Value{Json::objectValue};auto log=state["log"].isArray()?state["log"]:Json::Value{Json::arrayValue};Json::Value entry;entry["user_id"]=Json::Int64(user.id);entry["action"]=action;entry["round"]=state["round"];
      if(action=="guard"){state["guard_user_id"]=Json::Int64(user.id);const auto cardNo=intValue(match["rule_card_no"],-1);const auto rev=stringValue(match["rule_orientation"])=="reversed";auto key=std::to_string(user.id);auto cur=intValue(hp[key],100);std::int64_t heal=0;if(cardNo==3)heal=std::max<std::int64_t>(1,cur*0+8);if(cardNo==14)heal=rev?3:6;if(cardNo==17)heal=rev?3:6;if(heal>0)hp[key]=Json::Int64(cur+heal);if(cardNo==12&&!rev)hp[key]=Json::Int64(std::max<std::int64_t>(1,intValue(hp[key],cur)-5));entry["text"]="採取防禦姿態"+(heal>0?"，命運使生命恢復 "+std::to_string(heal):"");}else{const auto me=characterRaw(service->database(),user.id);const auto enemy=characterRaw(service->database(),target);const auto accuracy=std::clamp<std::int64_t>(55+intValue(me["agility"])-intValue(enemy["agility"])/2,15,95);const auto roll=randomInt(1,100);bool hit=roll<=accuracy;auto damage=std::max<std::int64_t>(1,intValue(me["strength"])/2+randomInt(1,10));const auto cardNo=intValue(match["rule_card_no"],-1);const auto rev=stringValue(match["rule_orientation"])=="reversed";auto fateAccuracy=accuracy;const auto round=intValue(state["round"],1);switch(cardNo){case 0:fateAccuracy+=rev?(state.isMember("last_miss")?20:-8):12;break;case 1:damage=damage*(rev?120:115)/100;fateAccuracy+=rev?-8:0;break;case 2:fateAccuracy+=rev?-10:(intValue(state["guard_user_id"])==user.id?18:0);break;case 3:damage=damage*(rev?110:100)/100;break;case 4:damage=damage*(rev?120:110)/100;break;case 5:if(round%3==0)damage=damage*(rev?85:100)/100;break;case 6:{auto streak=intValue(state["hit_streak_"+std::to_string(user.id)]);damage=damage*(100+std::min<std::int64_t>(25,streak*5))/100;break;}case 7:damage=damage*(rev?125:115)/100;fateAccuracy+=rev?-8:0;break;case 8:if(intValue(hp[std::to_string(user.id)],100)<40)damage=damage*(rev?115:125)/100;break;case 9:fateAccuracy-=10;damage=damage*(rev?90:120)/100;break;case 10:{auto wave=randomInt(-12,12)*(rev?2:1);fateAccuracy+=wave;damage=std::max<std::int64_t>(1,damage*(100+wave)/100);break;}case 11:damage=damage*(rev?110:90)/100;fateAccuracy+=rev?-5:10;break;case 12:if(intValue(state["guard_user_id"])==user.id)damage=damage*(rev?115:140)/100;break;case 13:if(intValue(hp[std::to_string(target)],100)<30)damage=damage*(rev?100:130)/100;break;case 14:damage=damage*(rev?112:100)/100;break;case 15:damage=damage*(rev?120:135)/100;break;case 17:fateAccuracy+=rev?-5:5;break;case 18:fateAccuracy+=rev?-5:randomInt(-15,8);break;case 19:damage=damage*(rev?108:112)/100;fateAccuracy+=rev?8:12;break;case 21:if(round>=(rev?8:10)){damage=damage*(100+std::min<std::int64_t>(50,(round-(rev?7:9))*5))/100;if(rev)fateAccuracy-=5;}break;default:break;}hit=roll<=std::clamp<std::int64_t>(fateAccuracy,8,98);if(intValue(state["guard_user_id"])==target)damage=std::max<std::int64_t>(1,damage*65/100);state["guard_user_id"]=Json::Int64(0);entry["roll"]=roll;entry["accuracy"]=Json::Int64(accuracy);entry["hit"]=hit;if(hit){const auto key=std::to_string(target);const auto next=std::max<std::int64_t>(0,intValue(hp[key],100)-damage);hp[key]=Json::Int64(next);entry["damage"]=Json::Int64(damage);entry["text"]="命中，造成 "+std::to_string(damage)+" 傷害";if(next<=0){state["winner_user_id"]=Json::Int64(user.id);match["status"]="finished";}}else entry["text"]="攻擊落空";state["last_miss"]=!hit;const auto streakKey="hit_streak_"+std::to_string(user.id);state[streakKey]=Json::Int64(hit?intValue(state[streakKey])+1:0);if(cardNo==6&&rev&&!hit)hp[std::to_string(user.id)]=Json::Int64(std::max<std::int64_t>(1,intValue(hp[std::to_string(user.id)],100)-4));if(cardNo==14&&hit)hp[std::to_string(user.id)]=Json::Int64(intValue(hp[std::to_string(user.id)],100)+(rev?2:4));if(cardNo==16&&round%(rev?3:4)==0){for(const auto&v:ps){auto k=std::to_string(intValue(v));hp[k]=Json::Int64(std::max<std::int64_t>(1,intValue(hp[k],100)-(rev?5:8)));}entry["text"]=stringValue(entry["text"])+"；高塔震盪席捲雙方";}if(cardNo==17)hp[std::to_string(user.id)]=Json::Int64(intValue(hp[std::to_string(user.id)],100)+(rev?2:4));if(cardNo==20&&!state.isMember("judgement_"+std::to_string(user.id))&&intValue(hp[std::to_string(user.id)],100)<=25){hp[std::to_string(user.id)]=Json::Int64(intValue(hp[std::to_string(user.id)],100)+(rev?8:15));state["judgement_"+std::to_string(user.id)]=true;entry["text"]=stringValue(entry["text"])+"；審判使你獲得一次救贖";}}
      log.append(entry);while(log.size()>60){Json::Value cut{Json::arrayValue};for(Json::ArrayIndex i=1;i<log.size();++i)cut.append(log[i]);log=std::move(cut);}state["log"]=log;state["hp"]=hp;if(intValue(state["winner_user_id"])>0){service->database()->execSqlSync("UPDATE fate_gap_pvp_matches SET status='finished',state=$1::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=$2",compactJson(state),id);appendMiyoiMemory(roomId,user.id,"玩家在命鬥場對局 #"+std::to_string(id)+" 獲勝","fate_duel_win",1,2,0);}else{const auto next=(idx+1)%2;state["turn_index"]=Json::Int64(next);if(next==0)state["round"]=Json::Int64(intValue(state["round"],1)+1);service->database()->execSqlSync("UPDATE fate_gap_pvp_matches SET state=$1::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=$2",compactJson(state),id);}Json::Value out;out["ok"]=true;out["match"]=loadFateMatch(id);reply(cb,std::move(out));},"命鬥場行動失敗");
  });

  reg1("/api/fate-gap/pvp/{1}/surrender",drogon::Post,[service,loadFateMatch,fateParticipants,fateHasParticipant,appendMiyoiMemory](const Request& req,Callback& cb,const std::string& idText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto id=positiveId(idText);auto match=loadFateMatch(id);const auto roomId=intValue(match["room_id"]);requireRoomAccess(service,user,roomId);if(stringValue(match["status"])!="active"||!fateHasParticipant(match,user.id))throw ApiError(drogon::k409Conflict,"目前不能投降");std::int64_t winner=0;for(const auto&v:fateParticipants(match))if(intValue(v)!=user.id)winner=intValue(v);auto state=match["state"].isObject()?match["state"]:Json::Value{Json::objectValue};state["winner_user_id"]=Json::Int64(winner);state["surrender_user_id"]=Json::Int64(user.id);service->database()->execSqlSync("UPDATE fate_gap_pvp_matches SET status='finished',state=$1::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=$2",compactJson(state),id);appendMiyoiMemory(roomId,user.id,"玩家在命鬥場對局 #"+std::to_string(id)+" 主動投降","fate_duel_surrender",0,0,0);Json::Value out;out["ok"]=true;out["match"]=loadFateMatch(id);reply(cb,std::move(out));},"命鬥場投降失敗");
  });

  reg1("/api/fate-gap/miyoi/{1}",drogon::Get,[service,miyoiId](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto npcId=miyoiId();Json::Value rel;const auto rows=service->database()->execSqlSync("SELECT row_to_json(r)::text AS json_text FROM npc_relationships r WHERE room_id=$1 AND npc_template_id=$2 AND user_id=$3",roomId,npcId,user.id);if(rows.empty()){rel["affinity"]=0;rel["trust"]=0;rel["fear"]=0;rel["hostility"]=0;rel["memories"]=Json::arrayValue;}else rel=rowJson(rows);Json::Value ai;ai["affinity"]=rel["affinity"];ai["trust"]=rel["trust"];ai["fear"]=rel["fear"];ai["hostility"]=rel["hostility"];ai["curiosity"]=Json::Int64(std::clamp<std::int64_t>(static_cast<std::int64_t>(rel["memories"].size())*2+std::max<std::int64_t>(0,intValue(rel["trust"]))/4,0,100));ai["annoyance"]=Json::Int64(std::clamp<std::int64_t>(intValue(rel["hostility"])+std::max<std::int64_t>(0,-intValue(rel["affinity"]))/2,0,100));ai["willing_to_fight"]=intValue(rel["trust"])>=35&&intValue(rel["hostility"])<40;ai["willing_to_reveal_secrets"]=intValue(rel["trust"])>=60;ai["mood"]=intValue(rel["hostility"])>=60?"警戒":intValue(rel["trust"])>=60?"放鬆":intValue(rel["affinity"])>=25?"親近":"平靜";ai["memories"]=rel["memories"];Json::Value out;out["npc_template_id"]=Json::Int64(npcId);out["ai"]=ai;reply(cb,std::move(out));},"讀取彌夜 AI 狀態失敗");
  });

  reg1("/api/fate-gap/miyoi/{1}/combat-decision",drogon::Get,[service,miyoiId](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto npcId=miyoiId();Json::Value rel;const auto rr=service->database()->execSqlSync("SELECT row_to_json(r)::text AS json_text FROM npc_relationships r WHERE room_id=$1 AND npc_template_id=$2 AND user_id=$3",roomId,npcId,user.id);if(rr.empty()){rel["trust"]=0;rel["hostility"]=0;}else rel=rowJson(rr);const auto monsters=service->database()->execSqlSync("SELECT COUNT(*)::BIGINT AS n,COALESCE(SUM(CASE WHEN mt.category ILIKE '%詭%' OR mt.config::text ILIKE '%anomal%' THEN 1 ELSE 0 END),0)::BIGINT AS weird FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0",roomId);const auto n=monsters.empty()?0:monsters[0]["n"].as<std::int64_t>();const auto weird=monsters.empty()?0:monsters[0]["weird"].as<std::int64_t>();const auto trust=intValue(rel["trust"]),hostility=intValue(rel["hostility"]);const bool willing=hostility<40&&(weird>0||trust>=35)&&(n>0);Json::Value out;out["willing"]=willing;out["active_threats"]=Json::Int64(n);out["anomaly_threats"]=Json::Int64(weird);out["reason"]=willing?(weird>0?"偵測到詭異／命運異常，彌夜願意主動協戰":"信任足夠，彌夜願意接受協戰邀請"):(n<=0?"目前沒有需要她介入的戰鬥":hostility>=40?"彌夜目前對你保持警戒":"信任尚不足，且沒有命運異常威脅");reply(cb,std::move(out));},"判斷彌夜協戰意願失敗");
  });

  reg1("/api/fate-gap/miyoi/{1}/join-combat",drogon::Post,[service,miyoiId,appendMiyoiMemory](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto npcId=miyoiId();if(npcId<=0)throw ApiError(drogon::k404NotFound,"彌夜 NPC 尚未建立");const auto rr=service->database()->execSqlSync("SELECT COALESCE(trust,0) AS trust,COALESCE(hostility,0) AS hostility FROM npc_relationships WHERE room_id=$1 AND npc_template_id=$2 AND user_id=$3",roomId,npcId,user.id);const auto trust=rr.empty()?0:rr[0]["trust"].as<std::int64_t>();const auto hostility=rr.empty()?0:rr[0]["hostility"].as<std::int64_t>();const auto threats=service->database()->execSqlSync("SELECT COUNT(*)::BIGINT AS n,COALESCE(SUM(CASE WHEN mt.category ILIKE '%詭%' OR mt.config::text ILIKE '%anomal%' THEN 1 ELSE 0 END),0)::BIGINT AS weird FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0",roomId);const auto n=threats.empty()?0:threats[0]["n"].as<std::int64_t>(),weird=threats.empty()?0:threats[0]["weird"].as<std::int64_t>();if(n<=0)throw ApiError(drogon::k409Conflict,"目前沒有可協戰的敵人");if(hostility>=40||(trust<35&&weird<=0))throw ApiError(drogon::k403Forbidden,"彌夜拒絕加入這場戰鬥");Json::Value st;st["combat_joined"]=true;st["support_role"]="fate_observer";st["joined_for_user_id"]=Json::Int64(user.id);st["anomaly_priority"]=weird>0;st["skills"]=Json::arrayValue;for(const auto*x:{"命運觀測","抽牌","預兆","改命"})st["skills"].append(x);service->database()->execSqlSync("INSERT INTO room_npcs(room_id,npc_template_id,status,state) VALUES($1,$2,'active',$3::jsonb) ON CONFLICT(room_id,npc_template_id) DO UPDATE SET status='active',state=room_npcs.state||EXCLUDED.state",roomId,npcId,compactJson(st));appendMiyoiMemory(roomId,user.id,weird>0?"彌夜因詭異／命運污染主動加入玩家戰鬥":"彌夜接受玩家邀請加入戰鬥","miyoi_combat_join",2,3,0);Json::Value out;out["ok"]=true;out["joined"]=true;out["support_state"]=st;reply(cb,std::move(out));},"彌夜加入戰鬥失敗");
  });

  // [功能備註｜70-cpp.31 彌夜情境對話 AI]
  reg1("/api/fate-gap/miyoi/{1}/dialogue",drogon::Get,[service,miyoiId](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto npcId=miyoiId();
      const auto rr=service->database()->execSqlSync("SELECT COALESCE(affinity,0) AS affinity,COALESCE(trust,0) AS trust,COALESCE(hostility,0) AS hostility FROM npc_relationships WHERE room_id=$1 AND npc_template_id=$2 AND user_id=$3",roomId,npcId,user.id);
      const auto trust=rr.empty()?0:rr[0]["trust"].as<std::int64_t>(),affinity=rr.empty()?0:rr[0]["affinity"].as<std::int64_t>(),hostility=rr.empty()?0:rr[0]["hostility"].as<std::int64_t>();
      const auto tr=service->database()->execSqlSync("SELECT COUNT(*)::BIGINT AS n,COALESCE(SUM(CASE WHEN mt.category ILIKE '%逆命%' OR mt.config::text ILIKE '%anti_fate%' OR mt.config::text ILIKE '%fate_interference%' THEN 1 ELSE 0 END),0)::BIGINT AS anti FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0",roomId);
      const auto threats=tr.empty()?0:tr[0]["n"].as<std::int64_t>(),anti=tr.empty()?0:tr[0]["anti"].as<std::int64_t>();std::string line,mood="平靜";
      if(hostility>=60){mood="警戒";line="『先別靠近牌桌。』彌夜把牌收進袖口，這次倒是完全沒拿錯邊。";}
      else if(anti>0){mood="專注";line="『有東西在碰命運線。』她難得沒有發呆，停了一下又補充：『……我的茶先幫我拿走，我怕等等又打翻。』";}
      else if(threats>0&&trust>=35){mood="認真";line="『我可以跟你去。』她站起來，走了兩步又回頭把忘在桌上的塔羅牌抱走。";}
      else if(trust>=60){mood="放鬆";line="『今天的線很安靜。要聊一下嗎？』她低頭找了半天，才發現水晶球正被自己抱著。";}
      else if(affinity>=25){mood="親近";line="『你又來了。』她把牌往你這邊推了一點：『先說好，今天不准笑我把正逆位看反……上次只是桌子歪。』";}
      else {line="『要占卜嗎？』她看了看空桌面，沉默兩秒：『……牌在我手上。嗯。』";}
      Json::Value out;out["mood"]=mood;out["line"]=line;out["threats"]=Json::Int64(threats);out["anti_fate_threats"]=Json::Int64(anti);out["trust"]=Json::Int64(trust);reply(cb,std::move(out));
    },"讀取彌夜情境對話失敗");
  });

  // [功能備註｜70-cpp.31 彌夜實際協戰回合]
  // C++ 伺服器決定彌夜每回合使用哪一種支援技能；前端不能指定技能或結果。
  reg1("/api/fate-gap/miyoi/{1}/combat-act",drogon::Post,[service,miyoiId,drawTarotCard,tarotOrientation,appendMiyoiMemory](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto npcId=miyoiId();
      if(npcId<=0)throw ApiError(drogon::k404NotFound,"彌夜 NPC 尚未建立");
      const auto nr=service->database()->execSqlSync("SELECT state::text AS state_text FROM room_npcs WHERE room_id=$1 AND npc_template_id=$2 AND status='active'",roomId,npcId);
      if(nr.empty())throw ApiError(drogon::k409Conflict,"彌夜目前沒有加入戰鬥");
      auto st=parseJson(nr[0]["state_text"].as<std::string>());if(!st.get("combat_joined",false).asBool())throw ApiError(drogon::k409Conflict,"彌夜目前沒有加入戰鬥");
      const auto monsters=service->database()->execSqlSync("SELECT rm.id,rm.current_hp,rm.max_hp,mt.name,mt.category,mt.config::text AS config_text FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0 ORDER BY (CASE WHEN mt.category ILIKE '%詭%' OR mt.config::text ILIKE '%anomal%' THEN 1 ELSE 0 END) DESC,rm.current_hp DESC,rm.id LIMIT 1",roomId);
      if(monsters.empty())throw ApiError(drogon::k409Conflict,"目前沒有可協戰的敵人");
      const auto targetId=monsters[0]["id"].as<std::int64_t>();const auto targetName=monsters[0]["name"].as<std::string>();
      const auto targetCategory=monsters[0]["category"].as<std::string>();const auto targetConfig=monsters[0]["config_text"].as<std::string>();
      const bool antiFate=targetCategory.find("逆命")!=std::string::npos||targetConfig.find("anti_fate")!=std::string::npos||targetConfig.find("unobservable")!=std::string::npos||targetConfig.find("fate_interference")!=std::string::npos;
      const bool fateNoise=antiFate&&randomInt(1,100)<=45;
      const auto members=service->database()->execSqlSync("SELECT user_id,hp,max_hp FROM room_members WHERE room_id=$1 ORDER BY CASE WHEN max_hp>0 THEN hp::double precision/max_hp ELSE 1 END ASC",roomId);
      std::int64_t lowHp=999999,lowMax=1,lowUser=0;for(const auto&m:members){const auto h=m["hp"].as<std::int64_t>(),mx=std::max<std::int64_t>(1,m["max_hp"].as<std::int64_t>());if(h*100/mx<lowHp*100/lowMax){lowHp=h;lowMax=mx;lowUser=m["user_id"].as<std::int64_t>();}}
      const auto roundRows=service->database()->execSqlSync("SELECT round FROM rooms WHERE id=$1",roomId);const auto round=roundRows.empty()?1:roundRows[0]["round"].as<std::int64_t>();
      const auto lastRound=intValue(st["last_combat_act_round"],0);if(lastRound==round)throw ApiError(drogon::k409Conflict,"彌夜本回合已經行動過");
      const bool fateUsed=st.get("fate_change_used",false).asBool();std::string skill,text;Json::Value effect{Json::objectValue};
      effect["anti_fate_target"]=antiFate;effect["fate_noise"]=fateNoise;
      if(fateNoise){
        skill="命運雜訊";st["forecast"]=Json::Value(Json::objectValue);st["forecast"]["reliable"]=false;st["forecast"]["target_id"]=Json::Int64(targetId);effect["interfered"]=true;
        text="命運線突然變成一片雜訊。彌夜盯著牌看了兩秒：『……它在改牌。這個我真的沒看漏。』";
      }else if(lowUser>0&&lowHp<=0&&!fateUsed){
        skill="改命";const auto restored=std::max<std::int64_t>(1,lowMax/4);service->database()->execSqlSync("UPDATE room_members SET hp=$1,combat_state='active' WHERE room_id=$2 AND user_id=$3",restored,roomId,lowUser);st["fate_change_used"]=true;effect["restored_user_id"]=Json::Int64(lowUser);effect["hp"]=Json::Int64(restored);text="彌夜將已經墜落的命運拉回了一線。";
      }else if(lowUser>0&&lowHp*100/lowMax<=35){
        skill="抽牌";auto card=drawTarotCard("major");auto orientation=tarotOrientation();const auto cardId=intValue(card["card_no"],-1);
        std::int64_t heal=std::max<std::int64_t>(2,lowMax/10);std::string fateExtra;
        switch(cardId){
          case 0: effect["next_check_luck"]=12; fateExtra=" 愚者讓下一次行動多了一點無法計算的幸運。"; break;
          case 1: effect["skill_power_percent"]=orientation=="reversed"?12:20; fateExtra=" 魔術師把隊伍的技藝暫時推向更高的顯化。"; break;
          case 2: service->database()->execSqlSync("UPDATE room_monsters SET state=state||jsonb_build_object('miyoi_hidden_read',true,'miyoi_fate_round',$1) WHERE id=$2",round,targetId); effect["reveal_hidden"]=true; fateExtra=" 女祭司掀開了一小角原本不該被看見的資訊。"; break;
          case 3: heal=std::max<std::int64_t>(3,lowMax*(orientation=="reversed"?12:22)/100); effect["healing_boost"]=true; fateExtra=" 皇后的牌面帶來豐饒與恢復。"; break;
          case 4: effect["party_guard_percent"]=orientation=="reversed"?8:18; fateExtra=" 皇帝替隊伍立起一道短暫秩序壁壘。"; break;
          case 5: effect["faith_cost_reduction_percent"]=orientation=="reversed"?0:15; effect["faith_instability"]=orientation=="reversed"; fateExtra=" 教皇讓信仰與大道的力量產生共鳴。"; break;
          case 6: effect["cooperation_bonus_percent"]=orientation=="reversed"?5:15; fateExtra=" 戀人將彼此的行動線短暫連在一起。"; break;
          case 7: service->database()->execSqlSync("UPDATE room_monsters SET state=state||jsonb_build_object('miyoi_fate_exposed',true,'miyoi_fate_round',$1) WHERE id=$2",round,targetId); effect["initiative_bonus"]=orientation=="reversed"?10:20; fateExtra=" 戰車讓敵人的行動軌跡暴露在前進路線上。"; break;
          case 8: service->database()->execSqlSync("UPDATE room_monsters SET state=state||jsonb_build_object('miyoi_fate_exposed',true,'miyoi_fate_round',$1) WHERE id=$2",round,targetId); effect["low_hp_power_percent"]=orientation=="reversed"?12:25; fateExtra=" 力量把危境轉成了反擊的支點。"; break;
          case 9: effect["search_weakness"]=true; fateExtra=" 隱者的孤燈照出了敵人最不顯眼的弱點。"; break;
          case 10: effect["reroll_token"]=1; effect["forced"]=orientation=="reversed"; fateExtra=" 命運之輪留下了一次重新轉動結果的機會。"; break;
          case 11: effect["variance_reduction_percent"]=orientation=="reversed"?5:20; fateExtra=" 正義暫時壓低了戰場結果的偏差。"; break;
          case 12: effect["sacrifice_guard"]=true; effect["spirit_bonus_percent"]=orientation=="reversed"?10:25; fateExtra=" 倒吊人要求以一部分自由交換另一種力量。"; break;
          case 13: effect["execute_threshold_percent"]=orientation=="reversed"?0:30; effect["recovery_suppressed"]=orientation=="reversed"; fateExtra=" 死神把戰鬥推向不可逆的轉折。"; break;
          case 14: heal=std::max<std::int64_t>(2,lowMax*(orientation=="reversed"?6:14)/100); effect["resource_efficiency_percent"]=orientation=="reversed"?-10:15; fateExtra=" 節制重新調整了隊伍的消耗與恢復。"; break;
          case 15: heal=std::max<std::int64_t>(1,lowMax/14); effect["risk"]=true; effect["damage_bonus_percent"]=orientation=="reversed"?15:25; fateExtra=" 惡魔提供力量，也把代價一起放上桌面。"; break;
          case 16: {const auto chip=std::max<std::int64_t>(1,monsters[0]["max_hp"].as<std::int64_t>()/(orientation=="reversed"?25:16));service->database()->execSqlSync("UPDATE room_monsters SET current_hp=GREATEST(1,current_hp-$1),state=state||jsonb_build_object('miyoi_tower_broken',true,'miyoi_fate_round',$2) WHERE id=$3",chip,round,targetId);effect["tower_chip"]=Json::Int64(chip);fateExtra=" 高塔直接撼動了敵人的穩定規則。";break;}
          case 17: heal=std::max<std::int64_t>(3,lowMax*(orientation=="reversed"?12:22)/100); effect["luck_bonus_percent"]=orientation=="reversed"?5:15; fateExtra=" 星星讓恢復與希望重新變得清晰。"; break;
          case 18: effect["illusion_evasion_percent"]=orientation=="reversed"?5:18; effect["anomaly_sense"]=true; fateExtra=" 月亮讓隊伍能從幻象與異常中捕捉真實輪廓。"; break;
          case 19: heal=std::max<std::int64_t>(3,lowMax*(orientation=="reversed"?10:20)/100); effect["all_minor_bonus_percent"]=orientation=="reversed"?8:12; fateExtra=" 太陽把戰場照得過於清楚，卻也帶來最直接的力量。"; break;
          case 20: effect["second_chance"]=1; effect["revive_percent"]=orientation=="reversed"?8:15; fateExtra=" 審判留下了一次回應失敗命運的機會。"; break;
          case 21: effect["world_complete"]=orientation!="reversed"; effect["all_minor_bonus_percent"]=orientation=="reversed"?6:15; fateExtra=" 世界短暫把分散的命運收束成完整的一圈。"; break;
          default: fateExtra=" 牌義沒有改變規則，只留下了模糊的餘韻。"; break;
        }
        service->database()->execSqlSync("UPDATE room_members SET hp=LEAST(max_hp,hp+$1) WHERE room_id=$2 AND user_id=$3",heal,roomId,lowUser);effect["card"]=card;effect["orientation"]=orientation;effect["healed_user_id"]=Json::Int64(lowUser);effect["heal"]=Json::Int64(heal);text="彌夜抽出「"+stringValue(card["name"])+"」"+(orientation=="reversed"?"・逆位":"・正位")+"，命運暫時偏向了傷勢最重的人。"+fateExtra;
      }else if(round%3==0){
        skill="預兆";service->database()->execSqlSync("UPDATE room_monsters SET state=state||jsonb_build_object('miyoi_omen_exposed',true,'miyoi_omen_round',$1) WHERE id=$2",round,targetId);effect["target_id"]=Json::Int64(targetId);effect["exposed"]=true;text="彌夜看見了「"+targetName+"」下一個破綻；它的命運軌跡被標記。";
      }else{
        skill="命運觀測";Json::Value forecast{Json::objectValue};forecast["target_id"]=Json::Int64(targetId);forecast["target_name"]=targetName;forecast["round"]=Json::Int64(round);forecast["hint"]="下一次危險最可能來自此目標；若它具有不可觀測能力，預兆可能失真。";st["forecast"]=forecast;effect["forecast"]=forecast;text="彌夜凝視命運線，提前指出「"+targetName+"」是目前最清晰的危險來源。";
      }
      st["last_combat_act_round"]=Json::Int64(round);st["last_skill"]=skill;st["last_skill_text"]=text;
      service->database()->execSqlSync("UPDATE room_npcs SET state=$1::jsonb WHERE room_id=$2 AND npc_template_id=$3",compactJson(st),roomId,npcId);
      Json::Value payload;payload["text"]="彌夜使用【"+skill+"】："+text;payload["npc_template_id"]=Json::Int64(npcId);payload["skill"]=skill;payload["effect"]=effect;service->addEvent(roomId,user.id,"miyoi_combat",payload);
      appendMiyoiMemory(roomId,user.id,"彌夜在戰鬥中使用【"+skill+"】","miyoi_combat_action",0,0,0);
      Json::Value out;out["ok"]=true;out["skill"]=skill;out["text"]=text;out["effect"]=effect;out["support_state"]=st;out["room"]=service->roomSnapshot(roomId);reply(cb,std::move(out));
    },"彌夜協戰回合失敗");
  });


  // [功能備註｜73-cpp.34.1 彌夜 × 無牌者特殊聯動]
  // 玩家可以在逆命詭異抹去牌義後協助彌夜重新拼回塔羅；完成三次協力後解鎖一次合作終結技。
  reg1("/api/fate-gap/anti-fate/{1}/link-state",drogon::Get,[service,miyoiId](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto npcId=miyoiId();
      Json::Value out;out["active"]=false;out["boss"]=Json::Value(Json::nullValue);out["miyoi"]=Json::Value(Json::objectValue);
      const auto br=service->database()->execSqlSync("SELECT rm.id,rm.current_hp,rm.max_hp,COALESCE(rm.state,'{}'::jsonb)::text AS state_text,mt.name FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0 AND (mt.category ILIKE '%逆命%' OR mt.config::text ILIKE '%anti_fate%' OR mt.config::text ILIKE '%fate_interference%') ORDER BY rm.current_hp DESC LIMIT 1",roomId);
      const auto nr=service->database()->execSqlSync("SELECT COALESCE(state,'{}'::jsonb)::text AS state_text,status FROM room_npcs WHERE room_id=$1 AND npc_template_id=$2",roomId,npcId);
      Json::Value mst=nr.empty()?Json::Value{Json::objectValue}:parseJson(nr[0]["state_text"].as<std::string>());
      const auto roundRows=service->database()->execSqlSync("SELECT round FROM rooms WHERE id=$1",roomId);const auto round=roundRows.empty()?1:roundRows[0]["round"].as<std::int64_t>();
      out["round"]=Json::Int64(round);out["miyoi"]["joined"]=!nr.empty()&&mst.get("combat_joined",false).asBool();out["miyoi"]["blank_card_stacks"]=Json::Int64(std::max<std::int64_t>(0,intValue(mst["blank_card_stacks"])));out["miyoi"]["restoration_progress"]=Json::Int64(std::max<std::int64_t>(0,intValue(mst["restoration_progress"])));out["miyoi"]["tarot_restored"]=mst.get("tarot_restored",false).asBool();out["miyoi"]["fate_suppressed"]=mst.get("fate_effect_suppressed",false).asBool()||intValue(mst["miyoi_silenced_until_round"])>=round;out["miyoi"]["finisher_used"]=mst.get("fate_link_finisher_used",false).asBool();
      if(!br.empty()){
        const auto hp=br[0]["current_hp"].as<std::int64_t>(),mx=std::max<std::int64_t>(1,br[0]["max_hp"].as<std::int64_t>());const auto phase=hp*100/mx<=25?4:hp*100/mx<=50?3:hp*100/mx<=75?2:1;auto bst=parseJson(br[0]["state_text"].as<std::string>());
        Json::Value boss;boss["id"]=Json::Int64(br[0]["id"].as<std::int64_t>());boss["name"]=br[0]["name"].as<std::string>();boss["hp"]=Json::Int64(hp);boss["max_hp"]=Json::Int64(mx);boss["phase"]=phase;boss["phase_name"]=phase==1?"無牌之初":phase==2?"因果錯位":phase==3?"命運吞噬":"逆寫終局";boss["last_skill"]=bst["last_anti_fate_skill"];
        out["active"]=true;out["boss"]=boss;out["finisher_ready"]=out["miyoi"]["joined"].asBool()&&out["miyoi"]["tarot_restored"].asBool()&&intValue(out["miyoi"]["restoration_progress"])>=3&&phase>=3&&!out["miyoi"]["finisher_used"].asBool();
      }else out["finisher_ready"]=false;
      reply(cb,std::move(out));
    },"讀取彌夜逆命聯動狀態失敗");
  });

  reg1("/api/fate-gap/miyoi/{1}/restore-tarot",drogon::Post,[service,miyoiId,appendMiyoiMemory](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto npcId=miyoiId();
      const auto br=service->database()->execSqlSync("SELECT rm.id FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0 AND (mt.category ILIKE '%逆命%' OR mt.config::text ILIKE '%anti_fate%' OR mt.config::text ILIKE '%fate_interference%') LIMIT 1",roomId);if(br.empty())throw ApiError(drogon::k409Conflict,"目前沒有需要修復命運的逆命詭異");
      const auto nr=service->database()->execSqlSync("SELECT COALESCE(state,'{}'::jsonb)::text AS state_text FROM room_npcs WHERE room_id=$1 AND npc_template_id=$2 AND status='active'",roomId,npcId);if(nr.empty())throw ApiError(drogon::k409Conflict,"彌夜目前沒有加入戰鬥");auto st=parseJson(nr[0]["state_text"].as<std::string>());if(!st.get("combat_joined",false).asBool())throw ApiError(drogon::k409Conflict,"彌夜目前沒有加入戰鬥");
      const auto rr=service->database()->execSqlSync("SELECT round FROM rooms WHERE id=$1",roomId);const auto round=rr.empty()?1:rr[0]["round"].as<std::int64_t>();if(intValue(st["last_restore_round"])==round)throw ApiError(drogon::k409Conflict,"本回合已有人協助彌夜修復牌組");
      auto stacks=std::max<std::int64_t>(0,intValue(st["blank_card_stacks"]));auto progress=std::max<std::int64_t>(0,intValue(st["restoration_progress"]));if(stacks<=0&&st.get("tarot_restored",false).asBool())throw ApiError(drogon::k409Conflict,"彌夜的牌組目前已經恢復");
      stacks=std::max<std::int64_t>(0,stacks-1);progress=std::min<std::int64_t>(3,progress+1);const bool restored=progress>=3||stacks<=0;
      st["blank_card_stacks"]=Json::Int64(stacks);st["restoration_progress"]=Json::Int64(progress);st["last_restore_round"]=Json::Int64(round);st["last_restore_user_id"]=Json::Int64(user.id);st["tarot_restored"]=restored;
      if(restored){st["fate_effect_suppressed"]=false;st["miyoi_silenced_until_round"]=0;st["fate_noise_bonus"]=0;st["major_rule_flip"]=false;st["all_fate_unstable"]=false;}
      service->database()->execSqlSync("UPDATE room_npcs SET state=$1::jsonb WHERE room_id=$2 AND npc_template_id=$3",compactJson(st),roomId,npcId);
      Json::Value event;event["user_id"]=Json::Int64(user.id);event["progress"]=Json::Int64(progress);event["blank_card_stacks"]=Json::Int64(stacks);event["restored"]=restored;event["text"]=restored?"最後一張空白牌重新浮現圖像，彌夜的塔羅再次回應命運。":"玩家替彌夜固定了一張正在褪色的牌面，命運線恢復了一部分。";service->addEvent(roomId,user.id,"miyoi_tarot_restore",event);appendMiyoiMemory(roomId,user.id,restored?"玩家協助彌夜完整恢復了被抹去的塔羅":"玩家在逆命戰鬥中協助彌夜修復牌面","miyoi_tarot_restore",1,2,0);
      Json::Value out;out["ok"]=true;out["progress"]=Json::Int64(progress);out["blank_card_stacks"]=Json::Int64(stacks);out["restored"]=restored;out["text"]=event["text"];reply(cb,std::move(out));
    },"協助彌夜修復塔羅失敗");
  });

  reg1("/api/fate-gap/miyoi/{1}/fate-finisher",drogon::Post,[service,miyoiId,appendMiyoiMemory](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{
      const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto npcId=miyoiId();
      const auto nr=service->database()->execSqlSync("SELECT COALESCE(state,'{}'::jsonb)::text AS state_text FROM room_npcs WHERE room_id=$1 AND npc_template_id=$2 AND status='active'",roomId,npcId);if(nr.empty())throw ApiError(drogon::k409Conflict,"彌夜目前沒有加入戰鬥");auto st=parseJson(nr[0]["state_text"].as<std::string>());if(!st.get("combat_joined",false).asBool()||!st.get("tarot_restored",false).asBool()||intValue(st["restoration_progress"])<3)throw ApiError(drogon::k409Conflict,"命運線尚未完整恢復");if(st.get("fate_link_finisher_used",false).asBool())throw ApiError(drogon::k409Conflict,"本場戰鬥已經使用過合作終結技");
      const auto br=service->database()->execSqlSync("SELECT rm.id,rm.current_hp,rm.max_hp,mt.name FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0 AND (mt.category ILIKE '%逆命%' OR mt.config::text ILIKE '%anti_fate%' OR mt.config::text ILIKE '%fate_interference%') ORDER BY rm.current_hp DESC LIMIT 1",roomId);if(br.empty())throw ApiError(drogon::k409Conflict,"目前沒有逆命詭異 Boss");const auto bossId=br[0]["id"].as<std::int64_t>(),hp=br[0]["current_hp"].as<std::int64_t>(),mx=std::max<std::int64_t>(1,br[0]["max_hp"].as<std::int64_t>());const auto phase=hp*100/mx<=25?4:hp*100/mx<=50?3:hp*100/mx<=75?2:1;if(phase<3)throw ApiError(drogon::k409Conflict,"無牌者的命運核心尚未暴露，現在無法發動合作終結技");
      const auto damage=std::max<std::int64_t>(1,mx*30/100);const auto after=std::max<std::int64_t>(0,hp-damage);service->database()->execSqlSync("UPDATE room_monsters SET current_hp=$1,status=CASE WHEN $1<=0 THEN 'defeated' ELSE status END,state=COALESCE(state,'{}'::jsonb)||jsonb_build_object('fate_link_broken',true,'tarot_meaning_restored',true),updated_at=CURRENT_TIMESTAMP WHERE room_id=$2 AND id=$3",after,roomId,bossId);
      st["fate_link_finisher_used"]=true;st["fate_effect_suppressed"]=false;st["major_rule_flip"]=false;st["all_fate_unstable"]=false;st["last_skill"]="命運之外・世界重構";service->database()->execSqlSync("UPDATE room_npcs SET state=$1::jsonb WHERE room_id=$2 AND npc_template_id=$3",compactJson(st),roomId,npcId);
      Json::Value event;event["boss_id"]=Json::Int64(bossId);event["damage"]=Json::Int64(damage);event["hp_after"]=Json::Int64(after);event["defeated"]=after<=0;event["skill"]="命運之外・世界重構";event["text"]="玩家抓住被重新拼回的命運線，彌夜翻開『世界』——被抹去的牌義沿著裂縫反寫回無牌者自身。";service->addEvent(roomId,user.id,"miyoi_fate_finisher",event);appendMiyoiMemory(roomId,user.id,"玩家與彌夜共同發動【命運之外・世界重構】","miyoi_fate_finisher",3,5,0);
      Json::Value out;out["ok"]=true;out["skill"]=event["skill"];out["text"]=event["text"];out["damage"]=Json::Int64(damage);out["hp_after"]=Json::Int64(after);out["defeated"]=after<=0;out["room"]=service->roomSnapshot(roomId);reply(cb,std::move(out));
    },"彌夜合作終結技失敗");
  });


  // [功能備註｜71-cpp.32 逆命詭異 Boss：階段／技能／命運污染]
  reg1("/api/fate-gap/anti-fate/{1}/boss-state",drogon::Get,[service](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);Json::Value out;out["bosses"]=Json::arrayValue;const auto rows=service->database()->execSqlSync("SELECT row_to_json(x)::text AS json_text FROM (SELECT rm.id,rm.current_hp,rm.max_hp,rm.state,mt.name,mt.category,mt.config FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0 AND (mt.category ILIKE '%逆命%' OR mt.config::text ILIKE '%anti_fate%')) x",roomId);for(const auto&r:rows){auto b=parseJson(r["json_text"].as<std::string>());const auto hp=std::max<std::int64_t>(0,intValue(b["current_hp"])),mx=std::max<std::int64_t>(1,intValue(b["max_hp"]));const auto pct=hp*100/mx;const auto phase=pct<=25?4:pct<=50?3:pct<=75?2:1;b["phase"]=phase;b["phase_name"]=phase==1?"無牌之初":phase==2?"因果錯位":phase==3?"命運吞噬":"逆寫終局";out["bosses"].append(b);}reply(cb,std::move(out));},"讀取逆命 Boss 狀態失敗");
  });

  reg1("/api/fate-gap/anti-fate/{1}/boss-act",drogon::Post,[service](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{const auto user=requireUser(service,req);const auto roomId=positiveId(roomText);requireRoomAccess(service,user,roomId);const auto rows=service->database()->execSqlSync("SELECT rm.id,rm.current_hp,rm.max_hp,COALESCE(rm.state,'{}'::jsonb)::text AS state_text,mt.name FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 AND rm.status NOT IN ('defeated','dead') AND rm.current_hp>0 AND (mt.category ILIKE '%逆命%' OR mt.config::text ILIKE '%anti_fate%') ORDER BY rm.current_hp DESC LIMIT 1",roomId);if(rows.empty())throw ApiError(drogon::k409Conflict,"目前沒有逆命詭異 Boss");const auto bossId=rows[0]["id"].as<std::int64_t>(),hp=rows[0]["current_hp"].as<std::int64_t>(),mx=std::max<std::int64_t>(1,rows[0]["max_hp"].as<std::int64_t>());auto st=parseJson(rows[0]["state_text"].as<std::string>());const auto phase=hp*100/mx<=25?4:hp*100/mx<=50?3:hp*100/mx<=75?2:1;const auto roll=randomInt(1,100);std::string skill,text;Json::Value effect{Json::objectValue};effect["phase"]=phase;
      if(phase==1){skill=roll<=55?"空白預言":"未定之噪";effect["tarot_interference_percent"]=35;text="牌面短暫失去圖像，下一次命運觀測更容易出現雜訊。";}
      else if(phase==2){skill=roll<=50?"因果倒置":"昨日之傷";effect["initiative_disorder"]=true;effect["reroll_corruption"]=true;text="原因與結果被調換，重新判定與行動順序開始失去可靠性。";}
      else if(phase==3){skill=roll<=55?"吞食未來":"抹去牌義";effect["miyoi_silence_rounds"]=1;effect["fate_effect_suppressed"]=true;text="它吞掉一段即將發生的未來，使塔羅效果短暫失效。";}
      else {skill=roll<=50?"逆寫終局":"無牌世界";effect["major_rule_flip"]=true;effect["all_fate_unstable"]=true;text="它開始直接反寫命運，正位與逆位的界線被強行扭曲。";}
      st["anti_fate_phase"]=phase;st["last_anti_fate_skill"]=skill;st["fate_interference"]=effect;st["last_anti_fate_act_at"]=Json::Int64(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count());service->database()->execSqlSync("UPDATE room_monsters SET state=$1::jsonb WHERE id=$2",compactJson(st),bossId);Json::Value payload;payload["text"]="逆命詭異使用【"+skill+"】："+text;payload["monster_id"]=Json::Int64(bossId);payload["effect"]=effect;service->addEvent(roomId,user.id,"anti_fate_boss",payload);Json::Value out;out["ok"]=true;out["boss_id"]=Json::Int64(bossId);out["phase"]=phase;out["skill"]=skill;out["text"]=text;out["effect"]=effect;out["state"]=st;reply(cb,std::move(out));},"逆命 Boss 行動失敗");
  });
