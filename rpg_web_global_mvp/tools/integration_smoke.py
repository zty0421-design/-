...
def run(args: argparse.Namespace) -> None:
    ...
    require(status.get("version") == "53-cpp.14", "unexpected C++ version")
    ...
    # 53-cpp.14: bulk import magic / recipe / shop through the shared C++ preview -> commit pipeline.
    bulk_magic_name = "批量魔法" + secrets.token_hex(2)
    bulk_recipe_name = "批量配方" + secrets.token_hex(2)
    bulk_shop_name = "批量商品" + secrets.token_hex(2)
    bulk_cases = [
        ("magics", [{"name": bulk_magic_name, "category": "批量測試", "rank": "G", "point_cost": 1, "description": "bulk", "prerequisite_ids": [], "effects": {"element_storage_cap_bonus": {"光": 1}}, "active": True}]),
    ...
    # 53-cpp.14: player self-avatar plus DM editing another player's avatar.
    ...
    # 53-cpp.14: direct avatar upload persistence. Tiny valid 1x1 PNG.
    ...
    # 53-cpp.14: structured rule engine create -> dry-run -> real PLAYER_ACTION -> state/log.
    ...
    # 53-cpp.14: real movement hooks + block_action enforcement.
    api.request(
        "POST", "/api/admin/rule-engine/rules", expected=200, token=admin_token,
        payload={
