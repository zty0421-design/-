...
def main():
    ...
    expect(version=='60-cpp.21' and 'kVersion = "60-cpp.21"' in read('include/trpg/CoreService.h') and 'C++ 相容版 60-cpp.21' in read('public/index.html') and '"version": "60-cpp.21"' in read('docs/api_parity.json'),
           '60-cpp.21 版本資訊一致','版本資訊不同步')
    expect('v60-cpp-21-monster-combat-entity-1' in read('public/sw.js'),'Service Worker 快取版本已更新','Service Worker 仍使用舊快取')
    ...
    expect(smoke.find('def passed(name: str)') < smoke.find('passed("PWA version endpoint")') and 'status.get("version") == "60-cpp.21"' in smoke,
