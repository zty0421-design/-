...
def main():
    ...
    expect(version=='61-cpp.22' and 'kVersion = "61-cpp.22"' in read('include/trpg/CoreService.h') and 'C++ 相容版 61-cpp.22' in read('public/index.html') and '"version": "61-cpp.22"' in read('docs/api_parity.json'),
           '61-cpp.22 版本資訊一致','版本資訊不同步')
    expect('v61-cpp-22-planar-map-1' in read('public/sw.js'),'Service Worker 快取版本已更新','Service Worker 仍使用舊快取')
    ...
    expect(smoke.find('def passed(name: str)') < smoke.find('passed("PWA version endpoint")') and 'status.get("version") == "61-cpp.22"' in smoke,
