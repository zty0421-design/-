...
def main():
    required=[
        ...
        'db/legacy_v39_migrations.sql','tests/security_test.cpp','tools/integration_smoke.py','docs/VERSION.txt','docs/api_parity.json','docs/FEATURE_MAP.md','docs/RULE_ENGINE_GUIDE.md','docs/SKILL_SYSTEM_OVERVIEW.md','docs/RITUAL_SYSTEM_OVERVIEW.md'
    ]
    ...
    expect(version=='64-cpp.25.1' and 'kVersion = "64-cpp.25.1"' in read('include/trpg/CoreService.h') and 'C++ 相容版 64-cpp.25.1' in read('public/index.html') and '"version": "64-cpp.25.1"' in read('docs/api_parity.json'),
           '64-cpp.25.1 版本資訊一致','版本資訊不同步')
    expect('v64-cpp-25-1-render-compile-hotfix-1' in read('public/sw.js'),'Service Worker 快取版本已更新','Service Worker 仍使用舊快取')
    ...
    expect(smoke.find('def passed(name: str)') < smoke.find('passed("PWA version endpoint")') and 'status.get("version") == "64-cpp.25.1"' in smoke,
    ...
    expect('[前端功能備註｜60-cpp.21.3 建房／開團核心事件]' in html and
           ...
           "if(actionForm)actionForm.onsubmit=submitActionCritical;" in html and
           html.find('bindCriticalRoomWheelEvents();') < html.find('try{bindGearAffixEvents();}'),
           '60-cpp.21.4 建房／加入／開團／提交行動／轉盤優先事件綁定存在', '60-cpp.21.4 核心房間事件仍可能被其他 bind 阻斷')
    ...
    # 60-cpp.21.4 player action submission hardening.
    ...
    expect(action_start>=0 and 'ruleEngineDegraded' in action_block and
           ...
           '60-cpp.21.4 action 後端：事件核心持久化＋規則／快照降級存在', '60-cpp.21.4 action 後端穩定性保護缺失')
    expect('service->roomSnapshot(roomId)' not in action_block and
           'if (triggered.get("blocked", false).asBool())' in action_block,
           '60-cpp.21.4 action 使用 viewer 快照且保留規則阻止', 'action 仍可能回傳未過濾快照或繞過規則阻止')
    expect('[前端功能備註｜60-cpp.21.4 提交行動核心事件]' in html and
           'async function submitActionCritical(event)' in html and
           html.find('if(actionForm)actionForm.onsubmit=submitActionCritical;') < html.find('try{bindGearAffixEvents();}'),
           '60-cpp.21.4 action 前端優先綁定存在', '提交行動仍可能被後續 JS 綁定錯誤拖垮')
    expect('action.get("rule_engine_degraded") is False' in smoke and
           'PLAYER_ACTION viewer snapshot missing' in smoke,
           'Render integration_smoke 驗證 action receipt／規則／viewer snapshot', 'action 新回傳格式尚未納入 Render integration smoke')
    ...
    expect('version_info.get("version") == "64-cpp.25.1"' in smoke and
           'character tier 0 -> 1 and fixed tier-1 attribute budget' in smoke and
           'ascension ritual advances big tier and grants the new fixed attribute budget' in smoke,
