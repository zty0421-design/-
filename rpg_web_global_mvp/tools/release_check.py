...
def main():
    ...
    expect(version=='64-cpp.25' and 'kVersion = "64-cpp.25"' in read('include/trpg/CoreService.h') and 'C++ 相容版 64-cpp.25' in read('public/index.html') and '"version": "64-cpp.25"' in read('docs/api_parity.json'),
           '64-cpp.25 版本資訊一致','版本資訊不同步')
    expect('v64-cpp-25-character-tier-1' in read('public/sw.js'),'Service Worker 快取版本已更新','Service Worker 仍使用舊快取')
    ...
    expect(smoke.find('def passed(name: str)') < smoke.find('passed("PWA version endpoint")') and 'status.get("version") == "64-cpp.25"' in smoke,
    ...
    expect('version_info.get("version") == "64-cpp.25"' in smoke and
