...
def main():
    ...
    expect(version=='73-cpp.34.1' and 'kVersion = "73-cpp.34.1"' in read('include/trpg/CoreService.h') and 'C++ 相容版 73-cpp.34.1' in read('public/index.html') and '"version": "73-cpp.34.1"' in read('docs/api_parity.json'),
           '67-cpp.28 版本資訊一致','版本資訊不同步')
    expect('v73-cpp-34-1-antifate-hotfix' in read('public/sw.js'),'Service Worker 快取版本已更新','Service Worker 仍使用舊快取')
    ...
    expect('fateMajorRule' in legacy and 'case 21:' in legacy and 'case 0:' in legacy, '73-cpp.34.1 22 張大阿爾克那命鬥規則接線存在','大阿爾克那完整規則缺失')
    expect('/api/fate-gap/miyoi/{1}/combat-decision' in legacy and '/api/fate-gap/miyoi/{1}/join-combat' in legacy, '73-cpp.34.1 彌夜協戰 AI API 存在','彌夜協戰 AI API 缺失')
    expect('/api/fate-gap/miyoi/{1}/combat-act' in legacy and 'last_combat_act_round' in legacy and 'fate_change_used' in legacy, '73-cpp.34.1 彌夜四技能實際協戰回合存在','彌夜實際協戰回合缺失')
    expect('/api/fate-gap/miyoi/{1}/dialogue' in legacy and 'anti_fate' in legacy and 'fate_interference' in legacy, '73-cpp.34.1 彌夜情境對話與逆命干擾存在','彌夜情境對話／逆命干擾缺失')
    expect('case 21:' in legacy and 'card[\"card_no\"]' in legacy, '73-cpp.34.1 22 張大阿爾協戰牌號接線完整','大阿爾協戰牌號或完整效果缺失')
    expect('/api/fate-gap/anti-fate/{1}/boss-state' in legacy and '/api/fate-gap/anti-fate/{1}/boss-act' in legacy and '逆寫終局' in legacy, '73-cpp.34.1 逆命 Boss 四階段 API 存在','逆命 Boss 階段接線缺失')
    expect('命運雜訊' in legacy and 'miyoi_fate_exposed' in legacy and 'tower_chip' in legacy, '73-cpp.34.1 塔羅協戰差異效果存在','塔羅協戰差異效果缺失')
    core=read('src/CoreService.cpp')
    expect('executeAntiFateBossAiAction' in core and 'isAntiFateBoss(attacker)' in core and 'anti_fate_ai' in core, '73-cpp.34.1 逆命 Boss 已接入正式怪物 AI 回合','逆命 Boss 仍未接入正式怪物 AI')
    expect('miyoi_silenced_until_round' in core and 'major_rule_flip' in core and 'anti_fate_phase' in core, '73-cpp.34.1 四階段命運污染會寫入實際戰鬥狀態','逆命 Boss 階段污染狀態接線缺失')
    expect(all(x in legacy for x in ['/api/fate-gap/anti-fate/{1}/link-state','/api/fate-gap/miyoi/{1}/restore-tarot','/api/fate-gap/miyoi/{1}/fate-finisher']), '73-cpp.34.1 彌夜逆命聯動 API 完整','彌夜逆命聯動 API 缺失')
    expect(all(x in html for x in ['被抹去的命運','restoreMiyoiTarot','miyoiFateFinisher','命運之外・世界重構']), '73-cpp.34.1 命隙聯動 UI 完整','命隙特殊聯動 UI 缺失')
    expect(all(x in core for x in ['blank_card_stacks','tarot_restored','fate_effect_suppressed']), '73-cpp.34.1 空白牌污染接入逆命 Boss AI','空白牌污染未接入逆命 Boss AI')
