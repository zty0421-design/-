...
def main():
    ...
    expect(version=='73-cpp.34.25.18' and 'kVersion = "73-cpp.34.25.18"' in read('include/trpg/CoreService.h') and 'C++ 相容版 73-cpp.34.25.18' in read('public/index.html') and '"version": "73-cpp.34.25.18"' in read('docs/api_parity.json'),
           '67-cpp.28 版本資訊一致','版本資訊不同步')
    expect('v73-cpp-34-25-18-mobile-progressive-render' in read('public/sw.js'),'Service Worker 快取版本已更新','Service Worker 仍使用舊快取')
    legacy=read('src/LegacyCompat.cpp'); html=read('public/index.html'); mig=read('db/legacy_v39_migrations.sql')
    expect(all(x in html for x in ['loadPostLoginDataInBackground','loadCharacterBootstrap','restoreLastRoomBackground']), '73-cpp.34.25.18 登入後非必要資料背景載入完整', '登入仍可能被大量資料載入阻塞')
    expect('後續資料依優先級背景載入' in html and 'setAuthBusy(false);\n    showApp();' in html, '73-cpp.34.25.18 認證成功立即進入主畫面並解鎖操作', '登入成功後仍可能停留在鎖定畫面')
    expect('for(let attempt=0;attempt<3;attempt++)' not in html and 'for(let loginTry=0;loginTry<2;loginTry++)' in html and 'setTimeout(()=>controller.abort(),75000)' in html, '73-cpp.34.25.18 登入僅在網路／gateway 錯誤重試一次且不會 28 秒過早取消', '登入重試策略或冷啟動等待時間不正確')
    expect(all(x in html for x in ['commitContentHTML(content,html)','content.__trpgRenderedHTML===next','try{bindView();}']), '73-cpp.34.25.18 相同主內容不重建 DOM／事件綁定失敗不清空畫面', '主內容去重或事件綁定韌性缺失')
    expect("queueSafeRender('background-loader')" not in html and 'optionalLoaders=[' not in html and "loadAdminViewInBackground(state.adminView||'control')" in html, '73-cpp.34.25.18 DM 登入改為核心資料＋目前分頁按需載入', 'DM 登入仍會一次載入大量背景模組')
    expect('content-visibility:auto' in html and '#content{contain:layout style' in html and 'lastRenderedNavigationView' in html, '73-cpp.34.25.18 長頁面延遲排版／主內容 containment／導覽去重完整', '渲染層級優化缺失')
    expect(html.count('observe(document.body,{childList:true,subtree:true})') == 0 and 'function scheduleDmEnhancements()' in html, '73-cpp.34.25.18 移除全域 MutationObserver storm', '仍存在 document.body subtree observer，可能造成 Chrome 無回應')
    expect(all(x in html for x in ["state.view!=='admin'",'dmEnhanceRunning','scheduleDmEnhancements();']), '73-cpp.34.25.18 DM 增強器只在提交 DM 畫面後單次執行', 'DM 增強掃描仍可能在登入頁或非 DM 頁持續執行')
    socketjs=read('public/native-socket.js'); roomsocket=read('src/RoomSocket.cpp')
    expect(all(x in html for x in ['loadAdminViewInBackground','promiseWithTimeout','data-admin-retry-view','dmAdminViewHTML']), '73-cpp.34.25.18 DM 切頁先顯示／背景載入／單頁重試完整', 'DM 畫面仍可能被 API 等待或單頁錯誤阻塞')
    expect(all(x in html for x in ['playerViewLoader','loadPlayerViewInBackground','playerViewSafeHTML','data-player-retry-view']), '73-cpp.34.25.18 玩家畫面按需載入／單頁容錯／重試完整', '玩家畫面仍可能被單一 API 或渲染錯誤拖垮')
    expect(all(x in html for x in ['myCharacterLoadGeneration','保留上一份可用資料','scheduleElementGenerationRefresh']), '73-cpp.34.25.18 角色卡短斷線保留與元素資料低優先刷新完整', '角色卡仍可能因附加 API 或短斷線消失')
    expect(all(x in html for x in ['character-core-ready','game-room-core-ready','queuedRenderDelayTimer','hiddenRenderPending']), '73-cpp.34.25.18 手機核心先畫／背景補資料／隱藏頁停止重繪完整', '手機漸進式渲染或背景頁抑制缺失')
    expect("content.__trpgRenderedHTML=''" in html and "try{bindView();}" in html, '73-cpp.34.25.18 錯誤卡快取與 bindView 非致命保護完整', '畫面錯誤後可能無法恢復')
    expect(all(x in socketjs for x in ['client:ping','heartbeat timeout','visibilitychange','scheduleReconnect','2200']), '73-cpp.34.25.18 WebSocket 心跳／快速重連／前景恢復完整', '即時連線心跳或重連保護缺失')
    expect('server:pong' in roomsocket and 'client:ping' in roomsocket, '73-cpp.34.25.18 C++ 即時心跳回應存在', 'C++ WebSocket 心跳回應缺失')
    expect('pendingSnapshotRooms' in roomsocket and 'runAfter(0.035' in roomsocket and 'broadcastSnapshotNow' in roomsocket, '73-cpp.34.25.18 房間快照廣播具 35ms 合併', '房間快照仍可能重複密集廣播')
    expect('scheduleRealtimeSupplementRefresh' in html and "queueSafeRender('room:snapshot')" in html and 'Promise.allSettled([loadTimeView(),loadRoomWorld()])' in html, '73-cpp.34.25.18 即時快照立即渲染且附加 API 節流', '即時快照仍會被重型 HTTP 載入阻塞')
    expect(all(x in socketjs for x in ['ticketFallbackUntil','openGeneration','droppedRoomEnter']), '73-cpp.34.25.18 WebSocket 票證失敗冷卻／競態保護／舊進房佇列清理完整', 'WebSocket 重連仍可能被票證逾時或舊 room:enter 拖慢')
    expect(all(x in roomsocket for x in ['sameRoom','presenceCache','recipientsByUser']), '73-cpp.34.25.18 C++ 進房冪等／presence 去重／同 user 快照共用完整', 'C++ 即時同步仍存在重複進房或重複快照成本')
    expect('backendHealthProbePromise' in html and 'backendHealthProbeLastAt' in html and 'realtimeSnapshotSignature' in html and 'Math.imul(hash,16777619)' in html, '73-cpp.34.25.18 前端健康探測與重複快照去重完整', '前端 reconnect storm 或重複快照仍可能造成額外負載')
    ...
    expect('fateMajorRule' in legacy and 'case 21:' in legacy and 'case 0:' in legacy, '73-cpp.34.25.18 22 張大阿爾克那命鬥規則接線存在','大阿爾克那完整規則缺失')
    expect('/api/fate-gap/miyoi/{1}/combat-decision' in legacy and '/api/fate-gap/miyoi/{1}/join-combat' in legacy, '73-cpp.34.25.18 彌夜協戰 AI API 存在','彌夜協戰 AI API 缺失')
    expect('/api/fate-gap/miyoi/{1}/combat-act' in legacy and 'last_combat_act_round' in legacy and 'fate_change_used' in legacy, '73-cpp.34.25.18 彌夜四技能實際協戰回合存在','彌夜實際協戰回合缺失')
    expect('/api/fate-gap/miyoi/{1}/dialogue' in legacy and 'anti_fate' in legacy and 'fate_interference' in legacy, '73-cpp.34.25.18 彌夜情境對話與逆命干擾存在','彌夜情境對話／逆命干擾缺失')
    expect('case 21:' in legacy and 'card[\"card_no\"]' in legacy, '73-cpp.34.25.18 22 張大阿爾協戰牌號接線完整','大阿爾協戰牌號或完整效果缺失')
    expect('/api/fate-gap/anti-fate/{1}/boss-state' in legacy and '/api/fate-gap/anti-fate/{1}/boss-act' in legacy and '逆寫終局' in legacy, '73-cpp.34.25.18 逆命 Boss 四階段 API 存在','逆命 Boss 階段接線缺失')
    expect('命運雜訊' in legacy and 'miyoi_fate_exposed' in legacy and 'tower_chip' in legacy, '73-cpp.34.25.18 塔羅協戰差異效果存在','塔羅協戰差異效果缺失')
    core=read('src/CoreService.cpp')
    expect('executeAntiFateBossAiAction' in core and 'isAntiFateBoss(attacker)' in core and 'anti_fate_ai' in core, '73-cpp.34.25.18 逆命 Boss 已接入正式怪物 AI 回合','逆命 Boss 仍未接入正式怪物 AI')
    expect('miyoi_silenced_until_round' in core and 'major_rule_flip' in core and 'anti_fate_phase' in core, '73-cpp.34.25.18 四階段命運污染會寫入實際戰鬥狀態','逆命 Boss 階段污染狀態接線缺失')
    expect(all(x in legacy for x in ['/api/fate-gap/anti-fate/{1}/link-state','/api/fate-gap/miyoi/{1}/restore-tarot','/api/fate-gap/miyoi/{1}/fate-finisher']), '73-cpp.34.25.18 彌夜逆命聯動 API 完整','彌夜逆命聯動 API 缺失')
    expect(all(x in html for x in ['被抹去的命運','restoreMiyoiTarot','miyoiFateFinisher','命運之外・世界重構']), '73-cpp.34.25.18 命隙聯動 UI 完整','命隙特殊聯動 UI 缺失')
    expect(all(x in core for x in ['blank_card_stacks','tarot_restored','fate_effect_suppressed']), '73-cpp.34.25.18 空白牌污染接入逆命 Boss AI','空白牌污染未接入逆命 Boss AI')
    expect("['library','fateGap'].includes(section)" in html, '73-cpp.34.25.18 命隙分頁可正確進入','命隙分頁仍被導回遊戲間大廳')
    expect('max-width:100vw !important' in html, '73-cpp.34.25.18 全站手機版寬度 hotfix 保留','全站手機版寬度 hotfix 缺失')
    expect('viewer room snapshot degraded for room' in read('src/WorldSystems.cpp') and 'snapshot[\"degraded\"] = true' in read('src/WorldSystems.cpp'), '73-cpp.34.25.18 進房 viewer 快照具降級保護','進房 viewer 快照缺少降級保護')
    expect('GET room viewer snapshot degraded for room' in core and 'resilientRoomSnapshot(self, roomId)' in core, '73-cpp.34.25.18 GET 房間具二次降級保護','GET 房間缺少二次降級保護')
    expect('部分房間附加資料載入失敗，已保留核心房間畫面' in html, '73-cpp.34.25.18 前端附加載入失敗不阻止進房','前端進房仍可能被附加載入阻斷')
    expect(all((ROOT / p).exists() for p in ['public/assets/miyoi/miyoi_avatar.jpg','public/assets/miyoi/miyoi_fullbody.jpg','public/assets/miyoi/miyoi_dialogue.jpg','public/assets/miyoi/miyoi_battle.jpg']), '73-cpp.34.25.18 彌夜正式美術素材完整', '彌夜正式美術素材缺失')
    expect('/assets/miyoi/miyoi_fullbody.jpg' in html and '/assets/miyoi/miyoi_avatar.jpg' in html, '73-cpp.34.25.18 命隙 UI 已接入彌夜正式立繪', '命隙 UI 尚未接入彌夜正式立繪')
    expect('wheelSpinState=new Map()' in html and '__wheelDelegatedBinding' in html and '輪盤轉動中…' in html, '73-cpp.34.25.18 輪盤具立即動畫與委派點擊保護', '輪盤手機點擊／動畫保護缺失')
    expect('if(options.isString())options=parseJson(options.asString()' in legacy, '73-cpp.34.25.18 後端輪盤選項相容 JSON 字串', '後端輪盤選項格式相容保護缺失')
    expect('chat-avatar' in html and 'chat-speaker' in html and 'data-chat-mention' in html, '73-cpp.34.25.18 聊天顯示頭像名字並可 @ 交流', '聊天室頭像／名字／交流 UI 缺失')
    expect('RoomSocket::broadcastSnapshotForRoom(roomId)' in legacy, '73-cpp.34.25.18 HTTP 說話會即時同步給房內玩家', 'HTTP 互動未即時廣播房間快照')
    expect("'avatar_url'" in core and "'portrait_url'" in core and 'character_room_bindings crb' in core, '73-cpp.34.25.18 聊天事件包含房間角色頭像與角色名', '聊天事件缺少角色頭像或房間角色綁定')
    expect('runAutonomousNpcSpeech' in legacy and 'npcJudgementLine' in legacy and 'autonomous_speech_chance' in legacy, '73-cpp.34.25.18 NPC 自主發言與情境判斷核心存在', 'NPC 自主發言核心缺失')
    expect('speaker_kind' in legacy and 'npc_avatar_url' in legacy and 'ai_judgement' in legacy, '73-cpp.34.25.18 NPC 聊天事件包含身份、頭像與判斷標記', 'NPC 聊天事件資料不完整')
    expect('npcAutonomousSpeech' in html and 'npcAutonomousSpeechChance' in html and 'NPC 判斷' in html, '73-cpp.34.25.18 DM 可控制 NPC 自主發言且聊天室可識別 NPC', 'NPC 自主交流前端設定或顯示缺失')
    expect('config:{...(state.npcTemplates||[]).find' in html and 'combat_side' in html and 'autonomous_speech_chance' in html, '73-cpp.34.25.18 NPC 表單會把 AI／戰鬥擴充欄位保存進 config', 'NPC 擴充欄位仍可能被 generic CRUD 丟棄')

    expect('monsterSkillBuilderHTML' in html and 'serializeMonsterSkillBuilder' in html and 'add-monster-skill-row' in html, '73-cpp.34.25.18 怪物卡可視化技能新增器存在', '怪物卡技能仍需手動輸入格式')
    expect(all(x in html for x in ['快速建立：','data-preset=\"normal\"','data-preset=\"elite\"','data-preset=\"boss\"','monster-form-section']), '73-cpp.34.25.18 怪物卡快速模板與分區表單存在', '怪物卡簡易建立 UI 缺失')
    expect("serializeMonsterSkillBuilder('active')" in html and "serializeMonsterSkillBuilder('passive')" in html and "serializeMonsterSkillBuilder('great')" in html, '73-cpp.34.25.18 主動／被動／大道技能皆可額外新增並保存', '怪物技能新增器未完整接入保存流程')
    ...
    expect('universalDerivedStats' in html and '體質 × 2' in html, '73-cpp.34.25.18 全域衍生屬性 UI 存在', '全域衍生屬性 UI 缺失')
    expect(all(x in html for x in ['monsterUseEndurance','monsterUseWill','monsterUseSanity']), '73-cpp.34.25.18 怪物可個別取消耐力／意志／理智', '怪物特殊屬性取消開關缺失')
    expect('const auto maxHp = constitution * 2' in core and 'derived_stats' in core, '73-cpp.34.25.18 怪物生命與衍生值由 C++ 強制計算', '怪物衍生值後端規則缺失')
    expect('body[\"max_hp\"]=Json::Int64(con*2)' in legacy and '修改 NPC 失敗' in legacy, '73-cpp.34.25.18 NPC 生命與衍生值由 C++ 強制計算', 'NPC 衍生值後端規則缺失')
    expect('dmEnhanceCreationPanels' in html and 'dmApplyEasyMode' in html and 'dm-easy-mode' in html, '73-cpp.34.25.18 DM 技能／配方／裝備簡易模式存在', 'DM 建立面板簡易模式缺失')
    expect(all(x in html for x in ['data-skill-preset=\"attack\"','data-eq-preset=\"growth\"','顯示進階設定']), '73-cpp.34.25.18 快速模板與進階切換存在', 'DM 快速模板或進階切換缺失')
    expect(all(x in migrations for x in ['CREATE TABLE IF NOT EXISTS admin_content_versions','CREATE TABLE IF NOT EXISTS admin_content_drafts']), '73-cpp.34.25.18 DM 版本歷史／草稿 migration 完整', 'DM 版本歷史或草稿 migration 缺失')
    expect(all(x in legacy for x in ['/api/admin/content/bulk-update','/api/admin/content/{1}/{2}/versions','/api/admin/content/{1}/{2}/dependencies','/api/admin/content-drafts/{1}']), '73-cpp.34.25.18 C++ 批量／版本／依賴／草稿 API 完整', 'DM 內容工作流 C++ API 缺失')
    expect(all(x in html for x in ['dmBulkToolbarHTML','dmBindServerDraft','dmOpenVersions','dmOpenDependencies','dm-content-history','dm-bulk-select']), '73-cpp.34.25.18 DM 內容工作流前端完整', 'DM 批量／草稿／版本／依賴 UI 缺失')
    expect(all(x in html for x in ['dmAdminStartPanelHTML','dmOpenCommandPalette','DM_FAVORITES_KEY','dm-mobile-dock']), '73-cpp.34.25.18 DM 常用／最近／快捷指令／手機快捷列完整', 'DM 編輯器快捷工作區缺失')
    expect(all(x in html for x in ['dmMakeSortable','dm-sort-up','dm-sort-down','#recipeMaterialList']), '73-cpp.34.25.18 技能／效果／配方可排序編輯器存在', 'DM 編輯器拖曳／上下排序缺失')

    expect(all(x in html for x in ['dmContentHealthPanelHTML','dmContentHealthIssues','dm-balance-inline','同品階屬性明顯離群']), '73-cpp.34.25.18 DM 內容健康與平衡檢查器存在', 'DM 內容健康或平衡檢查器缺失')
    expect(all(x in html for x in ['dmSmartCreateCommand','dmSmartSearchCommand','建立 怪物 名稱']), '73-cpp.34.25.18 DM 智慧指令可建立／搜尋內容', 'DM 智慧指令缺失')
    expect('/api/admin/editor-dashboard' in legacy and 'COUNT(DISTINCT rm.user_id)' in legacy and 'recent_versions' in legacy and 'recent_drafts' in legacy, '73-cpp.34.25.18 C++ DM 工作儀表板 API 完整', 'DM 工作儀表板 API 缺失')
    expect(all(x in html for x in ['dmEditorDashboardHTML','refreshDmDashboard','dm-dashboard-grid','state.dmEditorDashboard']), '73-cpp.34.25.18 DM 工作儀表板 UI 完整', 'DM 工作儀表板 UI 缺失')
    expect(all(x in html for x in ['bulk-preview-table','bulkImportValidOnlyBtn','只匯入有效資料']), '73-cpp.34.25.18 批量匯入逐筆預覽／有效資料匯入完整', '批量匯入逐筆預覽或部分匯入缺失')
    expect('CREATE TABLE IF NOT EXISTS admin_effect_templates' in migrations, '73-cpp.34.25.18 共用效果模板 migration 完整', '共用效果模板 migration 缺失')
    expect(all(x in legacy for x in ['/api/admin/effect-templates','/rename-sync','/safe-delete','/combat-center','/gm-command']), '73-cpp.34.25.18 C++ 效果模板／引用保護／戰鬥中心 API 完整', '34.20 C++ API 缺失')
    expect(all(x in html for x in ['dmEffectTemplatesHTML','dmCombatCenterHTML','dmRunGmCommand','dmSafeDelete','dmReferenceRename']), '73-cpp.34.25.18 DM 戰鬥中心／GM 指令／引用安全 UI 完整', '34.20 DM UI 缺失')
    expect(all(x in html for x in ['dmBossPhaseRowHTML','dmEnhanceBossBuilder','三階段 Boss 模板']), '73-cpp.34.25.18 Boss 視覺階段編輯器完整', 'Boss 視覺階段編輯器缺失')
    expect('CREATE TABLE IF NOT EXISTS admin_drop_pools' in migrations and '/api/admin/drop-pools' in legacy, '73-cpp.34.25.18 共用掉落池 C++／migration 完整', '共用掉落池後端或 migration 缺失')
    expect(all(x in html for x in ['dmDropPoolsHTML','dm-monster-drop-pool','drop_pool_ids']), '73-cpp.34.25.18 共用掉落池 DM／怪物引用 UI 完整', '共用掉落池前端引用缺失')
    expect(all(x in html for x in ['副本內容套組','dm-dungeon-monster','dm-dungeon-boss','packageCfg']), '73-cpp.34.25.18 副本快速內容套組完整', '副本內容套組缺失')
    expect(all(x in html for x in ['dmTaskFlowHTML','任務流程圖','dm-task-node']), '73-cpp.34.25.18 任務流程視覺總覽完整', '任務流程總覽缺失')
    expect(all(x in migrations for x in ['ALTER TABLE tasks ADD COLUMN IF NOT EXISTS task_steps','ALTER TABLE task_participants ADD COLUMN IF NOT EXISTS progress','CREATE TABLE IF NOT EXISTS room_task_anchors','CREATE TABLE IF NOT EXISTS task_step_actions']), '73-cpp.34.25.18 結構化任務步驟／錨點 migration 完整', '結構化任務步驟 migration 缺失')
    expect(all(x in legacy for x in ['normalizeTaskSteps','/api/rooms/{1}/tasks/{2}/steps/{3}/progress','cooperative_seal','place_anchor',"status='sealed'",'/api/rooms/{1}/task-anchors','/api/admin/tasks/{1}/step-progress']), '73-cpp.34.25.18 C++ NPC 協力封印／錨點／DM 判定完整', '任務步驟 C++ 執行核心缺失')
    expect(all(x in html for x in ['TASK_STEP_TYPES','taskStepRowHTML','taskStepBuilder','NPC 協力封印','安插錨點','progress-task-step','admin-task-step-manage']), '73-cpp.34.25.18 任務環節編輯／玩家進度／DM 校正 UI 完整', '任務環節前端缺失')
    expect('task-anchor-badge' in html and 'roomTaskAnchors' in html and '/task-anchors' in html, '73-cpp.34.25.18 任務錨點顯示於地圖節點', '任務錨點未接入地圖')
    expect('任務步驟尚未全部完成；DM 可選擇強制完成' in legacy and 'force:true' in html, '73-cpp.34.25.18 任務完成前步驟驗證／DM 強制完成完整', '任務步驟完成保護缺失')
    expect(all(x in migrations for x in ['task_variables JSONB','task_failure_conditions JSONB']), '73-cpp.34.25.18 任務變數／失敗條件 migration 完整', '任務變數或失敗條件 migration 缺失')
    expect(all(x in legacy for x in ['taskStepActive','taskFailureReason','applyTaskStepVariableChange','/api/admin/tasks/{1}/debug/{2}','/api/admin/tasks/{1}/variables']), '73-cpp.34.25.18 C++ 任務分支／失敗／變數／除錯器完整', '任務分支／失敗／變數／除錯器 C++ 缺失')
    expect(all(x in html for x in ['taskVariables','taskFailureConditions','task-step-activation-var','task-step-change-var','admin-task-debug','openAdminTaskDebugger']), '73-cpp.34.25.18 任務分支／失敗／變數／除錯 UI 完整', '任務分支／失敗／變數／除錯 UI 缺失')

    expect(all(x in legacy for x in ['advanceStructuredTaskStepsForEvent','taskStepDependenciesComplete','required_anchor_count','BOSS_PHASE_CHANGED']), '73-cpp.34.25.18 任務事件自動推進／前置依賴 C++ 完整', '任務事件自動推進或前置依賴缺失')
    expect('advanceStructuredTaskStepsForEvent(service, roomId, actorUserId, eventType, eventContext)' in read('src/RuleEngine.cpp'), '73-cpp.34.25.18 規則引擎事件自動接入任務', '規則事件未接入任務自動推進')
    expect(all(x in html for x in ['Boss 階段','規則事件名稱','封印前需要錨點數','由遊戲事件自動判定','task-step-auto-progress']), '73-cpp.34.25.18 任務自動事件／Boss／封印前置 UI 完整', '任務自動事件或封印前置 UI 缺失')
    expect("status='success' ORDER BY completed_at DESC LIMIT 1" in legacy and 'ev["ritual_id"]=inst["ritual_id"]' in legacy, '73-cpp.34.25.18 儀式成功任務判定修正完整', '儀式成功任務判定仍有缺口')

    expect(all(x in html for x in ['prewarmAuthServer','schedulePostLoginBootstrap','authProgress','postLoginBootstrapGeneration']), '73-cpp.34.25.18 登入預熱／錯峰 bootstrap 完整', '登入預熱或錯峰載入缺失')
    expect('不再每次登入額外 INSERT ... ON CONFLICT' in core and '/api/auth/login' in core, '73-cpp.34.25.18 C++ 登入熱路徑移除角色卡寫入', '登入熱路徑仍有不必要 DB 寫入')
    expect('requestAnimationFrame(()=>setTimeout' in html and 'connectSocket();' in html, '73-cpp.34.25.18 首屏先 render 再啟動即時連線', '登入首屏與 WebSocket 啟動順序未優化')
    expect(all(x in html for x in ['function isCompactClient()','const characterLoader=async()=>','const gameRoomLoader=async()=>','playerViewCacheKey']), '73-cpp.34.25.18 手機角色／遊戲間依目前分頁按需載入', '角色或遊戲間仍會一次載入所有重型資料')
    expect(all(x in html for x in ['playerViewLoadGeneration','state.playerViewLoadGeneration[cacheKey]','ttl=isCompactClient()?30000:15000']), '73-cpp.34.25.18 玩家頁 stale request 防護與手機快取 TTL 完整', '玩家切頁仍可能被舊請求覆蓋或重複載入')
    expect('backdrop-filter:none!important' in html and 'touch-action:manipulation' in html, '73-cpp.34.25.18 手機降低昂貴視覺效果並優化觸控', '手機仍可能因 blur/觸控延遲增加渲染成本')
    expect('setTimeout(()=>controller.abort(),75000)' in html and 'for(let loginTry=0;loginTry<2;loginTry++)' in html, '73-cpp.34.25.18 登入冷啟動長等待／網路重試完整', '登入仍可能過早逾時或缺少重試')
    expect('不要在使用者按登入的同時再額外打一支 health' in html and 'Promise.race([authWarmPromise,sleep(1500)])' in html, '73-cpp.34.25.18 登入避免 health/login 冷啟動競爭', '登入仍可能與 health 預熱競爭')
