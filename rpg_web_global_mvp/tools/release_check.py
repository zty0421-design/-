...
def main():
    ...
    expect(version=='60-cpp.21.2' and 'kVersion = "60-cpp.21.2"' in read('include/trpg/CoreService.h') and 'C++ 相容版 60-cpp.21.2' in read('public/index.html') and '"version": "60-cpp.21.2"' in read('docs/api_parity.json'),
           '60-cpp.21.2 版本資訊一致','版本資訊不同步')
    expect('v60-cpp-21-2-room-wheel-hotfix-1' in read('public/sw.js'),'Service Worker 快取版本已更新','Service Worker 仍使用舊快取')
    ...
    expect(smoke.find('def passed(name: str)') < smoke.find('passed("PWA version endpoint")') and 'status.get("version") == "60-cpp.21.2"' in smoke,
    ...
    # 60-cpp.21.2 room/wheel critical regression checks.
    expect('[功能備註｜60-cpp.21.2 開團穩定性]' in core and
           "UPDATE rooms SET status='active',saved=FALSE,round=1" in core and
           'room start combat reset degraded' in core,
           '60-cpp.21.2 開團後端降級保護存在', '60-cpp.21.2 開團降級保護缺失')
