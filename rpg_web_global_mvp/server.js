...
app.post('/api/admin/item-templates',auth,adminAuth,async(req,res)=>{try{
  ...
  const row=await get(`INSERT INTO item_templates(name,category,item_type,description,effects,consume_on_use) VALUES(?,?,?,?,?::jsonb,?) ON CONFLICT(name) DO UPDATE SET category=EXCLUDED.category,item_type=EXCLUDED.item_type,description=EXCLUDED.description,effects=EXCLUDED.effects,consume_on_use=EXCLUDED.consume_on_use,updated_at=CURRENT_TIMESTAMP RETURNING *`,[name,String(req.body.category||'道具').slice(0,80),type,String(req.body.description||''),JSON.stringify(effects),req.body.consume_on_use===undefined?true:Boolean(req.body.consume_on_use)]);
...
app.post('/api/admin/rooms/:id/map-nodes',auth,adminAuth,async(req,res)=>{try{await requireRoomAdmin(req.user.id,req.params.id);const connections=Array.isArray(req.body.connections)?req.body.connections.map(Number).filter(Boolean):[];const row=await get(`INSERT INTO room_map_nodes(room_id,name,description,image_url,connections,hidden,noise_threshold) VALUES(?,?,?,?,?::jsonb,?,?) RETURNING *`,[req.params.id,String(req.body.name||'未命名區域').slice(0,120),String(req.body.description||''),String(req.body.image_url||''),JSON.stringify(connections),Boolean(req.body.hidden),clampInt(req.body.noise_threshold,0,999,30)]);await syncBidirectionalNodeConnections(Number(req.params.id),row.id,[],connections);res.json({node:{...row,connections:parseJsonField(row.connections,[])}});}catch(error){res.status(error.status||500).json({error:error.message||'建立地圖節點失敗'});}});
...
app.patch('/api/admin/rooms/:id/map-nodes/:nodeId',auth,adminAuth,async(req,res)=>{try{
  ...
  const oldConnections=mapConnectionIds(node.connections);const row=await get(`UPDATE room_map_nodes SET name=?,description=?,image_url=?,connections=?::jsonb,hidden=?,noise_threshold=?,updated_at=CURRENT_TIMESTAMP WHERE id=? RETURNING *`,[String(req.body.name??node.name).trim().slice(0,120)||node.name,String(req.body.description??node.description),String(req.body.image_url??node.image_url),JSON.stringify(connections),req.body.hidden===undefined?node.hidden:Boolean(req.body.hidden),clampInt(req.body.noise_threshold??node.noise_threshold,0,999,30),node.id]);await syncBidirectionalNodeConnections(roomId,node.id,oldConnections,connections);res.json({node:{...row,connections:mapConnectionIds(row.connections)}});
