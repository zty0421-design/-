  ...
  return source
    ...
    .map(
      function (raw, index) {
        ...
        const maxAttribute =
          ['strength','agility','constitution','spirit','luck'].includes(
            String(item.max_attribute || '').toLowerCase()
          )
            ? String(item.max_attribute).toLowerCase()
            : '';

        const maxMultiplierNumber = Number(item.max_multiplier);
        const maxMultiplier = Number.isFinite(maxMultiplierNumber)
          ? Math.max(-1000, Math.min(1000, maxMultiplierNumber))
          : 1;

        const maxFlat = Math.floor(Number(item.max_flat) || 0);
        ...
        return {
          ...
          round_mode:
            ['fixed','max_percent','attribute'].includes(item.round_mode)
              ? item.round_mode
              : 'fixed',
          round_percent: Math.max(-1000,Math.min(1000,Number(item.round_percent)||0)),
          round_attribute:
            ['strength','agility','constitution','spirit','luck'].includes(String(item.round_attribute||''))
              ? String(item.round_attribute)
              : '',
          round_multiplier: Math.max(-1000,Math.min(1000,Number(item.round_multiplier)||0)),
          round_flat: Math.floor(Number(item.round_flat)||0),
          battle_start_mode:
            ['keep','fill','empty','fixed','percent'].includes(item.battle_start_mode)
              ? item.battle_start_mode
              : (item.reset_on_battle ? 'fixed' : 'keep'),
          battle_start_value: Number(item.battle_start_value ?? item.reset_value ?? 0)||0,
          battle_end_mode:
            ['keep','fill','empty','fixed','percent'].includes(item.battle_end_mode)
              ? item.battle_end_mode
              : 'keep',
          battle_end_value: Number(item.battle_end_value)||0,
          rest_mode:
            ['keep','fill','empty','fixed','percent'].includes(item.rest_mode)
              ? item.rest_mode
              : 'keep',
          rest_value: Number(item.rest_value)||0,
          player_editable:
            Boolean(
...
async function syncCharacterClassResources(
  ...
  const calculatedCharacter = parseCharacter(character);
  const finalAttributes = calculatedCharacter.final_attributes || {};
  ...
  for (
    ...
    const formulaAttribute = String(definition.max_attribute || '');
    const formulaBase = formulaAttribute ? Number(finalAttributes[formulaAttribute]) || 0 : 0;
    const formulaMax = formulaAttribute
      ? Math.max(
          definition.min,
          Math.floor(formulaBase * (Number(definition.max_multiplier) || 0) + (Number(definition.max_flat) || 0))
        )
      : definition.max;
    const resolvedDefinition = {
      ...definition,
      max: formulaMax,
      formula_max: Boolean(formulaAttribute),
      formula_attribute_value: formulaBase
    };
    const existing =
      currentMap &&
      ...
      Math.floor(
        clampNumber(
          ...
          existing.current !== undefined
            ? existing.current
            : resolvedDefinition.initial,
          resolvedDefinition.min,
          resolvedDefinition.max
        )
      );
...
function normalizeSkillResources(
  ...
  source.class_resource = {
    ...
    amount_mode:
      ['fixed','max_percent','attribute'].includes(raw.amount_mode)
        ? raw.amount_mode
        : 'fixed',
    max_percent:
      Math.max(0, Math.min(1000, Number(raw.max_percent) || 0)),
    amount_attribute:
      ['strength','agility','constitution','spirit','luck'].includes(String(raw.amount_attribute||''))
        ? String(raw.amount_attribute)
        : '',
    attribute_multiplier:
      Math.max(-1000, Math.min(1000, Number(raw.attribute_multiplier) || 0)),
    amount_flat:
      Math.floor(Number(raw.amount_flat) || 0)
  };
...
function calculateSkillClassResourceChange(
  ...
  const calculatedCharacter = character && character.final_attributes
    ? character
    : parseCharacter(character);
  const finalAttributes = calculatedCharacter?.final_attributes || {};
  let amount = Math.max(0, Math.floor(Number(effect.amount) || 0));
  if (effect.amount_mode === 'max_percent') {
    amount = Math.max(0, Math.ceil(max * (Number(effect.max_percent) || 0) / 100));
  } else if (effect.amount_mode === 'attribute') {
    const attributeValue = Number(finalAttributes[effect.amount_attribute]) || 0;
    amount = Math.max(0, Math.floor(
      attributeValue * (Number(effect.attribute_multiplier) || 0) + (Number(effect.amount_flat) || 0)
    ));
  }
...
async function resetCharacterClassResourcesForBattle(
  ...
  return applyCharacterClassResourceTransition(userId,'battle_start');
}

function classResourceTransitionValue(resource,event){
  const min=Number(resource.min)||0;
  const max=Math.max(min,Number(resource.max)||0);
  const current=Math.floor(clampNumber(resource.current,min,max));
  const mode=String(resource[`${event}_mode`]||'keep');
  const value=Number(resource[`${event}_value`])||0;
  if(mode==='fill')return max;
  if(mode==='empty')return min;
  if(mode==='fixed')return Math.floor(clampNumber(value,min,max));
  if(mode==='percent')return Math.floor(clampNumber(min+(max-min)*(value/100),min,max));
  return current;
}

async function applyCharacterClassResourceTransition(userId,event){
  const character =
    await getCharacterByUser(
      userId
    );
  ...
  for (
    ...
    const next=classResourceTransitionValue(resource,event);
...
async function applyRoomClassResourceTransition(roomId,event){
  const members=await all(`SELECT user_id FROM room_members WHERE room_id=? AND role='player'`,[roomId]);
  for(const member of members)await applyCharacterClassResourceTransition(member.user_id,event);
}
...
async function applyCharacterClassResourceRoundDelta(
  ...
  for (
    ...
    const mode=String(resource?.round_mode||'fixed');
    const max=Math.max(Number(resource?.min)||0,Number(resource?.max)||0);
    const finalAttributes=character.final_attributes||{};
    let delta=Math.floor(Number(resource?.round_delta)||0);
    if(mode==='max_percent')delta=Math.trunc(max*(Number(resource?.round_percent)||0)/100);
    if(mode==='attribute')delta=Math.trunc(
      (Number(finalAttributes[resource?.round_attribute])||0)*(Number(resource?.round_multiplier)||0)
      +(Number(resource?.round_flat)||0)
    );
...
app.use(function auditAdminRequests(req,res,next){
  const method=String(req.method||'GET').toUpperCase();
  const requestPath=String(req.originalUrl||req.url||'').split('?')[0];
  const shouldAudit=requestPath.startsWith('/api/admin/')&&(
    ['POST','PUT','PATCH','DELETE'].includes(method)||
    (method==='GET'&&/\/backups\/(?:site|rooms\/\d+|characters\/\d+)\/export$/.test(requestPath))
  );
  if(!shouldAudit)return next();
  const startedAt=Date.now();
  res.on('finish',function(){
    const inferred=inferAdminAuditTarget(requestPath);
    const context=req.auditContext||{};
    writeAuditLog({
      actor_user_id:req.user?.id||null,
      actor_username:String(req.user?.username||''),
      method,
      action:String(context.action||normalizeAdminAuditAction(method,requestPath)),
      path:requestPath,
      target_type:String(context.target_type||inferred.target_type||''),
      target_id:String(context.target_id||inferred.target_id||''),
      room_id:context.room_id||inferred.room_id||null,
      status_code:Number(res.statusCode)||0,
      summary:String(context.summary||'').slice(0,500),
      metadata:{duration_ms:Math.max(0,Date.now()-startedAt),ok:Number(res.statusCode)<400}
  ...
  async function (
    ...
    try {
      ...
      const createBody=currentUser&&currentUser.is_admin
        ? req.body
        : {name:req.body&&req.body.name,faction:req.body&&req.body.faction,gender:req.body&&req.body.gender,age:req.body&&req.body.age,height_cm:req.body&&req.body.height_cm,birthday:req.body&&req.body.birthday};
...
async function createCharacter(
  ...
  const age=Math.max(0,Math.min(9999,Math.floor(Number(body.age)||0)));
  const heightCm=Math.max(0,Math.min(9999,Math.floor(Number(body.height_cm)||0)));
  const birthday=String(body.birthday||'').trim().slice(0,40);
...
async function endBattleIfResolved(roomId){
  ...
  await applyRoomClassResourceTransition(roomId,'battle_end');
...
app.post('/api/rooms/:id/battle/end', auth, adminAuth, async function(req,res){
  try{
    ...
    await applyRoomClassResourceTransition(req.params.id,'battle_end');
...
app.post('/api/rooms/:id/resources/rest',auth,adminAuth,async function(req,res){
  try{
    await requireRoomAdmin(req.user.id,req.params.id);
    const room=await get(`SELECT battle_active FROM rooms WHERE id=?`,[req.params.id]);
    if(room?.battle_active)return res.status(400).json({error:'戰鬥中不能進行休息恢復'});
    await applyRoomClassResourceTransition(req.params.id,'rest');
    await addEvent(req.params.id,req.user.id,'system',{text:'🛏️ DM 進行休息，已套用職業能量恢復規則。'});
    const snapshot=await roomSnapshot(req.params.id);
    io.to('room:'+req.params.id).emit('room:snapshot',snapshot);
    res.json(snapshot);
  }catch(error){res.status(error.status||500).json({error:error.message||'休息恢復失敗'});}
});
...
app.post('/api/admin/npc-templates',auth,adminAuth,async(req,res)=>{try{
  ...
  const config={
    ...
    faction_id:Math.max(0,Number(req.body.faction_id||req.body.config?.faction_id)||0)||null
  };
  const effects=effectContainerWithList(req.body.effects,normalizeUnifiedEffects(req.body.effects));
...
app.patch('/api/admin/npc-templates/:id',auth,adminAuth,async(req,res)=>{try{
  ...
  if(req.body.faction_id!==undefined)config.faction_id=Math.max(0,Number(req.body.faction_id)||0)||null;
  const rawEffects=req.body.effects===undefined?parseJsonField(cur.effects,{}):req.body.effects;
  const effects=effectContainerWithList(rawEffects,normalizeUnifiedEffects(rawEffects));
...
async function migrateCharacterSystem() {
  ...
  statements.push(`CREATE TABLE IF NOT EXISTS character_savepoints (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  ...
  statements.push(`CREATE INDEX IF NOT EXISTS idx_character_savepoints_user_created ON character_savepoints(user_id,created_at DESC)`);
  ...
  statements.push(`CREATE INDEX IF NOT EXISTS idx_audit_logs_created ON audit_logs(created_at DESC)`);
  statements.push(`CREATE INDEX IF NOT EXISTS idx_audit_logs_actor ON audit_logs(actor_user_id,created_at DESC)`);
  statements.push(`CREATE INDEX IF NOT EXISTS idx_audit_logs_action ON audit_logs(action,created_at DESC)`);
  statements.push(`CREATE INDEX IF NOT EXISTS idx_audit_logs_room ON audit_logs(room_id,created_at DESC)`);
  ...
  /* =========================
     V30: 勢力 / NPC 對話 / NPC 商店
  ========================= */
  statements.push(`CREATE TABLE IF NOT EXISTS faction_templates (
    id BIGSERIAL PRIMARY KEY,
    name VARCHAR(160) NOT NULL UNIQUE,
    description TEXT NOT NULL DEFAULT '',
  ...
  statements.push(`CREATE TABLE IF NOT EXISTS character_reputations (
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  ...
  statements.push(`CREATE TABLE IF NOT EXISTS npc_dialogue_entries (
    id BIGSERIAL PRIMARY KEY,
    npc_template_id BIGINT NOT NULL REFERENCES npc_templates(id) ON DELETE CASCADE,
  ...
  statements.push(`CREATE TABLE IF NOT EXISTS npc_dialogue_history (
    room_id BIGINT NOT NULL REFERENCES rooms(id) ON DELETE CASCADE,
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    npc_template_id BIGINT NOT NULL REFERENCES npc_templates(id) ON DELETE CASCADE,
  ...
  statements.push(`CREATE TABLE IF NOT EXISTS npc_shop_items (
    id BIGSERIAL PRIMARY KEY,
    npc_template_id BIGINT NOT NULL REFERENCES npc_templates(id) ON DELETE CASCADE,
    name VARCHAR(160) NOT NULL,
    item_type VARCHAR(30) NOT NULL DEFAULT 'item',
  ...
  statements.push(`CREATE TABLE IF NOT EXISTS npc_shop_stock (
    room_id BIGINT NOT NULL REFERENCES rooms(id) ON DELETE CASCADE,
  ...
  statements.push(`CREATE INDEX IF NOT EXISTS idx_character_reputations_user ON character_reputations(user_id,reputation DESC)`);
  statements.push(`CREATE INDEX IF NOT EXISTS idx_npc_dialogue_npc ON npc_dialogue_entries(npc_template_id,active,sort_order,id)`);
  statements.push(`CREATE INDEX IF NOT EXISTS idx_npc_shop_npc ON npc_shop_items(npc_template_id,active,id)`);
  statements.push(`CREATE INDEX IF NOT EXISTS idx_npc_dialogue_history_user ON npc_dialogue_history(room_id,user_id,npc_template_id)`);

  /* =========================
     V31: 勢力外交 / NPC 態度 / 商店折扣
  ========================= */
  statements.push(`CREATE TABLE IF NOT EXISTS faction_relations (
  ...
  statements.push(`CREATE INDEX IF NOT EXISTS idx_faction_relations_source ON faction_relations(source_faction_id,relation_score DESC)`);
  statements.push(`ALTER TABLE npc_shop_items ADD COLUMN IF NOT EXISTS discount_faction_id BIGINT REFERENCES faction_templates(id) ON DELETE SET NULL`);
  statements.push(`ALTER TABLE npc_shop_items ADD COLUMN IF NOT EXISTS discount_per_100 NUMERIC(8,3) NOT NULL DEFAULT 0`);
  statements.push(`ALTER TABLE npc_shop_items ADD COLUMN IF NOT EXISTS max_discount_percent NUMERIC(8,3) NOT NULL DEFAULT 0`);
  statements.push(`ALTER TABLE npc_shop_items ADD COLUMN IF NOT EXISTS max_hostility INTEGER NOT NULL DEFAULT 100`);

  for (const sql of statements) {
    await run(sql);
  }

  /* v35.2：將所有非 ID 的 32 位整數欄位升級為 BIGINT。
     保留主鍵、序號與外鍵型別，避免破壞資料表關聯。 */
  await run(`
    DO $$
...
async function updatePlayerCharacter(userId, body) {
  ...
  const allowed = new Set([
    'name','gender','age','height_cm','birthday','self_intro','agility','strength','constitution','spirit','great_way','faith','attribute_points','gear_loadout'
  ]);
  ...
  if(body.gender!==undefined){updates.push('gender = ?');values.push(String(body.gender||'').trim().slice(0,40));}
  if(body.age!==undefined){updates.push('age = ?');values.push(Math.max(0,Math.min(9999,Math.floor(Number(body.age)||0))));}
  if(body.height_cm!==undefined){updates.push('height_cm = ?');values.push(Math.max(0,Math.min(9999,Math.floor(Number(body.height_cm)||0))));}
  if(body.birthday!==undefined){updates.push('birthday = ?');values.push(String(body.birthday||'').trim().slice(0,40));}

  const fifthMaxField=current.faction==='西國'?'faith':'great_way';
  const fifthCurrentField=current.faction==='西國'?'current_faith':'current_great_way';
  const wrongFifthField=current.faction==='西國'?'great_way':'faith';
  if(body[wrongFifthField]!==undefined){
    throw Object.assign(new Error(`你的角色只能修改${current.faction==='西國'?'信仰':'大道'}上限`),{status:403});
  }
  if(body[fifthMaxField]!==undefined){
    const nextMax=Math.max(0,Math.floor(Number(body[fifthMaxField])||0));
    const oldMax=Math.max(0,Number(current[fifthMaxField])||0);
    const currentValue=current[fifthCurrentField]===null||current[fifthCurrentField]===undefined
      ? oldMax
      : Math.max(0,Number(current[fifthCurrentField])||0);
    if(nextMax<currentValue){
      throw Object.assign(new Error(`新上限不能低於目前剩餘值 ${currentValue}；請先由技能、效果或 DM 降低目前值`),{status:400});
    }
    updates.push(`${fifthMaxField} = ?`);values.push(nextMax);
  }
...
async function migrateTaskSystem() {
  ...
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS prerequisite_task_ids JSONB NOT NULL DEFAULT '[]'::jsonb`);
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS chain_name VARCHAR(160) NOT NULL DEFAULT ''`);
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS chain_order INTEGER NOT NULL DEFAULT 0`);
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS required_faction_id BIGINT REFERENCES faction_templates(id) ON DELETE SET NULL`);
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS min_reputation INTEGER NOT NULL DEFAULT -9999`);
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS branch_group VARCHAR(160) NOT NULL DEFAULT ''`);
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS branch_label VARCHAR(160) NOT NULL DEFAULT ''`);
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS exclusive_group VARCHAR(160) NOT NULL DEFAULT ''`);
  await run(`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS ending_label VARCHAR(200) NOT NULL DEFAULT ''`);
  ...
  await run(`CREATE INDEX IF NOT EXISTS idx_tasks_chain ON tasks(chain_name,chain_order)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_tasks_required_faction ON tasks(required_faction_id,min_reputation)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_tasks_branch_group ON tasks(branch_group,chain_order)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_tasks_exclusive_group ON tasks(exclusive_group)`);
...
function normalizeTaskRewards(value) {
  ...
  rewards.profession_grants = (Array.isArray(value.profession_grants) ? value.profession_grants : [])
    ...
    .filter(Boolean)
    .slice(0,10);
  rewards.reputation_changes = (Array.isArray(value.reputation_changes) ? value.reputation_changes : [])
    .map(raw=>{
      const faction_id=Math.max(0,Number(raw?.faction_id)||0);
      const amount=Math.max(-100000,Math.min(100000,Math.floor(Number(raw?.amount)||0)));
      return faction_id&&amount?{faction_id,amount}:null;
    })
    .filter(Boolean)
    .slice(0,20);
  return rewards;
...
function normalizeTaskPrerequisites(value){
  return [...new Set((Array.isArray(value)?value:parseJsonField(value,[])).map(Number).filter(Boolean))].slice(0,100);
}
...
function parseTask(row, myStatus) {
  ...
  return {
    ...row,
    rewards: normalizeTaskRewards(parseJsonField(row.rewards, {})),
    prerequisite_task_ids: normalizeTaskPrerequisites(row.prerequisite_task_ids),
    chain_order: Math.max(0,Math.floor(Number(row.chain_order)||0)),
    min_reputation: Math.floor(Number(row.min_reputation)||-9999),
...
async function getUserFactionReputation(userId,factionId,client=null){
  if(!factionId)return 0;
  const sql='SELECT reputation FROM character_reputations WHERE user_id=$1 AND faction_id=$2';
  if(client){const result=await client.query(sql,[userId,factionId]);return Number(result.rows[0]?.reputation)||0;}
  const row=await get(`SELECT reputation FROM character_reputations WHERE user_id=? AND faction_id=?`,[userId,factionId]);
  return Number(row?.reputation)||0;
}

async function taskChainEligibility(userId,task){
  const parsed=parseTask(task);
  if(!parsed)return {eligible:false,reason:'找不到任務'};
  const participation=await get(`SELECT status FROM task_participants WHERE task_id=? AND user_id=?`,[parsed.id,userId]);
  if(participation?.status==='locked_out')return {eligible:false,reason:'你已選擇同一分支中的其他路線，這個任務已被鎖定',locked_out:true};
  const prerequisites=normalizeTaskPrerequisites(parsed.prerequisite_task_ids);
  if(prerequisites.length){
    const donePlaceholders=prerequisites.map(()=>'?').join(',');
    const rows=await all(`SELECT task_id FROM task_participants WHERE user_id=? AND status='completed' AND task_id IN (${donePlaceholders})`,[userId,...prerequisites]);
    const done=new Set(rows.map(row=>Number(row.task_id)));
    const missing=prerequisites.filter(id=>!done.has(Number(id)));
    if(missing.length){
      const missingPlaceholders=missing.map(()=>'?').join(',');
      const names=await all(`SELECT id,title FROM tasks WHERE id IN (${missingPlaceholders}) ORDER BY id`,missing);
      return {eligible:false,reason:`前置任務尚未完成：${names.map(x=>x.title).join('、')||missing.join('、')}`,missing_task_ids:missing};
    }
  }
  if(parsed.required_faction_id){
    const rep=await getUserFactionReputation(userId,Number(parsed.required_faction_id));
    if(rep<Number(parsed.min_reputation||0)){
      const faction=await get(`SELECT name FROM faction_templates WHERE id=?`,[parsed.required_faction_id]);
      return {eligible:false,reason:`${faction?.name||'指定勢力'}聲望不足，需要 ${Number(parsed.min_reputation)||0}（目前 ${rep}）`,reputation:rep};
    }
  }
  return {eligible:true,reason:'',reputation:parsed.required_faction_id?await getUserFactionReputation(userId,Number(parsed.required_faction_id)):null};
}

async function applyReputationChangeWithClient(client,userId,factionId,amount){
  const factionResult=await client.query('SELECT id,name FROM faction_templates WHERE id=$1',[factionId]);
  const faction=factionResult.rows[0];
  if(!faction)throw Object.assign(new Error('找不到聲望獎勵指定的勢力'),{status:404});
  const result=await client.query(`INSERT INTO character_reputations(user_id,faction_id,reputation) VALUES($1,$2,$3) ON CONFLICT(user_id,faction_id) DO UPDATE SET reputation=character_reputations.reputation+EXCLUDED.reputation,updated_at=CURRENT_TIMESTAMP RETURNING reputation`,[userId,factionId,amount]);
  return {faction_id:factionId,faction_name:faction.name,amount,reputation:Number(result.rows[0]?.reputation)||0};
}
  ...
  async function (req, res) {
    try {
      ...
      const tasks=await Promise.all(rows.map(async row=>{
        ...
        return {...task,eligibility:await taskChainEligibility(req.user.id,task)};
      }));
      res.json({tasks});
    } catch (error) {
...
app.post(
  ...
  async function (req, res) {
    try {
      ...
      const eligibility=await taskChainEligibility(req.user.id,task);
      if(!eligibility.eligible){
        return res.status(403).json({error:eligibility.reason||'尚未符合任務解鎖條件'});
      }
      ...
      const rows = await all(
        `
          SELECT
            ...
            creator.username AS creator_username,
            target.username AS target_username,
            COUNT(tp.user_id)::BIGINT AS participant_count
          FROM tasks t
          JOIN users creator
            ON creator.id = t.creator_id
          LEFT JOIN users target
...
app.post(
  ...
  async function (req, res) {
    try {
      ...
      const prerequisiteTaskIds=normalizeTaskPrerequisites(req.body.prerequisite_task_ids);
      const chainName=String(req.body.chain_name||'').trim().slice(0,160);
      const chainOrder=Math.max(0,Math.floor(Number(req.body.chain_order)||0));
      const requiredFactionId=Math.max(0,Number(req.body.required_faction_id)||0)||null;
      const minReputation=Math.max(-100000,Math.min(100000,Math.floor(Number(req.body.min_reputation)||0)));
      const branchGroup=String(req.body.branch_group||'').trim().slice(0,160);
      const branchLabel=String(req.body.branch_label||'').trim().slice(0,160);
      const exclusiveGroup=String(req.body.exclusive_group||'').trim().slice(0,160);
      const endingLabel=String(req.body.ending_label||'').trim().slice(0,200);
      if(requiredFactionId){
        const faction=await get(`SELECT id FROM faction_templates WHERE id=?`,[requiredFactionId]);
        if(!faction)return res.status(404).json({error:'找不到任務需求指定的勢力'});
      }
      ...
      const result = await run(
        ...
        [
          ...
          professionId,
          professionRank,
          JSON.stringify(prerequisiteTaskIds),
          chainName,
          chainOrder,
          requiredFactionId,
          minReputation,
...
app.patch(
  ...
  async function (req, res) {
    try {
      ...
      if(req.body.prerequisite_task_ids!==undefined){updates.push('prerequisite_task_ids = ?::jsonb');values.push(JSON.stringify(normalizeTaskPrerequisites(req.body.prerequisite_task_ids)));}
      if(req.body.chain_name!==undefined){updates.push('chain_name = ?');values.push(String(req.body.chain_name||'').trim().slice(0,160));}
      if(req.body.chain_order!==undefined){updates.push('chain_order = ?');values.push(Math.max(0,Math.floor(Number(req.body.chain_order)||0)));}
      if(req.body.required_faction_id!==undefined){updates.push('required_faction_id = ?');values.push(Math.max(0,Number(req.body.required_faction_id)||0)||null);}
      if(req.body.min_reputation!==undefined){updates.push('min_reputation = ?');values.push(Math.max(-100000,Math.min(100000,Math.floor(Number(req.body.min_reputation)||0))));}
      if(req.body.branch_group!==undefined){updates.push('branch_group = ?');values.push(String(req.body.branch_group||'').trim().slice(0,160));}
      if(req.body.branch_label!==undefined){updates.push('branch_label = ?');values.push(String(req.body.branch_label||'').trim().slice(0,160));}
      if(req.body.exclusive_group!==undefined){updates.push('exclusive_group = ?');values.push(String(req.body.exclusive_group||'').trim().slice(0,160));}
      if(req.body.ending_label!==undefined){updates.push('ending_label = ?');values.push(String(req.body.ending_label||'').trim().slice(0,200));}
...
app.post(
  ...
  async function (req, res) {
    ...
    try {
      ...
      for(const change of rewards.reputation_changes||[]){
        reputationEvents.push(await applyReputationChangeWithClient(client,targetUserId,Number(change.faction_id),Number(change.amount)||0));
      }
      ...
      const exclusiveGroup=String(task.exclusive_group||'').trim();
      if(exclusiveGroup){
        await client.query(`
          INSERT INTO task_participants(task_id,user_id,status)
          SELECT id,$1,'locked_out'
          FROM tasks
          WHERE exclusive_group=$2 AND id<>$3 AND status='open'
            AND (target_user_id IS NULL OR target_user_id=$1)
          ON CONFLICT(task_id,user_id) DO UPDATE
          SET status=CASE WHEN task_participants.status='completed' THEN task_participants.status ELSE 'locked_out' END
        `,[targetUserId,exclusiveGroup,task.id]);
      }
...
app.get('/api/rooms/:id/teams',auth,async(req,res)=>{try{
  ...
  const teams=await all(`SELECT rt.*,rmn.name AS map_node_name,COUNT(rtm.user_id)::BIGINT AS member_count FROM room_teams rt LEFT JOIN room_team_members rtm ON rtm.team_id=rt.id LEFT JOIN room_map_nodes rmn ON rmn.id=rt.current_map_node_id WHERE rt.room_id=? GROUP BY rt.id,rmn.name ORDER BY rt.id`,[roomId]);
...
/* =========================================================
   V39 BACKUP CENTER / AUDIT LOG
========================================================= */

const BACKUP_SCHEMA='trpg-online-backup';
const BACKUP_FORMAT_VERSION=1;
const BACKUP_IMPORT_MAX_BYTES=8*1024*1024;
const CHARACTER_BACKUP_TABLES=[
  {key:'statuses',table:'character_statuses'},
  {key:'great_ways',table:'character_great_ways'},
  {key:'faiths',table:'character_faiths'},
  {key:'reputations',table:'character_reputations'},
  {key:'clues',table:'character_clues'},
  {key:'rules',table:'rule_discoveries'},
  {key:'item_identifications',table:'item_identifications'},
  {key:'tasks',table:'task_participants'}
];

function normalizeAdminAuditAction(method,requestPath){
  return `${method} ${String(requestPath||'').replace(/\/\d+(?=\/|$)/g,'/:id')}`.slice(0,120);
}

function inferAdminAuditTarget(requestPath){
  const pathText=String(requestPath||'');
  let match=pathText.match(/\/rooms\/(\d+)/);
  if(match)return {target_type:'room',target_id:match[1],room_id:Number(match[1])};
  match=pathText.match(/\/(?:characters|players)\/(\d+)/);
  if(match)return {target_type:'character',target_id:match[1],room_id:null};
  match=pathText.match(/\/users\/(\d+)/);
  if(match)return {target_type:'user',target_id:match[1],room_id:null};
  return {target_type:'admin',target_id:'',room_id:null};
}

async function writeAuditLog(entry){
  await run(`INSERT INTO audit_logs(actor_user_id,actor_username,method,action,path,target_type,target_id,room_id,status_code,summary,metadata) VALUES((SELECT id FROM users WHERE id=?),?,?,?,?,?,?,(SELECT id FROM rooms WHERE id=?),?,?,?::jsonb)`,[
    entry.actor_user_id||null,
    String(entry.actor_username||'').slice(0,80),
    String(entry.method||'').slice(0,10),
    String(entry.action||'').slice(0,120),
    String(entry.path||'').slice(0,1000),
    String(entry.target_type||'').slice(0,60),
    String(entry.target_id||'').slice(0,160),
    entry.room_id||null,
...
function stableBackupStringify(value){
  ...
  if(typeof value==='object')return `{${Object.keys(value).sort().map(key=>`${JSON.stringify(key)}:${stableBackupStringify(value[key])}`).join(',')}}`;
  const encoded=JSON.stringify(value);
  return encoded===undefined?'null':encoded;
}

function isSensitiveBackupKey(key){
  return /^(?:password|password_hash|passwordhash|passwd|token|access_token|refresh_token|jwt|jwt_secret|secret|database_url|authorization|api_key)$/i.test(String(key||''));
}

function containsSensitiveBackupKey(value){
  if(!value||typeof value!=='object')return false;
  if(Array.isArray(value))return value.some(containsSensitiveBackupKey);
  return Object.entries(value).some(([key,item])=>isSensitiveBackupKey(key)||containsSensitiveBackupKey(item));
}
...
function sanitizeBackupValue(value){
  ...
  if(value&&typeof value==='object'){
    ...
    for(const [key,item] of Object.entries(value)){
      if(isSensitiveBackupKey(key))continue;
...
function backupChecksum(backup){
  return crypto.createHash('sha256').update(stableBackupStringify(backupDigestPayload(backup))).digest('hex');
}

function createBackupEnvelope(scope,target,data){
  const backup={
    ...
    scope,
    target:sanitizeBackupValue(target),
    data:sanitizeBackupValue(data)
  };
...
function validateBackupEnvelope(raw){
  if(!raw||typeof raw!=='object'||Array.isArray(raw))throw Object.assign(new Error('備份檔格式不正確'),{status:400});
  const byteSize=Buffer.byteLength(JSON.stringify(raw),'utf8');
  if(byteSize>BACKUP_IMPORT_MAX_BYTES)throw Object.assign(new Error('備份檔超過 8 MB 匯入上限'),{status:413});
  if(raw.schema!==BACKUP_SCHEMA||Number(raw.format_version)!==BACKUP_FORMAT_VERSION)throw Object.assign(new Error('這不是可支援的 TRPG 備份檔'),{status:400});
  if(!['site','room','character'].includes(String(raw.scope||'')))throw Object.assign(new Error('備份範圍不正確'),{status:400});
  if(!raw.target||typeof raw.target!=='object'||!raw.data||typeof raw.data!=='object')throw Object.assign(new Error('備份內容不完整'),{status:400});
  if(containsSensitiveBackupKey(raw))throw Object.assign(new Error('備份檔含有禁止匯入的登入憑證欄位'),{status:400});
  const expected=backupChecksum(raw);
  const actual=String(raw.checksum||'').toLowerCase();
  if(!/^[a-f0-9]{64}$/.test(actual))throw Object.assign(new Error('備份檔缺少有效校驗碼'),{status:400});
  const expectedBuffer=Buffer.from(expected,'hex'),actualBuffer=Buffer.from(actual,'hex');
  if(expectedBuffer.length!==actualBuffer.length||!crypto.timingSafeEqual(expectedBuffer,actualBuffer))throw Object.assign(new Error('備份檔校驗失敗，內容可能已損壞或被修改'),{status:400});
  return raw;
}

function safeBackupTableName(table){
  if(!/^[a-z_][a-z0-9_]*$/.test(String(table||'')))throw new Error('不安全的資料表名稱');
  return `"${table}"`;
}

async function buildCharacterCheckpoint(userId){
  const user=await getUser(userId);
  if(!user)throw Object.assign(new Error('找不到玩家'),{status:404});
  const character=await get(`SELECT * FROM character_cards WHERE user_id=?`,[userId]);
  if(!character)throw Object.assign(new Error('找不到角色卡'),{status:404});
  const related={};
  for(const item of CHARACTER_BACKUP_TABLES){
    related[item.key]=await all(`SELECT * FROM ${safeBackupTableName(item.table)} WHERE user_id=? ORDER BY 1`,[userId]);
  }
  return {
    version:39,
    created_at:new Date().toISOString(),
    user:{id:user.id,username:user.username,is_admin:Boolean(user.is_admin),created_at:user.created_at},
    character,
    related
  };
}
...
async function buildSiteArchive(){
  const excluded=new Set(['room_savepoints','character_savepoints']);
  const tableRows=await all(`SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE' ORDER BY table_name`);
  const tables={};
  const counts={};
  for(const item of tableRows){
    const table=String(item.table_name||'');
    if(!table||excluded.has(table))continue;
    const rows=table==='users'
      ?await all(`SELECT id,username,is_admin,created_at FROM users ORDER BY id`)
      :await all(`SELECT * FROM ${safeBackupTableName(table)} ORDER BY 1`);
    tables[table]=sanitizeBackupValue(rows);
    counts[table]=rows.length;
  }
  return {
    mode:'export_only',
    credential_policy:'passwords, tokens and server secrets are never exported',
    restore_policy:'site archives require a database administrator and are not restorable from the web interface',
    counts,
    tables
  };
}

async function getBackupTableColumns(client,table){
  safeBackupTableName(table);
  const result=await client.query(`SELECT column_name,data_type,udt_name,is_generated FROM information_schema.columns WHERE table_schema='public' AND table_name=$1 ORDER BY ordinal_position`,[table]);
  return result.rows;
}

function backupSqlValue(column,value){
  const jsonColumn=column.data_type==='json'||column.data_type==='jsonb'||column.udt_name==='json'||column.udt_name==='jsonb';
  if(!jsonColumn)return {value,cast:''};
  const parsed=typeof value==='string'?parseJsonField(value,value):value;
  return {value:JSON.stringify(parsed===undefined?null:parsed),cast:'::jsonb'};
}

async function restoreCharacterCheckpoint(userId,snapshot,createdBy){
  if(!snapshot||Number(snapshot.character?.user_id)!==Number(userId))throw Object.assign(new Error('角色備份目標不一致'),{status:400});
  const current=await buildCharacterCheckpoint(userId);
  const safetyLabel=`匯入前自動備份 ${new Date().toISOString().slice(0,16).replace('T',' ')}`;
  await run(`INSERT INTO character_savepoints(user_id,label,snapshot,created_by) VALUES(?,?,?::jsonb,?)`,[userId,safetyLabel,JSON.stringify(current),createdBy]);
  const client=await pool.connect();
  try{
    await client.query('BEGIN');
    const cardColumns=await getBackupTableColumns(client,'character_cards');
    const cardFields=cardColumns.filter(column=>column.is_generated==='NEVER'&&!['id','user_id','created_at'].includes(column.column_name)&&Object.prototype.hasOwnProperty.call(snapshot.character,column.column_name));
    if(!cardFields.length)throw Object.assign(new Error('角色備份沒有可還原的欄位'),{status:400});
    const cardValues=[];
    const cardSets=[];
    for(const column of cardFields){
      const prepared=backupSqlValue(column,snapshot.character[column.column_name]);
      cardValues.push(prepared.value);
      cardSets.push(`${safeBackupTableName(column.column_name)}=$${cardValues.length}${prepared.cast}`);
    }
    cardValues.push(userId);
    const updated=await client.query(`UPDATE character_cards SET ${cardSets.join(',')} WHERE user_id=$${cardValues.length}` ,cardValues);
    if(updated.rowCount!==1)throw Object.assign(new Error('找不到要還原的角色卡'),{status:404});

    for(const config of CHARACTER_BACKUP_TABLES){
      await client.query(`DELETE FROM ${safeBackupTableName(config.table)} WHERE user_id=$1`,[userId]);
      const columns=await getBackupTableColumns(client,config.table);
      const rows=Array.isArray(snapshot.related?.[config.key])?snapshot.related[config.key]:[];
      for(const sourceRow of rows){
        const row={...sourceRow,user_id:userId};
        const selected=columns.filter(column=>column.is_generated==='NEVER'&&column.column_name!=='id'&&Object.prototype.hasOwnProperty.call(row,column.column_name));
        if(!selected.some(column=>column.column_name==='user_id'))throw Object.assign(new Error(`角色備份區段 ${config.key} 缺少玩家欄位`),{status:400});
        const values=[];
        const placeholders=[];
        for(const column of selected){
          const prepared=backupSqlValue(column,row[column.column_name]);
          values.push(prepared.value);
          placeholders.push(`$${values.length}${prepared.cast}`);
        }
        await client.query(`INSERT INTO ${safeBackupTableName(config.table)}(${selected.map(column=>safeBackupTableName(column.column_name)).join(',')}) VALUES(${placeholders.join(',')})`,values);
      }
    }
    await client.query('COMMIT');
  }catch(error){
...
function roomBackupCounts(checkpoint){
  const keys=['members','characters','map_nodes','teams','monsters','npcs','dungeons','summons','statuses','combat_statuses','relations','containers','scheduled','chases'];
  return Object.fromEntries(keys.map(key=>[key,Array.isArray(checkpoint?.[key])?checkpoint[key].length:0]));
}

function characterBackupCounts(snapshot){
  const counts={character:snapshot?.character?1:0};
  for(const config of CHARACTER_BACKUP_TABLES)counts[config.key]=Array.isArray(snapshot?.related?.[config.key])?snapshot.related[config.key].length:0;
  return counts;
}

async function inspectBackupForRestore(raw,actorId,issueToken=true){
  const backup=validateBackupEnvelope(raw);
  if(backup.scope==='site'){
    return {
      valid:true,
      restorable:false,
      scope:'site',
      ...
      warning:'全站備份不含密碼與伺服器密鑰，僅供下載保存與資料庫管理員手動救援；網頁不提供全站覆寫。'
    };
  }
  const targetId=Number(backup.target?.id);
  if(!Number.isSafeInteger(targetId)||targetId<=0)throw Object.assign(new Error('備份目標編號不正確'),{status:400});
  let target,counts,confirmText,warning;
  if(backup.scope==='room'){
    target=await get(`SELECT id,name,code,status FROM rooms WHERE id=?`,[targetId]);
    if(!target)throw Object.assign(new Error('目前網站找不到這個房間，房間備份只能回復既有房間'),{status:404});
    if(Number(backup.data?.checkpoint?.room?.id)!==targetId)throw Object.assign(new Error('房間備份內容與目標不一致'),{status:400});
    counts=roomBackupCounts(backup.data.checkpoint);
    confirmText=`還原房間 #${targetId}`;
    warning:'房間還原會回復既有房間中的可變狀態；已刪除的地圖、怪物或其他結構不會由此重建。執行前會自動建立安全存檔。';
  }else{
    const user=await getUser(targetId);
    const character=await get(`SELECT user_id,name,faction,level FROM character_cards WHERE user_id=?`,[targetId]);
    if(!user||!character)throw Object.assign(new Error('目前網站找不到這個玩家角色，角色備份只能回復既有角色'),{status:404});
    if(Number(backup.data?.checkpoint?.character?.user_id)!==targetId)throw Object.assign(new Error('角色備份內容與目標不一致'),{status:400});
    target={id:targetId,username:user.username,name:character.name,faction:character.faction,level:character.level};
    counts=characterBackupCounts(backup.data.checkpoint);
    confirmText=`還原角色 #${targetId}`;
    warning:'角色還原會精確替換角色卡、狀態、大道、信仰、聲望、線索、規則情報、道具鑑定與任務參與狀態；帳號密碼及房間成員資格不受影響。';
  }
  const result={valid:true,restorable:true,scope:backup.scope,target,counts,confirm_text:confirmText,warning,checksum:backup.checksum};
  if(issueToken)result.restore_token=jwt.sign({purpose:'backup_restore',scope:backup.scope,target_id:String(targetId),checksum:backup.checksum,actor_id:String(actorId)},JWT_SECRET,{expiresIn:'10m'});
  return result;
}

app.get('/api/admin/backups/site/export',auth,adminAuth,async(req,res)=>{try{
  req.auditContext={action:'backup.export.site',target_type:'site',target_id:'site',summary:'匯出不含登入憑證的全站資料封存'};
  const backup=createBackupEnvelope('site',{id:'site',name:'全站資料封存'},await buildSiteArchive());
  res.setHeader('Content-Disposition',`attachment; filename="trpg-site-${new Date().toISOString().slice(0,10)}.json"`);
  res.json(backup);
}catch(error){console.error('SITE BACKUP ERROR',error);res.status(error.status||500).json({error:error.message||'匯出全站備份失敗'});}});

app.get('/api/admin/backups/rooms/:id/export',auth,adminAuth,async(req,res)=>{try{
  const roomId=Number(req.params.id);await requireRoomAdmin(req.user.id,roomId);
  const checkpoint=await buildRoomCheckpoint(roomId);
  req.auditContext={action:'backup.export.room',target_type:'room',target_id:String(roomId),room_id:roomId,summary:`匯出房間「${checkpoint.room.name}」狀態備份`};
  const backup=createBackupEnvelope('room',{id:roomId,name:checkpoint.room.name,code:checkpoint.room.code},{mode:'existing_state_restore',checkpoint});
  res.setHeader('Content-Disposition',`attachment; filename="trpg-room-${roomId}-${new Date().toISOString().slice(0,10)}.json"`);
  res.json(backup);
}catch(error){console.error('ROOM BACKUP ERROR',error);res.status(error.status||500).json({error:error.message||'匯出房間備份失敗'});}});

app.get('/api/admin/backups/characters/:userId/export',auth,adminAuth,async(req,res)=>{try{
  const userId=Number(req.params.userId);const checkpoint=await buildCharacterCheckpoint(userId);
  req.auditContext={action:'backup.export.character',target_type:'character',target_id:String(userId),summary:`匯出角色「${checkpoint.character.name}」備份`};
  const backup=createBackupEnvelope('character',{id:userId,username:checkpoint.user.username,name:checkpoint.character.name},{mode:'exact_character_restore',checkpoint});
  res.setHeader('Content-Disposition',`attachment; filename="trpg-character-${userId}-${new Date().toISOString().slice(0,10)}.json"`);
  res.json(backup);
}catch(error){console.error('CHARACTER BACKUP ERROR',error);res.status(error.status||500).json({error:error.message||'匯出角色備份失敗'});}});

app.get('/api/admin/backups/character-savepoints',auth,adminAuth,async(req,res)=>{try{
  const limit=Math.max(10,Math.min(100,Number(req.query.limit)||30));
  const rows=await all(`SELECT cs.id,cs.user_id,cs.label,cs.created_by,cs.created_at,u.username,c.name AS character_name,creator.username AS created_by_username FROM character_savepoints cs LEFT JOIN users u ON u.id=cs.user_id LEFT JOIN character_cards c ON c.user_id=cs.user_id LEFT JOIN users creator ON creator.id=cs.created_by ORDER BY cs.id DESC LIMIT ?`,[limit]);
  res.json({savepoints:rows,limit});
}catch(error){res.status(500).json({error:'讀取角色安全存檔失敗'});}});

app.get('/api/admin/backups/character-savepoints/:id/preview',auth,adminAuth,async(req,res)=>{try{
  const row=await get(`SELECT cs.*,u.username,c.name AS character_name FROM character_savepoints cs LEFT JOIN users u ON u.id=cs.user_id LEFT JOIN character_cards c ON c.user_id=cs.user_id WHERE cs.id=?`,[req.params.id]);
  if(!row)return res.status(404).json({error:'找不到角色安全存檔'});
  const checkpoint=parseJsonField(row.snapshot,{});
  const backup=createBackupEnvelope('character',{id:Number(row.user_id),username:row.username||checkpoint.user?.username||'',name:row.character_name||checkpoint.character?.name||''},{mode:'exact_character_restore',source_savepoint_id:row.id,checkpoint});
  const preview=await inspectBackupForRestore(backup,req.user.id,true);
  res.json({backup,preview,savepoint:{id:row.id,user_id:row.user_id,label:row.label,created_at:row.created_at}});
}catch(error){res.status(error.status||500).json({error:error.message||'預覽角色安全存檔失敗'});}});

app.post('/api/admin/backups/preview',auth,adminAuth,async(req,res)=>{try{
  req.auditContext={action:'backup.preview',target_type:'backup',target_id:'',summary:'驗證備份檔與預覽還原範圍'};
  const preview=await inspectBackupForRestore(req.body?.backup,req.user.id,true);
  req.auditContext={...req.auditContext,target_type:preview.scope,target_id:String(preview.target?.id||preview.scope),room_id:preview.scope==='room'?Number(preview.target?.id)||null:null};
  res.json({preview});
}catch(error){res.status(error.status||500).json({error:error.message||'備份檔預覽失敗'});}});

app.post('/api/admin/backups/restore',auth,adminAuth,async(req,res)=>{try{
  const backup=validateBackupEnvelope(req.body?.backup);
  if(backup.scope==='site')return res.status(400).json({error:'全站備份不能從網頁覆寫，請交由資料庫管理員處理'});
  const preview=await inspectBackupForRestore(backup,req.user.id,false);
  let approval;
  try{approval=jwt.verify(String(req.body?.restore_token||''),JWT_SECRET);}catch{return res.status(400).json({error:'還原許可已失效，請重新預覽備份檔'});}
  if(approval.purpose!=='backup_restore'||String(approval.actor_id)!==String(req.user.id)||approval.scope!==backup.scope||String(approval.target_id)!==String(preview.target.id)||approval.checksum!==backup.checksum)return res.status(400).json({error:'還原許可與目前備份檔不一致，請重新預覽'});
  if(String(req.body?.confirm_text||'')!==preview.confirm_text)return res.status(400).json({error:`請完整輸入確認文字：${preview.confirm_text}`});
  const targetId=Number(preview.target.id);
  req.auditContext={action:`backup.restore.${backup.scope}`,target_type:backup.scope,target_id:String(targetId),room_id:backup.scope==='room'?targetId:null,summary:`從校驗碼 ${backup.checksum.slice(0,12)}… 還原${backup.scope==='room'?'房間':'角色'} #${targetId}`};
  if(backup.scope==='room'){
    await requireRoomAdmin(req.user.id,targetId);
    const safety=await buildRoomCheckpoint(targetId);
    await run(`INSERT INTO room_savepoints(room_id,label,snapshot,created_by) VALUES(?,?,?::jsonb,?)`,[targetId,`匯入前自動備份 ${new Date().toISOString().slice(0,16).replace('T',' ')}`,JSON.stringify(safety),req.user.id]);
    await restoreRoomCheckpoint(targetId,backup.data.checkpoint);
    await addEvent(targetId,req.user.id,'restore',{text:'⏪ DM 從外部備份檔還原房間狀態。',backup_checksum:backup.checksum.slice(0,16)});
    const snapshot=await roomSnapshot(targetId);io.to('room:'+targetId).emit('room:snapshot',snapshot);emitTeamUpdate(targetId,null);
    return res.json({ok:true,message:'房間狀態已還原；還原前狀態已自動保存。',room:snapshot});
  }
  const character=await restoreCharacterCheckpoint(targetId,backup.data.checkpoint,req.user.id);
  res.json({ok:true,message:'角色資料已還原；還原前狀態已自動保存。',character});
}catch(error){console.error('BACKUP RESTORE ERROR',error);res.status(error.status||500).json({error:error.message||'還原備份失敗'});}});

app.get('/api/admin/audit-logs',auth,adminAuth,async(req,res)=>{try{
  ...
  if(action){clauses.push(`al.action ILIKE ?`);values.push(`%${action}%`);}
  const roomId=Number(req.query.room_id);if(Number.isSafeInteger(roomId)&&roomId>0){clauses.push(`al.room_id=?`);values.push(roomId);}
  const actorId=Number(req.query.actor_user_id);if(Number.isSafeInteger(actorId)&&actorId>0){clauses.push(`al.actor_user_id=?`);values.push(actorId);}
  const statusCode=Number(req.query.status_code);if(Number.isInteger(statusCode)&&statusCode>=100&&statusCode<=599){clauses.push(`al.status_code=?`);values.push(statusCode);}
  values.push(limit);
  const rows=await all(`SELECT al.*,COALESCE(u.username,al.actor_username,'') AS actor_display FROM audit_logs al LEFT JOIN users u ON u.id=al.actor_user_id ${clauses.length?`WHERE ${clauses.join(' AND ')}`:''} ORDER BY al.id DESC LIMIT ?`,values);
  res.json({logs:rows.map(row=>({...row,metadata:parseJsonField(row.metadata,{})})),limit});
}catch(error){res.status(500).json({error:'讀取管理操作紀錄失敗'});}});
...
/* =========================================================
   V30: 勢力 / 聲望 / NPC 對話商店 / 任務鏈
========================================================= */
function normalizeRelationDelta(value){
  const raw=value&&typeof value==='object'&&!Array.isArray(value)?value:{};
  return {
    affinity:Math.max(-100,Math.min(100,Math.floor(Number(raw.affinity)||0))),
    trust:Math.max(-100,Math.min(100,Math.floor(Number(raw.trust)||0))),
    fear:Math.max(-100,Math.min(100,Math.floor(Number(raw.fear)||0))),
    hostility:Math.max(-100,Math.min(100,Math.floor(Number(raw.hostility)||0)))
  };
}
async function v30Relationship(roomId,npcId,userId){
  const row=await get(`SELECT * FROM npc_relationships WHERE room_id=? AND npc_template_id=? AND user_id=?`,[roomId,npcId,userId]);
  return row?{...row,memories:parseJsonField(row.memories,[])}:{room_id:roomId,npc_template_id:npcId,user_id:userId,affinity:0,trust:0,fear:0,hostility:0,memories:[]};
}
async function v30NpcInstanceForUser(roomId,npcId,userId,isAdmin=false){
  const row=await get(`SELECT rn.map_node_id,rn.status AS room_status,nt.* FROM room_npcs rn JOIN npc_templates nt ON nt.id=rn.npc_template_id WHERE rn.room_id=? AND rn.npc_template_id=?`,[roomId,npcId]);
  if(!row)throw Object.assign(new Error('找不到房間 NPC'),{status:404});
  if(!isAdmin){
    await requireRoomMember(userId,roomId);
    const viewerNodeId=await getViewerMapNodeId(userId,roomId);
    if(row.map_node_id&&String(row.map_node_id)!==String(viewerNodeId||''))throw Object.assign(new Error('NPC 不在你目前的位置'),{status:403});
  }
  return row;
}
async function v30InventoryHas(userId,itemName){
  if(!itemName)return true;
  const character=await getCharacterByUser(userId);if(!character)return false;
  return inventoryQuantity(normalizeInventory(character.inventory),itemName)>0;
}
async function v30TaskState(userId,taskId){
  if(!taskId)return null;
  return get(`SELECT status FROM task_participants WHERE task_id=? AND user_id=?`,[taskId,userId]);
}
async function v30DialogueCheck(roomId,userId,npc,entry){
  const rel=await v30Relationship(roomId,npc.id,userId);
  if(Number(rel.affinity)<Number(entry.min_affinity??-100))return {ok:false,reason:'好感不足'};
  if(Number(rel.trust)<Number(entry.min_trust??-100))return {ok:false,reason:'信任不足'};
  if(Number(rel.hostility)>Number(entry.max_hostility??100))return {ok:false,reason:'NPC 敵意過高'};
  const factionId=Number(entry.required_faction_id||0)||Number(parseWorldConfig(npc.config).faction_id||0);
  if(Number(entry.required_faction_id||0)){
    const rep=await getUserFactionReputation(userId,factionId);
    if(rep<Number(entry.min_reputation||0))return {ok:false,reason:'勢力聲望不足'};
  }
  if(entry.required_item&&!(await v30InventoryHas(userId,entry.required_item)))return {ok:false,reason:`需要道具：${entry.required_item}`};
  if(entry.required_task_id){const st=await v30TaskState(userId,entry.required_task_id);if(st?.status!=='completed')return {ok:false,reason:'前置任務尚未完成'};}
  if(entry.once_per_user){const used=await get(`SELECT 1 AS ok FROM npc_dialogue_history WHERE room_id=? AND user_id=? AND dialogue_entry_id=?`,[roomId,userId,entry.id]);if(used)return {ok:false,reason:'這段對話已經使用過'};}
  return {ok:true,reason:'',relationship:rel,faction_id:factionId};
}
async function getFactionRelationScore(sourceFactionId,targetFactionId){
  if(!sourceFactionId||!targetFactionId||String(sourceFactionId)===String(targetFactionId))return 0;
  const direct=await get(`SELECT relation_score FROM faction_relations WHERE source_faction_id=? AND target_faction_id=?`,[sourceFactionId,targetFactionId]);
  if(direct)return Number(direct.relation_score)||0;
  const reverse=await get(`SELECT relation_score FROM faction_relations WHERE source_faction_id=? AND target_faction_id=?`,[targetFactionId,sourceFactionId]);
  return Number(reverse?.relation_score)||0;
}
async function strongestPlayerFaction(userId){
  return get(`SELECT cr.faction_id,cr.reputation,ft.name FROM character_reputations cr JOIN faction_templates ft ON ft.id=cr.faction_id WHERE cr.user_id=? AND ft.active=TRUE ORDER BY ABS(cr.reputation) DESC,cr.reputation DESC LIMIT 1`,[userId]);
}
async function v31NpcAttitude(roomId,userId,npc,relationship=null){
  const rel=relationship||await v30Relationship(roomId,npc.id,userId);
  const npcFactionId=Number(parseWorldConfig(npc.config).faction_id||0)||null;
  const npcRep=npcFactionId?await getUserFactionReputation(userId,npcFactionId):0;
  const playerFaction=await strongestPlayerFaction(userId);
  const diplomatic=npcFactionId&&playerFaction?.faction_id?await getFactionRelationScore(npcFactionId,Number(playerFaction.faction_id)):0;
  const score=(Number(rel.affinity)||0)*0.42+(Number(rel.trust)||0)*0.30-(Number(rel.hostility)||0)*0.62-(Number(rel.fear)||0)*0.12+Math.max(-30,Math.min(30,npcRep/10))+Math.max(-20,Math.min(20,diplomatic/5));
  const key=score>=35?'friendly':score>=15?'warm':score>-15?'neutral':score>-35?'wary':'hostile';
  const labels={friendly:'友善',warm:'親近',neutral:'中立',wary:'戒備',hostile:'敵對'};
  return {key,label:labels[key],score:Math.round(score),npc_faction_reputation:npcRep,diplomatic_score:diplomatic,player_faction:playerFaction||null};
}
async function v31ShopPrice(userId,item){
  const base=Math.max(0,Math.floor(Number(item.price)||0));
  const factionId=Number(item.discount_faction_id||0)||null;
  const per100=Math.max(0,Number(item.discount_per_100)||0);
  const cap=Math.max(0,Math.min(95,Number(item.max_discount_percent)||0));
  const reputation=factionId?await getUserFactionReputation(userId,factionId):0;
  const positive=Math.max(0,reputation);
  const discount=Math.max(0,Math.min(cap,(positive/100)*per100));
  return {base_price:base,effective_price:Math.max(0,Math.ceil(base*(1-discount/100))),discount_percent:Math.round(discount*100)/100,discount_reputation:reputation,discount_faction_id:factionId};
}
async function v30ShopCheck(roomId,userId,npc,item){
  const rel=await v30Relationship(roomId,npc.id,userId);
  if(Number(rel.affinity)<Number(item.min_affinity??-100))return {ok:false,reason:`需要好感 ${Number(item.min_affinity??-100)}`,relationship:rel};
  if(Number(rel.hostility)>Number(item.max_hostility??100))return {ok:false,reason:`NPC 對你的敵意過高（${Number(rel.hostility)||0}/${Number(item.max_hostility??100)}）`,relationship:rel};
  if(item.required_faction_id){const rep=await getUserFactionReputation(userId,Number(item.required_faction_id));if(rep<Number(item.min_reputation||0))return {ok:false,reason:`需要聲望 ${Number(item.min_reputation)||0}`,relationship:rel};}
  const attitude=await v31NpcAttitude(roomId,userId,npc,rel);
  if(attitude.key==='hostile')return {ok:false,reason:'NPC 目前對你抱持敵對態度，拒絕交易',relationship:rel,attitude};
  return {ok:true,reason:'',relationship:rel,attitude};
}

app.get('/api/factions',auth,async(req,res)=>{try{
  const [factions,reps,relations]=await Promise.all([all(`SELECT * FROM faction_templates WHERE active=TRUE ORDER BY name,id`),all(`SELECT cr.*,ft.name,ft.description,ft.alignment FROM character_reputations cr JOIN faction_templates ft ON ft.id=cr.faction_id WHERE cr.user_id=? ORDER BY cr.reputation DESC,ft.name`,[req.user.id]),all(`SELECT fr.*,a.name AS source_name,b.name AS target_name FROM faction_relations fr JOIN faction_templates a ON a.id=fr.source_faction_id JOIN faction_templates b ON b.id=fr.target_faction_id WHERE a.active=TRUE AND b.active=TRUE ORDER BY a.name,b.name`)]);
  res.json({factions,reputations:reps,relations});
}catch(error){res.status(500).json({error:'讀取勢力／聲望失敗'});}});
app.get('/api/admin/factions',auth,adminAuth,async(req,res)=>{try{const [factions,reputations,relations]=await Promise.all([all(`SELECT * FROM faction_templates ORDER BY id`),all(`SELECT cr.*,ft.name,ft.description,ft.alignment FROM character_reputations cr JOIN faction_templates ft ON ft.id=cr.faction_id WHERE cr.user_id=? ORDER BY cr.reputation DESC,ft.name`,[req.user.id]),all(`SELECT fr.*,a.name AS source_name,b.name AS target_name FROM faction_relations fr JOIN faction_templates a ON a.id=fr.source_faction_id JOIN faction_templates b ON b.id=fr.target_faction_id ORDER BY a.name,b.name`)]);res.json({factions,reputations,relations});}catch(error){res.status(500).json({error:'讀取勢力失敗'});}});
app.put('/api/admin/faction-relations',auth,adminAuth,async(req,res)=>{try{const source=Number(req.body.source_faction_id),target=Number(req.body.target_faction_id);if(!source||!target||source===target)return res.status(400).json({error:'請選擇兩個不同勢力'});const score=Math.max(-100,Math.min(100,Math.floor(Number(req.body.relation_score)||0)));const row=await get(`INSERT INTO faction_relations(source_faction_id,target_faction_id,relation_score,label,notes) VALUES(?,?,?,?,?) ON CONFLICT(source_faction_id,target_faction_id) DO UPDATE SET relation_score=EXCLUDED.relation_score,label=EXCLUDED.label,notes=EXCLUDED.notes,updated_at=CURRENT_TIMESTAMP RETURNING *`,[source,target,score,String(req.body.label||'').trim().slice(0,80),String(req.body.notes||'').slice(0,4000)]);res.json({ok:true,relation:row});}catch(error){res.status(500).json({error:'儲存勢力關係失敗'});}});
app.delete('/api/admin/faction-relations/:source/:target',auth,adminAuth,async(req,res)=>{try{await run(`DELETE FROM faction_relations WHERE source_faction_id=? AND target_faction_id=?`,[req.params.source,req.params.target]);res.json({ok:true});}catch(error){res.status(500).json({error:'刪除勢力關係失敗'});}});
app.post('/api/admin/factions',auth,adminAuth,async(req,res)=>{try{const name=String(req.body.name||'').trim().slice(0,160);if(!name)return res.status(400).json({error:'請輸入勢力名稱'});const alignment=['friendly','neutral','hostile','mixed'].includes(String(req.body.alignment))?String(req.body.alignment):'neutral';const row=await get(`INSERT INTO faction_templates(name,description,alignment,active) VALUES(?,?,?,?) RETURNING *`,[name,String(req.body.description||'').slice(0,10000),alignment,req.body.active===undefined?true:Boolean(req.body.active)]);res.json({ok:true,faction:row});}catch(error){res.status(error.code==='23505'?409:500).json({error:error.code==='23505'?'勢力名稱已存在':'建立勢力失敗'});}});
app.patch('/api/admin/factions/:id',auth,adminAuth,async(req,res)=>{try{const current=await get(`SELECT * FROM faction_templates WHERE id=?`,[req.params.id]);if(!current)return res.status(404).json({error:'找不到勢力'});const alignment=['friendly','neutral','hostile','mixed'].includes(String(req.body.alignment))?String(req.body.alignment):current.alignment;const row=await get(`UPDATE faction_templates SET name=?,description=?,alignment=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? RETURNING *`,[String(req.body.name??current.name).trim().slice(0,160),String(req.body.description??current.description).slice(0,10000),alignment,req.body.active===undefined?current.active:Boolean(req.body.active),req.params.id]);res.json({ok:true,faction:row});}catch(error){res.status(error.code==='23505'?409:500).json({error:error.code==='23505'?'勢力名稱已存在':'修改勢力失敗'});}});
app.delete('/api/admin/factions/:id',auth,adminAuth,async(req,res)=>{try{await run(`DELETE FROM faction_templates WHERE id=?`,[req.params.id]);res.json({ok:true});}catch(error){res.status(500).json({error:'刪除勢力失敗'});}});
app.patch('/api/admin/players/:userId/reputations/:factionId',auth,adminAuth,async(req,res)=>{try{const userId=Number(req.params.userId),factionId=Number(req.params.factionId);if(!await getUser(userId))return res.status(404).json({error:'找不到玩家'});const faction=await get(`SELECT * FROM faction_templates WHERE id=?`,[factionId]);if(!faction)return res.status(404).json({error:'找不到勢力'});const mode=req.body.mode==='delta'?'delta':'set';const value=Math.max(-100000,Math.min(100000,Math.floor(Number(req.body.value)||0)));const row=mode==='delta'?await get(`INSERT INTO character_reputations(user_id,faction_id,reputation) VALUES(?,?,?) ON CONFLICT(user_id,faction_id) DO UPDATE SET reputation=character_reputations.reputation+EXCLUDED.reputation,updated_at=CURRENT_TIMESTAMP RETURNING *`,[userId,factionId,value]):await get(`INSERT INTO character_reputations(user_id,faction_id,reputation) VALUES(?,?,?) ON CONFLICT(user_id,faction_id) DO UPDATE SET reputation=EXCLUDED.reputation,updated_at=CURRENT_TIMESTAMP RETURNING *`,[userId,factionId,value]);res.json({ok:true,reputation:{...row,faction_name:faction.name}});}catch(error){res.status(500).json({error:'修改聲望失敗'});}});

app.get('/api/admin/npc-social',auth,adminAuth,async(req,res)=>{try{
  const [dialogues,shop]=await Promise.all([
    all(`SELECT nde.*,nt.name AS npc_name,ft.name AS faction_name FROM npc_dialogue_entries nde JOIN npc_templates nt ON nt.id=nde.npc_template_id LEFT JOIN faction_templates ft ON ft.id=nde.required_faction_id ORDER BY nt.name,nde.sort_order,nde.id`),
    all(`SELECT nsi.*,nt.name AS npc_name,ft.name AS faction_name,df.name AS discount_faction_name FROM npc_shop_items nsi JOIN npc_templates nt ON nt.id=nsi.npc_template_id LEFT JOIN faction_templates ft ON ft.id=nsi.required_faction_id LEFT JOIN faction_templates df ON df.id=nsi.discount_faction_id ORDER BY nt.name,nsi.id`)
  ]);
  res.json({dialogues:dialogues.map(x=>({...x,relation_delta:normalizeRelationDelta(parseJsonField(x.relation_delta,{}))})),shop_items:shop});
}catch(error){res.status(500).json({error:'讀取 NPC 互動資料失敗'});}});
app.post('/api/admin/npc-dialogues',auth,adminAuth,async(req,res)=>{try{const npcId=Number(req.body.npc_template_id);if(!npcId||!await get(`SELECT id FROM npc_templates WHERE id=?`,[npcId]))return res.status(404).json({error:'找不到 NPC'});const choice=String(req.body.choice_text||'').trim().slice(0,300);if(!choice)return res.status(400).json({error:'請輸入玩家對話選項'});const row=await get(`INSERT INTO npc_dialogue_entries(npc_template_id,choice_text,npc_text,required_faction_id,min_reputation,min_affinity,min_trust,max_hostility,required_item,required_task_id,grant_task_id,relation_delta,once_per_user,sort_order,active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?::jsonb,?,?,?) RETURNING *`,[npcId,choice,String(req.body.npc_text||'').slice(0,12000),Math.max(0,Number(req.body.required_faction_id)||0)||null,Math.max(-100000,Math.min(100000,Math.floor(Number(req.body.min_reputation)||0))),Math.max(-100,Math.min(100,Math.floor(Number(req.body.min_affinity)||0))),Math.max(-100,Math.min(100,Math.floor(Number(req.body.min_trust)||0))),Math.max(0,Math.min(100,Math.floor(Number(req.body.max_hostility??100)||100))),String(req.body.required_item||'').trim().slice(0,120),Math.max(0,Number(req.body.required_task_id)||0)||null,Math.max(0,Number(req.body.grant_task_id)||0)||null,JSON.stringify(normalizeRelationDelta(req.body.relation_delta)),Boolean(req.body.once_per_user),Math.floor(Number(req.body.sort_order)||0),req.body.active===undefined?true:Boolean(req.body.active)]);res.json({ok:true,dialogue:{...row,relation_delta:normalizeRelationDelta(row.relation_delta)}});}catch(error){res.status(500).json({error:'建立 NPC 對話失敗'});}});
app.patch('/api/admin/npc-dialogues/:id',auth,adminAuth,async(req,res)=>{try{const cur=await get(`SELECT * FROM npc_dialogue_entries WHERE id=?`,[req.params.id]);if(!cur)return res.status(404).json({error:'找不到對話'});const row=await get(`UPDATE npc_dialogue_entries SET choice_text=?,npc_text=?,required_faction_id=?,min_reputation=?,min_affinity=?,min_trust=?,max_hostility=?,required_item=?,required_task_id=?,grant_task_id=?,relation_delta=?::jsonb,once_per_user=?,sort_order=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? RETURNING *`,[String(req.body.choice_text??cur.choice_text).trim().slice(0,300),String(req.body.npc_text??cur.npc_text).slice(0,12000),req.body.required_faction_id===undefined?cur.required_faction_id:(Math.max(0,Number(req.body.required_faction_id)||0)||null),Math.floor(Number(req.body.min_reputation??cur.min_reputation)||0),Math.floor(Number(req.body.min_affinity??cur.min_affinity)||0),Math.floor(Number(req.body.min_trust??cur.min_trust)||0),Math.floor(Number(req.body.max_hostility??cur.max_hostility)||100),String(req.body.required_item??cur.required_item).trim().slice(0,120),req.body.required_task_id===undefined?cur.required_task_id:(Math.max(0,Number(req.body.required_task_id)||0)||null),req.body.grant_task_id===undefined?cur.grant_task_id:(Math.max(0,Number(req.body.grant_task_id)||0)||null),JSON.stringify(normalizeRelationDelta(req.body.relation_delta===undefined?parseJsonField(cur.relation_delta,{}):req.body.relation_delta)),req.body.once_per_user===undefined?cur.once_per_user:Boolean(req.body.once_per_user),Math.floor(Number(req.body.sort_order??cur.sort_order)||0),req.body.active===undefined?cur.active:Boolean(req.body.active),req.params.id]);res.json({ok:true,dialogue:{...row,relation_delta:normalizeRelationDelta(row.relation_delta)}});}catch(error){res.status(500).json({error:'修改 NPC 對話失敗'});}});
app.delete('/api/admin/npc-dialogues/:id',auth,adminAuth,async(req,res)=>{try{await run(`DELETE FROM npc_dialogue_entries WHERE id=?`,[req.params.id]);res.json({ok:true});}catch(error){res.status(500).json({error:'刪除 NPC 對話失敗'});}});

app.post('/api/admin/npc-shop-items',auth,adminAuth,async(req,res)=>{try{const npcId=Number(req.body.npc_template_id);if(!npcId||!await get(`SELECT id FROM npc_templates WHERE id=?`,[npcId]))return res.status(404).json({error:'找不到 NPC'});const name=String(req.body.name||'').trim().slice(0,160);if(!name)return res.status(400).json({error:'請輸入商品名稱'});const itemType=['item','equipment','weapon'].includes(String(req.body.item_type))?String(req.body.item_type):'item';const currency=req.body.currency==='game'?'game':'weird';const row=await get(`INSERT INTO npc_shop_items(npc_template_id,name,item_type,item_category,description,currency,price,quantity,stock,required_faction_id,min_reputation,min_affinity,discount_faction_id,discount_per_100,max_discount_percent,max_hostility,active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *`,[npcId,name,itemType,String(req.body.item_category||'').slice(0,80),String(req.body.description||'').slice(0,8000),currency,Math.max(0,Math.floor(Number(req.body.price)||0)),Math.max(1,Math.floor(Number(req.body.quantity)||1)),Math.max(-1,Math.floor(Number(req.body.stock??-1)||-1)),Math.max(0,Number(req.body.required_faction_id)||0)||null,Math.max(-100000,Math.min(100000,Math.floor(Number(req.body.min_reputation)||0))),Math.max(-100,Math.min(100,Math.floor(Number(req.body.min_affinity)||0))),Math.max(0,Number(req.body.discount_faction_id)||0)||null,Math.max(0,Number(req.body.discount_per_100)||0),Math.max(0,Math.min(95,Number(req.body.max_discount_percent)||0)),Math.max(0,Math.min(100,Math.floor(Number(req.body.max_hostility??100)||100))),req.body.active===undefined?true:Boolean(req.body.active)]);res.json({ok:true,item:row});}catch(error){res.status(500).json({error:'建立 NPC 商品失敗'});}});
app.patch('/api/admin/npc-shop-items/:id',auth,adminAuth,async(req,res)=>{try{const cur=await get(`SELECT * FROM npc_shop_items WHERE id=?`,[req.params.id]);if(!cur)return res.status(404).json({error:'找不到商品'});const itemType=req.body.item_type===undefined?cur.item_type:(['item','equipment','weapon'].includes(String(req.body.item_type))?String(req.body.item_type):'item');const currency=req.body.currency===undefined?cur.currency:(req.body.currency==='game'?'game':'weird');const row=await get(`UPDATE npc_shop_items SET name=?,item_type=?,item_category=?,description=?,currency=?,price=?,quantity=?,stock=?,required_faction_id=?,min_reputation=?,min_affinity=?,discount_faction_id=?,discount_per_100=?,max_discount_percent=?,max_hostility=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? RETURNING *`,[String(req.body.name??cur.name).trim().slice(0,160),itemType,String(req.body.item_category??cur.item_category).slice(0,80),String(req.body.description??cur.description).slice(0,8000),currency,Math.max(0,Math.floor(Number(req.body.price??cur.price)||0)),Math.max(1,Math.floor(Number(req.body.quantity??cur.quantity)||1)),Math.max(-1,Math.floor(Number(req.body.stock??cur.stock)||-1)),req.body.required_faction_id===undefined?cur.required_faction_id:(Math.max(0,Number(req.body.required_faction_id)||0)||null),Math.floor(Number(req.body.min_reputation??cur.min_reputation)||0),Math.floor(Number(req.body.min_affinity??cur.min_affinity)||0),req.body.discount_faction_id===undefined?cur.discount_faction_id:(Math.max(0,Number(req.body.discount_faction_id)||0)||null),Math.max(0,Number(req.body.discount_per_100??cur.discount_per_100)||0),Math.max(0,Math.min(95,Number(req.body.max_discount_percent??cur.max_discount_percent)||0)),Math.max(0,Math.min(100,Math.floor(Number(req.body.max_hostility??cur.max_hostility)||100))),req.body.active===undefined?cur.active:Boolean(req.body.active),req.params.id]);res.json({ok:true,item:row});}catch(error){res.status(500).json({error:'修改 NPC 商品失敗'});}});
app.delete('/api/admin/npc-shop-items/:id',auth,adminAuth,async(req,res)=>{try{await run(`DELETE FROM npc_shop_items WHERE id=?`,[req.params.id]);res.json({ok:true});}catch(error){res.status(500).json({error:'刪除 NPC 商品失敗'});}});

app.get('/api/rooms/:id/npcs/:npcId/interact',auth,async(req,res)=>{try{
  const roomId=Number(req.params.id),npcId=Number(req.params.npcId);const user=await getUser(req.user.id);const isAdmin=Boolean(user?.is_admin);const npc=await v30NpcInstanceForUser(roomId,npcId,req.user.id,isAdmin);const relation=await v30Relationship(roomId,npcId,req.user.id);const factionId=Number(parseWorldConfig(npc.config).faction_id||0)||null;const faction=factionId?await get(`SELECT * FROM faction_templates WHERE id=?`,[factionId]):null;const reputation=factionId?await getUserFactionReputation(req.user.id,factionId):0;const attitude=await v31NpcAttitude(roomId,req.user.id,npc,relation);
  const dialogueRows=await all(`SELECT * FROM npc_dialogue_entries WHERE npc_template_id=? AND active=TRUE ORDER BY sort_order,id`,[npcId]);const dialogues=[];for(const entry of dialogueRows){const check=await v30DialogueCheck(roomId,req.user.id,npc,entry);if(check.ok)dialogues.push({...entry,relation_delta:normalizeRelationDelta(parseJsonField(entry.relation_delta,{}))});}
  const shopRows=await all(`SELECT * FROM npc_shop_items WHERE npc_template_id=? AND active=TRUE ORDER BY id`,[npcId]);const shop_items=[];for(const item of shopRows){const check=await v30ShopCheck(roomId,req.user.id,npc,item);const pricing=await v31ShopPrice(req.user.id,item);let remaining=Number(item.stock);if(Number(item.stock)>=0){const sr=await get(`SELECT remaining FROM npc_shop_stock WHERE room_id=? AND shop_item_id=?`,[roomId,item.id]);remaining=sr?Number(sr.remaining):Number(item.stock);}shop_items.push({...item,...pricing,can_buy:check.ok&&remaining!==0,lock_reason:check.ok?(remaining===0?'售罄':''):check.reason,remaining});}
  res.json({npc:{id:npc.id,name:npc.name,role_name:npc.role_name,description:npc.description,faction_id:factionId},faction,reputation,relationship:relation,attitude,dialogues,shop_items});
}catch(error){res.status(error.status||500).json({error:error.message||'讀取 NPC 互動失敗'});}});

app.post('/api/rooms/:id/npcs/:npcId/dialogue/:entryId',auth,async(req,res)=>{try{
  const roomId=Number(req.params.id),npcId=Number(req.params.npcId),entryId=Number(req.params.entryId);const user=await getUser(req.user.id);const isAdmin=Boolean(user?.is_admin);const npc=await v30NpcInstanceForUser(roomId,npcId,req.user.id,isAdmin);const entry=await get(`SELECT * FROM npc_dialogue_entries WHERE id=? AND npc_template_id=? AND active=TRUE`,[entryId,npcId]);if(!entry)return res.status(404).json({error:'找不到可用對話'});const check=await v30DialogueCheck(roomId,req.user.id,npc,entry);if(!check.ok)return res.status(403).json({error:check.reason});
  const relation=check.relationship||await v30Relationship(roomId,npcId,req.user.id);const delta=normalizeRelationDelta(parseJsonField(entry.relation_delta,{}));const memories=parseJsonField(relation.memories,[]);memories.push({text:`玩家選擇「${entry.choice_text}」；NPC 回應「${String(entry.npc_text||'').slice(0,500)}」`,at:new Date().toISOString(),type:'dialogue'});const affinity=Math.max(-100,Math.min(100,Number(relation.affinity||0)+delta.affinity)),trust=Math.max(-100,Math.min(100,Number(relation.trust||0)+delta.trust)),fear=Math.max(0,Math.min(100,Number(relation.fear||0)+delta.fear)),hostility=Math.max(0,Math.min(100,Number(relation.hostility||0)+delta.hostility));await run(`INSERT INTO npc_relationships(room_id,npc_template_id,user_id,affinity,trust,fear,hostility,memories) VALUES(?,?,?,?,?,?,?,?::jsonb) ON CONFLICT(room_id,npc_template_id,user_id) DO UPDATE SET affinity=EXCLUDED.affinity,trust=EXCLUDED.trust,fear=EXCLUDED.fear,hostility=EXCLUDED.hostility,memories=EXCLUDED.memories,updated_at=CURRENT_TIMESTAMP`,[roomId,npcId,req.user.id,affinity,trust,fear,hostility,JSON.stringify(memories.slice(-100))]);if(entry.once_per_user)await run(`INSERT INTO npc_dialogue_history(room_id,user_id,npc_template_id,dialogue_entry_id) VALUES(?,?,?,?) ON CONFLICT DO NOTHING`,[roomId,req.user.id,npcId,entryId]);let granted_task=null;if(entry.grant_task_id){const task=await getTask(entry.grant_task_id);if(task&&task.status==='open'&&(!task.target_user_id||String(task.target_user_id)===String(req.user.id))){const existingTaskState=await get(`SELECT status FROM task_participants WHERE task_id=? AND user_id=?`,[task.id,req.user.id]);if(!existingTaskState){const eligibility=await taskChainEligibility(req.user.id,task);if(eligibility.eligible){await run(`INSERT INTO task_participants(task_id,user_id,status) VALUES(?,?,'accepted') ON CONFLICT(task_id,user_id) DO NOTHING`,[task.id,req.user.id]);granted_task=parseTask(task,'accepted');}}}}const character=await getCharacterByUser(req.user.id);const attitude=await v31NpcAttitude(roomId,req.user.id,npc,{affinity,trust,fear,hostility,memories});await addEvent(roomId,req.user.id,'npc_dialogue',{text:`💬 ${character?.name||user?.username||'玩家'} 與「${npc.name}」交談。NPC 態度：${attitude.label}`,npc_template_id:npcId,dialogue_entry_id:entryId,attitude:attitude.key});res.json({ok:true,reply:String(entry.npc_text||''),relationship:{affinity,trust,fear,hostility},attitude,granted_task});
}catch(error){res.status(error.status||500).json({error:error.message||'NPC 對話失敗'});}});

app.post('/api/rooms/:id/npcs/:npcId/shop/:shopItemId/buy',auth,async(req,res)=>{const client=await pool.connect();try{
  const roomId=Number(req.params.id),npcId=Number(req.params.npcId),shopItemId=Number(req.params.shopItemId);const user=await getUser(req.user.id);const isAdmin=Boolean(user?.is_admin);const npc=await v30NpcInstanceForUser(roomId,npcId,req.user.id,isAdmin);const item=await get(`SELECT * FROM npc_shop_items WHERE id=? AND npc_template_id=? AND active=TRUE`,[shopItemId,npcId]);if(!item)return res.status(404).json({error:'找不到 NPC 商品'});const check=await v30ShopCheck(roomId,req.user.id,npc,item);if(!check.ok)return res.status(403).json({error:check.reason});
  await client.query('BEGIN');const locked=(await client.query('SELECT * FROM npc_shop_items WHERE id=$1 AND active=TRUE FOR SHARE',[shopItemId])).rows[0];if(!locked)throw Object.assign(new Error('商品目前不可購買'),{status:404});if(Number(locked.stock)>=0){await client.query(`INSERT INTO npc_shop_stock(room_id,shop_item_id,remaining) VALUES($1,$2,$3) ON CONFLICT(room_id,shop_item_id) DO NOTHING`,[roomId,shopItemId,Number(locked.stock)]);const sr=(await client.query(`SELECT remaining FROM npc_shop_stock WHERE room_id=$1 AND shop_item_id=$2 FOR UPDATE`,[roomId,shopItemId])).rows[0];if(Number(sr?.remaining)<=0)throw Object.assign(new Error('商品已售罄'),{status:400});}
  const charResult=await client.query('SELECT * FROM character_cards WHERE user_id=$1 FOR UPDATE',[req.user.id]);const character=parseCharacter(charResult.rows[0]);if(!character)throw Object.assign(new Error('找不到角色卡'),{status:404});const pricing=await v31ShopPrice(req.user.id,locked);const price=pricing.effective_price;if(locked.currency==='game'){if(!character.game_coin_unlocked)throw Object.assign(new Error('你尚未解鎖遊戲幣'),{status:403});if(Number(character.game_coins||0)<price)throw Object.assign(new Error(`遊戲幣不足，需要 ${price}`),{status:400});character.game_coins=Math.max(0,Number(character.game_coins)||0)-price;}else{if(Number(character.weird_coins||0)<price)throw Object.assign(new Error(`詭幣不足，需要 ${price}`),{status:400});character.weird_coins=Math.max(0,Number(character.weird_coins)||0)-price;}
  const quantity=Math.max(1,Math.floor(Number(locked.quantity)||1));let inventory=normalizeInventory(character.inventory),equipment=Array.isArray(character.equipment)?[...character.equipment]:[],weapons=Array.isArray(character.weapons)?[...character.weapons]:[];if(locked.item_type==='equipment'){for(let i=0;i<quantity;i++)equipment.push(locked.name);}else if(locked.item_type==='weapon'){for(let i=0;i<quantity;i++)weapons.push(locked.name);}else{ensureInventoryCanAdd(character,inventory,locked.name,locked.item_category||'');inventory=changeInventoryQuantity(inventory,locked.name,quantity,locked.item_category||'');}await client.query(`UPDATE character_cards SET weird_coins=$1,game_coins=$2,inventory=$3::jsonb,equipment=$4::jsonb,weapons=$5::jsonb,updated_at=CURRENT_TIMESTAMP WHERE user_id=$6`,[character.weird_coins,character.game_coins,JSON.stringify(inventory),JSON.stringify(equipment),JSON.stringify(weapons),req.user.id]);if(Number(locked.stock)>=0)await client.query(`UPDATE npc_shop_stock SET remaining=GREATEST(0,remaining-1),updated_at=CURRENT_TIMESTAMP WHERE room_id=$1 AND shop_item_id=$2`,[roomId,shopItemId]);await client.query('COMMIT');const characterName=(await getCharacterByUser(req.user.id))?.name||user?.username||'玩家';await addEvent(roomId,req.user.id,'npc_shop',{text:`🛒 ${characterName} 向「${npc.name}」購買了 ${locked.name} ×${quantity}，支付 ${price}${pricing.discount_percent>0?`（聲望折扣 ${pricing.discount_percent}%）`:''}`,npc_template_id:npcId,shop_item_id:shopItemId,price,discount_percent:pricing.discount_percent});res.json({ok:true,item:{...locked,...pricing},character:await getCharacterByUser(req.user.id)});
}catch(error){await client.query('ROLLBACK').catch(()=>{});res.status(error.status||500).json({error:error.message||'NPC 商店購買失敗'});}finally{client.release();}});
