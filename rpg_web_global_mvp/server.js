...
function calculateCharacter(
  ...
  const ownedEquipment = Array.isArray(character.equipment) ? character.equipment : [];
  const ownedWeapons = Array.isArray(character.weapons) ? character.weapons : [];
  ...
  const equippedEquipment = pickOwned(ownedEquipment, loadoutEquipment, !hasExplicitLoadout);
  const equippedWeapons = pickOwned(ownedWeapons, loadoutWeapons, !hasExplicitLoadout);
  ...
  for (const status of activeStatuses) {
    ...
    const effects = parseJsonField(status.effects, {});
    const statFlat = effects.stat_flat && typeof effects.stat_flat === 'object' ? effects.stat_flat : {};
    const statPct = effects.stat_percent && typeof effects.stat_percent === 'object' ? effects.stat_percent : {};
    allPct += num(effects.all_attributes_percent);
    for (const field of Object.keys(flat)) {
      flat[field] += num(statFlat[field]);
      pct[field] += num(statPct[field]);
    }
    const affinityMods = normalizeElementAffinityModifier(effects.element_affinity || {});
    for (const element of ELEMENTS) statusElementAffinityModifiers[element] += affinityMods[element];
  }
...
async function getCharacterByUser(
  ...
  try {
    statuses = await all(
      `SELECT id,status_template_id,name,description,remaining_rounds,effects,source,active,created_at,updated_at
      ...
      summons.map(
        function (summon) {
          return {
            ...
            attributes: parseJsonField(summon.attributes, {}),
            element_affinity: parseJsonField(summon.element_affinity, {}),
...
app.post('/api/character/gear/:kind/:index/use', auth, async function(req,res){
  ...
  try{
    ...
    const itemName = String(gear.consume_item_name || '').trim();
    const category = String(gear.consume_item_category || '').trim();
    const quantity = Math.max(0, Math.floor(Number(gear.consume_quantity)||0));
    let inventory = normalizeInventory(character.inventory);
    ...
    await client.query(
      `UPDATE character_cards SET inventory=$1::jsonb,updated_at=CURRENT_TIMESTAMP WHERE user_id=$2`,
...
app.post(
  ...
  async function (req, res) {
    try {
      const character = await requireCharacter(req.user.id);
      const skill = (Array.isArray(character.skills)?character.skills:[]).find(item=>String(item&&item.id)===String(req.params.skillId));
      ...
      if(roomId){
        ...
        const config=gate.config||skillCombatConfig(skill);
        await addEvent(roomId,req.user.id,'skill',{
          text:`${character.name} 使用技能「${skill.name||'未命名技能'}」${resourceText}${config.turn_cost===0?'（不消耗回合數）':''}`,
          skill_id:skill.id,
        ...
        if(gate.battle&&config.turn_cost>0)await advanceBattleAction(roomId,req.user.id,`使用「${skill.name||'技能'}」`);
...
function parseSummonInstance(
  ...
  return {
    ...
    element_affinity: parseJsonField(row.element_affinity, {}),
...
app.post('/api/character/summons/:id/skills/:skillTemplateId/use',auth,async function(req,res){
  try{
    const summon=await get(`SELECT si.*,st.skills FROM summon_instances si LEFT JOIN summon_templates st ON st.id=si.template_id WHERE si.id=? AND si.owner_user_id=? AND si.status='active'`,[req.params.id,req.user.id]);
    if(!summon)return res.status(404).json({error:'找不到你的召喚物'});

    const skillIds=parseJsonField(summon.skills,[]).map(Number);
    const skillTemplateId=Number(req.params.skillTemplateId);
    if(!skillIds.includes(skillTemplateId))return res.status(403).json({error:'這個召喚物沒有此技能'});

    const template=await get(`SELECT * FROM skill_templates WHERE id=?`,[skillTemplateId]);
    if(!template)return res.status(404).json({error:'召喚物技能模板已不存在'});
    const skill=buildCharacterSkillFromTemplate(template,{type:'summon_template',summon_id:summon.id});
    const character=await requireCharacter(req.user.id);
    const roomId=Number(summon.room_id)||null;
    const gate=await prepareBattleSkillUse(roomId,req.user.id,character,skill);

    if(gate.charging){
      const snapshot=roomId?await roomSnapshot(roomId):null;
      if(snapshot)io.to('room:'+roomId).emit('room:snapshot',snapshot);
      return res.json({ok:true,charging:true,room:snapshot,charge_turns:gate.config.charge_turns});
    }

    if(roomId){
      const config=gate.config||skillCombatConfig(skill);
      await addEvent(roomId,req.user.id,'summon_skill',{
        text:`召喚物「${summon.name}」使用技能「${template.name}」${config.turn_cost===0?'（不消耗回合數）':''}`,
        summon_id:summon.id,
        skill_template_id:template.id,
        turn_cost:config.turn_cost,
        charged:Boolean(gate.charged)
      });
      if(gate.battle&&config.turn_cost>0)await advanceBattleAction(roomId,req.user.id,`指揮「${summon.name}」使用技能「${template.name}」`);
      const snapshot=await roomSnapshot(roomId);
      io.to('room:'+roomId).emit('room:snapshot',snapshot);
      return res.json({ok:true,charging:false,room:snapshot});
    }

    res.json({ok:true,charging:false});
  }catch(error){
    console.error('USE SUMMON SKILL ERROR:',error);
    res.status(error.status||500).json({error:error.message||'召喚物使用技能失敗'});
  }
});
...
app.patch('/api/admin/summon-templates/:id',auth,adminAuth,async function(req,res){
  try{
    ...
    const row=await get(`UPDATE summon_templates SET name=?,description=?,growth_mode=?,control_mode=?,behavior_mode=?,self_ai_level=?,language_level=?,speech_enabled=?,loyalty=?,attributes=?::jsonb,element_affinity=?::jsonb,traits=?::jsonb,skills=?::jsonb,config=?::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=? RETURNING *`,[
    ...
    res.json({summon_template:{...row,attributes:parseJsonField(row.attributes,{}),element_affinity:parseJsonField(row.element_affinity,{}),traits:parseJsonField(row.traits,[]),skills:parseJsonField(row.skills,[]),config:parseJsonField(row.config,{})}});
...
app.patch('/api/rooms/:id/scene', auth, adminAuth, async function(req,res){
  try{
    ...
    await run(
      `UPDATE rooms SET map_name=?,map_image_url=?,map_description=?,game_day=?,game_hour=?,game_minute=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`,
      [mapName,mapImageUrl,mapDescription,gameDay,gameHour,gameMinute,req.params.id]
    );
...
app.post('/api/rooms/:id/time/advance', auth, adminAuth, async function(req,res){
  try{
    ...
    await run(`UPDATE rooms SET game_day=?,game_hour=?,game_minute=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`,[gameDay,gameHour,gameMinute,req.params.id]);
...
/* =========================================================
   V13 COMBAT ACTION / CHARGE HELPERS
========================================================= */

function skillCombatConfig(skill) {
  const data = parseJsonField(skill && skill.data, {});
  return {
    turn_cost: data.turn_cost === 0 ? 0 : 1,
    charge_turns: Math.max(0, Math.min(20, Math.floor(Number(data.charge_turns) || 0))),
...
async function battleMemberOrder(roomId) {
  const rows = await all(`
    SELECT rm.user_id, rm.joined_at, rm.turn_actions_remaining, rm.skill_charge_state, cc.*
    FROM room_members rm
    JOIN character_cards cc ON cc.user_id = rm.user_id
  ...
  return rows
    .map(row => ({
      ...
      character: parseCharacter(row),
      turn_actions_remaining: Math.max(0, Number(row.turn_actions_remaining) || 0),
      skill_charge_state: parseJsonField(row.skill_charge_state, {})
    }))
    .sort((a,b) => {
      const agiA = Math.max(0, Number(a.character && a.character.final_agility) || 0);
      const agiB = Math.max(0, Number(b.character && b.character.final_agility) || 0);
...
async function initializeBattleTurnState(roomId) {
  const members = await battleMemberOrder(roomId);
  for (const member of members) {
    await run(`UPDATE room_members SET turn_actions_remaining=?, skill_charge_state='{}'::jsonb WHERE room_id=? AND user_id=?`,[
      Math.max(1, Number(member.character && member.character.round_count) || 1),
      roomId,
      member.user_id
    ]);
  }
...
async function advanceBattleAction(roomId, actingUserId, reason) {
  ...
  const members = await battleMemberOrder(roomId);
  if (!members.length) return {advanced:false};

  let currentIndex = members.findIndex(m => String(m.user_id) === String(room.current_actor_id));
  if (currentIndex < 0) currentIndex = 0;
  const current = members[currentIndex];

  if (actingUserId && String(current.user_id) !== String(actingUserId)) {
    throw Object.assign(new Error('現在不是你的行動'), {status:403});
  }

  let remaining = Math.max(0, Number(current.turn_actions_remaining) || 0);
  if (!remaining) remaining = Math.max(1, Number(current.character && current.character.round_count) || 1);
  remaining = Math.max(0, remaining - 1);

  const charge = parseJsonField(current.skill_charge_state, {});
  if (charge && charge.status === 'charging' && Number(charge.remaining_turns) > 0) {
    charge.remaining_turns = Math.max(0, Number(charge.remaining_turns) - 1);
    if (charge.remaining_turns <= 0) charge.status = 'ready';
  }
  ...
  if (remaining > 0) {
    await addEvent(roomId, current.user_id, 'turn_action', {
      text: `${current.character.name} 消耗 1 回合數${reason ? `（${reason}）` : ''}，本輪剩餘 ${remaining}`,
      remaining
    });
    return {advanced:true, actor_id:current.user_id, round:room.round, remaining};
  }
  ...
  if (wrapped) {
    nextRound += 1;
    for (const member of members) {
      await applyCharacterClassResourceRoundDelta(member.user_id);
      const fresh = await getCharacterByUser(member.user_id);
      await run(`UPDATE room_members SET turn_actions_remaining=? WHERE room_id=? AND user_id=?`,[
        Math.max(1, Number(fresh && fresh.round_count) || 1),
        roomId,
        member.user_id
      ]);
    }
  ...
  await run(`UPDATE rooms SET round=?, current_actor_id=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,[
    nextRound,
    next.user_id,
    roomId
  ]);

  await addEvent(roomId, null, 'turn', {
    round: nextRound,
    actor_id: next.user_id,
    text: `輪到 ${next.character.name} 行動`
  });

  return {advanced:true, actor_id:next.user_id, round:nextRound, remaining:null};
}

async function prepareBattleSkillUse(roomId, userId, character, skill) {
  const config = skillCombatConfig(skill);
  if (!roomId) return {ready:true, battle:false, config};

  const room = await get(`SELECT * FROM rooms WHERE id=?`,[roomId]);
  if (!room) throw Object.assign(new Error('找不到房間'),{status:404});
  if (!room.battle_active) return {ready:true, battle:false, config};

  const member = await get(`SELECT * FROM room_members WHERE room_id=? AND user_id=?`,[roomId,userId]);
  if (!member) throw Object.assign(new Error('你不是房間成員'),{status:403});

  if (config.turn_cost > 0 && String(room.current_actor_id) !== String(userId)) {
    throw Object.assign(new Error('現在不是你的行動'),{status:403});
  }
  ...
  if (charge && charge.skill_id && String(charge.skill_id) !== String(skill.id)) {
    throw Object.assign(new Error(`目前正在為「${charge.skill_name || '其他技能'}」蓄力，請先完成或取消`),{status:400});
  }

  if (charge && String(charge.skill_id) === String(skill.id) && charge.status === 'ready') {
    await run(`UPDATE room_members SET skill_charge_state='{}'::jsonb WHERE room_id=? AND user_id=?`,[roomId,userId]);
    return {ready:true, battle:true, config, charged:true};
  }

  if (charge && String(charge.skill_id) === String(skill.id) && charge.status === 'charging') {
    throw Object.assign(new Error(`技能仍在蓄力，剩餘 ${Math.max(0,Number(charge.remaining_turns)||0)} 回合數`),{status:400});
  }

  const nextCharge = {
    skill_id: skill.id,
    skill_name: skill.name || '未命名技能',
    status: 'charging',
    remaining_turns: config.charge_turns,
    started_at_round: Math.max(1, Number(room.round) || 1)
  };
  await run(`UPDATE room_members SET skill_charge_state=?::jsonb WHERE room_id=? AND user_id=?`,[
    JSON.stringify(nextCharge), roomId, userId
  ]);
  await addEvent(roomId,userId,'charge',{
    text:`${character.name} 開始為技能「${skill.name || '未命名技能'}」蓄力（需要 ${config.charge_turns} 回合數）`,
    skill_id:skill.id,
    remaining_turns:config.charge_turns
  });

  if (config.turn_cost > 0) {
    await advanceBattleAction(roomId,userId,'開始蓄力');
  }
...
app.post('/api/rooms/:id/battle/start', auth, adminAuth, async function(req,res){
  try{
    ...
    const members=await initializeBattleTurnState(req.params.id);
    for(const member of members){
      await resetCharacterClassResourcesForBattle(member.user_id);
    }

    const first=members[0]||null;
    await run(`UPDATE rooms SET battle_active=TRUE,round=1,current_actor_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`,[first?first.user_id:null,req.params.id]);
    await addEvent(req.params.id,req.user.id,'system',{text:first?`⚔️ DM 開始戰鬥輪。由 ${first.character.name} 先行動。`:'⚔️ DM 開始戰鬥輪。'});
    const snapshot=await roomSnapshot(req.params.id);
    io.to('room:'+req.params.id).emit('room:snapshot',snapshot);
    res.json(snapshot);
  }catch(error){
...
app.post('/api/rooms/:id/battle/end', auth, adminAuth, async function(req,res){
  try{
    await requireRoomAdmin(req.user.id,req.params.id);
    await run(`UPDATE rooms SET battle_active=FALSE,current_actor_id=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?`,[req.params.id]);
...
app.post(
  ...
  async function (req, res) {
    try {
      ...
      const currentActor = roomInfo.room.current_actor_id;
      if (!currentActor) return res.status(400).json({error:'目前沒有可行動的玩家'});
      await advanceBattleAction(req.params.id,currentActor,'DM 推進');
...
app.post('/api/admin/monster-templates',auth,adminAuth,async(req,res)=>{try{
  ...
  const config={...parseWorldConfig(req.body.config),ai_actions:splitLines(req.body.ai_actions||req.body.config?.ai_actions),secret:String(req.body.secret||req.body.config?.secret||'').slice(0,4000),element:String(req.body.element||req.body.config?.element||'').slice(0,20)};
  const row=await get(`INSERT INTO monster_templates(name,category,rank,description,public_hint,image_url,max_hp,investigation_dc,attributes,skills,drops,ai_level,ai_behavior,config) VALUES(?,?,?,?,?,?,?,?,?::jsonb,?::jsonb,?::jsonb,?,?,?::jsonb) RETURNING *`,[
    name,String(req.body.category||'怪物').slice(0,80),String(req.body.rank||'G').slice(0,10),String(req.body.description||''),String(req.body.public_hint||'').slice(0,2000),String(req.body.image_url||''),Math.max(1,Number(req.body.max_hp)||100),Math.max(1,Math.min(100,Number(req.body.investigation_dc)||50)),JSON.stringify(req.body.attributes||{}),JSON.stringify(splitLines(req.body.skills)),JSON.stringify(splitLines(req.body.drops)),Math.max(0,Math.min(10,Number(req.body.ai_level)||1)),String(req.body.ai_behavior||'balanced').slice(0,40),JSON.stringify(config)
  ]);res.json({monster:{...row,attributes:parseJsonField(row.attributes,{}),skills:parseWorldList(row.skills),drops:parseWorldList(row.drops),config:parseWorldConfig(row.config)}});
...
app.post('/api/admin/npc-templates',auth,adminAuth,async(req,res)=>{try{
  ...
  const config={...parseWorldConfig(req.body.config),ai_lines:splitLines(req.body.ai_lines||req.body.config?.ai_lines),ai_actions:splitLines(req.body.ai_actions||req.body.config?.ai_actions)};
  const row=await get(`INSERT INTO npc_templates(name,role_name,description,image_url,ai_level,ai_behavior,speech_enabled,in_game_room,config) VALUES(?,?,?,?,?,?,?,?,?::jsonb) RETURNING *`,[
    name,String(req.body.role_name||'').slice(0,120),String(req.body.description||''),String(req.body.image_url||''),Math.max(0,Math.min(10,Number(req.body.ai_level)||1)),String(req.body.ai_behavior||'balanced').slice(0,40),req.body.speech_enabled!==false,Boolean(req.body.in_game_room),JSON.stringify(config)
  ]);res.json({npc:{...row,config:parseWorldConfig(row.config)}});
...
app.patch('/api/admin/npc-templates/:id',auth,adminAuth,async(req,res)=>{try{
  ...
  const row=await get(`UPDATE npc_templates SET name=?,role_name=?,description=?,image_url=?,ai_level=?,ai_behavior=?,speech_enabled=?,in_game_room=?,config=?::jsonb,updated_at=CURRENT_TIMESTAMP WHERE id=? RETURNING *`,[
    String(req.body.name??cur.name).trim().slice(0,120),String(req.body.role_name??cur.role_name).slice(0,120),String(req.body.description??cur.description),String(req.body.image_url??cur.image_url),Math.max(0,Math.min(10,Number(req.body.ai_level??cur.ai_level)||1)),String(req.body.ai_behavior??cur.ai_behavior).slice(0,40),req.body.speech_enabled===undefined?cur.speech_enabled:Boolean(req.body.speech_enabled),req.body.in_game_room===undefined?cur.in_game_room:Boolean(req.body.in_game_room),JSON.stringify(config),req.params.id
  ]);res.json({npc:{...row,config:parseWorldConfig(row.config)}});
...
app.get('/api/rooms/:id/world',auth,async(req,res)=>{try{
  const currentUser=await getUser(req.user.id);const isAdmin=Boolean(currentUser&&currentUser.is_admin);if(!isAdmin)await requireRoomMember(req.user.id,req.params.id);
  const monsters=await all(`SELECT rm.*,mt.name,mt.category,mt.rank,mt.description,mt.public_hint,mt.image_url,mt.investigation_dc,mt.attributes,mt.skills,mt.drops,mt.ai_level,mt.ai_behavior,mt.config,md.encountered,md.investigated,md.last_roll,md.last_rate FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id LEFT JOIN monster_discoveries md ON md.room_monster_id=rm.id AND md.user_id=? WHERE rm.room_id=? ORDER BY rm.id`,[req.user.id,req.params.id]);
  ...
  for(const m of monsters){
    ...
    if(full){visibleMonsters.push({...m,revealed:true,attributes:parseJsonField(m.attributes,{}),skills:parseWorldList(m.skills),drops:parseWorldList(m.drops),config:parseWorldConfig(m.config)});}
    else{visibleMonsters.push({id:m.id,monster_template_id:m.monster_template_id,status:m.status,revealed:false,encountered:true,name:'未知怪物',public_hint:m.public_hint||'你只知道這裡有某種異常存在。',image_url:'',last_roll:m.last_roll,last_rate:m.last_rate});}
  }
  const npcs=(await all(`SELECT rn.status,rn.state,nt.* FROM room_npcs rn JOIN npc_templates nt ON nt.id=rn.npc_template_id WHERE rn.room_id=? ORDER BY nt.id`,[req.params.id])).map(n=>({...n,state:parseJsonField(n.state,{}),config:parseWorldConfig(n.config)}));
  const dungeons=(await all(`SELECT rd.status,rd.state,dt.* FROM room_dungeons rd JOIN dungeon_templates dt ON dt.id=rd.dungeon_template_id WHERE rd.room_id=? ORDER BY dt.id`,[req.params.id])).map(d=>({...d,state:parseJsonField(d.state,{}),config:parseWorldConfig(d.config)}));
  res.json({monsters:visibleMonsters,npcs,dungeons});
}catch(e){console.error(e);res.status(e.status||500).json({error:e.message||'讀取世界資料失敗'});}});

app.post('/api/admin/rooms/:id/monsters',auth,adminAuth,async(req,res)=>{try{await requireRoomAdmin(req.user.id,req.params.id);const t=await get(`SELECT * FROM monster_templates WHERE id=?`,[req.body.template_id]);if(!t)return res.status(404).json({error:'找不到怪物卡'});const row=await get(`INSERT INTO room_monsters(room_id,monster_template_id,status,current_hp,max_hp) VALUES(?,?, 'hidden', ?, ?) RETURNING *`,[req.params.id,t.id,Math.max(1,Number(t.max_hp)||100),Math.max(1,Number(t.max_hp)||100)]);res.json({ok:true,instance:row});}catch(e){res.status(e.status||500).json({error:e.message||'加入怪物失敗'});}});
app.post('/api/admin/rooms/:id/monsters/:monsterId/encounter',auth,adminAuth,async(req,res)=>{try{await requireRoomAdmin(req.user.id,req.params.id);const inst=await get(`SELECT * FROM room_monsters WHERE id=? AND room_id=?`,[req.params.monsterId,req.params.id]);if(!inst)return res.status(404).json({error:'找不到房間怪物'});await run(`UPDATE room_monsters SET status='encountered',updated_at=CURRENT_TIMESTAMP WHERE id=?`,[inst.id]);const members=await all(`SELECT user_id FROM room_members WHERE room_id=? AND role='player'`,[req.params.id]);for(const m of members){await run(`INSERT INTO monster_discoveries(room_monster_id,user_id,encountered,investigated) VALUES(?,?,TRUE,FALSE) ON CONFLICT(room_monster_id,user_id) DO UPDATE SET encountered=TRUE,updated_at=CURRENT_TIMESTAMP`,[inst.id,m.user_id]);}await addEvent(req.params.id,req.user.id,'encounter',{text:'⚠️ 玩家遭遇了未知怪物。',monster_id:inst.id});res.json({ok:true});}catch(e){res.status(e.status||500).json({error:e.message||'設定遭遇失敗'});}});
...
app.post('/api/admin/rooms/:id/npcs',auth,adminAuth,async(req,res)=>{try{await requireRoomAdmin(req.user.id,req.params.id);await run(`INSERT INTO room_npcs(room_id,npc_template_id) VALUES(?,?) ON CONFLICT(room_id,npc_template_id) DO NOTHING`,[req.params.id,req.body.template_id]);res.json({ok:true});}catch(e){res.status(e.status||500).json({error:e.message||'加入 NPC 失敗'});}});
...
app.post('/api/admin/rooms/:id/dungeons',auth,adminAuth,async(req,res)=>{try{await requireRoomAdmin(req.user.id,req.params.id);await run(`INSERT INTO room_dungeons(room_id,dungeon_template_id) VALUES(?,?) ON CONFLICT(room_id,dungeon_template_id) DO NOTHING`,[req.params.id,req.body.template_id]);res.json({ok:true});}catch(e){res.status(e.status||500).json({error:e.message||'加入副本失敗'});}});
...
app.post('/api/rooms/:id/ai/step',auth,adminAuth,async(req,res)=>{try{
  await requireRoomAdmin(req.user.id,req.params.id);const messages=[];
  const monsters=await all(`SELECT rm.id,mt.name,mt.ai_level,mt.ai_behavior,mt.config FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=? AND rm.status='encountered'`,[req.params.id]);for(const m of monsters){if(Number(m.ai_level)<=0)continue;const config=parseWorldConfig(m.config);const action=worldAiChoice(parseWorldList(config.ai_actions),m.ai_behavior)||'觀察玩家的動向';messages.push(`👹 ${m.name}：${action}`);}
  const npcs=await all(`SELECT nt.* FROM room_npcs rn JOIN npc_templates nt ON nt.id=rn.npc_template_id WHERE rn.room_id=? AND rn.status='active'`,[req.params.id]);for(const n of npcs){if(Number(n.ai_level)<=0)continue;const config=parseWorldConfig(n.config);const line=worldAiChoice(parseWorldList(config.ai_lines),n.ai_behavior);const action=worldAiChoice(parseWorldList(config.ai_actions),n.ai_behavior);if(line&&n.speech_enabled)messages.push(`🧑 ${n.name}：「${line}」`);else if(action)messages.push(`🧑 ${n.name}：${action}`);}
  const dungeons=await all(`SELECT dt.* FROM room_dungeons rd JOIN dungeon_templates dt ON dt.id=rd.dungeon_template_id WHERE rd.room_id=? AND rd.status='available'`,[req.params.id]);for(const d of dungeons){if(Number(d.ai_level)<=0)continue;const config=parseWorldConfig(d.config);const event=worldAiChoice(parseWorldList(config.ai_events),d.ai_behavior);if(event)messages.push(`🏚️ ${d.name}：${event}`);}
  if(!messages.length)messages.push('AI 本次沒有觸發新的行動。');for(const text of messages)await addEvent(req.params.id,req.user.id,'ai',{text});const snapshot=await roomSnapshot(req.params.id);io.to('room:'+req.params.id).emit('room:snapshot',snapshot);res.json({ok:true,messages,snapshot});
}catch(e){res.status(e.status||500).json({error:e.message||'AI 推進失敗'});}});

app.get('/api/game-room/npcs',auth,async(req,res)=>{try{const rows=await all(`SELECT * FROM npc_templates WHERE in_game_room=TRUE ORDER BY id`);res.json({npcs:rows.map(r=>({...r,config:parseWorldConfig(r.config)}))});}catch(e){res.status(500).json({error:'讀取遊戲間 NPC 失敗'});}});
...
async function updatePlayerCharacter(userId, body) {
  ...
  if (body.gear_loadout !== undefined) {
    ...
    const ownedEquipment=(Array.isArray(current.equipment)?current.equipment:[]).map(item=>String(item&&typeof item==='object'?(item.name||item.title||''):item||''));
    const ownedWeapons=(Array.isArray(current.weapons)?current.weapons:[]).map(item=>String(item&&typeof item==='object'?(item.name||item.title||''):item||''));
    const validate=(wanted,owned,label)=>{
      const pool=[...owned];
      for(const name of wanted){const idx=pool.indexOf(name);if(idx<0)throw Object.assign(new Error(`你沒有可裝備的${label}「${name}」`),{status:400});pool.splice(idx,1);}
    };
    validate(wantedEquipment,ownedEquipment,'裝備'); validate(wantedWeapons,ownedWeapons,'武器');
...
app.post('/api/rooms/:id/actions',auth,async(req,res)=>{try{
  ...
  await addEvent(room.id,req.user.id,'action',{text:`🗣️ ${actorName} 宣告行動：${action}`,action});
  ...
  for(const raw of rules){
    ...
    if(location&&!String(room.map_name||'').toLocaleLowerCase().includes(location))continue;
  ...
  const snapshot=await roomSnapshot(room.id);io.to('room:'+room.id).emit('room:snapshot',snapshot);
  res.json({ok:true,triggered,snapshot});
...
function parsedStatus(row){return row?{...row,effects:parseJsonField(row.effects,{})}:null;}
...
app.get('/api/admin/clues',auth,adminAuth,async(req,res)=>{try{const rows=await all(`SELECT ct.*,hr.name AS linked_rule_name FROM clue_templates ct LEFT JOIN horror_rules hr ON hr.id=ct.linked_rule_id ORDER BY ct.id DESC`);res.json({clues:rows});}catch(error){res.status(500).json({error:'讀取線索失敗'});}});
app.post('/api/admin/clues',auth,adminAuth,async(req,res)=>{try{const name=String(req.body.name||'').trim().slice(0,160);if(!name)return res.status(400).json({error:'線索名稱不能為空'});const row=await get(`INSERT INTO clue_templates(name,category,description,image_url,linked_rule_id,reveal_level) VALUES(?,?,?,?,?,?) RETURNING *`,[name,String(req.body.category||'一般線索').slice(0,80),String(req.body.description||''),String(req.body.image_url||''),Number(req.body.linked_rule_id)||null,Math.max(1,Math.min(3,Number(req.body.reveal_level)||1))]);res.json({clue:row});}catch(error){console.error(error);res.status(500).json({error:'建立線索失敗'});}});
...
app.get('/api/admin/status-templates',auth,adminAuth,async(req,res)=>{try{const rows=await all(`SELECT * FROM status_templates ORDER BY id DESC`);res.json({statuses:rows.map(parsedStatus)});}catch(error){res.status(500).json({error:'讀取狀態模板失敗'});}});
app.post('/api/admin/status-templates',auth,adminAuth,async(req,res)=>{try{const name=String(req.body.name||'').trim().slice(0,120);if(!name)return res.status(400).json({error:'狀態名稱不能為空'});const effects=req.body.effects&&typeof req.body.effects==='object'?req.body.effects:{};const row=await get(`INSERT INTO status_templates(name,category,description,duration_rounds,effects) VALUES(?,?,?,?,?::jsonb) RETURNING *`,[name,String(req.body.category||'狀態').slice(0,60),String(req.body.description||''),Math.max(0,Math.floor(Number(req.body.duration_rounds)||0)),JSON.stringify(effects)]);res.json({status:parsedStatus(row)});}catch(error){res.status(500).json({error:'建立狀態模板失敗'});}});
...
app.post('/api/admin/players/:id/statuses',auth,adminAuth,async(req,res)=>{try{const t=await get(`SELECT * FROM status_templates WHERE id=?`,[req.body.template_id]);if(!t)return res.status(404).json({error:'找不到狀態模板'});const row=await get(`INSERT INTO character_statuses(user_id,status_template_id,name,description,remaining_rounds,effects,source) VALUES(?,?,?,?,?,?::jsonb,?) RETURNING *`,[req.params.id,t.id,t.name,t.description,req.body.remaining_rounds===undefined?t.duration_rounds:Math.max(0,Number(req.body.remaining_rounds)||0),JSON.stringify(parseJsonField(t.effects,{})),String(req.body.source||'DM')]);res.json({status:parsedStatus(row),character:await getCharacterByUser(req.params.id)});}catch(error){console.error(error);res.status(500).json({error:'套用狀態失敗'});}});
...
app.post('/api/admin/item-templates',auth,adminAuth,async(req,res)=>{try{const name=String(req.body.name||'').trim().slice(0,120);if(!name)return res.status(400).json({error:'道具名稱不能為空'});const type=['normal','buff','revive','profession_book','skill_book'].includes(req.body.item_type)?req.body.item_type:'normal';const row=await get(`INSERT INTO item_templates(name,category,item_type,description,effects,consume_on_use) VALUES(?,?,?,?,?::jsonb,?) ON CONFLICT(name) DO UPDATE SET category=EXCLUDED.category,item_type=EXCLUDED.item_type,description=EXCLUDED.description,effects=EXCLUDED.effects,consume_on_use=EXCLUDED.consume_on_use,updated_at=CURRENT_TIMESTAMP RETURNING *`,[name,String(req.body.category||'道具').slice(0,80),type,String(req.body.description||''),JSON.stringify(req.body.effects&&typeof req.body.effects==='object'?req.body.effects:{}),req.body.consume_on_use===undefined?true:Boolean(req.body.consume_on_use)]);res.json({item:parsedItemTemplate(row)});}catch(error){console.error(error);res.status(500).json({error:'建立道具模板失敗'});}});
...
app.post('/api/character/items/use',auth,async(req,res)=>{const client=await pool.connect();try{
  await client.query('BEGIN');const roomId=Number(req.body.room_id)||null;const itemName=String(req.body.name||'').trim();if(!itemName){await client.query('ROLLBACK');return res.status(400).json({error:'請選擇道具'});}const cr=await client.query(`SELECT * FROM character_cards WHERE user_id=$1 FOR UPDATE`,[req.user.id]);const character=parseCharacter(cr.rows[0]);if(!character){await client.query('ROLLBACK');return res.status(404).json({error:'找不到角色'});}let inventory=normalizeInventory(character.inventory);if(inventoryQuantity(inventory,itemName)<=0){await client.query('ROLLBACK');return res.status(400).json({error:'你沒有這個道具'});}const templateResult=await client.query(`SELECT * FROM item_templates WHERE name=$1`,[itemName]);const template=parsedItemTemplate(templateResult.rows[0]);if(!template){await client.query('ROLLBACK');return res.status(400).json({error:'這個道具尚未建立使用效果'});}const effects=template.effects||{};
  let message=`使用了「${itemName}」`;
  if(template.item_type==='revive'){
    if(!roomId){await client.query('ROLLBACK');return res.status(400).json({error:'復活道具只能在房間中使用'});}const mr=await client.query(`SELECT * FROM room_members WHERE room_id=$1 AND user_id=$2 FOR UPDATE`,[roomId,req.user.id]);const member=mr.rows[0];if(!member){await client.query('ROLLBACK');return res.status(403).json({error:'你不在這個房間'});}if(Number(member.hp)>0){await client.query('ROLLBACK');return res.status(400).json({error:'只有生命值為 0 的角色才能使用復活道具'});}const pct=Math.max(1,Math.min(100,Number(effects.revive_hp_percent)||20));const nextHp=Math.max(1,Math.floor(Number(member.max_hp||character.max_hp||1)*pct/100));await client.query(`UPDATE room_members SET hp=$1 WHERE room_id=$2 AND user_id=$3`,[nextHp,roomId,req.user.id]);message+=`，生命恢復至 ${nextHp}`;
  }else if(template.item_type==='buff'){
    const duration=Math.max(0,Math.floor(Number(effects.duration_rounds)||0));await client.query(`INSERT INTO character_statuses(user_id,name,description,remaining_rounds,effects,source) VALUES($1,$2,$3,$4,$5::jsonb,$6)`,[req.user.id,itemName,template.description,duration,JSON.stringify(effects),`道具：${itemName}`]);message+=duration?`，獲得 ${duration} 回合加成`:'，獲得加成狀態';
  }else{
  ...
  if(roomId)await addEvent(roomId,req.user.id,'item',{text:`${character.name}${message}`,item_name:itemName});await client.query('COMMIT');const updated=await getCharacterByUser(req.user.id);const snapshot=roomId?await roomSnapshot(roomId):null;if(snapshot)io.to('room:'+roomId).emit('room:snapshot',snapshot);res.json({ok:true,character:updated,room:snapshot,message});
...
app.get('/api/rooms/:id/map-nodes',auth,async(req,res)=>{try{const user=await getUser(req.user.id);if(!user?.is_admin)await requireRoomMember(req.user.id,req.params.id);const room=await get(`SELECT current_map_node_id FROM rooms WHERE id=?`,[req.params.id]);const rows=await all(`SELECT * FROM room_map_nodes WHERE room_id=? AND (? OR hidden=FALSE) ORDER BY id`,[req.params.id,Boolean(user?.is_admin)]);res.json({current_map_node_id:room?.current_map_node_id||null,nodes:rows.map(r=>({...r,connections:parseJsonField(r.connections,[])}))});}catch(error){res.status(error.status||500).json({error:error.message||'讀取地圖節點失敗'});}});
app.post('/api/admin/rooms/:id/map-nodes',auth,adminAuth,async(req,res)=>{try{await requireRoomAdmin(req.user.id,req.params.id);const row=await get(`INSERT INTO room_map_nodes(room_id,name,description,image_url,connections,hidden) VALUES(?,?,?,?,?::jsonb,?) RETURNING *`,[req.params.id,String(req.body.name||'未命名區域').slice(0,120),String(req.body.description||''),String(req.body.image_url||''),JSON.stringify(Array.isArray(req.body.connections)?req.body.connections:[]),Boolean(req.body.hidden)]);res.json({node:{...row,connections:parseJsonField(row.connections,[])}});}catch(error){res.status(error.status||500).json({error:error.message||'建立地圖節點失敗'});}});
