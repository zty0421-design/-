...
app.post('/api/admin/professions',auth,adminAuth,async function(req,res){
  try{
    const name=String(req.body.name||'').trim().slice(0,100); if(!name)return res.status(400).json({error:'職業名稱不能為空'});
    const category=['main','sub','both'].includes(req.body.category)?req.body.category:'both';
    const row=await get(`INSERT INTO profession_templates (name,category,rank,description,config) VALUES (?,?,?,?,?::jsonb) RETURNING *`,[name,category,String(req.body.rank||'G').slice(0,10),String(req.body.description||''),JSON.stringify(req.body.config||{})]);
    res.json({profession:row});
  }catch(error){console.error('CREATE PROFESSION ERROR:',error);res.status(error.code==='23505'?409:500).json({error:error.code==='23505'?'這個職業名稱已存在':'建立職業失敗'});}
});
app.delete('/api/admin/professions/:id',auth,adminAuth,async function(req,res){
  try{const r=await run(`DELETE FROM profession_templates WHERE id = ?`,[req.params.id]);if(!r.changes)return res.status(404).json({error:'找不到職業'});res.json({ok:true});}
...
app.post('/api/admin/skill-templates',auth,adminAuth,async function(req,res){
  try{
    const name=String(req.body.name||'').trim().slice(0,120);if(!name)return res.status(400).json({error:'技能名稱不能為空'});
    const row=await get(`INSERT INTO skill_templates (name,category,skill_type,level,description,check_name,damage_formula,extra_tag,summon_tag,resources,data) VALUES (?,?,?,?,?,?,?,?,?,?::jsonb,?::jsonb) RETURNING *`,[name,String(req.body.category||'一般').slice(0,60),String(req.body.skill_type||'active').slice(0,30),Math.max(1,Number(req.body.level)||1),String(req.body.description||''),String(req.body.check_name||'').slice(0,60),String(req.body.damage_formula||'').slice(0,200),String(req.body.extra_tag||'').slice(0,60),Boolean(req.body.summon_tag),JSON.stringify(req.body.resources||{}),JSON.stringify(req.body.data||{})]);
    res.json({skill:row});
  }catch(error){console.error('CREATE SKILL TEMPLATE ERROR:',error);res.status(500).json({error:'建立技能失敗'});}
});
...
async function updatePlayerCharacter(userId, body) {
  ...
  if(body[identityField]!==undefined){
    const list=Array.isArray(body[identityField])
      ? body[identityField].map(v=>String(v||'').trim().slice(0,100)).filter(Boolean).slice(0,10)
      : [];
