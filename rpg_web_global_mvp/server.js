...
function calculateCharacter(
  ...
  return {
    ...
    attribute_name: fifthField === 'faith' ? '信仰' : '大道',
...
async function updatePlayerCharacter(userId, body) {
  ...
  const oldTotal = fields.reduce(
    (sum, field) => sum + Math.max(0, Number(current[field]) || 0),
    0
  );
  ...
  for (const field of fields) {
    if (next[field] > cap) {
      throw Object.assign(
        new Error(`${field} 已達目前 ${tier} 階上限 ${cap}，必須完成對應屬性試煉`),
        { status: 400 }
      );
    }
  }
  ...
  if (newTotal + newPoints !== oldTotal + oldPoints) {
    throw Object.assign(
      new Error('屬性點數不正確，玩家不能自行創造或消耗屬性點'),
      { status: 400 }
    );
  }
  ...
  if (body.name !== undefined) {
    updates.push('name = ?');
    values.push(String(body.name).trim().slice(0, 50));
  }

  for (const field of ['equipment', 'weapons']) {
    if (body[field] !== undefined) {
      updates.push(`${field} = ?::jsonb`);
      values.push(JSON.stringify(Array.isArray(body[field]) ? body[field] : []));
    }
  }

  if (!updates.length) {
    throw Object.assign(new Error('沒有可更新欄位'), { status: 400 });
  }
  ...
  await run(
    `UPDATE character_cards SET ${updates.join(', ')} WHERE user_id = ?`,
    values
  );
