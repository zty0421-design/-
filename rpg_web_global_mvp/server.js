...
async function syncCharacterSourceEffects(userId, rawCharacter) {
  ...
  if (!character) return {profession_effects:{list:[]},bloodline_effects:{list:[]}};
  ...
  await run(
    `UPDATE character_cards SET profession_effects=?::jsonb,bloodline_effects=?::jsonb,updated_at=CURRENT_TIMESTAMP WHERE user_id=?`,
    [JSON.stringify(professionEffects), JSON.stringify(bloodlineEffects), userId]
  );
  return {profession_effects:professionEffects,bloodline_effects:bloodlineEffects};
}
...
app.patch('/api/rooms/:id/scene', auth, adminAuth, async function(req,res){
  try{
    ...
    await run(
      `UPDATE rooms SET map_name=?,map_image_url=?,map_description=?,game_day=?,game_hour=?,game_minute=?,time_hidden=?,time_distortion_offset_minutes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`,
      [mapName,mapImageUrl,mapDescription,gameDay,gameHour,gameMinute,timeHidden,timeDistortionOffset,req.params.id]
    );
...
app.post('/api/rooms/:id/time/advance', auth, adminAuth, async function(req,res){
  try{
    ...
    if(nextTotal>currentTotal){await decayWorldNoise(Number(req.params.id),nextTotal-currentTotal);await processScheduledWorldEvents(Number(req.params.id),currentTotal,nextTotal);await checkAllBossTimePhases(Number(req.params.id),nextTotal);}
...
async function buildRoomTimeView(roomId,userId) {
  const room = await get(
    `SELECT id,game_day,game_hour,game_minute,time_hidden,time_distortion_offset_minutes,current_map_node_id
       FROM rooms
      WHERE id=?`,
    [roomId]
  );
  ...
  if (viewerNodeId) {
    const node = await get(
      `SELECT name,time_offset_minutes FROM room_map_nodes WHERE id=? AND room_id=?`,
      [viewerNodeId,roomId]
    );
    if (node) {
  ...
  const observedOffset = seesTrueTime ? 0 : globalOffset + nodeOffset;
  ...
  if (isAdmin) {
    response.admin = {
      ...
      player_observed:totalMinutesToGameTime(actualTotal + globalOffset + nodeOffset)
    };
  } else if (capability.true_time) {
    response.actual = actual;
    response.distortion_detected = (globalOffset + nodeOffset) !== 0;
  }
...
async function resolveCombatAttack({roomId,attackerType,attackerId=null,attackerUserId=null,attackerName,attackerSource,skill,config,targetType,targetId,damageMultiplier=1}){
  const target=await getCombatTarget(roomId,targetType,targetId);
  ...
  if(combatActorSide(attackerActor)===combatTargetSide(target))throw Object.assign(new Error('不能攻擊同一陣營的目標'),{status:400});
  ...
  let affinityMultiplier=1,counterMultiplier=1;
  if(hit){
    ...
    finalDamage=Math.max(0,Math.floor(baseDamage*Math.max(0,Number(config.damage_multiplier)||1)*Math.max(0,Number(damageMultiplier)||1)*affinityMultiplier*counterMultiplier*reactionMultiplier));
...
async function buildCombatOrder(roomId) {
  ...
  for(const p of players){
    if(Number(p.member.hp)<=0)continue;
...
async function combatActorAlive(roomId,actor) {
  ...
  if(actor.type==='player'){const row=await get(`SELECT hp,combat_state FROM room_members WHERE room_id=? AND user_id=?`,[roomId,actor.id]);return Boolean(row&&Number(row.hp)>0&&row.combat_state!=='dead');}
...
async function initializeBattleTurnState(roomId) {
  ...
  for(const actor of order){
    if(actor.type==='player'){
      ...
      await run(`UPDATE room_members SET turn_actions_remaining=?,skill_charge_state='{}'::jsonb,max_hp=?,hp=LEAST(hp,?),max_spirit=?,spirit=LEAST(spirit,?),combat_state=CASE WHEN hp>0 THEN 'active' ELSE 'downed' END WHERE room_id=? AND user_id=?`,[Math.max(1,Number(ch?.round_count)||1),Math.max(1,Number(ch?.max_hp)||1),Math.max(1,Number(ch?.max_hp)||1),Math.max(0,Number(ch?.final_spirit)||0),Math.max(0,Number(ch?.final_spirit)||0),roomId,actor.id]);
    }else{
...
async function executeAiCombatActor(roomId,actor){
  ...
  if(!chosen){
    const templates=await loadWorldSkillTemplates(entity.skills);chosen=chooseAiCombatSkill(entity,templates);
  }
  if(!chosen){
...
app.patch('/api/rooms/:id/combat/reaction',auth,async(req,res)=>{try{
  await requireRoomMember(req.user.id,req.params.id);const mode=String(req.body.mode||'dodge')==='defense'?'defense':'dodge';await run(`UPDATE room_members SET combat_reaction=? WHERE room_id=? AND user_id=?`,[mode,req.params.id,req.user.id]);const snapshot=await roomSnapshot(req.params.id);io.to('room:'+req.params.id).emit('room:snapshot',snapshot);res.json({ok:true,mode,room:snapshot});
}catch(e){res.status(e.status||500).json({error:e.message||'設定防禦方式失敗'});}});
...
app.post('/api/admin/npc-templates',auth,adminAuth,async(req,res)=>{try{
  ...
  const effects=effectContainerWithList(req.body.effects,normalizeUnifiedEffects(req.body.effects));
  const row=await get(`INSERT INTO npc_templates(name,role_name,description,image_url,max_hp,attributes,element_affinity,effects,skills,ai_level,ai_behavior,speech_enabled,in_game_room,config) VALUES(?,?,?,?,?,?::jsonb,?::jsonb,?::jsonb,?::jsonb,?,?,?,?,?::jsonb) RETURNING *`,[
    name,String(req.body.role_name||'').slice(0,120),String(req.body.description||''),String(req.body.image_url||''),Math.max(1,Number(req.body.max_hp)||100),JSON.stringify(req.body.attributes||{}),JSON.stringify(req.body.element_affinity||{}),JSON.stringify(effects),JSON.stringify(splitLines(req.body.skills)),Math.max(0,Math.min(10,Number(req.body.ai_level)||1)),String(req.body.ai_behavior||'balanced').slice(0,40),req.body.speech_enabled!==false,Boolean(req.body.in_game_room),JSON.stringify(config)
...
app.post('/api/admin/horror-rules',auth,adminAuth,async(req,res)=>{try{
  ...
  const row=await get(`INSERT INTO horror_rules(room_id,name,rule_text,public_hint,location_contains,start_minute,end_minute,keywords,match_mode,consequence,active,truth_state,active_from_total,active_until_total,change_note) VALUES(?,?,?,?,?,?,?,?::jsonb,?,?,?,?,?,?,?) RETURNING *`,[
    roomId,name,String(req.body.rule_text||''),String(req.body.public_hint||''),String(req.body.location_contains||'').trim().slice(0,160),parseClockMinute(req.body.start_time,0),parseClockMinute(req.body.end_time,1439),JSON.stringify(keywords),req.body.match_mode==='all'?'all':'any',String(req.body.consequence||''),req.body.active===undefined?true:Boolean(req.body.active),truthState,activeFrom,activeUntil,String(req.body.change_note||'').slice(0,4000)
...
app.get('/api/item-templates',auth,async(req,res)=>{try{const rows=await all(`SELECT * FROM item_templates ORDER BY category,name`);res.json({items:rows.map(parsedItemTemplate)});}catch(error){res.status(500).json({error:'讀取道具模板失敗'});}});
...
app.post('/api/admin/item-templates',auth,adminAuth,async(req,res)=>{try{
  ...
  const row=await get(`INSERT INTO item_templates(name,category,item_type,description,effects,consume_on_use,grants_time_view,grants_true_time) VALUES(?,?,?,?,?::jsonb,?,?,?) ON CONFLICT(name) DO UPDATE SET category=EXCLUDED.category,item_type=EXCLUDED.item_type,description=EXCLUDED.description,effects=EXCLUDED.effects,consume_on_use=EXCLUDED.consume_on_use,grants_time_view=EXCLUDED.grants_time_view,grants_true_time=EXCLUDED.grants_true_time,updated_at=CURRENT_TIMESTAMP RETURNING *`,[name,String(req.body.category||'道具').slice(0,80),type,String(req.body.description||''),JSON.stringify(effects),req.body.consume_on_use===undefined?true:Boolean(req.body.consume_on_use),grantsTimeView,grantsTrueTime]);
...
app.post('/api/admin/rooms/:id/map-nodes',auth,adminAuth,async(req,res)=>{try{await requireRoomAdmin(req.user.id,req.params.id);const connections=Array.isArray(req.body.connections)?req.body.connections.map(Number).filter(Boolean):[];const row=await get(`INSERT INTO room_map_nodes(room_id,name,description,image_url,connections,hidden,noise_threshold,time_offset_minutes) VALUES(?,?,?,?,?::jsonb,?,?,?) RETURNING *`,[req.params.id,String(req.body.name||'未命名區域').slice(0,120),String(req.body.description||''),String(req.body.image_url||''),JSON.stringify(connections),Boolean(req.body.hidden),clampInt(req.body.noise_threshold,0,999,30),clampInt(req.body.time_offset_minutes,-10080,10080,0)]);await syncBidirectionalNodeConnections(Number(req.params.id),row.id,[],connections);res.json({node:{...row,connections:parseJsonField(row.connections,[])}});}catch(error){res.status(error.status||500).json({error:error.message||'建立地圖節點失敗'});}});
...
async function activateMonsterBossPhase(roomId,monsterId,nextIndex,phase,reason=''){
  ...
  await run(`UPDATE room_monsters SET boss_phase_index=?,boss_phase_name=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`,[nextIndex,phase.name,monsterId]);
  await addEvent(roomId,null,'boss_phase',{text:`👑 ${row.name} 進入「${phase.name}」${phase.description?`：${phase.description}`:''}`,monster_id:monsterId,phase_index:nextIndex,phase,reason});return true;
}
...
async function processScheduledWorldEvents(roomId,fromTotal,toTotal){
  ...
  for(const ev of events){let occurrences=[];if(ev.repeat_mode==='daily'){
      const startDay=Math.floor(fromTotal/1440)+1,endDay=Math.floor(toTotal/1440)+1;for(let day=startDay;day<=endDay&&day-startDay<370;day++){const total=((day-1)*1440)+clampInt(ev.trigger_minute,0,1439,0);if(total>fromTotal&&total<=toTotal&&(!ev.last_trigger_total||total>Number(ev.last_trigger_total)))occurrences.push(total);}
    }else{const total=((Math.max(1,Number(ev.trigger_day)||1)-1)*1440)+clampInt(ev.trigger_minute,0,1439,0);if(total>fromTotal&&total<=toTotal&&(!ev.last_trigger_total||Number(ev.last_trigger_total)<total))occurrences.push(total);}
    for(const total of occurrences){const day=Math.floor(total/1440)+1,minute=total%1440,hour=Math.floor(minute/60),min=minute%60;const prefix=`⏰ 第 ${day} 天 ${String(hour).padStart(2,'0')}:${String(min).padStart(2,'0')}`;const content=String(ev.content||ev.name||'事件發生');
      let delivered=false;
      ...
      await run(`UPDATE scheduled_world_events SET trigger_count=trigger_count+1,last_trigger_total=?,active=CASE WHEN repeat_mode='once' THEN FALSE ELSE active END,updated_at=CURRENT_TIMESTAMP WHERE id=?`,[total,ev.id]);fired.push({id:ev.id,total});
    }
  }return fired;
}
...
app.patch('/api/admin/rooms/:id/map-nodes/:nodeId',auth,adminAuth,async(req,res)=>{try{
  ...
  const oldConnections=mapConnectionIds(node.connections);const row=await get(`UPDATE room_map_nodes SET name=?,description=?,image_url=?,connections=?::jsonb,hidden=?,noise_threshold=?,time_offset_minutes=?,updated_at=CURRENT_TIMESTAMP WHERE id=? RETURNING *`,[String(req.body.name??node.name).trim().slice(0,120)||node.name,String(req.body.description??node.description),String(req.body.image_url??node.image_url),JSON.stringify(connections),req.body.hidden===undefined?node.hidden:Boolean(req.body.hidden),clampInt(req.body.noise_threshold??node.noise_threshold,0,999,30),clampInt(req.body.time_offset_minutes??node.time_offset_minutes,-10080,10080,0),node.id]);await syncBidirectionalNodeConnections(roomId,node.id,oldConnections,connections);res.json({node:{...row,connections:mapConnectionIds(row.connections)}});
