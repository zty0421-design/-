...
app.post('/api/rooms/:id/battle/start', auth, adminAuth, async function(req,res){
  try{
    ...
    const current=await get(`SELECT user_id FROM room_members WHERE room_id=? AND role='player' ORDER BY joined_at ASC LIMIT 1`,[req.params.id]);
    const members=await all(`SELECT user_id FROM room_members WHERE room_id=? AND role='player'`,[req.params.id]);
    for(const member of members){
      await resetCharacterClassResourcesForBattle(member.user_id);
    }

    await run(`UPDATE rooms SET battle_active=TRUE,round=1,current_actor_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`,[current?current.user_id:null,req.params.id]);
    await addEvent(req.params.id,req.user.id,'system',{text:'⚔️ DM 開始戰鬥輪。'});
    const snapshot=await roomSnapshot(req.params.id);
    io.to('room:'+req.params.id).emit('room:snapshot',snapshot);
    res.json(snapshot);
  }catch(error){
