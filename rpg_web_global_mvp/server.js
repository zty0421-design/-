...
async function updatePlayerCharacter(userId, body) {
  ...
  if (body.bloodline !== undefined) {
    const bloodline = String(body.bloodline || '').trim().slice(0,120);
    updates.push('bloodline = ?');
    values.push(bloodline);
  }

  if (body.self_intro !== undefined) {
    const selfIntro = String(body.self_intro || '').trim().slice(0,4000);
    updates.push('self_intro = ?');
    values.push(selfIntro);
  }
