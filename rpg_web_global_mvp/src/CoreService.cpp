...
Json::Value executeConfiguredCombatSkill(const std::shared_ptr<CoreService>& service,
  ...
  for (const auto& [oneTargetType, oneTargetId] : targetRefs) {
    for (std::size_t index = 0; index < hitSpecs.size(); ++index) {
      ...
      action["damage_multiplier"] = hit.isMember("damage_multiplier") ? hit["damage_multiplier"] : Json::Value{1.0};
...
void CoreService::initializeDatabase() {
  const std::vector<std::string> migrations = {
      ...
      R"SQL(UPDATE character_cards SET attribute_points=GREATEST(0,
...
Json::Value CoreService::characterByUser(std::int64_t userId) const {
  ...
  const auto attributeBudget = characterTierBudget(characterTier);
  ...
  character["attribute_remaining_expected"] = Json::Int64(std::max<std::int64_t>(0, attributeBudget - attributeSpent));
  ...
  drogon::app().registerHandler(
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback) {
        runHandler(callback, [&] {
          ...
          const auto budget = characterTierBudget(tier);
          ...
          for (std::size_t index = 0; index < fields.size(); ++index) {
            ...
            if (attributes[index] > budget) {
              throw ApiError(drogon::k400BadRequest, "單項基礎屬性不能超過目前大階的總屬性額度");
            }
          }
          ...
          if (body.isMember(allowedFifth)) {
            ...
            if (nextMax > budget) {
              throw ApiError(drogon::k400BadRequest, "第五基礎屬性不能超過目前大階的總屬性額度");
            }
          ...
          if (editsAttributes) {
            ...
            if (spent > budget || spent + newPoints != budget) {
              throw ApiError(drogon::k400BadRequest,
                             "屬性點數不正確：敏捷、力量、體質、精神、第五基礎屬性與剩餘點數總和必須等於目前大階額度");
            }
          }
