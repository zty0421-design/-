...
std::vector<std::pair<std::int64_t, double>> poolEntries(
    const Db& db, std::int64_t poolId, const std::set<std::int64_t>& excluded = {}) {
  const auto rows = db->execSqlSync(
      "SELECT e.affix_id,e.weight FROM affix_pool_entries e "
  ...
  for (const auto& row : rows) {
    ...
    out.emplace_back(id, row["weight"].as<double>());
  }
  return out;
}
...
Json::Value expandTarget(const Db& db, Json::Value target) {
  ...
  const auto ids = target["current_affixes"].isArray()
                       ? target["current_affixes"]
  ...
  target["affixes"] = std::move(expanded);
  ...
  if (integer(target["pool_id"]) <= 0) {
    const auto pool = poolFor(db, profile, str(target["target_type"]), str(target["rank"], "G"), target["data"]);
    if (pool) target["pool_id"] = Json::Int64(*pool);
  }
  const auto poolId = integer(target["pool_id"]);
  if (poolId > 0) target["pool_entries"] = poolWithEntries(db, poolId)["entries"];
  else target["pool_entries"] = Json::arrayValue;
  return target;
}
...
Json::Value createTarget(const Db& db, std::int64_t ownerUserId, const std::string& targetType,
  ...
  const auto rows = db->execSqlSync(
      "INSERT INTO affix_targets(owner_user_id,target_type,target_key,display_name,rank,pool_id,max_affixes,current_affixes,data) "
      "VALUES($1,$2,$3,$4,$5,NULLIF($6,0),$7,'[]'::jsonb,$8::jsonb) "
      "ON CONFLICT(owner_user_id,target_type,target_key) DO UPDATE SET "
      "display_name=EXCLUDED.display_name,rank=EXCLUDED.rank,pool_id=COALESCE(affix_targets.pool_id,EXCLUDED.pool_id),"
      "max_affixes=CASE WHEN affix_targets.max_affixes>0 THEN affix_targets.max_affixes ELSE EXCLUDED.max_affixes END,"
      "data=EXCLUDED.data,updated_at=CURRENT_TIMESTAMP RETURNING row_to_json(affix_targets)::text AS json_text",
  ...
  if (pool && integer(profile["initial_affixes"]) > 0 && target["current_affixes"].isArray() &&
    ...
    const auto initial = std::min<std::int64_t>(integer(profile["initial_affixes"]),
                                                std::max<std::int64_t>(0, integer(target["max_affixes"])));
    Json::Value current{Json::arrayValue};
    std::set<std::int64_t> used;
    for (std::int64_t i = 0; i < initial; ++i) {
      const auto candidates = poolEntries(db, *pool, used);
      if (candidates.empty()) break;
      const auto affixId = weightedPick(candidates);
      used.insert(affixId);
...
Json::Value poolWithEntries(const Db& db, std::int64_t poolId) {
  ...
  const auto entries = db->execSqlSync(
      "SELECT row_to_json(x)::text AS json_text FROM (SELECT e.pool_id,e.affix_id,e.weight,e.active,"
      "a.name,a.rank,a.category,a.description FROM affix_pool_entries e JOIN affix_definitions a ON a.id=e.affix_id "
      "WHERE e.pool_id=$1 ORDER BY a.rank,a.name,a.id) x", poolId);
  auto list = listJson(entries);
  double total = 0.0;
  for (const auto& item : list) if (item.get("active", true).asBool()) total += std::max(0.0, item["weight"].asDouble());
  for (Json::ArrayIndex i = 0; i < list.size(); ++i) {
    const auto w = std::max(0.0, list[i]["weight"].asDouble());
    list[i]["probability"] = total > 0.0 ? (w / total * 100.0) : 0.0;
  }
...
void applyGearAffixMigrations(const std::shared_ptr<CoreService>& service) {
  ...
  const std::array<std::pair<const char*, const char*>, 8> defaults = {{
      {"head", "頭部"}, {"body", "身體"}, {"hands", "手部"}, {"legs", "腿部"},
      {"feet", "腳部"}, {"accessory_1", "飾品 1"}, {"accessory_2", "飾品 2"}, {"special", "特殊裝備"}}};
  int order = 10;
  for (const auto& [code, name] : defaults) {
    db->execSqlSync(
        "INSERT INTO equipment_slot_types(code,name,sort_order,active) VALUES($1,$2,$3,TRUE) ON CONFLICT(code) DO NOTHING",
        code, name, order);
    order += 10;
  }
...
void registerGearAffixRoutes(const std::shared_ptr<CoreService>& service) {
  ...
  register0("/api/admin/affixes", drogon::Post, [service, db](const Request& req, Callback& cb) {
    runHandler(cb,[&]{requireUser(service,req,true);auto body=bodyOrEmpty(req);const auto name=trimmed(str(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"詞條名稱不能空白");const auto rank=normalizeRank(str(body["rank"],"G"));const auto rows=db->execSqlSync("INSERT INTO affix_definitions(name,rank,category,description,effects,tags,active) VALUES($1,$2,$3,$4,$5::jsonb,$6::jsonb,$7) RETURNING row_to_json(affix_definitions)::text AS json_text",name,rank,trimmed(str(body["category"],"特殊")),str(body["description"]),compactJson(body["effects"].isObject()?body["effects"]:Json::Value{Json::objectValue}),compactJson(body["tags"].isArray()?body["tags"]:Json::Value{Json::arrayValue}),body.get("active",true).asBool());Json::Value out;out["affix"]=rowJson(rows);reply(cb,std::move(out));});
  });
  register1("/api/admin/affixes/{1}", drogon::Patch, [service, db](const Request& req, Callback& cb,const std::string&idText){
    runHandler(cb,[&]{requireUser(service,req,true);const auto id=positiveId(idText);auto body=bodyOrEmpty(req);const auto name=trimmed(str(body["name"]));const auto rank=normalizeRank(str(body["rank"],"G"));db->execSqlSync("UPDATE affix_definitions SET name=$1,rank=$2,category=$3,description=$4,effects=$5::jsonb,tags=$6::jsonb,active=$7,updated_at=CURRENT_TIMESTAMP WHERE id=$8",name,rank,trimmed(str(body["category"],"特殊")),str(body["description"]),compactJson(body["effects"].isObject()?body["effects"]:Json::Value{Json::objectValue}),compactJson(body["tags"].isArray()?body["tags"]:Json::Value{Json::arrayValue}),body.get("active",true).asBool(),id);Json::Value out;out["ok"]=true;reply(cb,std::move(out));});
  });
  ...
  register0("/api/admin/affix-pools", drogon::Post, [service, db](const Request& req, Callback& cb){runHandler(cb,[&]{requireUser(service,req,true);auto body=bodyOrEmpty(req);auto name=trimmed(str(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"詞條池名稱不能空白");auto type=trimmed(str(body["target_type"],"*"));auto rank=normalizeRank(str(body["rank"],"G"));const auto rows=db->execSqlSync("INSERT INTO affix_pools(name,target_type,rank,description,active) VALUES($1,$2,$3,$4,$5) RETURNING id",name,type,rank,str(body["description"]),body.get("active",true).asBool());Json::Value out;out["pool"]=poolWithEntries(db,rows[0]["id"].as<std::int64_t>());reply(cb,std::move(out));});});
  register1("/api/admin/affix-pools/{1}", drogon::Patch,[service,db](const Request&req,Callback&cb,const std::string&idText){runHandler(cb,[&]{requireUser(service,req,true);const auto id=positiveId(idText);auto body=bodyOrEmpty(req);db->execSqlSync("UPDATE affix_pools SET name=$1,target_type=$2,rank=$3,description=$4,active=$5,updated_at=CURRENT_TIMESTAMP WHERE id=$6",trimmed(str(body["name"])),trimmed(str(body["target_type"],"*")),normalizeRank(str(body["rank"],"G")),str(body["description"]),body.get("active",true).asBool(),id);Json::Value out;out["pool"]=poolWithEntries(db,id);reply(cb,std::move(out));});});
  register2("/api/admin/affix-pools/{1}/entries/{2}",drogon::Patch,[service,db](const Request&req,Callback&cb,const std::string&poolText,const std::string&affixText){runHandler(cb,[&]{requireUser(service,req,true);const auto poolId=positiveId(poolText),affixId=positiveId(affixText);auto body=bodyOrEmpty(req);const auto weight=std::max(0.0,body.get("weight",1.0).asDouble());db->execSqlSync("INSERT INTO affix_pool_entries(pool_id,affix_id,weight,active) VALUES($1,$2,$3,$4) ON CONFLICT(pool_id,affix_id) DO UPDATE SET weight=EXCLUDED.weight,active=EXCLUDED.active,updated_at=CURRENT_TIMESTAMP",poolId,affixId,weight,body.get("active",true).asBool());Json::Value out;out["pool"]=poolWithEntries(db,poolId);reply(cb,std::move(out));});});
  ...
  auto rerollHandler=[service,db](const Request&req,Callback&cb,const std::string&idText,bool targeted){runHandler(cb,[&]{const auto user=requireUser(service,req);const auto targetId=positiveId(idText);const auto rows=db->execSqlSync("SELECT row_to_json(t)::text AS json_text FROM affix_targets t WHERE id=$1",targetId);if(rows.empty())throw ApiError(drogon::k404NotFound,"找不到詞條目標");auto target=rowJson(rows);if(integer(target["owner_user_id"])!=user.id&&!user.isAdmin)throw ApiError(drogon::k403Forbidden,"不能洗煉其他玩家的物品");const auto owner=integer(target["owner_user_id"]);auto profile=profileFor(db,str(target["target_type"]),normalizeRank(str(target["rank"],"G")));if(profile.isNull())throw ApiError(drogon::k400BadRequest,"此類型／品階尚未設定洗煉規則");const auto pool=poolFor(db,profile,str(target["target_type"]),str(target["rank"]),target["data"]);if(!pool)throw ApiError(drogon::k400BadRequest,"尚未設定可用詞條池");auto body=bodyOrEmpty(req);auto current=target["current_affixes"].isArray()?target["current_affixes"]:Json::Value{Json::arrayValue};const auto slot=integer(body["slot"],0);if(slot<0||slot>=static_cast<std::int64_t>(current.size()))throw ApiError(drogon::k400BadRequest,"要刷新的是既有詞條欄位");const auto code=str(profile[targeted?"targeted_currency_code":"random_currency_code"],"trait_core");const auto cost=std::max<std::int64_t>(0,integer(profile[targeted?"targeted_cost":"random_cost"],targeted?5:1));spendCurrency(db,owner,code,cost);try{std::int64_t next=0;if(targeted){next=integer(body["affix_id"]);if(next<=0)throw ApiError(drogon::k400BadRequest,"請指定詞條");const auto valid=db->execSqlSync("SELECT 1 FROM affix_pool_entries e JOIN affix_definitions a ON a.id=e.affix_id WHERE e.pool_id=$1 AND e.affix_id=$2 AND e.active=TRUE AND a.active=TRUE",*pool,next);if(valid.empty())throw ApiError(drogon::k400BadRequest,"指定詞條不在此詞條池");}else{std::set<std::int64_t> excluded;for(Json::ArrayIndex i=0;i<current.size();++i)if(static_cast<std::int64_t>(i)!=slot)excluded.insert(integer(current[i]));next=weightedPick(poolEntries(db,*pool,excluded));}current[static_cast<Json::ArrayIndex>(slot)]=Json::Int64(next);db->execSqlSync("UPDATE affix_targets SET current_affixes=$1::jsonb,pool_id=$2,updated_at=CURRENT_TIMESTAMP WHERE id=$3",compactJson(current),*pool,targetId);}catch(...){refundCurrency(db,owner,code,cost);throw;}const auto updated=db->execSqlSync("SELECT row_to_json(t)::text AS json_text FROM affix_targets t WHERE id=$1",targetId);Json::Value out;out["target"]=expandTarget(db,rowJson(updated));out["currencies"]=currenciesFor(db,owner);reply(cb,std::move(out));},targeted?"指定詞條失敗":"刷新詞條失敗");};
