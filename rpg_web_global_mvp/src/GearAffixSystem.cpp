...
// 成長設定與進度都保存在目標 data；有效品階與每級加成只由 C++ 計算。
const std::array<std::string, 10>& growthRanks() {
  static const std::array<std::string, 10> ranks = {
      "G", "F", "E", "D", "C", "B", "A", "S", "SS", "SSS"};
  return ranks;
}

std::int64_t growthRankIndex(const std::string& raw) {
  const auto rank = normalizeRank(raw);
  const auto& ranks = growthRanks();
  const auto it = std::find(ranks.begin(), ranks.end(), rank);
  return it == ranks.end() ? 0 : static_cast<std::int64_t>(std::distance(ranks.begin(), it));
}

std::int64_t growthXpNeeded(const Json::Value& data, std::int64_t level) {
  const auto base = std::clamp<std::int64_t>(integer(data["growth_xp_base"], 10), 1, 1000000000LL);
  const auto step = std::clamp<std::int64_t>(integer(data["growth_xp_step"], 5), 0, 1000000000LL);
  const auto safeLevel = std::max<std::int64_t>(1, level);
  const long double raw = static_cast<long double>(base) +
                          static_cast<long double>(step) * static_cast<long double>(safeLevel - 1);
  return static_cast<std::int64_t>(std::min<long double>(raw, 1000000000000.0L));
}

Json::Value normalizeGrowthData(Json::Value data, const std::string& fallbackRank) {
  if (!data.isObject()) data = Json::Value{Json::objectValue};
  if (!data.get("growth_enabled", false).asBool()) return data;

  const auto baseRank = normalizeRank(str(data["growth_base_rank"], fallbackRank));
  auto maxRank = normalizeRank(str(data["growth_max_rank"], "SSS"));
  if (growthRankIndex(maxRank) < growthRankIndex(baseRank)) maxRank = baseRank;
  const auto maxLevel = std::clamp<std::int64_t>(integer(data["growth_max_level"], 10), 1, 1000);
  const auto level = std::clamp<std::int64_t>(integer(data["growth_level"], 1), 1, maxLevel);
  auto xp = std::max<std::int64_t>(0, integer(data["growth_xp"]));
  if (level >= maxLevel) xp = 0;
  const auto rankEvery = std::clamp<std::int64_t>(integer(data["growth_rank_every"], 5), 0, 1000);
  const auto affixEvery = std::clamp<std::int64_t>(integer(data["growth_affix_every"], 5), 0, 1000);
  const auto affixEach = std::clamp<std::int64_t>(integer(data["growth_affix_bonus_each"], 1), 0, 1000);
  const auto damagePerLevel = std::clamp(data.get("growth_damage_percent_per_level", 2.0).asDouble(), 0.0, 10000.0);

  auto effectiveIndex = growthRankIndex(baseRank);
  if (rankEvery > 0) effectiveIndex += (level - 1) / rankEvery;
  effectiveIndex = std::min<std::int64_t>(effectiveIndex, growthRankIndex(maxRank));
  effectiveIndex = std::clamp<std::int64_t>(effectiveIndex, 0, static_cast<std::int64_t>(growthRanks().size() - 1));
  const auto effectiveRank = growthRanks()[static_cast<std::size_t>(effectiveIndex)];
  const auto affixBonus = affixEvery > 0 ? ((level - 1) / affixEvery) * affixEach : 0;
  const auto damageMultiplier = 1.0 + (damagePerLevel * static_cast<double>(level - 1) / 100.0);

  data["growth_enabled"] = true;
  data["growth_base_rank"] = baseRank;
  data["growth_max_rank"] = maxRank;
  data["growth_max_level"] = Json::Int64(maxLevel);
  data["growth_level"] = Json::Int64(level);
  data["growth_xp"] = Json::Int64(xp);
  data["growth_total_xp"] = Json::Int64(std::max<std::int64_t>(0, integer(data["growth_total_xp"])));
  data["growth_xp_base"] = Json::Int64(std::clamp<std::int64_t>(integer(data["growth_xp_base"], 10), 1, 1000000000LL));
  data["growth_xp_step"] = Json::Int64(std::clamp<std::int64_t>(integer(data["growth_xp_step"], 5), 0, 1000000000LL));
  data["growth_xp_on_use"] = Json::Int64(std::clamp<std::int64_t>(integer(data["growth_xp_on_use"], 1), 0, 1000000000LL));
  data["growth_rank_every"] = Json::Int64(rankEvery);
  data["growth_affix_every"] = Json::Int64(affixEvery);
  data["growth_affix_bonus_each"] = Json::Int64(affixEach);
  data["growth_damage_percent_per_level"] = damagePerLevel;
  data["growth_effective_rank"] = effectiveRank;
  data["growth_affix_bonus"] = Json::Int64(affixBonus);
  data["growth_damage_multiplier"] = damageMultiplier;
  data["growth_xp_needed"] = Json::Int64(level >= maxLevel ? 0 : growthXpNeeded(data, level));
  return data;
}

Json::Value growthSummary(const Json::Value& rawData, const std::string& fallbackRank) {
  const auto data = normalizeGrowthData(rawData, fallbackRank);
  Json::Value out{Json::objectValue};
  const bool enabled = data.get("growth_enabled", false).asBool();
  out["enabled"] = enabled;
  if (!enabled) return out;
  const auto level = integer(data["growth_level"], 1);
  const auto maxLevel = integer(data["growth_max_level"], 10);
  const auto xp = integer(data["growth_xp"]);
  const auto needed = integer(data["growth_xp_needed"]);
  out["level"] = Json::Int64(level);
  out["max_level"] = Json::Int64(maxLevel);
  out["xp"] = Json::Int64(xp);
  out["xp_needed"] = Json::Int64(needed);
  out["total_xp"] = Json::Int64(integer(data["growth_total_xp"]));
  out["progress_percent"] = needed > 0 ? std::clamp(100.0 * static_cast<double>(xp) / static_cast<double>(needed), 0.0, 100.0) : 100.0;
  out["effective_rank"] = str(data["growth_effective_rank"], fallbackRank);
  out["affix_bonus"] = Json::Int64(integer(data["growth_affix_bonus"]));
  out["damage_multiplier"] = data.get("growth_damage_multiplier", 1.0).asDouble();
  out["damage_percent_per_level"] = data.get("growth_damage_percent_per_level", 0.0).asDouble();
  return out;
}

Json::Value growthResultJson(const GearGrowthResult& result) {
  Json::Value out{Json::objectValue};
  out["enabled"] = result.enabled;
  out["changed"] = result.changed;
  out["levelled"] = result.levelled;
  out["rank_changed"] = result.rankChanged;
  out["target_id"] = Json::Int64(result.targetId);
  out["level"] = Json::Int64(result.level);
  out["xp"] = Json::Int64(result.xp);
  out["xp_needed"] = Json::Int64(result.xpNeeded);
  out["total_xp"] = Json::Int64(result.totalXp);
  out["xp_gained"] = Json::Int64(result.xpGained);
  out["max_affixes"] = Json::Int64(result.maxAffixes);
...
Json::Value expandTarget(const Db& db, Json::Value target) {
  ...
  // [功能備註｜65-cpp.26.14 可成長性裝備] 每次回傳都由 C++ 正規化成長快照，前端只負責顯示。
  target["data"] = normalizeGrowthData(objectOrEmpty(target["data"]), str(target["rank"], "G"));
  target["growth"] = growthSummary(target["data"], str(target["rank"], "G"));
  if (target["growth"].get("enabled", false).asBool()) {
    target["rank"] = str(target["growth"]["effective_rank"], str(target["rank"], "G"));
  }
...
Json::Value createTarget(const Db& db, std::int64_t ownerUserId, const std::string& targetType,
  ...
  const auto incomingRank = normalizeRank(rank);
  auto mergedData = data.isObject() ? data : Json::Value{Json::objectValue};

  // [功能備註｜65-cpp.26.14 可成長性裝備]
  // 玩家角色卡裡保存的是裝備模板／基礎設定；等級與 XP 存在 affix target。
  // 每次同步時只更新設定，保留既有動態進度，避免刷新角色卡把裝備洗回 Lv.1。
  const auto existingRows = db->execSqlSync(
      "SELECT data::text AS data_text FROM affix_targets "
      "WHERE owner_user_id=$1 AND target_type=$2 AND target_key=$3 LIMIT 1",
      ownerUserId, targetType, targetKey);
  if (!existingRows.empty() && !existingRows[0]["data_text"].isNull() &&
      mergedData.get("growth_enabled", false).asBool()) {
    const auto existingData = parseJson(existingRows[0]["data_text"].as<std::string>(),
                                        Json::Value{Json::objectValue});
    if (existingData.get("growth_enabled", false).asBool()) {
      for (const auto* key : {"growth_level", "growth_xp", "growth_total_xp"}) {
        if (existingData.isMember(key)) mergedData[key] = existingData[key];
      }
    }
  }
  mergedData = normalizeGrowthData(std::move(mergedData), incomingRank);
  const auto effectiveRank = mergedData.get("growth_enabled", false).asBool()
                                 ? str(mergedData["growth_effective_rank"], incomingRank)
                                 : incomingRank;
  auto profile = profileFor(db, targetType, effectiveRank);
  const auto pool = poolFor(db, profile, targetType, effectiveRank, mergedData);
  const auto baseMaxAffixes = std::max<std::int64_t>(0, integer(profile["max_affixes"], 0));
  const auto growthAffixBonus = std::max<std::int64_t>(0, integer(mergedData["growth_affix_bonus"], 0));
  const auto maxAffixes = baseMaxAffixes + growthAffixBonus;
  const auto maxSpecialAffixes = std::max<std::int64_t>(0, integer(mergedData["max_special_affixes"], 0));
  const auto intrinsicEffects = arrayOrEmpty(mergedData["intrinsic_effects"]);
  const auto rows = db->execSqlSync(
      ...
      "pool_id=COALESCE(EXCLUDED.pool_id,affix_targets.pool_id),"
      "max_affixes=GREATEST(affix_targets.max_affixes,EXCLUDED.max_affixes),"
      "max_special_affixes=GREATEST(affix_targets.max_special_affixes,EXCLUDED.max_special_affixes),"
      ...
      ownerUserId, targetType, targetKey, displayName, effectiveRank,
      pool ? static_cast<std::int64_t>(*pool) : static_cast<std::int64_t>(0), maxAffixes,
      maxSpecialAffixes, compactJson(intrinsicEffects), compactJson(mergedData));
...
void syncHeldTargets(const std::shared_ptr<CoreService>& service, std::int64_t userId) {
  ...
  for (const auto& [field, type] : sources) {
    ...
    for (const auto& raw : character[field]) {
      ...
      const auto instanceId = trimmed(str(data["gear_instance_id"]));
      const auto key = !instanceId.empty()
                           ? std::string(type) + ":instance:" + instanceId
                           : std::string(type) + ":" + name + ":" + std::to_string(n);
      ...
      const bool enabled = data.get("affix_enabled", false).asBool() ||
                           data.get("growth_enabled", false).asBool() ||
...
void syncHeldGearTargets(const std::shared_ptr<CoreService>& service, std::int64_t userId) {
  syncHeldTargets(service, userId);
}
...
GearGrowthResult grantGearGrowthXp(const std::shared_ptr<CoreService>& service,
                                   ...
                                   std::int64_t amount) {
  GearGrowthResult result;
  result.targetId = targetId;
  if (!service || targetId <= 0 || ownerUserId <= 0 || amount <= 0) return result;
  const auto db = service->database();
  const auto rows = db->execSqlSync(
      "SELECT row_to_json(t)::text AS json_text FROM affix_targets t "
      "WHERE t.id=$1 AND t.owner_user_id=$2 LIMIT 1",
      targetId, ownerUserId);
  if (rows.empty()) return result;
  auto target = rowJson(rows);
  auto data = normalizeGrowthData(objectOrEmpty(target["data"]), str(target["rank"], "G"));
  result.enabled = data.get("growth_enabled", false).asBool();
  if (!result.enabled) return result;

  const auto oldLevel = integer(data["growth_level"], 1);
  const auto oldRank = str(data["growth_effective_rank"], str(target["rank"], "G"));
  const auto maxLevel = std::clamp<std::int64_t>(integer(data["growth_max_level"], 10), 1, 1000);
  if (oldLevel >= maxLevel) {
    result.level = oldLevel;
    result.rank = oldRank;
    result.totalXp = integer(data["growth_total_xp"]);
    result.maxAffixes = integer(target["max_affixes"]);
    return result;
  }

  const auto gained = std::clamp<std::int64_t>(amount, 1, 1000000000000LL);
  auto level = oldLevel;
  auto xp = std::max<std::int64_t>(0, integer(data["growth_xp"])) + gained;
  auto totalXp = std::max<std::int64_t>(0, integer(data["growth_total_xp"])) + gained;
  while (level < maxLevel) {
    const auto needed = growthXpNeeded(data, level);
    if (xp < needed) break;
    xp -= needed;
    ++level;
  }
  ...
  data["growth_level"] = Json::Int64(level);
  data["growth_xp"] = Json::Int64(xp);
  data["growth_total_xp"] = Json::Int64(totalXp);
  data = normalizeGrowthData(std::move(data), str(target["rank"], "G"));

  const auto newRank = str(data["growth_effective_rank"], oldRank);
  auto profile = profileFor(db, str(target["target_type"]), newRank);
  const auto pool = poolFor(db, profile, str(target["target_type"]), newRank, data);
  const auto desiredMaxAffixes = std::max<std::int64_t>(0, integer(profile["max_affixes"])) +
                                 std::max<std::int64_t>(0, integer(data["growth_affix_bonus"]));
  const auto oldMaxAffixes = std::max<std::int64_t>(0, integer(target["max_affixes"]));
  const auto nextMaxAffixes = std::max(oldMaxAffixes, desiredMaxAffixes);
  auto currentAffixes = arrayOrEmpty(target["current_affixes"]);
  // 新成長槽位若有可用一般詞條池，會自動長出新詞條；既有詞條永遠不被覆蓋。
  if (pool && nextMaxAffixes > oldMaxAffixes &&
      static_cast<std::int64_t>(currentAffixes.size()) < nextMaxAffixes) {
    std::set<std::int64_t> used;
    for (const auto& raw : currentAffixes) {
      const auto id = integer(raw);
      if (id > 0) used.insert(id);
    }
    while (static_cast<std::int64_t>(currentAffixes.size()) < nextMaxAffixes) {
      try {
        const auto affixId = pickAffixFromPool(db, *pool, used, "normal");
        used.insert(affixId);
        currentAffixes.append(Json::Int64(affixId));
      } catch (const ApiError&) {
        break;
      }
    }
  }
  const auto updated = db->execSqlSync(
      "UPDATE affix_targets SET rank=$1,pool_id=COALESCE(NULLIF($2,0),pool_id),"
      "max_affixes=GREATEST(max_affixes,$3),current_affixes=$4::jsonb,data=$5::jsonb,updated_at=CURRENT_TIMESTAMP "
      "WHERE id=$6 AND owner_user_id=$7 RETURNING row_to_json(affix_targets)::text AS json_text",
      newRank, pool ? static_cast<std::int64_t>(*pool) : static_cast<std::int64_t>(0),
      nextMaxAffixes, compactJson(currentAffixes), compactJson(data), targetId, ownerUserId);
  auto nextTarget = updated.empty() ? target : rowJson(updated);

  result.changed = true;
  result.levelled = level > oldLevel;
  result.rankChanged = newRank != oldRank;
  result.level = level;
  result.xp = integer(data["growth_xp"]);
  result.xpNeeded = integer(data["growth_xp_needed"]);
  result.totalXp = totalXp;
  result.xpGained = gained;
  result.maxAffixes = integer(nextTarget["max_affixes"], desiredMaxAffixes);
  result.rank = newRank;
  return result;
}
...
GearGrowthResult grantHeldGearGrowthXpByIndex(const std::shared_ptr<CoreService>& service,
                                              ...
                                              std::int64_t amount) {
  GearGrowthResult result;
  if (!service || ownerUserId <= 0 || index < 0 || amount <= 0) return result;
  const std::string field = targetType == "weapon" ? "weapons" :
                            targetType == "equipment" ? "equipment" : "";
  if (field.empty()) return result;
  syncHeldTargets(service, ownerUserId);
  const auto character = service->characterByUser(ownerUserId);
  if (!character[field].isArray() || index >= static_cast<std::int64_t>(character[field].size())) return result;
  const auto selectedRaw = character[field][static_cast<Json::ArrayIndex>(index)];
  const auto targetName = gearName(selectedRaw);
  if (targetName.empty()) return result;
  const auto selectedData = gearData(selectedRaw, targetType);
  const auto instanceId = trimmed(str(selectedData["gear_instance_id"]));
  std::int64_t occurrence = 0;
  if (instanceId.empty()) {
    for (std::int64_t i = 0; i <= index; ++i) {
      if (gearName(character[field][static_cast<Json::ArrayIndex>(i)]) == targetName) ++occurrence;
    }
  }
  const auto key = !instanceId.empty()
                       ? targetType + ":instance:" + instanceId
                       : targetType + ":" + targetName + ":" + std::to_string(std::max<std::int64_t>(1, occurrence));
  const auto rows = service->database()->execSqlSync(
      "SELECT id FROM affix_targets WHERE owner_user_id=$1 AND target_type=$2 AND target_key=$3 LIMIT 1",
      ownerUserId, targetType, key);
  if (rows.empty()) return result;
  return grantGearGrowthXp(service, ownerUserId, rows[0]["id"].as<std::int64_t>(), amount);
}
...
void registerGearAffixRoutes(const std::shared_ptr<CoreService>& service) {
  ...
  // [功能備註｜65-cpp.26.14 可成長性裝備] DM 可直接給指定玩家某件持有武器／裝備成長 XP。
  register1("/api/admin/players/{1}/gear-growth", drogon::Post,
            [service](const Request& req, Callback& cb, const std::string& idText) {
    runHandler(cb, [&] {
      requireUser(service, req, true);
      const auto userId = positiveId(idText);
      const auto body = bodyOrEmpty(req);
      const auto type = trimmed(str(body["target_type"]));
      if (type != "weapon" && type != "equipment")
        throw ApiError(drogon::k400BadRequest, "成長目標只能是 weapon 或 equipment");
      const auto index = integer(body["index"], -1);
      if (index < 0) throw ApiError(drogon::k400BadRequest, "裝備索引錯誤");
      const auto xp = std::clamp<std::int64_t>(integer(body["xp"]), 1, 1000000000000LL);
      auto growth = grantHeldGearGrowthXpByIndex(service, userId, type, index, xp);
      if (!growth.enabled) throw ApiError(drogon::k400BadRequest, "這件裝備沒有啟用成長系統");
      Json::Value out;
      out["ok"] = true;
      out["growth"] = growthResultJson(growth);
