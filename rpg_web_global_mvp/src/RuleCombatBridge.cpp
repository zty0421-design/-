...
Json::Value spawnMonster(const std::shared_ptr<CoreService>& service,
  ...
  const auto templates = service->database()->execSqlSync(
      "SELECT id,name,max_hp,attributes::text AS attributes_text,great_way::text AS great_way_text FROM monster_templates WHERE id=$1",
      templateId);
  if (templates.empty()) {
    out["error"] = "找不到怪物模板";
    out["applied"] = false;
  ...
  // [功能備註｜60-cpp.21 怪物大道] 規則引擎生成怪物時也初始化大道資源，與手動加入房間一致。
  const auto fifth = greatWay.get("enabled", false).asBool()
      ? std::max<std::int64_t>(0, integer(greatWay["resource_max"]))
      : std::max({std::int64_t{0}, integer(attributes["great_way"]), integer(attributes["faith"]), integer(attributes["fifth"])});
