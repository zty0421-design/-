      ...
      for(const auto&r:rows){
        ...
        try{
          ...
          if(kind=="knowledge_shop"){
            ...
            if(kind=="magics"){x["element"]=normalizeMagicElement(stringValue(x["element"]));x["tree_x"]=std::clamp<std::int64_t>(intValue(x["tree_x"]),0,20);x["tree_y"]=std::clamp<std::int64_t>(intValue(x["tree_y"]),0,30);if(!x.isMember("icon"))x["icon"]="✦";}
