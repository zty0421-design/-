...
void syncBidirectionalMapConnections(const std::shared_ptr<CoreService>& service,
  ...
  for (const auto& row : rows) {
    ...
    for (const auto& value : links) {
      const auto id = intValue(value);
      if (id == nodeId) { hasNode = true; continue; }
...
void registerLegacyV39Routes(const std::shared_ptr<CoreService>& service) {
  ...
  // 玩家「提交行動」的核心是把 action event 寫入 events。規則引擎仍可正常 block_action；
  ...
  reg1("/api/rooms/{1}/actions", drogon::Post,
    ...
    runHandler(cb, [&] {
      ...
      requireRoomAccess(service, user, roomId);
      const auto body = bodyOrEmpty(req);
      const auto action = trimmed(stringValue(body["action"], stringValue(body["text"])));
      if (action.empty()) throw ApiError(drogon::k400BadRequest, "請輸入要提交的行動");
      if (action.size() > 8000) throw ApiError(drogon::k400BadRequest, "行動內容過長");

      Json::Value payload = body;
      payload["action"] = action;
      payload["text"] = action;
      payload["noise"] = Json::Int64(std::clamp<std::int64_t>(intValue(body["noise"]), 0, 100));

      Json::Value triggered{Json::objectValue};
      triggered["event_type"] = "PLAYER_ACTION";
      triggered["matched"] = Json::arrayValue;
      triggered["blocked"] = false;
      bool ruleEngineDegraded = false;
      try {
        // [功能備註｜規則引擎] 玩家宣告任何動作時先送入 C++ 條件式規則引擎。
        triggered = evaluateRuleEvent(service, roomId, user.id, "PLAYER_ACTION", payload, false);
      } catch (const std::exception& error) {
        // 規則資料異常不可讓整個房間失去「提交行動」能力；DM 可從 Render log 追查。
        ruleEngineDegraded = true;
        LOG_WARN << "PLAYER_ACTION rule engine degraded for room " << roomId
                 << ", user " << user.id << ": " << error.what();
      }
      if (triggered.get("blocked", false).asBool()) {
        throw ApiError(drogon::k403Forbidden, "此行動被規則阻止");
      }

      // events 是行動宣告的核心持久化；這一步若真的失敗才回傳提交失敗。
      service->addEvent(roomId, user.id, "action", payload);

      Json::Value out;
      out["ok"] = true;
      out["action"] = payload;
      out["triggered"] = triggered["matched"].isArray() ? triggered["matched"] : Json::Value{Json::arrayValue};
      out["rule_engine"] = triggered;
      out["rule_engine_degraded"] = ruleEngineDegraded;

      // 玩家回傳必須使用 viewer-filtered snapshot，避免把其他玩家不可見聊天／資訊直接帶回。
      // 完整快照是附加資料，失敗時保留 action 成功結果，前端稍後再重新載入。
      try {
        out["snapshot"] = roomSnapshotForViewer(service, roomId, user.id);
      } catch (const std::exception& error) {
        out["snapshot_degraded"] = true;
        LOG_WARN << "PLAYER_ACTION snapshot degraded for room " << roomId
                 << ": " << error.what();
      }
      reply(cb, std::move(out));
    }, "提交行動失敗");
  ...
  reg2("/api/admin/rooms/{1}/map-nodes/{2}", drogon::Delete,
    ...
    runHandler(cb, [&] {
      requireUser(service, req, true);
      const auto roomId = positiveId(roomIdText); const auto nodeId = positiveId(nodeIdText); Json::Value noLinks{Json::arrayValue}; syncBidirectionalMapConnections(service, roomId, nodeId, noLinks);
      const auto rows = service->database()->execSqlSync("DELETE FROM room_map_nodes WHERE id=$1 AND room_id=$2 RETURNING id", nodeId, roomId);
