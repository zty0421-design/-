    ...
    for (const auto& step : steps) {
      ...
      if (sharedStep) {
        ...
        const auto recent = service->database()->execSqlSync(
            ...
            roomId, intValue(task["id"]), stepId, std::string("auto:") + eventName);
        if (!recent.empty()) continue;
      }
      ...
      service->database()->execSqlSync(
          ...
          roomId, intValue(task["id"]), stepId, userId, std::string("auto:") + eventName, nodeId > 0 ? nodeId : 0, compactJson(actionData));
      if (sharedStep) {
...
void registerLegacyV39Routes(const std::shared_ptr<CoreService>& service) {
  ...
  reg0("/api/admin/effect-templates", drogon::Get,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);Json::Value out;out["templates"]=selectRows0(service->database(),"SELECT row_to_json(e)::text AS json_text FROM admin_effect_templates e WHERE active=TRUE ORDER BY category,name,id");reply(cb,std::move(out));},"讀取共用效果模板失敗");
  });
    ...
    runHandler(cb, [&] {
      ...
      if (type == "place_anchor") {
        ...
        service->database()->execSqlSync(
            ...
            roomId, taskId, stepId, user.id, currentNodeId, anchorName.empty() ? "任務錨點" : anchorName,
            compactJson(Json::Value{Json::objectValue}));
        const auto countRows = sharedStep
          ? service->database()->execSqlSync(
              "SELECT count(*) AS count FROM room_task_anchors WHERE room_id=$1 AND task_id=$2 AND step_id=$3", roomId, taskId, stepId)
          : service->database()->execSqlSync(
              "SELECT count(*) AS count FROM room_task_anchors WHERE room_id=$1 AND task_id=$2 AND step_id=$3 AND user_id=$4",
              roomId, taskId, stepId, user.id);
      ...
      service->database()->execSqlSync(
          "INSERT INTO task_step_actions(room_id,task_id,step_id,user_id,action_type,map_node_id,data) VALUES($1,$2,$3,$4,$5,NULLIF($6,0),$7::jsonb)",
          roomId, taskId, stepId, user.id, type, currentNodeId > 0 ? currentNodeId : 0, compactJson(actionData));
