...
void registerLegacyV39Routes(const std::shared_ptr<CoreService>& service) {
  ...
  reg1("/api/rooms/{1}/world", drogon::Get,
    ...
    runHandler(cb, [&] {
      ...
      out["monsters"] = selectRows1(service->database(),
          "SELECT row_to_json(x)::text AS json_text FROM (SELECT rm.*,mt.name,mt.category,mt.rank,mt.description,mt.public_hint,mt.image_url,mt.attributes,mt.skills,mt.passive_skills,mt.great_way,mt.boss_phases,mt.drops,mt.ai_level,mt.ai_behavior,mt.config,mt.element_affinity,mt.effects FROM room_monsters rm JOIN monster_templates mt ON mt.id=rm.monster_template_id WHERE rm.room_id=$1 ORDER BY rm.id) x", roomId);
  ...
  reg0("/api/admin/magic-studies",drogon::Post,[service](const Request& req,Callback& cb){
    runHandler(cb,[&]{requireUser(service,req,true);auto body=bodyOrEmpty(req);const auto name=trimmed(stringValue(body["name"]));if(name.empty())throw ApiError(drogon::k400BadRequest,"魔法名稱不能空白");body["name"]=name.substr(0,120);body["rank"]=normalizeContentRank(stringValue(body["rank"],"G"));body["element"]=normalizeMagicElement(stringValue(body["element"]));body["tree_x"]=std::clamp<std::int64_t>(intValue(body["tree_x"]),0,40);body["tree_y"]=std::clamp<std::int64_t>(intValue(body["tree_y"]),0,60);body["icon"]=trimmed(stringValue(body["icon"],"✦")).substr(0,40);body["node_style"]=normalizeMagicNodeStyle(body["node_style"],stringValue(body["category"],"一般魔法"));body["point_cost"]=Json::Int64(std::max<std::int64_t>(0,intValue(body["point_cost"],1)));validateMagicTreePrerequisites(service->database(),stringValue(body["element"]),body["prerequisite_ids"]);Json::Value out;out["ok"]=true;out["study"]=genericInsert(service->database(),"magic_studies",body);reply(cb,std::move(out));},"建立魔法失敗");
  });
  reg1("/api/admin/magic-studies/{1}",drogon::Patch,[service](const Request& req,Callback& cb,const std::string&idText){runHandler(cb,[&]{requireUser(service,req,true);const auto id=positiveId(idText);auto body=bodyOrEmpty(req);if(body.isMember("rank"))body["rank"]=normalizeContentRank(stringValue(body["rank"]));if(body.isMember("element"))body["element"]=normalizeMagicElement(stringValue(body["element"]));if(body.isMember("tree_x"))body["tree_x"]=std::clamp<std::int64_t>(intValue(body["tree_x"]),0,40);if(body.isMember("tree_y"))body["tree_y"]=std::clamp<std::int64_t>(intValue(body["tree_y"]),0,60);if(body.isMember("node_style"))body["node_style"]=normalizeMagicNodeStyle(body["node_style"],stringValue(body["category"]));const auto existing=service->database()->execSqlSync("SELECT element,prerequisite_ids FROM magic_studies WHERE id=$1",id);if(existing.empty())throw ApiError(drogon::k404NotFound,"找不到魔法節點");const auto element=body.isMember("element")?stringValue(body["element"]):(existing[0]["element"].isNull()?std::string{"無"}:existing[0]["element"].as<std::string>());Json::Value prereqs;if(body.isMember("prerequisite_ids"))prereqs=body["prerequisite_ids"];else if(!existing[0]["prerequisite_ids"].isNull())prereqs=parseJson(existing[0]["prerequisite_ids"].as<std::string>());validateMagicTreePrerequisites(service->database(),element,prereqs,id);Json::Value out;out["ok"]=true;out["study"]=genericUpdate(service->database(),"magic_studies",id,body);reply(cb,std::move(out));},"更新魔法節點失敗");});
      ...
      for(const auto&r:rows){
        ...
        try{
          ...
          if(kind=="knowledge_shop"){
            ...
            if(!x.isMember("price"))x["price"]=Json::Int64(0);if(intValue(x["price"])<0)throw ApiError(drogon::k400BadRequest,"price 不可小於 0");
            if(!x.isMember("stock"))x["stock"]=-1;if(!x.isMember("purchase_limit"))x["purchase_limit"]=1;if(!x.isMember("description"))x["description"]="";if(!x.isMember("active"))x["active"]=true;
          }else if(kind=="skills"){
