...
Json::Value ritualStudyForViewer(const Json::Value& study,const Json::Value& research,bool learned) {
  auto out=study;
  const bool requires=study.get("requires_research",false).asBool();
  ...
  out["research_complete"]=!requires||progress>=100||learned;
  out["learned"]=learned;
  if(!requires||progress>=100||learned)return out;
