    ...
    runHandler(cb, [&] {
      ...
      for (auto& task : tasks) {
        task["participants"] = selectRows1(service->database(),
            "SELECT row_to_json(x)::text AS json_text FROM (SELECT tp.user_id,tp.status,tp.accepted_at,tp.completed_at,u.username,c.name AS character_name FROM task_participants tp JOIN users u ON u.id=tp.user_id LEFT JOIN character_cards c ON c.user_id=tp.user_id WHERE tp.task_id=$1 ORDER BY tp.completed_at NULLS FIRST,tp.accepted_at,u.username) x",
            intValue(task["id"]));
      }
...
reg1("/api/rooms/{1}/actions", drogon::Post,
  ...
  auto fateMajorRule=[](std::int64_t cardNo,bool reversed){
    ...
    if(cardNo<0||cardNo>21)return std::string("無特殊規則");return std::string(reversed?rev[cardNo]:upright[cardNo]);
  };
  ...
  reg1("/api/fate-gap/miyoi/{1}/combat-act",drogon::Post,[service,miyoiId,drawTarotCard,tarotOrientation,appendMiyoiMemory](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{
      ...
      auto st=parseJsonText(nr[0]["state_text"].as<std::string>());if(!st.get("combat_joined",false).asBool())throw ApiError(drogon::k409Conflict,"彌夜目前沒有加入戰鬥");
  ...
  reg1("/api/fate-gap/anti-fate/{1}/link-state",drogon::Get,[service,miyoiId](const Request& req,Callback& cb,const std::string& roomText){
    runHandler(cb,[&]{
      ...
      Json::Value mst=nr.empty()?Json::Value{Json::objectValue}:parseJsonText(nr[0]["state_text"].as<std::string>());
      const auto roundRows=service->database()->execSqlSync("SELECT round FROM rooms WHERE id=$1",roomId);const auto round=roundRows.empty()?1:roundRows[0]["round"].as<std::int64_t>();
      ...
      if(!br.empty()){
        const auto hp=br[0]["current_hp"].as<std::int64_t>(),mx=std::max<std::int64_t>(1,br[0]["max_hp"].as<std::int64_t>());const auto phase=hp*100/mx<=25?4:hp*100/mx<=50?3:hp*100/mx<=75?2:1;auto bst=parseJsonText(br[0]["state_text"].as<std::string>());
