## 73-cpp.34.8 怪物卡易用建立／可追加技能
- 怪物卡分為基本、技能、進階戰鬥、AI/Boss、大道、掉落六區。
- 主動、被動與大道技能可按按鈕追加多筆，不再要求手動記住管線格式。
- 提供普通怪物／菁英怪物／Boss 三種快速起始模板。

## 73-cpp.34.7 NPC 自主交流／情境判斷
- 房間內可對話 NPC 會觀察同場景玩家的「說話／行動／待機」，依 AI 等級與 AI 行為自主決定是否發言。
- 被玩家直接 `@NPC 名稱` 或點名時優先回答。
- NPC 能針對危險、調查、戰鬥、求援等關鍵情境說出判斷；一般情況仍會保留自訂 AI 台詞作為角色口吻。
- 聊天室的 NPC 發言與玩家相同，顯示 NPC 頭像、名字與時間，並標示「NPC 判斷」。
- DM 的 NPC 卡新增「自主觀察／發言」與「自主發言率 %」，可逐 NPC 關閉或調整。
- 每次玩家互動最多兩位 NPC 插話，並有 12 秒短冷卻，避免洗版。


## 73-cpp.34.6 聊天頭像／角色名／即時交流
- 房間聊天室每則說話、行動、待機都顯示目前房間角色名稱與頭像。
- 點擊角色名稱／頭像可快速在說話輸入框加入 @角色名。
- HTTP 說話／行動提交後會由 C++ RoomSocket 主動廣播最新 viewer snapshot，房內其他玩家不需重新整理即可看到交流內容。
- 事件快照使用 character_room_bindings 取得該房間實際綁定角色，避免多角色帳號顯示錯誤名稱／頭像。
...
### 73-cpp.34.25.26 紙本清單第四批
- 諸神圖鑑：`pantheonHTML()`／`adminPantheonHTML()`；神祇真名、別名、立繪、神話紀錄、神明 AI 設定。
- 神明 AI 神諭：`POST /api/pantheon/{1}/pray`（C++ `LegacyCompat.cpp`），伺服器判定回應、虔誠與神眷。
- 合成差異動畫：`playCraftResultAnimation()`；配方 `config.craft_animation` 可選 auto/forge/alchemy/ritual/weave。
- 效能：神祇圖鑑圖片 `loading=lazy` + `decoding=async`；合成動畫重用單一 overlay；祈禱後僅重新讀信仰體系。


### 73-cpp.34.25.27 紙本清單第五批
- 世界樹：依既有房間地圖節點與 connections 產生階層視圖。
- 合成研究：locked_preview 僅回傳名稱、品階、類別與研究方式，不回傳材料／成品／成功率。
- 效能：世界頁怪物/NPC/副本/容器採漸進渲染，手機 6 張、桌機 12 張一批。

### 73-cpp.34.25.28 紙本清單第六批
- 卡片／怪物卡／NPC 卡資訊密度與手機折疊。
- 魔法學七元素進度總覽、儀式學掌握進度。
- 每版固定渲染／載入優化：lazy image、content-visibility、零額外 API。


## 73-cpp.34.25.33 紙本第十一批：遊戲群

- `game_groups / game_group_members / game_group_messages`：跨房間持久群組。
- `/api/game-groups`：列出／建立。
- `/api/game-groups/join`：邀請碼加入。
- `/api/game-groups/{id}`：成員限定詳情；群主／DM 修改與刪除。
- `/api/game-groups/{id}/messages`：成員留言。
- 前端採按需載入，避免增加登入與房間首屏請求。


## 73-cpp.34.25.34 紙本第十二批：學術道具／特殊職業道具深化

- `item_templates.study_domains` 正式接上 DM 道具編輯器與玩家背包：魔法學／儀式學／神秘學可複選。
- `item_templates.profession_requirement_mode` 支援 any/main/sub；C++ 使用道具 API 實際驗證。
- 玩家背包可按學術用途與特殊職業道具篩選，無額外網路請求。
- 修正道具使用 template_id 接線，並保留 name fallback。
