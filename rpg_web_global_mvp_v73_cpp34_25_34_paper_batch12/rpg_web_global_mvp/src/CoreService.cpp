...
Json::Value elementalDamageModifier(const Json::Value& attackerSource,
  ...
  auto element=trimmed(rawElement);
  // 73-cpp.34.25.20：元素別名統一。陽=光、陰=暗；資料庫與戰鬥運算只保存標準名稱。
  if(element=="陽") element="光";
  else if(element=="陰") element="暗";
...
void CoreService::initializeDatabase() {
  const std::vector<std::string> migrations = {
      ...
      "ALTER TABLE character_cards ADD COLUMN IF NOT EXISTS personality_profile JSONB NOT NULL DEFAULT '{}'::jsonb",
...
Json::Value CoreService::characterByUser(std::int64_t userId) const {
  ...
  auto normalizeElementPercentMap=[&](const Json::Value& raw){
    ...
    if(raw.isObject()){
      if(raw.isMember("陽")) out["光"]=Json::Int64(std::clamp<std::int64_t>(integerValue(raw["陽"]),0,900));
      if(raw.isMember("陰")) out["暗"]=Json::Int64(std::clamp<std::int64_t>(integerValue(raw["陰"]),0,900));
    }
    return out;
  };
  ...
  auto normalized=[&](const Json::Value& raw){auto out=zeroMap();if(raw.isObject()){for(const auto*e:storedElements)out[e]=Json::Int64(std::max<std::int64_t>(0,integerValue(raw[e])));if(raw.isMember("陽"))out["光"]=Json::Int64(std::max<std::int64_t>(0,integerValue(raw["陽"])));if(raw.isMember("陰"))out["暗"]=Json::Int64(std::max<std::int64_t>(0,integerValue(raw["陰"])));}return out;};
  auto addBonus=[&](Json::Value& target,const Json::Value& raw,std::int64_t multiplier=1){if(!raw.isObject()||multiplier==0)return;for(const auto*e:storedElements)target[e]=Json::Int64(std::max<std::int64_t>(0,integerValue(target[e])+integerValue(raw[e])*multiplier));if(raw.isMember("陽"))target["光"]=Json::Int64(std::max<std::int64_t>(0,integerValue(target["光"])+integerValue(raw["陽"])*multiplier));if(raw.isMember("陰"))target["暗"]=Json::Int64(std::max<std::int64_t>(0,integerValue(target["暗"])+integerValue(raw["陰"])*multiplier));};
  ...
  drogon::app().registerHandler(
      ...
      [self](const drogon::HttpRequestPtr& request, Callback&& callback) {
        runHandler(callback, [&] {
          ...
          // 73-cpp.34.25.7：登入熱路徑只做帳號查詢 + bcrypt 驗證 + JWT；角色卡已在註冊／DM bootstrap 建立，
          // 不再每次登入額外 INSERT ... ON CONFLICT，省下一次 PostgreSQL round-trip 與寫鎖。
